...
class RetryableAIExecutorTest {
  ...
  void executeWithCircuitBreaker_SuccessfulOperation_ReturnsResult() {
    ...
    String result =
        executor.executeWithCircuitBreaker("test-provider", "test-service", cb, () -> "Success!");
  ...
  void executeWithCircuitBreaker_OpenCircuit_ThrowsException() {
    ...
    for (int i = 0; i < 10; i++) {
      try {
        executor.executeWithCircuitBreaker(
            "test-provider",
            "test-service",
            cb,
            () -> {
    ...
    assertThrows(
        RetryableAIExecutor.CircuitBreakerOpenException.class,
        () ->
            executor.executeWithCircuitBreaker(
                "test-provider", "test-service", cb, () -> "Should fail"));
  }
  ...
  void executeWithCircuitBreaker_QuotaExceeded_ThrowsQuotaException() {
    ...
    assertThrows(
        RetryableAIExecutor.QuotaExceededException.class,
        () ->
            executor.executeWithCircuitBreaker(
                "test-provider", "test-service", cb, () -> "Result"));
  }
  ...
  void executeReactive_SuccessfulOperation_ReturnsMono() {
    ...
    Mono<String> result =
        executor.executeReactive("test-provider", "test-service", Mono.just("Reactive Success!"));

    StepVerifier.create(result).expectNext("Reactive Success!").verifyComplete();
  }
  ...
  void executeReactive_QuotaExceeded_ReturnsErrorMono() {
    ...
    Mono<String> result =
        executor.executeReactive("test-provider", "test-service", Mono.just("Should fail"));
  ...
  void executeWithCircuitBreakerReactive_SuccessfulOperation_ReturnsMono() {
    ...
    Mono<String> result =
        executor.executeWithCircuitBreakerReactive(
  ...
  void isRetryable_HttpTimeout_ReturnsTrue() {
    ...
    try {
      ...
      boolean result =
          (boolean) method.invoke(executor, new java.net.http.HttpTimeoutException("timeout"));
      assertTrue(result);
    } catch (Exception e) {
  ...
  void isRetryable_IOException_ReturnsTrue() {
    try {
      java.lang.reflect.Method method =
          RetryableAIExecutor.class.getDeclaredMethod("isRetryable", Throwable.class);
      method.setAccessible(true);

      boolean result =
          (boolean) method.invoke(executor, new java.io.IOException("connection reset"));
      assertTrue(result);
    } catch (Exception e) {
      fail("Reflection failed: " + e.getMessage());
    }
  ...
  void isRetryable_RateLimit_ReturnsTrue() {
    try {
      ...
      boolean result =
          (boolean) method.invoke(executor, new RuntimeException("Rate limit exceeded 429"));
      assertTrue(result);
    } catch (Exception e) {
      fail("Reflection failed: " + e.getMessage());
    }
  ...
  void isRetryable_QuotaExceeded_ReturnsFalse() {
    try {
      ...
      boolean result =
          (boolean)
              method.invoke(
                  executor, new RetryableAIExecutor.QuotaExceededException("Quota exceeded"));
      assertFalse(result);
    } catch (Exception e) {
      fail("Reflection failed: " + e.getMessage());
    }
