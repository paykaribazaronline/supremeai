...
/**
 * Unit tests for RequirementAnalyzer.
 * Tests vagueness detection and clarification generation for login, bugs, and app building scenarios.
 */
class RequirementAnalyzerTest {
    ...
    void testAnalyzeRequirement_detailedLoginPrompt() {
        String prompt = "I need to implement JWT-based authentication with Spring Security for a React frontend, including refresh tokens and role-based access control";
        RequirementClarification clarification = analyzer.analyzeRequirement(prompt);
    ...
    void testAnalyzeRequirement_detailedBugReport() {
        String prompt = "NullPointerException at line 45 in UserService.java when calling userRepository.findById with null id";
        RequirementClarification clarification = analyzer.analyzeRequirement(prompt);

        assertNull(clarification, "Detailed bug report with exception should not require clarification");
    }

    @Test
    void testAnalyzeRequirement_detailedAppDescription() {
        String prompt = "Build an e-commerce application with product catalog, shopping cart, Stripe payment integration, and order management system";
        RequirementClarification clarification = analyzer.analyzeRequirement(prompt);
    ...
    void testAnalyzeRequirement_shortPromptWithoutKeywords() {
        ...
        assertNull(clarification, "Short prompt without trigger keywords should not require clarification");
    }
    ...
    void testLoginClarificationOptionsContent() {
        ...
        assertTrue(options.get(0).getHint().toLowerCase().contains("mobile") || options.get(0).getHint().toLowerCase().contains("spa"));
    ...
    void testAppBuildClarificationOptionsContent() {
        ...
        assertTrue(options.stream().anyMatch(o -> o.getOptionText().toLowerCase().contains("e-commerce")));
        assertTrue(options.stream().anyMatch(o -> o.getOptionText().toLowerCase().contains("dashboard")));
