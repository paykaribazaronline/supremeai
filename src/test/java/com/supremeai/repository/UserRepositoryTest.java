...
class UserRepositoryTest {
  ...
  void findByEmail_shouldReturnEmpty_whenEmailNotFound() {
    ...
    StepVerifier.create(userRepository.findByEmail("nonexistent@example.com")).verifyComplete();
  }
  ...
  void findByFirebaseUid_shouldReturnUser_whenUidExists() {
    ...
    StepVerifier.create(userRepository.findByFirebaseUid("uid-456"))
        .expectNextMatches(
            u -> "uid-456".equals(u.getFirebaseUid()) && UserTier.PRO.equals(u.getTier()))
        .verifyComplete();
  }
  ...
  void findByFirebaseUid_shouldReturnEmpty_whenUidNotFound() {
    ...
    StepVerifier.create(userRepository.findByFirebaseUid("unknown-uid")).verifyComplete();
  }
  ...
  void deleteById_shouldRemoveUser() {
    ...
    StepVerifier.create(userRepository.deleteById("uid-delete")).verifyComplete();
  }
}
