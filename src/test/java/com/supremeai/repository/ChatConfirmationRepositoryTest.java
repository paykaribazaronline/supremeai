...
class ChatConfirmationRepositoryTest {
  ...
  @Mock private ChatConfirmationRepository repository;
  ...
  void findByItemId_shouldReturnConfirmationsForItem() {
    ...
    StepVerifier.create(repository.findByItemId("rule-1")).expectNextCount(2).verifyComplete();
  }
  ...
  void findByItemId_shouldReturnEmpty_whenNotFound() {
    ...
    StepVerifier.create(repository.findByItemId("no-item")).verifyComplete();
  }
  ...
  void findByChatId_shouldReturnEmpty_whenNotFound() {
    ...
    StepVerifier.create(repository.findByChatId("no-chat")).verifyComplete();
  }
  ...
  void findByItemTypeAndItemId_shouldReturnMatchingConfirmations() {
    ...
    StepVerifier.create(repository.findByItemTypeAndItemId("command", "cmd-1"))
        .expectNextMatches(
            conf -> "command".equals(conf.getItemType()) && "cmd-1".equals(conf.getItemId()))
        .verifyComplete();
  }
  ...
  void findByItemTypeAndItemId_shouldReturnEmpty_whenNoMatch() {
    ...
    StepVerifier.create(repository.findByItemTypeAndItemId("rule", "nonexistent")).verifyComplete();
  }
