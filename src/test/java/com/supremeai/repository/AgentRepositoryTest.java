...
class AgentRepositoryTest {
  ...
  void findStatusById_shouldReturnUnknownStatus_whenAgentHasNullStatus() {
    ...
    StepVerifier.create(agentRepository.findStatusById("agent-2"))
        .expectNextMatches(s -> "agent-2".equals(s.getAgentId()) && "UNKNOWN".equals(s.getStatus()))
        .verifyComplete();
  }
  ...
  void findStatusById_shouldReturnEmpty_whenAgentNotFound() {
    ...
    StepVerifier.create(agentRepository.findStatusById("nonexistent")).verifyComplete();
  }
  ...
  void updateStatus_shouldSaveAgentWithNewStatus() {
    ...
    StepVerifier.create(agentRepository.updateStatus("agent-3", newStatus)).verifyComplete();
  }
  ...
  void updateStatus_shouldComplete_whenAgentNotFound() {
    ...
    StepVerifier.create(agentRepository.updateStatus("missing", status)).verifyComplete();
