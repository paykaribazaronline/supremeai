...
class UserSimulatorProfileRepositoryTest {
  ...
  @Mock private UserSimulatorProfileRepository repository;
  ...
  void findByUserId_shouldReturnEmpty_whenNotFound() {
    ...
    StepVerifier.create(repository.findByUserId("no-user")).verifyComplete();
  }
  ...
  void findByActiveInstallsGreaterThan_shouldReturnProfilesWithMinInstalls() {
    ...
    when(repository.findByActiveInstallsGreaterThan(5))
        .thenReturn(Flux.fromIterable(List.of(p1, p2)));
  ...
  void findByActiveInstallsGreaterThan_shouldReturnEmpty_whenNoneMatch() {
    ...
    StepVerifier.create(repository.findByActiveInstallsGreaterThan(100)).verifyComplete();
  }
  ...
  void findByUserTier_shouldReturnEmpty_whenNoMatch() {
    ...
    StepVerifier.create(repository.findByUserTier(UserTier.ENTERPRISE)).verifyComplete();
  }
