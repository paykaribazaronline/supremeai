...
class UserApiKeyRepositoryTest {
  ...
  void findByApiKey_shouldReturnEmpty_whenNotFound() {
    ...
    StepVerifier.create(userApiKeyRepository.findByApiKey("sk-nonexistent")).verifyComplete();
  }
  ...
  void findByUserId_shouldReturnAllKeysForUser() {
    ...
    when(userApiKeyRepository.findByUserId("user-2"))
  ...
  void findByUserId_shouldReturnEmpty_whenNoKeys() {
    ...
    StepVerifier.create(userApiKeyRepository.findByUserId("user-no-keys")).verifyComplete();
  }
  ...
  void findByUserIdAndProvider_shouldReturnEmpty_whenNoMatch() {
    when(userApiKeyRepository.findByUserIdAndProvider("user-3", "Groq")).thenReturn(Flux.empty());
  ...
  void findByUserIdAndStatus_shouldReturnEmpty_whenNoMatchingStatus() {
    when(userApiKeyRepository.findByUserIdAndStatus("user-4", "error")).thenReturn(Flux.empty());
  ...
  void countByUserIdAndStatus_shouldReturnZero_whenNoneMatch() {
    when(userApiKeyRepository.countByUserIdAndStatus("user-6", "inactive"))
