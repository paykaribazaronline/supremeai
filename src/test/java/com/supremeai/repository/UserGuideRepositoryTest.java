...
class UserGuideRepositoryTest {
  ...
  @Mock private UserGuideRepository repository;

  @Test
  void findByIsPublished_shouldReturnPublishedGuides() {
    UserGuide g1 =
        new UserGuide("guide-1", Map.of("en", "Getting Started"), Map.of("en", "Intro guide"));
    g1.setIsPublished(true);
    UserGuide g2 =
        new UserGuide("guide-2", Map.of("en", "Advanced"), Map.of("en", "Advanced topics"));
  ...
  void findByIsPublished_shouldReturnEmpty_whenNoPublished() {
    ...
    StepVerifier.create(repository.findByIsPublished(true)).verifyComplete();
  }

  @Test
  void findByCategoryAndShouldReturnMatchingGuides() {
    UserGuide g =
        new UserGuide("guide-3", Map.of("en", "Security Guide"), Map.of("en", "Security tips"));
    ...
    StepVerifier.create(repository.findByCategoryAndIsPublished("security", true))
        .expectNextMatches(
            guide ->
                "security".equals(guide.getCategory())
                    && Boolean.TRUE.equals(guide.getIsPublished()))
        .verifyComplete();
  }
  ...
  void findByTagsContaining_shouldReturnEmpty_whenNoMatch() {
    ...
    StepVerifier.create(repository.findByTagsContaining("nonexistent")).verifyComplete();
  }
  ...
  void save_shouldPersistGuide() {
    UserGuide g =
        new UserGuide("guide-new", Map.of("en", "New Guide"), Map.of("en", "New description"));
