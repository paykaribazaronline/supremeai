...
class UserLanguagePreferenceRepositoryTest {
  ...
  @Mock private UserLanguagePreferenceRepository repository;
  ...
  void save_shouldPersistPreference() {
    ...
    StepVerifier.create(repository.save(pref))
        .expectNextMatches(
            p -> "fr".equals(p.getLanguageCode()) && "French".equals(p.getLanguageName()))
        .verifyComplete();
  }
  ...
  void delete_shouldRemovePreference() {
    ...
    StepVerifier.create(repository.deleteById("pref-delete")).verifyComplete();
  }
}
