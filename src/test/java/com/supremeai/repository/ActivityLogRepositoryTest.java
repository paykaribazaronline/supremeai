...
class ActivityLogRepositoryTest {
  ...
  void findByCategoryOrderByTimestampDesc_shouldReturnLogsForCategory() {
    ActivityLog log1 =
        new ActivityLog("LOGIN", "user1", "auth", "info", "User logged in", "success", "127.0.0.1");
    log1.setId("log-1");
    ActivityLog log2 =
        new ActivityLog(
            "LOGOUT", "user1", "auth", "info", "User logged out", "success", "127.0.0.1");
    log2.setId("log-2");
  ...
  void findBySeverityOrderByTimestampDesc_shouldReturnLogsForSeverity() {
    ActivityLog log =
        new ActivityLog(
            "ERROR", "user1", "system", "critical", "System failure", "failure", "127.0.0.1");
    log.setId("log-3");
    ...
    StepVerifier.create(activityLogRepository.findBySeverityOrderByTimestampDesc("critical"))
        .expectNextMatches(
            l -> "critical".equals(l.getSeverity()) && "failure".equals(l.getOutcome()))
        .verifyComplete();
  }
  ...
  void save_shouldPersistActivityLog() {
    ActivityLog log =
        new ActivityLog(
            "ACTION", "user2", "data", "warning", "Data modified", "success", "10.0.0.1");
  ...
  void findAll_shouldReturnAllLogs() {
    ...
    StepVerifier.create(activityLogRepository.findAll()).expectNextCount(2).verifyComplete();
  }
}
