...
public class TestFirebaseConfig {
  ...
  /**
   * Provides a mock GoogleCredentials bean for test environments. This prevents tests from
   * attempting to load actual service account keys or application default credentials, which might
   * not be available or desired in tests.
   */
  @Bean
  ...
  /**
   * Dynamically registers properties for Firebase emulator hosts during tests. This ensures tests
   * connect to local emulators (e.g., Firestore on 8081) and uses the correct project ID
   * supremeai-a.
   */
