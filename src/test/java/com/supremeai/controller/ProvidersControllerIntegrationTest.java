...
class ProvidersControllerIntegrationTest {
  ...
  void setUp() {
    providersController =
        new com.supremeai.controller.ProvidersController(providerAdminService, discoveryService);
  ...
  void testProviderKey_InvalidKey_ReturnsUnauthorized() {
    when(providerAdminService.validateKey("OpenAI", "bad-key")).thenReturn(Mono.just(false));
  ...
  void discoverModels_ReturnsModelList() {
    when(discoveryService.discoverModels(anyString()))
        .thenReturn(Flux.just(Map.of("model", "gpt-4")));
  ...
  void getHealthStats_ReturnsHealthData() {
    ...
    when(providerAdminService.getHealthStats())
        .thenReturn(
            Mono.just(
                Map.of(
                    "total", 2,
  ...
  void suggestRoles_ValidProvider_ReturnsRoles() {
    ...
    when(providerAdminService.suggestRoles(provider))
  ...
  void removeProvider_ValidRequest_Success() {
    ...
    ResponseEntity<ApiResponse<String>> result = providersController.cleanupDeadProviders().block();
