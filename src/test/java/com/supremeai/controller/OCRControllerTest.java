...
class OCRControllerTest {
  ...
  @Mock private NativeVisionService nativeVisionService;

  @Mock private VisionService visionService;

  @InjectMocks private OCRController ocrController;
  ...
  void setUp() {
    mockFile =
        new MockMultipartFile(
            "file", "test-doc.pdf", "application/pdf", "dummy content".getBytes());
  }
  ...
  void processOCR_NativeVisionSuccess_ExtractsText() {
    ...
    NativeVisionService.NativeVisionResult mockResult =
        mock(NativeVisionService.NativeVisionResult.class);
  ...
  void processOCR_FallbackLocalExtraction_EnglishText() {
    when(nativeVisionService.isModelLoaded()).thenReturn(false);
    MockMultipartFile englishFile =
        new MockMultipartFile("file", "image.png", "image/png", "dummy content".getBytes());
    ...
    StepVerifier.create(ocrController.processOCR(englishFile, "en"))
        .expectNextMatches(
            response -> {
              assertEquals(HttpStatus.OK, response.getStatusCode());
              assertTrue(
                  response.getBody().get("text").toString().contains("Sample extracted text"));
              return true;
            })
        .verifyComplete();
  }
