...
class SystemLearningControllerTest {
  ...
  @Mock private SystemLearningService learningService;

  @Mock private EnhancedLearningService enhancedService;

  @Mock private com.supremeai.service.CyberSecuritySkillService cyberSkillService;
  ...
  void getAllLearning_ReturnsAllLearnings() {
    SystemLearning learning2 =
        new SystemLearning("learn-2", "Another Topic", "code_generation", "Code content");
  ...
  void getByCategory_ValidCategory_ReturnsFilteredLearnings() {
    SystemLearning nlLearning =
        new SystemLearning("learn-3", "NLP Topic", "natural_language", "NLP content");
    when(learningService.getByCategory("natural_language"))
        .thenReturn(Flux.just(testLearning, nlLearning));
  ...
  void getSystemWisdom_ReturnsHighConfidenceLearnings() {
    ...
    when(learningService.getAllLearning()).thenReturn(Flux.just(highConf1, highConf2, lowConf));
  ...
  void getBestPractices_ValidCategory_ReturnsFilteredLearnings() {
    SystemLearning bp1 =
        new SystemLearning("bp1", "Best Practice 1", "code_generation", "Content 1");
    bp1.setQualityScore(0.95);
    bp1.setSuccess(true);

    SystemLearning bp2 =
        new SystemLearning("bp2", "Best Practice 2", "code_generation", "Content 2");
    bp2.setQualityScore(0.88);
    bp2.setSuccess(true);

    SystemLearning lowQuality =
        new SystemLearning("low-1", "Low Quality", "code_generation", "Content 3");
    lowQuality.setQualityScore(0.5);
    lowQuality.setSuccess(true);

    when(enhancedService.getBestPractices("code_generation", 0.8)).thenReturn(Flux.just(bp1, bp2));
  ...
  void getBestPractices_HighThreshold_ReturnsEmpty() {
    when(enhancedService.getBestPractices("any_category", 0.99)).thenReturn(Flux.empty());
  ...
  void triggerCyberResearch_ReturnsResearchResults() {
    ...
    when(cyberSkillService.initiateLearningCycle("SQL Injection"))
  ...
  void getCyberStatus_ReturnsCombinedStatus() {
    ...
    Map<String, Object> auditReport =
        Map.of("auditId", "audit-123", "vulnerabilitiesFound", 0, "resilienceScore", 0.99);
