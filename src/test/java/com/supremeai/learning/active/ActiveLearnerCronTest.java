...
/**
 * Unit tests for ActiveLearnerCron.
 * Tests the scheduled learning job that scrapes trending issues and records them.
 */
class ActiveLearnerCronTest {
    ...
    void testNightlyInternetLearning_invokesScraperAndKnowledgeBase() {
        ...
        var issues = java.util.Arrays.asList(
        ...
        verify(knowledgeBase, times(2)).recordSuccessWithPermission(anyString(), anyString(), anyString(), eq(100L), eq(0.9));
    }
    ...
    private ActiveInternetScraper.ScrapedIssue createMockIssue(String title, String solution, String source) {
    ...
    void testNightlyInternetLearning_multipleIssuesRecorded() {
        ...
        verify(scraper).scrapeTrendingIssues();
        verify(knowledgeBase, times(10)).recordSuccessWithPermission(anyString(), anyString(), anyString(), eq(100L), eq(0.9));
    }
    ...
    void testNightlyInternetLearning_knowledgeBaseErrorDoesNotPropagate() {
        ...
        when(scraper.scrapeTrendingIssues()).thenReturn(issues);
        doThrow(new RuntimeException("DB error")).when(knowledgeBase).recordSuccessWithPermission(anyString(), anyString(), anyString(), anyLong(), anyDouble());
        ...
        verify(knowledgeBase, times(1)).recordSuccessWithPermission(anyString(), anyString(), anyString(), anyLong(), anyDouble());
    }
