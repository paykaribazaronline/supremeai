...
/**
 * Unit tests for GlobalKnowledgeBase. Tests memory storage, retrieval, versioning, and scoring
 * logic.
 */
class GlobalKnowledgeBaseTest {
  ...
  @Mock private AdminDashboardService adminDashboard;

  @Mock private SolutionMemoryRepository solutionMemoryRepository;
  ...
  void setUp() {
    ...
    when(solutionMemoryRepository.save(any()))
  ...
  void testRecordSuccessWithPermission_newSolutionCreatesMemory() {
    ...
    knowledgeBase.recordSuccessWithPermission(errorSig, successfulCode, "openai", 50L, 0.9).block();
  ...
  void testGetSolutions_emptyListForUnknownSignature() {
    List<SolutionMemory> solutions =
        knowledgeBase.getSolutions("unknown_error").collectList().block();
  ...
  void testSaveSolutionMemory_noRepository_returnsMonoJust() {
    ...
    knowledgeBase.saveSolutionMemory(mem).subscribe(saved -> assertEquals(mem, saved));
