...
/**
 * Unit tests for EnhancedContentSanitizerService. Tests comprehensive PII detection, malicious code
 * scanning, quality scoring.
 */
class EnhancedContentSanitizerServiceTest {

  private EnhancedContentSanitizerService sanitizer;

  @Mock private CodeImmunitySystem mockImmunity;
  ...
  void testSanitizeAndValidate_nullSolution_returnsInvalid() {
    EnhancedContentSanitizerService.SanitizationResult result =
        sanitizer.sanitizeAndValidate(null, "test");
  ...
  void testSanitizeAndValidate_contentTooShort_rejected() {
    ...
    EnhancedContentSanitizerService.SanitizationResult result =
        sanitizer.sanitizeAndValidate(mem, "test");
    assertFalse(result.isValid());
  ...
  void testSanitizeAndValidate_maliciousPattern_detected() {
    ...
    assertFalse(result.isValid());
    assertTrue(
        result.getMessage().contains("Malicious") || result.getMessage().contains("malicious"));
  }
  ...
  void testSanitizeAndValidate_lowSecurityScoreFromUntrustedSource_rejected() {
    ...
    EnhancedContentSanitizerService.SanitizationResult result =
        sanitizer.sanitizeAndValidate(mem, "random-site.com");
  ...
  void testSanitizeAndValidate_lowQualityScore_rejected() {
    ...
    EnhancedContentSanitizerService.SanitizationResult result =
        sanitizer.sanitizeAndValidate(mem, "random.com");
    assertFalse(result.isValid());
  }
  ...
  void testSanitizeAndValidate_validAllChecksPass() {
    ...
    EnhancedContentSanitizerService.SanitizationResult result =
        sanitizer.sanitizeAndValidate(mem, "github.com");
  ...
  void testMaskPII_multiplePIIs() {
    String input =
        "Email: test@test.com, Token: ghp_1234567890123456789012345678901234567890, AWS: AKIAIOSFODNN7EXAMPLE12345";
    String masked = sanitizer.maskPII(input);
    ...
    assertTrue(
        (boolean)
            ReflectionTestUtils.invokeMethod(
                sanitizer, "isTrustedSource", "Official Documentation"));
    assertTrue(
        (boolean) ReflectionTestUtils.invokeMethod(sanitizer, "isTrustedSource", "wikipedia.org"));
    assertTrue(
        (boolean) ReflectionTestUtils.invokeMethod(sanitizer, "isTrustedSource", "github.com"));
    assertTrue(
        (boolean)
            ReflectionTestUtils.invokeMethod(sanitizer, "isTrustedSource", "stackoverflow.com"));
    assertTrue(
        (boolean) ReflectionTestUtils.invokeMethod(sanitizer, "isTrustedSource", "docs.spring.io"));
    assertFalse(
        (boolean)
            ReflectionTestUtils.invokeMethod(sanitizer, "isTrustedSource", "random-blog.com"));
  }
  ...
  void testContainsMaliciousPatterns_detectsEval() {
    ...
    assertTrue(
        (boolean) ReflectionTestUtils.invokeMethod(sanitizer, "containsMaliciousPatterns", code));
  }
  ...
  private double invokeCalculateSecurityScore(String code) {
    try {
      var method =
          EnhancedContentSanitizerService.class.getDeclaredMethod(
              "calculateSecurityScore", String.class);
  ...
  private double invokeCalculateQualityScore(String code, String source) {
    try {
      var method =
          EnhancedContentSanitizerService.class.getDeclaredMethod(
              "calculateQualityScore", String.class, String.class);
      method.setAccessible(true);
