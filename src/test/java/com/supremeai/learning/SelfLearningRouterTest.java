...
/**
 * Unit tests for SelfLearningRouter (Q-Learning based router). Tests route selection, reward
 * updates, and top-N retrieval.
 */
class SelfLearningRouterTest {
  ...
  void testRouteTask_initializesQValues() {
    ...
    SelfLearningRouter.RoutingDecision decision =
        router.routeTask("code_generation", "sig1", agents);
  ...
  void testRouteTask_returnsBestAgentBasedOnQValues() {
    ...
    try {
      ...
      java.util.Map<String, java.util.Map<String, Double>> qTable =
          (java.util.Map<String, java.util.Map<String, Double>>) qTableField.get(router);
      String state = "bug_fix:" + "sig2".hashCode();
      qTable.put(
          state,
          new java.util.concurrent.ConcurrentHashMap<>(Map.of("agentA", 10.0, "agentB", 1.0)));
    } catch (Exception e) {
      ...
      threadList[i] =
          new Thread(
              () -> {
                for (int j = 0; j < 100; j++) {
                  router.routeTask(
                      "concurrent_task_" + j, "sig" + index, List.of("agent1", "agent2"));
                }
              });
    }
