...
/**
 * Unit tests for GeneticAlgorithm. Tests population evolution, selection, crossover, and mutation.
 */
class GeneticAlgorithmTest {
  ...
  void testCrossover_producesChildFromParents() throws Exception {
    ...
    var crossoverMethod =
        GeneticAlgorithm.class.getDeclaredMethod("crossover", AgentConfig.class, AgentConfig.class);
    ...
    assertTrue(
        childDwVal >= 0.0 && childDwVal <= 1.0, "Child decisionWeight should be within [0,1]");
  }
  ...
  void testEvaluateFitness_returnsValueBetween0And1() throws Exception {
    ...
    var evaluateMethod =
        GeneticAlgorithm.class.getDeclaredMethod("evaluateFitness", AgentConfig.class);
  ...
  void testElitism_preservesTopTwo() throws Exception {
    ...
    assertTrue(
        val0 > 0.9 || val1 > 0.9,
        "Top agents from previous generation should be preserved via elitism");
  }
}
