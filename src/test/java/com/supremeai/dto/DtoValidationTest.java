 ...
 *
 * <p>Currently tests 25+ validation scenarios across 12 DTO types.
 */
class DtoValidationTest {
  ...
  @ParameterizedTest
  @CsvSource({"en, fr", "es, de", "zh, ja", "bn, en"})
  ...
  void testTranslationRequest_InvalidFromLanguage_Size(String lang) {
    ...
    Set<ConstraintViolation<TranslationRequest>> violations = validator.validate(dto);
    assertFalse(
        violations.isEmpty(), "Language code '" + lang + "' should violate size constraint");
  }
  ...
  void testApiKeyBulkRequest_NullKeyIds() {
    ...
    Set<ConstraintViolation<ApiKeyBulkRequest>> violations = validator.validate(dto);
    assertFalse(violations.isEmpty());
    assertTrue(violations.stream().anyMatch(v -> v.getPropertyPath().toString().equals("keyIds")));
  }
  ...
  void testApiKeyUsageRequest_BlankStartDate() {
    ...
    assertFalse(violations.isEmpty());
    assertTrue(
        violations.stream().anyMatch(v -> v.getPropertyPath().toString().equals("startDate")));
  }
