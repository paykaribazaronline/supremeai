...
class MultiAIConsensusServiceTest {
  ...
  @Mock private KnowledgeFeedbackService feedbackService;

  @Mock private ContextualAIRankingService contextualRankingService;

  @Mock private AIProvider mockProvider1;

  @Mock private AIProvider mockProvider2;
  ...
  void setUp() {
    ...
    try {
      java.lang.reflect.Field providerFactoryField =
          MultiAIConsensusService.class.getDeclaredField("providerFactory");
      providerFactoryField.setAccessible(true);
      providerFactoryField.set(service, providerFactory);

      java.lang.reflect.Field selfHealingField =
          MultiAIConsensusService.class.getDeclaredField("selfHealingService");
      selfHealingField.setAccessible(true);
      selfHealingField.set(service, selfHealingService);

      java.lang.reflect.Field feedbackField =
          MultiAIConsensusService.class.getDeclaredField("feedbackService");
      feedbackField.setAccessible(true);
      feedbackField.set(service, feedbackService);

      java.lang.reflect.Field rankingField =
          MultiAIConsensusService.class.getDeclaredField("contextualRankingService");
  ...
  void askAllAIs_shouldReturnConsensusResultWithMultipleProviders() {
    ...
    verify(contextualRankingService, times(2))
        .recordTaskOutcome(anyString(), any(), eq(true), anyLong(), anyDouble());
  }
    ...
    StepVerifier.create(result)
        .assertNext(
            consensus -> {
              assertEquals(question, consensus.getQuestion());
              assertEquals(
                  ThirdOpinionConstants.NO_PROVIDER_RESPONSE, consensus.getConsensusAnswer());
  ...
  void askAllAIs_shouldHandleTimeout() {
    ...
    when(selfHealingService.executeWithRetry(any(), anyInt(), anyLong()))
        .thenReturn(Mono.delay(java.time.Duration.ofMillis(200)).thenReturn("Answer"))
        .thenReturn(Mono.delay(java.time.Duration.ofMillis(200)).thenReturn("Answer"));
  ...
  void askContextualAIs_shouldSelectBestProvidersAutomatically() {
    ...
    when(contextualRankingService.selectBestProvider(question, null)).thenReturn(selection);

    when(contextualRankingService.getRankingsForTask(
            ContextualAIRankingService.TaskType.CODE_GENERATION))
        .thenReturn(
            Arrays.asList(
                new ContextualAIRankingService.ProviderRanking(
                    "openai", "CODE_GENERATION", 95.0, 500.0, 4.2, 100),
                new ContextualAIRankingService.ProviderRanking(
                    "anthropic", "CODE_GENERATION", 88.0, 600.0, 3.9, 80)));
    ...
    verify(contextualRankingService)
        .getRankingsForTask(ContextualAIRankingService.TaskType.CODE_GENERATION);
  }
  ...
  void calculateConsensus_shouldDetermineStrongConsensus() {
    ...
    try {
      java.lang.reflect.Method method =
          MultiAIConsensusService.class.getDeclaredMethod(
              "calculateConsensus", String.class, List.class, long.class);
      method.setAccessible(true);
      ConsensusResult result =
          (ConsensusResult) method.invoke(service, question, votes, totalTimeMs);
  ...
  void calculateConsensus_shouldHandleSingleVote() {
    ...
    List<ProviderVote> votes =
        Arrays.asList(new ProviderVote("openai", "Simple answer", 0.8, System.currentTimeMillis()));
