...
class AuthenticationServiceTest {
  ...
  void testRegister_Success() {
    try (MockedStatic<FirebaseAuth> firebaseAuthMock = mockStatic(FirebaseAuth.class)) {
      ...
      Mono<User> result =
          authenticationService.register("new@example.com", "password123", "New User", "127.0.0.1");

      StepVerifier.create(result)
          .expectNextMatches(
              u -> u.getFirebaseUid().equals("new-uid") && u.getEmail().equals("new@example.com"))
          .verifyComplete();
    }
  }
  ...
  void testFirebaseLogin_Success() {
    try (MockedStatic<FirebaseAuth> firebaseAuthMock = mockStatic(FirebaseAuth.class)) {
      ...
      Mono<Map<String, Object>> result =
          authenticationService.firebaseLogin("valid-token", "127.0.0.1");

      StepVerifier.create(result)
          .expectNextMatches(
              res -> res.get("token").equals("access-token") && res.get("status").equals("success"))
          .verifyComplete();
    }
  }
