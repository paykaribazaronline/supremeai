...
class AIProviderDiscoveryServiceTest {
  ...
  @Mock private WebClient.RequestHeadersSpec getHeaderSpec;

  @Mock private WebClient.ResponseSpec responseSpec;

  @InjectMocks private AIProviderDiscoveryService discoveryService;
  ...
  void testDiscoverModels_Success() {
    ...
    StepVerifier.create(result)
        .expectNextMatches(
            m -> "gpt2".equals(m.get("name")) && "huggingface".equals(m.get("provider")))
        .expectNextMatches(
            m ->
                "openai/gpt-3.5-turbo".equals(m.get("name"))
                    && "openrouter".equals(m.get("provider")))
        .verifyComplete();
  }
  ...
  void testValidateKey_Failure() {
    when(providerFactory.getProvider(anyString(), anyString()))
        .thenThrow(new RuntimeException("Invalid provider"));
