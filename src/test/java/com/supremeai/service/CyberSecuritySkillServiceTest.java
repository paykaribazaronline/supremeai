...
class CyberSecuritySkillServiceTest {
  ...
  @Mock private IntelligenceController intelligenceController;
  ...
  void constructor_InitializesWithCoreTechniques() {
    ...
    StepVerifier.create(skills).expectNextCount(2).verifyComplete();
  }
              ...
              assertTrue(
                  list.stream()
                      .allMatch(
                          s ->
                              s.get("severity") != null
                                  && (s.get("severity").equals("CRITICAL")
                                      || s.get("severity").equals("HIGH"))));
              return true;
            })
        .verifyComplete();
  }
  ...
  void getActiveProtections_AfterInit_ReturnsProtections() {
    ...
    StepVerifier.create(protections).verifyComplete();
  }
  ...
  void initiateLearningCycle_ValidTopic_CreatesTechniqueAndProtection() {
    Mono<Map<String, Object>> result =
        cyberSecuritySkillService.initiateLearningCycle("SQL Injection");
  ...
  void initiateLearningCycle_CreatesProtection() {
    ...
    StepVerifier.create(protections).expectNextCount(1).verifyComplete();
  }
