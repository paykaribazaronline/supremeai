...
class EnhancedLearningServiceTest {
  ...
  @Mock private SystemLearningRepository repository;

  @Mock private SystemLearningService systemLearningService;

  @InjectMocks private EnhancedLearningService enhancedLearningService;
  ...
  void setUp() {
    ...
    doAnswer(invocation -> Mono.just(invocation.getArgument(0)))
        .when(systemLearningService)
        .addLearning(any(SystemLearning.class));
  }
  ...
  void learnFromNLPInteraction_ValidInput_ReturnsSavedLearning() {
    ...
    Mono<SystemLearning> result =
        enhancedLearningService.learnFromNLPInteraction(
            "Write Python code", "Python is great", "gpt4", 0.9, Map.of("context", "test"));
  ...
  void learnFromNLPInteraction_NullQualityScore_SavesWithSuccessFalse() {
    ...
    Mono<SystemLearning> result =
        enhancedLearningService.learnFromNLPInteraction("Short", "Response", "provider", 0.5, null);

    StepVerifier.create(result).expectNextMatches(l -> !l.getSuccess()).verifyComplete();
  }
  ...
  void learnFromAPIUsage_SuccessfulCall_ReturnsSavedLearning() {
    ...
    Mono<SystemLearning> result =
        enhancedLearningService.learnFromAPIUsage(
            "/api/generate", "openai", 150L, true, Map.of("model", "gpt4"));
  ...
  void learnFromAPIUsage_FailedCall_SavesWithSuccessFalse() {
    ...
    Mono<SystemLearning> result =
        enhancedLearningService.learnFromAPIUsage("/api/generate", "openai", 5000L, false, null);
  ...
  void learnFromAppGeneration_NullBuildMetrics_UsesDefaultQuality() {
    ...
    StepVerifier.create(result).expectNextMatches(l -> l.getQualityScore() == 1.0).verifyComplete();
  }
  ...
  void learnPredictivePattern_ValidPattern_ReturnsSavedLearning() {
    ...
    Mono<SystemLearning> result =
        enhancedLearningService.learnPredictivePattern(
  ...
  void getBestPractices_ValidCategory_ReturnsFilteredFlux() {
    ...
    when(repository.findByCategory("code_generation")).thenReturn(Flux.just(bp1, bp2, lowQuality));
  ...
  void getPredictiveRecommendations_MatchingTask_ReturnsRecommendations() {
    ...
    Flux<SystemLearning> result =
        enhancedLearningService.getPredictiveRecommendations("code_generation", Map.of());
  ...
  void getPredictiveRecommendations_NoMatch_ReturnsEmptyFlux() {
    ...
    Flux<SystemLearning> result =
        enhancedLearningService.getPredictiveRecommendations("unknown", Map.of());
  ...
  void applyLearning_ExistingId_IncrementsTimesApplied() {
    ...
    StepVerifier.create(result).expectNextMatches(l -> l.getTimesApplied() == 6).verifyComplete();
  }
