...
class SelfHealingServiceTest {
  ...
  void executeWithRetry_shouldRetryOnFailureAndEventuallySucceed() {
    Supplier<Mono<String>> taskSupplier =
  ...
  void executeWithRetry_shouldLogReasoningOnFinalFailure() {
    Supplier<Mono<String>> taskSupplier =
        () -> Mono.error(new RuntimeException("Persistent failure"));

    StepVerifier.create(selfHealingService.executeWithRetry(taskSupplier, 2, 100))
        .expectError(RuntimeException.class)
        .verify();

    verify(reasoningService)
        .logReasoning(
            anyString(), eq("Execution Attempt Failed"), anyString(), eq("SelfHealingService"));
  }
  ...
  void analyzeError_shouldReturnCorrectActionForDependencyError() {
    ...
    try {
      java.lang.reflect.Method method =
          SelfHealingService.class.getDeclaredMethod("analyzeError", String.class);
      method.setAccessible(true);

      String result =
          (String) method.invoke(service, "Dependency resolution failed for some reason");
