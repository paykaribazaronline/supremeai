...
class AutoProviderDiscoveryServiceTest {
  ...
  @InjectMocks private AutoProviderDiscoveryService autoDiscoveryService;
        ...
        .expectNextMatches(
            provider ->
                provider.getId().startsWith("huggingface_")
                    && provider
                        .getName()
                        .equals("HuggingFace - meta-llama/Meta-Llama-3-8B-Instruct")
  ...
  void testScanForNewProviders_ResilientFallback() {
    ...
    when(responseSpec.bodyToFlux(Map.class))
        .thenReturn(Flux.error(new RuntimeException("API Down")));
