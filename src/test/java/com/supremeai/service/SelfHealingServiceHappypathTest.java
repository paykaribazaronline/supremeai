 ...
 *
 * Avoids Mockito mock-matching ambiguity by constructing real lightweight
 * RootCauseAnalysisService stubs (anonymous subclasses) and injecting them
 * via reflection. Only the knowledge-base side-effects are mocked.
 */
...
class SelfHealingServiceHappypathTest {
    ...
    void setUp() throws Exception {
        ...
        lenient().when(globalKnowledgeBase.recordSuccessWithPermission(
    ...
    /**
     * GIVEN RCA analyses returns canAutoFix=true and confidence=0.93 (> 0.8)
     * WHEN SelfHealingService.analyzeError() is called
     * THEN the response is success=true and carries the RCA analysis.
     */
    @Test
    void autoFixPath_returnsSuccessResponse() {
        SupremeAIResponse resp = shs.analyzeError(
                "Cannot invoke toString() on null reference",
                new NullPointerException("null"), ctx("x.toString()"));

        assert resp != null : "response must not be null";
        assert resp.isSuccess()
                : "Expected success=true when RCA.canAutoFix=true and confidence>0.8";
        assert resp.getMessage() != null && !resp.getMessage().isBlank()
                : "Success response must carry a non-empty message";
        assert resp.getRootCauseAnalysis() != null
                : "Success response must echo the RCA analysis";
    }

    /**
     * GIVEN RCA returns non-auto-fixable analysis with confidence=0.60 (> 0.5)
     * WHEN SelfHealingService.analyzeError() is called
     * THEN the response is non-success but carries RCA details and an explainer.
     */
    @Test
    void reviewPath_nonSuccessWithRcaDetails() {
        ...
        SupremeAIResponse resp = shs.analyzeError(
                ...
                new RuntimeException("cannot find symbol: class Foo"), ctx(""));
        ...
        assert resp.getRootCauseAnalysis() != null
                : "Non-success response must carry the RCA analysis";
    }

    /**
     * GIVEN RCA throws a runtime error
     * WHEN SelfHealingService.analyzeError() is called
     * THEN SHS catches the error, writes an unknown-error artifact to GKB,
     *  calls recordFailedCorrection() to learn from the RCA failure,
     *  and returns a non-success response.
     */
    @Test
    void rcaThrows_recordsFailureOnMlPredictor() {
        ...
        SupremeAIResponse resp = shs.analyzeError(
                "Something broke", new RuntimeException("rca-down"), new UserContext());
        ...
        assert recordFailedCalls.get() == 1
                : "Expected recordFailedCorrection() to be called once; was called " + recordFailedCalls.get();
    }

    /**
     * GIVEN userContext is null (production edge case)
     * WHEN SelfHealingService.analyzeError() is called
     * THEN SHS replaces null with "" for codeContext and does not throw.
     */
    @Test
    void nullUserContext_replaceWithEmptyCodeContext() {
        ...
        SupremeAIResponse resp = shs.analyzeError(
                "edge-case error", new RuntimeException("null-ctx"), null);
