...
class DatabaseSchemaMigrationServiceTest {
    ...
    void testRunAutoMigrations_PendingMigration_Success() throws ExecutionException, InterruptedException {
        // Mock RTDB showing no previous migration applied
        when(firebaseRealtimeService.getData("migration_status")).thenReturn(Mono.just(Collections.emptyMap()));
        ...
        when(writeBatch.update(any(DocumentReference.class), anyString(), any(), anyString(), any(), anyString(), any())).thenReturn(writeBatch);
        ...
        when(firebaseRealtimeService.setData(eq("migration_status/v2_firestore_applied"), any())).thenAnswer(invocation -> {
            latch.countDown();
            return Mono.empty();
        });
        ...
        verify(firebaseRealtimeService, times(1)).setData("migration_status/v2_firestore_applied", true);
    }

    @Test
    void testRunAutoMigrations_AlreadyApplied_NoAction() {
        // Mock RTDB showing migration already applied
        when(firebaseRealtimeService.getData("migration_status")).thenReturn(Mono.just(Map.of("v2_firestore_applied", true)));
