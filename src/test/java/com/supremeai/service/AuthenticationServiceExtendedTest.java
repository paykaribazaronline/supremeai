...
class AuthenticationServiceExtendedTest {
  ...
  void firebaseLogin_NewUser_CreatesUserAndReturnsTokens() throws Exception {
    try (MockedStatic<FirebaseAuth> firebaseAuthMock = mockStatic(FirebaseAuth.class)) {
      ...
      Mono<Map<String, Object>> result =
          authenticationService.firebaseLogin("valid-new-token", "127.0.0.1");
  ...
  void firebaseLogin_NewUser_AdminEmailBecomesAdmin() throws Exception {
    try (MockedStatic<FirebaseAuth> firebaseAuthMock = mockStatic(FirebaseAuth.class)) {
      ...
      Mono<Map<String, Object>> result =
          authenticationService.firebaseLogin("admin-token", "127.0.0.1");
  ...
  void firebaseLogin_ExistingUser_AdminClaimEscalatesToAdmin() throws Exception {
    try (MockedStatic<FirebaseAuth> firebaseAuthMock = mockStatic(FirebaseAuth.class)) {
      ...
      when(jwtUtil.generateAccessToken(anyString(), anyString()))
          .thenReturn("access-token-upgraded");
      when(jwtUtil.generateRefreshToken(anyString(), anyString()))
          .thenReturn("refresh-token-upgraded");
      lenient().when(configService.getConfig()).thenReturn(null);
      lenient()
          .when(activityLogRepository.save(any()))
          .thenReturn(Mono.just(new com.supremeai.model.ActivityLog()));
      ...
      Mono<Map<String, Object>> result =
          authenticationService.firebaseLogin("valid-existing-token", "127.0.0.1");
  ...
  void firebaseLogin_InvalidToken_FirebaseThrowsException() throws Exception {
    try (MockedStatic<FirebaseAuth> firebaseAuthMock = mockStatic(FirebaseAuth.class)) {
      ...
      Mono<Map<String, Object>> result =
          authenticationService.firebaseLogin("invalid-token", "127.0.0.1");
  ...
  void register_WithNullDisplayName_UsesEmailPrefix() throws Exception {
    try (MockedStatic<FirebaseAuth> firebaseAuthMock = mockStatic(FirebaseAuth.class)) {
      ...
      Mono<User> result =
          authenticationService.register("test@example.com", "SecurePass456", null, "127.0.0.1");
