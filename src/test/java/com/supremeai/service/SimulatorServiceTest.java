...
public class SimulatorServiceTest {
  ...
  @Mock private UserSimulatorProfileRepository profileRepository;
  ...
  @Mock private SimulatorDeploymentService deploymentService;
  ...
  void installApp_quotaAvailable_installsSuccessfully() {
    ...
    when(profileRepository.save(any(UserSimulatorProfile.class)))
        .thenReturn(Mono.just(savedProfile));
  ...
  void uninstallApp_appExists_removesSuccessfully() {
    ...
    StepVerifier.create(simulatorService.uninstallApp("user-123", "app-abc")).verifyComplete();
  ...
  void stopSession_activeSession_clearsSession() {
    ...
    StepVerifier.create(simulatorService.stopSession("user-123")).verifyComplete();
