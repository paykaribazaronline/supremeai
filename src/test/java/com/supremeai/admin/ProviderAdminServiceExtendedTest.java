...
class ProviderAdminServiceExtendedTest {
  ...
  @Mock private AIProviderDiscoveryService discoveryService;

  @Mock private ProviderRoleSuggestionService roleSuggestionService;

  @Mock private AdminProviderValidationService adminProviderValidationService;

  @Mock private ProviderTypeRegistry providerTypeRegistry;
  ...
  void addProvider_EmptyApiKey_ThrowsException() {
    ...
    when(discoveryService.validateKey(anyString(), eq(""))).thenReturn(Mono.just(false));
  ...
  void getHealthStats_AllActive_ReturnsFullHealth() {
    APIProvider p1 = new APIProvider();
    p1.setStatus("active");
    APIProvider p2 = new APIProvider();
    p2.setStatus("active");
    APIProvider p3 = new APIProvider();
    p3.setStatus("active");
  ...
  void getHealthStats_MixedStatus_CalculatesCorrectly() {
    ...
    APIProvider p3 = new APIProvider();
    p3.setStatus("dead");
    APIProvider p4 = new APIProvider();
  ...
  void suggestRoles_NullProvider_ReturnsEmptyOrHandlesGracefully() {
    when(roleSuggestionService.suggestRoles(null)).thenReturn(Collections.emptyList());
  ...
  void patchCapability_NonExistentProvider_ReturnsEmpty() {
    ...
    StepVerifier.create(
            providerAdminService.patchCapability(
                "nonexistent", Map.of("canCommunicate", true), "admin-1"))
        .verifyComplete();
  }
  ...
  void removeDeadProviders_OnlyDeadRemoved() {
    ...
    StepVerifier.create(providerAdminService.removeDeadProviders("admin-1")).verifyComplete();
