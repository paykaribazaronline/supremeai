...
public class AppLifecycleIntegrationTest {
  ...
  public void testCompleteUserAndProjectLifecycle() throws Exception {
    ...
    mockMvc
        .perform(get("/api/projects").header("Authorization", "Bearer " + authToken))
    ...
    mockMvc
        .perform(
            delete("/api/projects/" + projectId).header("Authorization", "Bearer " + authToken))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true));
  }
  ...
  public void testAdminWorkflow() throws Exception {
    ...
    // 1. Get plans
    mockMvc
        .perform(get("/api/admin/plans").header("Authorization", "Bearer " + adminToken))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
    ...
    mockMvc
        .perform(
            get("/api/admin/providers/rankings").header("Authorization", "Bearer " + adminToken))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
