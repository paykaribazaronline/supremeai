...
class AIProviderFactoryEnhancedTest {
  ...
  void setUp() throws Exception {
    ...
    when(mockService.getActiveKey(anyString()))
        .thenAnswer(
            inv -> {
    ...
    List<com.supremeai.model.APIProvider> providers =
    ...
    when(mockRepository.findByStatus(anyString()))
        .thenReturn(reactor.core.publisher.Flux.fromIterable(providers));
    ...
    com.supremeai.model.ProviderTypeConfig mockTypeConfig =
        Mockito.mock(com.supremeai.model.ProviderTypeConfig.class);
    ...
    com.github.benmanes.caffeine.cache.LoadingCache<String, Boolean> healthCache =
        (com.github.benmanes.caffeine.cache.LoadingCache<String, Boolean>)
            getField(factory, "providerHealthCache");
  ...
  void getBestProviderForTask_withRankingException_shouldFallBackToDefault() {
    when(mockRankingService.getRankingsForTask(any()))
        .thenThrow(new RuntimeException("Ranking error"));
  ...
  void providersCreatedByFactory_shouldHaveCorrectNames() {
    ...
    for (Map.Entry<String, String> entry : expectedProviders.entrySet()) {
      ...
      assertEquals(
          entry.getValue(), provider.getName(), "Provider name mismatch for: " + entry.getKey());
      if (entry.getValue().equals("stub-local")) {
        assertTrue(
            provider instanceof StubLocalProvider, "Provider type mismatch for: " + entry.getKey());
      } else {
        assertTrue(
            provider instanceof SupremeCloudProvider,
