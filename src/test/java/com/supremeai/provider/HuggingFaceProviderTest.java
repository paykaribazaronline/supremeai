...
class HuggingFaceProviderTest {
  ...
  void setUp() throws IOException {
    ...
    String baseUrl =
        mockWebServer
            .url("/models/meta-llama/Llama-3.3-70B-Instruct/v1/chat/completions")
            .toString();
    provider = new HuggingFaceProvider("hf-test-key");
    try {
      java.lang.reflect.Field field =
          HuggingFaceProvider.class.getSuperclass().getDeclaredField("baseUrl");
  ...
  void createRequestBody_shouldUseMessagesFormat() throws Exception {
    java.lang.reflect.Method method =
        HuggingFaceProvider.class.getDeclaredMethod("createRequestBody", String.class);
  ...
  void extractResponse_shouldParseChoicesResponse() throws Exception {
    String jsonResponse =
        objectMapper.writeValueAsString(
            Map.of(
                "choices",
                List.of(Map.of("message", Map.of("content", "Hello from HuggingFace!")))));

    java.lang.reflect.Method method =
        HuggingFaceProvider.class.getDeclaredMethod("extractResponse", String.class);
    method.setAccessible(true);
  ...
  void extractResponse_shouldReturnDefault_whenMessageNull() throws Exception {
    ...
    String jsonResponse =
        objectMapper.writeValueAsString(Map.of("choices", List.of(Map.of("message", responseMap))));
  ...
  void httpIntegration_shouldReturnGeneratedText() throws Exception {
    String responseBody =
        objectMapper.writeValueAsString(
            Map.of("choices", List.of(Map.of("message", Map.of("content", "Mocked HF response")))));
