...
class OllamaProviderTest {
  ...
  void setUp() throws IOException {
    ...
    try {
      java.lang.reflect.Field field =
          OllamaProvider.class.getSuperclass().getDeclaredField("baseUrl");
  ...
  void createRequestBody_shouldDisableStreaming() throws Exception {
    java.lang.reflect.Method method =
        OllamaProvider.class.getDeclaredMethod("createRequestBody", String.class);
  ...
  void extractResponse_shouldParseValidResponse() throws Exception {
    String jsonResponse =
        objectMapper.writeValueAsString(
            Map.of(
                "choices",
                java.util.List.of(Map.of("message", Map.of("content", "Hello from Ollama!")))));

    java.lang.reflect.Method method =
        OllamaProvider.class.getDeclaredMethod("extractResponse", String.class);
    method.setAccessible(true);
  ...
  void httpIntegration_shouldReturnGeneratedText() throws Exception {
    String responseBody =
        objectMapper.writeValueAsString(
            Map.of(
                "choices",
                java.util.List.of(Map.of("message", Map.of("content", "Mocked Ollama response")))));
  ...
  void httpIntegration_shouldHandleHttpError() throws Exception {
    mockWebServer.enqueue(
        new MockResponse().setResponseCode(404).setBody("{\"error\": \"Model not found\"}"));
