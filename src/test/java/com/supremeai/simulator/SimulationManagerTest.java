...
public class SimulationManagerTest {
  ...
  @Mock private SimulationScenarioRepository scenarioRepository;

  @Mock private SimulationResultRepository resultRepository;

  @Mock private ControllerEngine controllerEngine;

  @Mock private ResultAnalyzer resultAnalyzer;

  @InjectMocks private SimulationManager simulationManager;
  ...
  void createScenario_shouldSaveAndReturnScenario() {
    ...
    StepVerifier.create(
            simulationManager.createScenario(
                "user-1", "app-1", "Test Scenario", "Description", params))
        .expectNextMatches(
            savedScen -> {
  ...
  void executeSimulation_whenScenarioExists_shouldInitiateSimulation() {
    when(scenarioRepository.findById("scen-1")).thenReturn(Mono.just(scenario));
    when(resultRepository.save(any(SimulationResult.class)))
        .thenAnswer(
            invocation -> {
