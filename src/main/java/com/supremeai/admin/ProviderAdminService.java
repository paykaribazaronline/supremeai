...
/**
 * Service for managing AI providers. Extracted from ProvidersController to maintain 3-layer
 * architecture.
 */
@Service
public class ProviderAdminService {
    ...
    return validateKey(provider.getType(), provider.getApiKey())
        .flatMap(
            valid -> {
              if (!valid) {
                return Mono.error(
                    new IllegalArgumentException("Invalid API key or provider unreachable"));
              }
              ...
              log.info(
                  "[PROVIDER] Added provider {} with status ACTIVE (validation passed)",
                  provider.getName());
              return saveProviderWithLog(
                  provider,
                  adminUserId,
                  "ADD_PROVIDER",
                  "Added provider: " + provider.getName() + " [Status: ACTIVE]");
            });
  }
            ...
            existing -> {
              ...
              if (keyChanged) {
                return validateKey(provider.getType(), provider.getApiKey())
                    .flatMap(
                        valid -> {
                          ...
                          provider.setConsecutiveErrorDays(0);
                          provider.setLastValidated(LocalDateTime.now());
                          if ("dead".equals(existing.getStatus())
                              || "error".equals(existing.getStatus())
                              || "rotating".equals(existing.getStatus())) {
                            provider.setStatus("active");
                          ...
                          return saveProviderWithLog(
                              ...
                              adminUserId,
                              "UPDATE_PROVIDER",
                              "Updated provider with new key: " + provider.getName());
                        });
              } else {
                ...
                return saveProviderWithLog(
                    ...
                    adminUserId,
                    "UPDATE_PROVIDER",
                    "Updated provider: " + provider.getName());
              }
            });
  }
  ...
  public Mono<APIProvider> reviveProvider(String id, String adminUserId) {
    return providerRepository
        ...
        .flatMap(
            provider -> {
              if (!"dead".equals(provider.getStatus())
                  && !"error".equals(provider.getStatus())
                  && !"rotating".equals(provider.getStatus())) {
                return Mono.error(
                    new IllegalStateException("Provider is not dead, rotating, or in error state"));
              }
              provider.setStatus("active");
              provider.setConsecutiveErrorDays(0);
              provider.setDeadReason(null);
              ...
              return saveProviderWithLog(
                  ...
                  adminUserId,
                  "REVIVE_PROVIDER",
                  "Revived provider: " + provider.getName());
            });
  }
              ...
              return providerRepository
                  ...
                  .then(
                      logAction(
                          ...
                          "PROVIDER_MANAGEMENT",
                          "WARN",
                          "Deleted provider: " + id + " (" + providerName + ")"))
                  .then();
            });
  }
  ...
  public Mono<Boolean> validateKey(String type, String apiKey) {
    if (apiKey == null || type == null) {
      log.warn(
          "[VALIDATION] Rejected: apiKey is null={} or type is null={}",
          apiKey == null,
          type == null);
    ...
    return discoveryService
        .validateKey(type, apiKey)
        .timeout(java.time.Duration.ofSeconds(10))
        .doOnError(
            e ->
                log.error(
                    "[VALIDATION] Key validation FAILED for type={}, keyLength={}: {}",
                    type,
                    apiKey.length(),
                    e.toString()))
        .onErrorReturn(false);
    ...
    return providerRepository
        ...
        .flatMap(
            provider -> {
              ...
              log.info(
                  "[PROVIDER] Deactivated provider {} (reason: {})", provider.getName(), reason);
              return saveProviderWithLog(
                  provider,
                  adminUserId,
                  "DEACTIVATE_PROVIDER",
  ...
  public Mono<APIProvider> patchCapability(
      String id, Map<String, Object> updates, String adminUserId) {
    return providerRepository
        .findById(id)
        .flatMap(
            provider -> {
              ...
              return saveProviderWithLog(
                  ...
                  adminUserId,
                  "UPDATE_CAPABILITY",
                  "Updated capabilities for: " + provider.getName());
            });
  }
              ...
              return providerRepository
                  ...
                  .then(
                      logAction(
                          ...
                          "PROVIDER_MANAGEMENT",
                          "WARN",
                          "Deleted invalid/dead provider: " + id + " (" + name + ")"));
            })
        .then();
  }
            ...
            p -> {
              ...
              if ((oldName == null || oldName.trim().isEmpty())) {
                if (oldId != null && !oldId.trim().isEmpty()) {
                  ...
                  log.warn(
                      "Sanitizer: Found corrupted provider with both null ID and name. Deleting.");
              ...
              // Reset status to active if in error, rotating, null, or inactive to allow immediate
              // availability
              if (p.getStatus() == null
                  || "inactive".equals(p.getStatus())
                  || "error".equals(p.getStatus())
                  || "rotating".equals(p.getStatus())) {
                p.setStatus("active");
                changed = true;
              }

              // Auto-detect deploymentSource based on type and accountEmail
              if (p.getDeploymentSource() == null
                  || p.getDeploymentSource().isBlank()
                  || !isValidDeploymentSource(p.getDeploymentSource())) {
                String detectedSource =
                    detectDeploymentSource(p.getType(), p.getAccountEmail(), p.getBaseUrl());
                if (!detectedSource.equalsIgnoreCase(p.getDeploymentSource())) {
                  p.setDeploymentSource(detectedSource);
                  changed = true;
                  log.info(
                      "Sanitizer: Auto-set deploymentSource for {} from type={}, accountEmail={} -> {}",
                      p.getName(),
                      p.getType(),
                      p.getAccountEmail(),
                      detectedSource);
              ...
              if (p.getApiKey() != null
                  && (p.getApiKey().contains("{") || p.getApiKey().contains("="))) {
                log.warn(
                    "Sanitizer: Detected potentially corrupted API key format for {}. Attempting to extract real key...",
                    p.getName());
                ...
                if (extractedKey != null && !extractedKey.isEmpty()) {
                  ...
                  p.setStatus("active");
                  log.info(
                      "Sanitizer: Successfully extracted and updated API key to: {} for {}",
                      extractedKey,
                      p.getName());
                } else {
  ...
  /**
   * Infer a provider type category from the provider name. This is a lightweight heuristic used
   * only during sanitization of existing records that are missing a typeId. The authoritative
   * source for provider type config is the Firestore {@code provider_types} collection accessed via
   * {@link ProviderTypeRegistry}.
   */
    ...
    for (ProviderTypeConfig config : allTypes.values()) {
      ...
      if (n.contains(config.getTypeId().toUpperCase())
          || (config.getDisplayName() != null
              && n.contains(config.getDisplayName().toUpperCase()))) {
        return config.getTypeId();
      }
    }
  ...
  /**
   * Resolve the default model for a provider type from the dynamic registry. Never returns a
   * hardcoded brand/model ID — falls back to {@code unknown-model} if the type has no entry in
   * {@code provider_types}.
   */
  ...
  private String detectDeploymentSource(String type, String accountEmail, String baseUrl) {
    ...
    if (type != null) {
      ...
      if (upperType.equals("GOOGLE")
          || upperType.equals("GEMINI")
          || upperType.equals("VERTEX")
          || upperType.equals("CLOUD_RUN")) {
        return "gcloud";
      }
  ...
  private Mono<APIProvider> saveProviderWithLog(
      APIProvider provider, String userId, String action, String details) {
    Mono<APIProvider> savedMono = providerRepository.save(provider);
    if (savedMono == null) {
      return Mono.just(provider);
    }
    return savedMono.flatMap(
        saved ->
            logAction(userId, action, "PROVIDER_MANAGEMENT", "INFO", details).thenReturn(saved));
  }

  private Mono<ActivityLog> logAction(
      String userId, String action, String category, String severity, String details) {
