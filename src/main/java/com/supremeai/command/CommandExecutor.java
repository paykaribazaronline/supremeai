 ...
 *
 * <p>Maintains list of all commands and executes them with proper: - Permission checking -
...
public class CommandExecutor {
  ...
  public CommandResult execute(
      String commandName, Map<String, Object> params, CommandContext context) {
    long startTime = System.currentTimeMillis();

    try {
      // 1. Check if command exists
      if (!commands.containsKey(commandName)) {
        return CommandResult.error(commandName, "NOT_FOUND", "Command not found: " + commandName);
      }
      ...
      for (String required : command.getRequiredPermissions()) {
        if (!context.hasPermission(required)) {
          logger.warn(
              "❌ Permission denied for user {} to run {}", context.getUsername(), commandName);
      ...
      return CommandResult.error(
          commandName, "EXECUTION_ERROR", "Command execution failed: " + e.getMessage());
    }
  }
