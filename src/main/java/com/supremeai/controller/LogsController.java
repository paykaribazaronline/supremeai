    ...
    if (severity != null && !severity.isEmpty()) {
      return wrapList(
          activityLogRepository
              .findBySeverityOrderByTimestampDesc(severity.toUpperCase())
              .take(limit),
          "logs");
    } else if (category != null && !category.isEmpty()) {
      return wrapList(
          activityLogRepository
              .findByCategoryOrderByTimestampDesc(category.toUpperCase())
              .take(limit),
          "logs");
    } else {
            ...
            e ->
                Mono.just(
                    ResponseEntity.status(500)
                        .body(ApiResponse.error("Failed to clear logs: " + e.getMessage()))));
  }
}
