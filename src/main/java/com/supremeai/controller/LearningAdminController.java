...
public class LearningAdminController {
  ...
  @Autowired
  public LearningAdminController(
      LearningModeControl modeControl,
  ...
  public ResponseEntity<Map<String, Object>> getStatus() {
    ...
    response.put(
        ...
        configService.getEffectiveSetting("learning_interval_minutes", 60L));
  ...
  public ResponseEntity<Map<String, Object>> updateInterval(@RequestBody Map<String, Object> body) {
    ...
    configService
        .updateSetting("learning_interval_minutes", interval)
        .subscribe(); // Fire and forget update
    return ResponseEntity.ok(Map.of("status", "success", "interval", interval));
  }

  /** Set learning mode (AGGRESSIVE, BALANCED, MANUAL, PAUSED). POST { "mode": "AGGRESSIVE" } */
  @PostMapping("/mode")
  public ResponseEntity<Map<String, String>> setMode(@RequestBody Map<String, String> body) {
    ...
    try {
      LearningModeControl.LearningMode mode =
          LearningModeControl.LearningMode.valueOf(modeStr.toUpperCase());
      ...
      return ResponseEntity.badRequest()
          .body(Map.of("error", "Invalid mode. Use: AGGRESSIVE, BALANCED, MANUAL, PAUSED"));
    }
  }
  ...
  public ResponseEntity<Map<String, String>> resume() {
    ...
    return ResponseEntity.ok(
        Map.of(
            ...
            "resumed",
            "message",
            "Learning resumed to previous mode: " + modeControl.getCurrentMode()));
  }
  ...
  public ResponseEntity<Map<String, Object>> updateQuota(@RequestBody QuotaUpdateRequest request) {
    ...
    return ResponseEntity.ok(Map.of("status", "updated", "quota", quotaService.getQuotaStats()));
  }
    ...
    if (!modeControl.allowManualTrigger()) {
      return ResponseEntity.status(403)
          .body(
              Map.of(
                  "error",
                  "Manual trigger not allowed in current mode: " + modeControl.getCurrentMode()));
    }
  ...
  /** GET /api/admin/learning/sources Returns all registered learning sources. */
  @GetMapping("/sources")
  public ResponseEntity<List<Map<String, Object>>> getAllSources() {
    List<Map<String, Object>> result =
        sourceRepository.findAll().map(this::toSourceMap).collectList().block(BLOCK_TIMEOUT);
    return ResponseEntity.ok(result != null ? result : List.of());
  }

  /**
   * POST /api/admin/learning/sources Add a new learning source. Auto-detects topic from URL. Body:
   * { "url": "https://example.com", "manualFocus": "marketing", "priority": 5, "notes": "..." }
   */
  @PostMapping("/sources")
  public ResponseEntity<Map<String, Object>> addSource(@RequestBody Map<String, Object> body) {
    ...
    if (url == null || url.isBlank()) {
      return ResponseEntity.badRequest()
          .body(Map.of("status", "error", "message", "URL is required"));
    }
    ...
    try {
      ...
      return ResponseEntity.badRequest()
          .body(Map.of("status", "error", "message", "Invalid URL format"));
    }
    ...
    if (exists) {
      return ResponseEntity.badRequest()
          .body(
              Map.of("status", "error", "message", "This URL is already in the learning sources"));
    }
    ...
    source.setPriority(
        body.get("priority") != null ? ((Number) body.get("priority")).intValue() : 5);
    ...
    return ResponseEntity.ok(
        Map.of(
            ...
            "success",
            "message",
            "Learning source added. Detected focus: " + saved.getEffectiveFocus(),
            "source",
            toSourceMap(saved)));
  }

  /**
   * POST /api/admin/learning/sources/detect-focus Preview what focus the system would detect for a
   * URL (before adding). Body: { "url": "https://example.com" }
   */
  ...
  /** DELETE /api/admin/learning/sources/{id} Remove a learning source. */
  ...
  /** POST /api/admin/learning/sources/{id}/toggle Enable or disable a learning source. */
  ...
  public static class QuotaUpdateRequest {
    ...
    public Integer getPerUserDailyMax() {
      return perUserDailyMax;
    }

    public void setPerUserDailyMax(Integer perUserDailyMax) {
      this.perUserDailyMax = perUserDailyMax;
    }

    public Integer getGlobalDailyMax() {
      return globalDailyMax;
    }

    public void setGlobalDailyMax(Integer globalDailyMax) {
