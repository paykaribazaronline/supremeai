...
public class OCRController {
            ...
            () -> {
              ...
              try {
                if (nativeVisionService != null && nativeVisionService.isModelLoaded()) {
                  ...
                  NativeVisionService.NativeVisionResult nativeResult =
                      nativeVisionService
                          .processImageNative(
                              base64, NativeVisionService.VisionTaskType.TEXT_EXTRACTION)
  ...
  public Mono<ResponseEntity<Map<String, Object>>> exportResult(
      @PathVariable String id, @RequestParam(defaultValue = "json") String format) {
    return Mono.fromCallable(
            () -> {
              OCRResult result = historyStore.get(id);
              if (result == null) {
                return ResponseEntity.ok(
                    Map.of("success", true, "data", Map.of("text", "no data")));
              }
              return ResponseEntity.ok(
                  Map.of(
                      "success",
    ...
    return Mono.just(
        ResponseEntity.ok(
            Map.of(
                "status", "UP",
                "native_ocr",
                    nativeVisionService != null && nativeVisionService.isModelLoaded()
                        ? "READY"
                        : "FALLBACK",
