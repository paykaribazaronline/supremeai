...
public class TranslationController {
  ...
  public Mono<ResponseEntity<TranslationResponse>> translate(
    ...
    return translationService
        .translate(request.getText(), request.getFromLanguage(), request.getToLanguage())
  ...
  public Mono<ResponseEntity<TranslationResponse>> translateFromEnglish(
      @Valid @RequestBody TranslationRequest request) {
    return translationService
        .translateFromEnglish(request.getText(), request.getToLanguage())
        .map(translatedText -> ResponseEntity.ok(new TranslationResponse(translatedText, true)))
        .onErrorReturn(ResponseEntity.badRequest().build());
  }
  ...
  public Mono<ResponseEntity<TranslationResponse>> translateToEnglish(
      @Valid @RequestBody TranslationRequest request) {
    return translationService
        .translateToEnglish(request.getText(), request.getFromLanguage())
        .map(translatedText -> ResponseEntity.ok(new TranslationResponse(translatedText, true)))
        .onErrorReturn(ResponseEntity.badRequest().build());
  }
