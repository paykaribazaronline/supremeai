 ...
 *
 * <p>Endpoints: - GET /api/admin/tasks - List all task assignments - GET /api/admin/tasks/:taskType
 * - Get assignment for specific task - POST /api/admin/tasks/:taskType/assign - Manually assign
 * providers to task - POST /api/admin/tasks/:taskType/unassign - Remove provider from task - POST
 * /api/admin/tasks/rebalance - Trigger rebalance - GET /api/admin/providers/:id/capabilities - Get
 * provider capability scores - POST /api/admin/providers/:id/benchmark - Trigger benchmark
 */
@RestController
...
public class TaskAssignmentController {
  ...
  @Autowired private TaskProviderAssignmentRepository assignmentRepo;

  @Autowired private ProviderRepository providerRepo;

  @Autowired private AutomaticTaskAssigner taskAssigner;

  @Autowired private ProviderCapabilityAnalyzer capabilityAnalyzer;
  ...
  public Mono<ResponseEntity<Map<String, Object>>> getStats() {
    return taskAssigner.getAssignmentStats().map(stats -> ResponseEntity.ok(stats));
  }

  /** Get assignment for a specific task */
  @GetMapping("/{taskType}")
  public Mono<ResponseEntity<TaskProviderAssignment>> getAssignment(@PathVariable String taskType) {
  ...
  public Mono<ResponseEntity<Map<String, Object>>> assignProviders(
      @PathVariable String taskType, @Valid @RequestBody AssignmentRequest request) {
            ...
            updated -> {
              ...
              response.put(
                  ...
                  "Task '"
                      + taskType
                      + "' updated with "
                      + request.getProviderIds().size()
                      + " providers");
              response.put("assignment", updated);
              log.info(
                  "✏️ Admin manually assigned {} providers to task '{}'",
                  request.getProviderIds().size(),
                  taskType);
  ...
  public Mono<ResponseEntity<Map<String, Object>>> autoAssign(@PathVariable String taskType) {
              ...
              return taskAssigner
                  ...
                  .then(
                      Mono.fromCallable(
                          () -> {
                            response.put(
                                "message", "Auto-assignment triggered for task '" + taskType + "'");
                            response.put("success", true);
                            return ResponseEntity.ok(response);
                          }));
            });
    ...
    return capabilityAnalyzer
        ...
        .map(
            scores -> {
              ...
              log.info("📊 Manual benchmark complete for {}: {} scores", providerId, scores.size());
  ...
  public static class AssignmentRequest {
    ...
    public void setMinProviders(Integer minProviders) {
      this.minProviders = minProviders;
    }

    public Integer getMaxProviders() {
      return maxProviders;
    }

    public void setMaxProviders(Integer maxProviders) {
