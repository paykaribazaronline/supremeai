...
public class BenchmarkController {
  ...
  @Autowired private AIValidationHarnessService harnessService;

  @Autowired private SWEBenchValidationService sweBenchService;

  @Autowired private BenchmarkResultRepository benchmarkResultRepository;

  /**
   * BV-05: Public endpoint for transparent AI quality reporting. Returns the latest benchmark
   * results.
   */
  @GetMapping("/public")
        ...
        .map(
            result -> {
              Map<String, Object> publicReport =
                  Map.of(
                      "supremeAIAccuracy",
                      result.getSupremeAiAccuracy() + "%",
                      "topBaseline",
                      result.getTopProviderName() + " (" + result.getTopProviderAccuracy() + "%)",
                      "lastRun",
                      result.getTimestamp(),
                      "status",
                      "Live");
  ...
  /**
   * BV-03: Self-Ranking Dashboard Data. Returns the accuracy of SupremeAI vs each provider on
   * recent queries.
   */
  ...
  public Mono<ResponseEntity<Map<String, Object>>> getRankingDashboard() {
    return benchmarkResultRepository
        ...
        .map(
            results -> {
              Map<String, Double> providerRates =
                  results.stream()
                      .collect(Collectors.toMap(r -> r.getProviderName(), r -> r.getAccuracy()));
  ...
  @PreAuthorize("hasRole('ADMIN')")
  public Mono<ResponseEntity<Map<String, Object>>> triggerSweBench(
      @RequestBody Map<String, List<String>> request) {
    List<String> baselines =
        request.getOrDefault("providers", List.of("openai", "anthropic", "google"));
    return sweBenchService.runSweBenchSuite(baselines).map(ResponseEntity::ok);
  }
}
