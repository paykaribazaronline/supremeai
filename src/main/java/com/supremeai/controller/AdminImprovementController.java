  ...
  public Mono<ResponseEntity<ApiResponse<Map<String, Object>>>> getPendingImprovements() {
    return adminDashboardService
        ...
        .map(
            pending ->
                ResponseEntity.ok(
                    ApiResponse.ok(Map.of("pending", pending, "count", pending.size()))));
  }
            ...
            success -> {
              if (success) {
                return ResponseEntity.ok(
                    ApiResponse.ok(Map.of("status", "approved", "proposalId", proposalId)));
              } else {
                return ResponseEntity.status(404)
                    .body(
                        ApiResponse.error("Proposal not found", Map.of("proposalId", proposalId)));
              }
            });
  }
    ...
    return adminDashboardService
        ...
        .map(
            success -> {
              if (success) {
                return ResponseEntity.ok(
                    ApiResponse.ok(Map.of("status", "rejected", "proposalId", proposalId)));
              } else {
                return ResponseEntity.status(404)
                    .body(
                        ApiResponse.error("Proposal not found", Map.of("proposalId", proposalId)));
