...
public class AnalysisController {
  ...
  public Mono<ResponseEntity<ApiResponse<AnalysisResponse>>> runAnalysis(
      ...
      @RequestParam(value = "projectType", required = false, defaultValue = "generic")
          String projectType,
      @RequestParam(value = "zipFile", required = false) MultipartFile zipFile,
      @RequestParam(value = "ragEnabled", required = false, defaultValue = "false")
          boolean ragEnabled,
      @RequestParam(value = "incrementalEnabled", required = false, defaultValue = "false")
          boolean incrementalEnabled,
      @RequestParam(value = "fixesEnabled", required = false, defaultValue = "true")
          boolean fixesEnabled,
      @RequestParam(value = "projectId", required = false) String projectId,
      @RequestParam(value = "baselineCommit", required = false) String baselineCommit,
      @RequestParam Map<String, String> allParams) {
    log.info(
        "[Analysis] Received analysis request - gitUrl: {}, hasZip: {}, rag: {}, incremental: {}, fixes: {}",
        gitUrl,
        zipFile != null,
        ragEnabled,
        incrementalEnabled,
            ...
            () -> {
              try {
                ...
                return ResponseEntity.status(500)
                    .body(
                        ApiResponse.<AnalysisResponse>error("Analysis failed: " + e.getMessage()));
              }
            })
  ...
  @GetMapping("/{jobId}")
  public Mono<ResponseEntity<ApiResponse<AnalysisResponse>>> getJobStatus(
            ...
            e -> {
              ...
              return Mono.just(
                  ResponseEntity.status(500)
                      .body(
                          ApiResponse.<AnalysisResponse>error(
  ...
  public Mono<ResponseEntity<ApiResponse<List<AnalysisFix>>>> getFixes(@PathVariable String jobId) {
    return fixSuggestionService
        .getFixesForJob(jobId)
        .collectList()
  ...
  public Mono<ResponseEntity<ApiResponse<AnalysisFix>>> applyFix(
    ...
    return fixSuggestionService
        .applyFix(jobId, fixId)
        .map(fix -> ResponseEntity.ok(ApiResponse.ok(fix)))
        .onErrorResume(
            e -> {
              log.error(
                  "[Analysis] Error applying fix {} for job {}: {}", fixId, jobId, e.getMessage());
              return Mono.just(
                  ResponseEntity.status(500)
                      .body(
                          ApiResponse.<AnalysisFix>error(
  ...
  public Mono<ResponseEntity<ApiResponse<String>>> clearCache(@PathVariable String projectId) {
    return projectAnalysisService
        ...
        .then(
            Mono.just(ResponseEntity.ok(ApiResponse.ok("Cache cleared for project: " + projectId))))
        .onErrorResume(
            e -> {
              log.error(
                  "[Analysis] Error clearing cache for project {}: {}", projectId, e.getMessage());
              return Mono.just(
                  ResponseEntity.status(500)
  ...
  public Mono<ResponseEntity<ApiResponse<Map<String, Object>>>> health() {
    ...
    health.put(
        "agents",
        Map.of("SECURITY", true, "QUALITY", true, "DEPENDENCIES", true, "ARCHITECTURE", true));
    health.put("phase", "3");
