...
public class ProvidersController extends BaseAdminController<APIProvider, String> {
  ...
  public ProvidersController(
      ProviderAdminService providerAdminService, AIProviderDiscoveryService discoveryService) {
  ...
  @PostMapping("/add")
  public Mono<ResponseEntity<ApiResponse<Map<String, Object>>>> addProvider(
      @RequestBody APIProvider provider) {
    return getCurrentAdminUserId()
        .flatMap(
            adminUserId ->
                providerAdminService
                    .addProvider(provider, adminUserId)
                    .map(
                        saved ->
                            ResponseEntity.ok(
                                ApiResponse.ok(
                                    Map.of(
                                        "message", "Provider added", "provider", (Object) saved)))))
        .onErrorResume(
            e ->
                Mono.just(
  ...
  @PutMapping("/{id}")
  public Mono<ResponseEntity<ApiResponse<Map<String, Object>>>> updateProviderById(
      @PathVariable String id, @RequestBody APIProvider provider) {
    return getCurrentAdminUserId()
        .flatMap(
            adminUserId ->
                providerAdminService
                    .updateProvider(id, provider, adminUserId)
                    .map(
                        saved -> {
                          if (saved == null)
                            return ResponseEntity.status(404)
                                .body(ApiResponse.<Map<String, Object>>error("Provider not found"));
                          return ResponseEntity.ok(
                              ApiResponse.ok(
                                  Map.of(
                                      "message", "Provider updated", "provider", (Object) saved)));
                        }))
        .onErrorResume(
            e ->
                Mono.just(
  ...
  public Mono<ResponseEntity<ApiResponse<Map<String, Object>>>> reviveProvider(
      @PathVariable String id) {
    return getCurrentAdminUserId()
        .flatMap(
            adminUserId ->
                providerAdminService
                    .reviveProvider(id, adminUserId)
                    .map(
                        saved -> {
                          if (saved == null)
                            return ResponseEntity.status(404)
  ...
  public Mono<ResponseEntity<ApiResponse<Map<String, Object>>>> activateProvider(
      @PathVariable String id) {
    return getCurrentAdminUserId()
        .flatMap(
            adminUserId ->
                providerAdminService
                    .activateProvider(id, adminUserId)
                    .map(
                        saved -> {
                          if (saved == null)
                            return ResponseEntity.status(404)
  ...
  public Mono<ResponseEntity<ApiResponse<Map<String, Object>>>> deactivateProvider(
    ...
    return getCurrentAdminUserId()
        .flatMap(
            adminUserId ->
                providerAdminService
                    .deactivateProvider(id, reason, adminUserId)
                    .map(
                        saved -> {
                          if (saved == null)
                            return ResponseEntity.status(404)
                                .body(ApiResponse.<Map<String, Object>>error("Provider not found"));
                          log.info(
                              "[API] Provider {} deactivated by admin {} (reason: {})",
                              id,
                              adminUserId,
                              reason);
                          return ResponseEntity.ok(
  ...
  public Mono<ResponseEntity<ApiResponse<String>>> deleteProvider(@PathVariable String id) {
    return getCurrentAdminUserId()
        .flatMap(
            adminUserId ->
                providerAdminService
                    .deleteProvider(id, adminUserId)
                    .thenReturn(ResponseEntity.ok(ApiResponse.ok("Provider deleted"))))
        .onErrorResume(
            e ->
                Mono.just(
                    ResponseEntity.status(500).body(ApiResponse.<String>error(e.getMessage()))));
  }
  ...
  public Mono<ResponseEntity<ApiResponse<Map<String, Object>>>> testProviderKey(
    ...
    log.info(
        "[TEST-KEY] Received test-key request: name={}, apiKeyLength={}",
        name,
        apiKey != null ? apiKey.length() : "null");

    if (name == null || apiKey == null) {
      log.warn(
          "[TEST-KEY] Rejected: missing parameters. name={}, apiKey present={}",
          name,
          apiKey != null);
      return Mono.just(
          ResponseEntity.badRequest().body(ApiResponse.error("name and apiKey are required")));
    }

    return providerAdminService
        .validateKey(name, apiKey)
                    ...
                    ? ResponseEntity.ok(
                        ApiResponse.<Map<String, Object>>ok(
                            Map.<String, Object>of(
                                "message", "Key validated successfully", "valid", true)))
                    : ResponseEntity.status(401)
                        .body(
                            ApiResponse.<Map<String, Object>>error(
                                "Invalid key or provider unreachable")))
        .onErrorResume(
            e -> {
              log.error(
                  "[TEST-KEY] Unexpected error during validation for name={}: {}",
                  name,
                  e.toString());
              return Mono.just(
        ...
        .onErrorResume(
            e ->
                Mono.just(
                    ResponseEntity.status(500)
                        .body(
                            ApiResponse.<List<Map<String, Object>>>error(
  ...
  @GetMapping("/{id}/roles")
  public Mono<ResponseEntity<ApiResponse<List<String>>>> getConfiguredRoles(
      @PathVariable String id) {
    return providerAdminService
        .getAllProviders()
                ...
                () ->
                    Mono.just(
                        ResponseEntity.status(404)
                            .body(ApiResponse.<List<String>>error("Provider not found: " + id)))))
        .onErrorResume(
            e ->
                Mono.just(
                    ResponseEntity.status(500)
  ...
  public Mono<ResponseEntity<ApiResponse<Map<String, Object>>>> updateCapability(
    ...
    return getCurrentAdminUserId()
        .flatMap(
            adminUserId ->
                providerAdminService
                    .patchCapability(id, updates, adminUserId)
                    .map(
                        saved -> {
                          if (saved == null)
                            return ResponseEntity.status(404)
  ...
  public Mono<ResponseEntity<ApiResponse<String>>> sanitizeProviders() {
    return getCurrentAdminUserId()
        .flatMap(
            adminUserId ->
                providerAdminService
                    .sanitizeProviders(adminUserId)
                    .then(
                        Mono.just(
                            ResponseEntity.ok(ApiResponse.ok("Provider sanitization completed")))))
        .onErrorResume(
            e ->
                Mono.just(
                    ResponseEntity.status(500).body(ApiResponse.<String>error(e.getMessage()))));
  ...
  public Mono<ResponseEntity<ApiResponse<String>>> cleanupDeadProviders() {
    return getCurrentAdminUserId()
        .flatMap(
            adminUserId ->
                providerAdminService
                    .removeDeadProviders(adminUserId)
