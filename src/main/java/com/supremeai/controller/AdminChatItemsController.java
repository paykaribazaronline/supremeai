...
public class AdminChatItemsController {
  ...
  public Mono<ResponseEntity<Map<String, Object>>> getRules(
    ...
    return chatProcessingService
        .getRules(active_only)
        .map(rules -> ResponseEntity.ok(Map.of("success", true, "rules", rules)));
  }
  ...
  public Mono<ResponseEntity<Map<String, Object>>> getRule(@PathVariable String id) {
    return chatProcessingService
        .getItemById("rule", id)
        .map(rule -> ResponseEntity.ok(Map.of("success", true, "item", rule)))
        .switchIfEmpty(
            Mono.just(
                ResponseEntity.status(404)
                    .body(Map.of("success", false, "message", "Rule not found"))));
  }
  ...
  public Mono<ResponseEntity<Map<String, Object>>> getPlan(@PathVariable String id) {
    return chatProcessingService
        .getItemById("plan", id)
        .map(plan -> ResponseEntity.ok(Map.of("success", true, "item", plan)))
        .switchIfEmpty(
            Mono.just(
                ResponseEntity.status(404)
                    .body(Map.of("success", false, "message", "Plan not found"))));
  }
  ...
  public Mono<ResponseEntity<Map<String, Object>>> getCommands(
      @RequestParam(defaultValue = "true") boolean active_only) {
    return chatProcessingService
        .getCommands(active_only)
        .map(commands -> ResponseEntity.ok(Map.of("success", true, "commands", commands)));
  }
  ...
  public Mono<ResponseEntity<Map<String, Object>>> getCommand(@PathVariable String id) {
    return chatProcessingService
        .getItemById("command", id)
        .map(command -> ResponseEntity.ok(Map.of("success", true, "item", command)))
        .switchIfEmpty(
            Mono.just(
                ResponseEntity.status(404)
                    .body(Map.of("success", false, "message", "Command not found"))));
  }
  ...
  public Mono<ResponseEntity<Map<String, Object>>> getConfirmations(
    ...
    return chatProcessingService
        .getConfirmationHistory(item_id, chat_id)
        .map(history -> ResponseEntity.ok(Map.of("success", true, "confirmations", history)));
  }
}
