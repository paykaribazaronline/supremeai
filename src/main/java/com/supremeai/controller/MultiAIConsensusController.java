...
public class MultiAIConsensusController {
  ...
  /**
   * POST /api/consensus/vote Body: {"question": "...", "providers": ["openai","anthropic","groq"]}
   */
  @PostMapping("/vote")
  ...
  public Mono<ResponseEntity<Object>> voteOnQuestion(@RequestBody Map<String, Object> request) {
    ...
    List<String> providers =
        (List<String>) request.getOrDefault("providers", providerFactory.getAvailableProviderIds());
  ...
  public Mono<ResponseEntity<Object>> getHistory(@RequestParam(defaultValue = "20") int limit) {
    return consensusService
        ...
        .map(
            history -> {
              Map<String, Object> response = Map.of("history", history, "count", history.size());
              return ResponseEntity.ok((Object) response);
            });
  }

  /** GET /api/consensus/health Health check for consensus service */
  @GetMapping("/health")
  ...
  /** POST /api/consensus/compare-strategies Compare different voting strategies */
