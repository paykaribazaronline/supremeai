...
/**
 * Enhanced Learning Controller - Exposes all learning type endpoints Covers NLP, Multimodal,
 * Ecosystem, App Generation, and Predictive learning
 */
@RestController
...
public class EnhancedLearningController {
  ...
  public Flux<SystemLearning> getBestPractices(
      @PathVariable String category, @RequestParam(defaultValue = "0.7") double minConfidence) {
  ...
  public Flux<SystemLearning> getPredictiveRecommendations(@PathVariable String taskType) {
  ...
  public Mono<SystemLearning> recordNLPLearning(@RequestBody Map<String, Object> payload) {
    ...
    double qualityScore =
        payload.containsKey("qualityScore")
            ? ((Number) payload.get("qualityScore")).doubleValue()
            : 0.5;
    @SuppressWarnings("unchecked")
    Map<String, Object> context = (Map<String, Object>) payload.getOrDefault("context", Map.of());

    return enhancedLearningService.learnFromNLPInteraction(
        userInput, aiResponse, provider, qualityScore, context);
  }
  ...
  public Mono<SystemLearning> recordMultimodalLearning(@RequestBody Map<String, Object> payload) {
    ...
    double accuracyScore =
        payload.containsKey("accuracyScore")
            ? ((Number) payload.get("accuracyScore")).doubleValue()
            : 0.5;

    return enhancedLearningService.learnFromMultimodalInteraction(
        textPrompt, imageUrl, generatedCode, provider, accuracyScore);
  }
  ...
  public Mono<SystemLearning> recordAPIUsageLearning(@RequestBody Map<String, Object> payload) {
    ...
    String provider = (String) payload.getOrDefault("provider", "unknown");
    long responseTimeMs =
        payload.containsKey("responseTimeMs")
            ? ((Number) payload.get("responseTimeMs")).longValue()
            : 0;
    boolean success = payload.containsKey("success") ? (Boolean) payload.get("success") : false;
    @SuppressWarnings("unchecked")
    Map<String, Object> requestMeta =
        (Map<String, Object>) payload.getOrDefault("requestMeta", Map.of());

    return enhancedLearningService.learnFromAPIUsage(
        apiEndpoint, provider, responseTimeMs, success, requestMeta);
  }

  /** Manually record App Generation learning */
  @PostMapping("/app-generation")
  public Mono<SystemLearning> recordAppGenerationLearning(
    ...
    boolean buildSuccess =
        payload.containsKey("buildSuccess") ? (Boolean) payload.get("buildSuccess") : false;
    String apkPath = (String) payload.getOrDefault("apkPath", "");
    @SuppressWarnings("unchecked")
    Map<String, Object> buildMetrics =
        (Map<String, Object>) payload.getOrDefault("buildMetrics", Map.of());
    String agentUsed = (String) payload.getOrDefault("agentUsed", "unknown");

    return enhancedLearningService.learnFromAppGeneration(
        requirement, generatedAppType, buildSuccess, apkPath, buildMetrics, agentUsed);
  }
  ...
  public Mono<SystemLearning> recordPredictiveLearning(@RequestBody Map<String, Object> payload) {
    ...
    Map<String, Object> patternData =
        (Map<String, Object>) payload.getOrDefault("patternData", Map.of());
    double confidence =
        payload.containsKey("confidence")
            ? ((Number) payload.get("confidence")).doubleValue()
            : 0.5;
    ...
    return enhancedLearningService.learnPredictivePattern(
        patternType, patternData, confidence, basedOnLearningId);
  }
