...
/**
 * REST endpoints for querying the Global Knowledge Base. Accessible by authenticated users (not
 * just admin) to retrieve known solutions. Modifications remain admin-only.
 */
@RestController
...
public class KnowledgeBaseController {
  ...
  /**
   * Get best solution for a specific error signature. Example: GET
   * /api/knowledge/solution?error=NullPointerException%20at%20Main.java:42
   */
  @GetMapping("/solution")
  ...
  /**
   * Learn a new solution (requires admin approval). This endpoint records a successful fix that
  ...
  public Mono<String> recordFailure(@RequestBody RecordFailureRequest request) {
    return globalKnowledgeBase
        .recordFailure(request.getErrorSignature(), request.getFailedCode())
