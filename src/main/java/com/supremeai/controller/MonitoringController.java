...
/** Admin Monitoring Controller - Phase 4 Exposes production health metrics and system status. */
@RestController
...
public class MonitoringController {
  ...
  @Autowired private ProductionHealthMonitor healthMonitor;

  @Autowired private MonitoringLogRepository monitoringLogRepository;

  /** GET /api/admin/monitoring/logs Fetch recent system logs from Firestore. */
  @GetMapping("/logs")
  public Mono<ApiResponse<List<MonitoringLog>>> getRecentLogs(
      @RequestParam(defaultValue = "100") int limit) {
    return monitoringLogRepository
        .findByOrderByTimestampDesc()
        .take(limit)
        .collectList()
        .map(logs -> ApiResponse.success("Monitoring logs retrieved successfully", logs));
  }

  /** DELETE /api/admin/monitoring/logs Clear all system logs. */
  @DeleteMapping("/logs")
  ...
  /** GET /api/admin/monitoring/health Comprehensive system health status. */
  @GetMapping("/health")
  ...
  /** GET /api/admin/monitoring/dashboard Lightweight dashboard summary. */
  ...
  /** POST /api/admin/monitoring/reset Reset metrics (admin only). */
  @PostMapping("/reset")
  ...
  /**
   * GET /api/admin/monitoring/test-sentry Triggers a test exception to verify Sentry integration.
   */
        ...
        () -> {
          ...
          try {
            throw new RuntimeException(
                "SupremeAI Monitoring Test: Sentry integration check at "
