...
/**
 * Quality Control Controller - Phase 4 Exposes quality scoring and validation endpoints for admin
 * dashboard.
 */
@RestController
...
public class QualityController {
  ...
  @Autowired private QualityScoringService qualityService;

  /** POST /api/admin/quality/assess Run quality assessment on an API submission. */
  @PostMapping("/assess")
  ...
  /** POST /api/admin/quality/run-tests Execute automated tests against a connector. */
  ...
  public ResponseEntity<Map<String, Object>> runTests(
    ...
    if (apiId == null) {
      return ResponseEntity.badRequest().body(Map.of("error", "apiId is required"));
    }
  ...
  /** GET /api/admin/quality/assessments/pending List all assessments requiring manual review. */
  ...
  /** GET /api/admin/quality/assessment/{apiId} Get quality assessment for a specific API. */
  @GetMapping("/assessment/{apiId}")
  public ResponseEntity<Map<String, Object>> getAssessment(@PathVariable String apiId) {
    QualityScoringService.QualityAssessment assessment = qualityService.getAssessment(apiId);
