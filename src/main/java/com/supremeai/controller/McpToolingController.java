...
/**
 * McpToolingController exposes SupremeAI's internal tools and local knowledge as an MCP (Model
 * Context Protocol) server. This allows AI agents to directly interact with browser scraping and
...
public class McpToolingController {
  ...
  public Mono<ResponseEntity<Map<String, String>>> readFile(
      @RequestBody Map<String, String> request) {
    return projectContextService
        .readFile(request.get("path"))
  ...
  /**
   * Scrapes a given URL and returns its content in LLM-ready markdown format. This exposes the
   * BROWSER_SCRAPPER functionality via MCP.
   *
  ...
  public Mono<ResponseEntity<Map<String, String>>> scrapeUrl(
      @RequestBody Map<String, String> request) {
    String url = request.get("url");
    if (url == null || url.isBlank()) {
      return Mono.just(
          ResponseEntity.badRequest().body(Map.of("error", "URL is required for scraping.")));
    }
    return firecrawlService
        ...
        .onErrorResume(
            ...
            e -> Mono.just(ResponseEntity.status(500).body(Map.of("error", e.getMessage()))))
        .onErrorResume(
            e ->
                Mono.just(
                    ResponseEntity.internalServerError()
                        .body(Map.of("error", "Failed to scrape URL: " + e.getMessage()))));
  }

  /**
   * Searches the local knowledge base for entries matching the query. This exposes the Local
   * Knowledge functionality via MCP.
   *
  ...
  @PostMapping(
      ...
      consumes = MediaType.APPLICATION_JSON_VALUE,
      produces = MediaType.APPLICATION_JSON_VALUE)
  public Mono<ResponseEntity<List<SystemLearning>>> searchKnowledge(
      @RequestBody Map<String, String> request) {
