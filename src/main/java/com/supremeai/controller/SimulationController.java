...
public class SimulationController {
    public SimulationController(SimulationManager simulationManager, SimulationScenarioRepository scenarioRepository, SimulationResultRepository resultRepository) {
     ...
     * Create a new simulation scenario configuration.
     */
    ...
    public Mono<ResponseEntity<SimulationScenario>> createScenario(
        ...
        return simulationManager.createScenario(userId, appId, name, description, parameters)
            .map(scenario -> ResponseEntity.status(HttpStatus.CREATED).body(scenario));
    }

    /**
     * POST /api/simulate/execute
     * Trigger execution of a simulator scenario.
     */
    ...
    public Mono<ResponseEntity<SimulationResult>> executeSimulation(
        ...
        String userId = auth.getName();
        return simulationManager.executeSimulation(userId, scenarioId)
     ...
     * Get the runtime status of an execution.
     */
    @GetMapping("/status")
    @PreAuthorize("isAuthenticated()")
    ...
    /**
     * GET /api/simulate/results
     * Retrieve all simulation results for the authenticated user, or filtered by appId.
     */
    @GetMapping("/results")
    @PreAuthorize("isAuthenticated()")
    public Mono<ResponseEntity<Flux<SimulationResult>>> getResults(
        ...
        if (appId != null && !appId.trim().isEmpty()) {
            results = resultRepository.findByAppId(appId)
     ...
     * Retrieve all simulation scenarios for the authenticated user, or filtered by appId.
     */
    ...
    public Mono<ResponseEntity<Flux<SimulationScenario>>> getScenarios(
        ...
        if (appId != null && !appId.trim().isEmpty()) {
            scenarios = scenarioRepository.findByAppId(appId)
                .filter(scen -> scen.getUserId().equals(userId));
        } else {
