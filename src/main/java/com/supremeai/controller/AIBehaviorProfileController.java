...
public class AIBehaviorProfileController {
  ...
  @Autowired private AIBehaviorProfileService profileService;

  @GetMapping("/project/{projectId}")
  public Mono<ResponseEntity<AIBehaviorProfile>> getProfileByProject(
  ...
  @PostMapping
  public Mono<ResponseEntity<AIBehaviorProfile>> saveProfile(
      @RequestBody AIBehaviorProfile profile) {
    return profileService
        .saveProfile(profile)
  ...
  public Mono<ResponseEntity<Void>> deleteProfile(@PathVariable String id) {
    return profileService.deleteProfile(id).then(Mono.just(ResponseEntity.noContent().build()));
  }

  @GetMapping("/default")
  public Mono<ResponseEntity<AIBehaviorProfile>> getDefaultProfile() {
    return profileService.getDefaultProfile().map(profile -> ResponseEntity.ok(profile));
  }
