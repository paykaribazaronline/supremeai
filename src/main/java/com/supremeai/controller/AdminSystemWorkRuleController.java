 ...
 *
 * <p>Endpoints ───────── GET /api/admin/system-work-rules — list all rules GET
 * /api/admin/system-work-rules/active — list active rules only GET
 * /api/admin/system-work-rules/{ruleKey} — fetch single rule POST /api/admin/system-work-rules —
 * create PUT /api/admin/system-work-rules/{ruleKey} — update (full replace) DELETE
 * /api/admin/system-work-rules/{ruleKey} — delete POST /api/admin/system-work-rules/seed/defaults —
 * seed 6 well-known defaults POST /api/admin/system-work-rules/sync/{ruleKey}— re-trigger conflict
 * propagation
 */
@RestController
...
public class AdminSystemWorkRuleController {
  ...
  @Autowired private SystemWorkRuleService ruleService;
  ...
  @PostMapping
  public Mono<ResponseEntity<ApiResponse<SystemWorkRule>>> create(
      @RequestBody SystemWorkRule rule) {
    return ruleService
        .saveRule(rule)
        .map(
            saved -> {
    ...
    return ruleService
        ...
        .flatMap(
            existing -> {
              ...
              if (updates.getStructuredValue() != null)
                existing.setStructuredValue(updates.getStructuredValue());
        ...
        .defaultIfEmpty(
            ResponseEntity.status(404)
                .body(ApiResponse.<SystemWorkRule>error("Rule not found: " + ruleKey)))
        .doOnError(
            e -> log.error("[AdminSystemWorkRuleController] Update failed: {}", e.getMessage(), e));
  }
  ...
  public Mono<ResponseEntity<ApiResponse<Void>>> delete(@PathVariable String ruleKey) {
    return ruleService
        ...
        .defaultIfEmpty(
            ResponseEntity.status(404).body(ApiResponse.<Void>error("Rule not found: " + ruleKey)))
        .doOnError(
            e -> log.error("[AdminSystemWorkRuleController] Delete failed: {}", e.getMessage(), e));
  }
  ...
  public Mono<ResponseEntity<ApiResponse<Map<String, Object>>>> sync(@PathVariable String ruleKey) {
    ...
    return ruleService
        ...
        .flatMap(
            rule -> {
              ...
              String targetField =
                  rule.getTargetField() != null ? rule.getTargetField() : rule.getRuleKey();
              return ruleService
                  .saveRule(rule)
                  .map(
                      saved -> {
        ...
        .defaultIfEmpty(
            ResponseEntity.status(404).body(ApiResponse.error("Rule not found: " + ruleKey)));
  }
}
