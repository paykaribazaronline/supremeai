 ...
 *
 * <p>Security: All endpoints require authentication via AuthenticationFilter (Firebase). Users can
 * only access their own profile. Admin endpoints require ROLE_ADMIN.
 */
@RestController
...
public class SimulatorController {
  ...
  @Autowired private UserSimulatorProfileRepository profileRepository;
  ...
  public Mono<ResponseEntity<UserSimulatorProfile>> getProfile(Authentication auth) {
    ...
    return simulatorService.getProfile(userId).map(ResponseEntity::ok);
  }

  /** POST /api/simulator/profile Update current user's simulator profile (quota, device) */
  @PostMapping("/profile")
  @PreAuthorize("isAuthenticated()")
  public Mono<ResponseEntity<UserSimulatorProfile>> updateProfile(
    ...
    return simulatorService.updateProfile(userId, request).map(ResponseEntity::ok);
  }
  ...
  public Mono<ResponseEntity<Map<String, Object>>> installApp(
      Authentication auth, @RequestBody Map<String, String> request) {
    ...
    if (appId == null || appId.trim().isEmpty()) {
      return Mono.just(ResponseEntity.badRequest().body(Map.of("error", "appId is required")));
    }

    return simulatorService
        .installApp(userId, appId, deviceProfile)
        .map(
            result -> {
  ...
  /** DELETE /api/simulator/install/{appId} Uninstall an app from simulator */
  ...
  public Mono<ResponseEntity<Map<String, Object>>> uninstallApp(
      Authentication auth, @PathVariable String appId) {
  ...
  /** GET /api/simulator/installed List all installed apps for authenticated user */
  ...
  public Mono<ResponseEntity<Map<String, Object>>> getInstalledApps(Authentication auth) {
    ...
    return simulatorService
        ...
        .map(
            profile -> {
              java.util.List<Map<String, Object>> apps =
                  profile.getInstalledApps().stream()
                      .map(
                          app -> {
  ...
  public Mono<ResponseEntity<Map<String, Object>>> startSession(
      Authentication auth, @RequestParam String appId) {

    String userId = auth.getName();

    return simulatorService
        .startSession(userId, appId)
        .map(
            result -> {
              Map<String, Object> response = new HashMap<>();
  ...
  /** POST /api/simulator/session/stop Stop current simulator session */
  ...
  /** GET /api/simulator/session/status Get current session status */
  ...
  /** GET /api/simulator/admin/usage Get simulator usage across all users (Admin only) */
  ...
  /** POST /api/simulator/admin/set-quota/{userId} Override user's install quota (Admin only) */
  @PostMapping("/admin/set-quota/{userId}")
  @PreAuthorize("hasRole('ADMIN')")
  public Mono<ResponseEntity<UserSimulatorProfile>> adminSetQuota(
      Authentication auth, @PathVariable String userId, @RequestParam int quota) {
