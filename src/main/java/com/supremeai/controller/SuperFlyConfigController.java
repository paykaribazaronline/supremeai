...
public class SuperFlyConfigController {
  ...
  /**
   * Get the SuperFly dynamic settings. Accessible by guests, users, and admins since it's a general
   * feature.
   */
  @GetMapping
  ...
  public Mono<ResponseEntity<ApiResponse<SuperFlyConfig>>> getSettings() {
    return repository
        .findById("superfly_settings")
        .switchIfEmpty(
            Mono.defer(
                () -> {
                  log.info(
                      "[SuperFly] No settings found in Firestore. Seeding default configuration...");
            ...
            e -> {
              ...
              return Mono.just(
                  ResponseEntity.status(500)
                      .body(
                          ApiResponse.error(
                              "Failed to load SuperFly configurations: " + e.getMessage())));
            });
  }
  ...
  public Mono<ResponseEntity<ApiResponse<SuperFlyConfig>>> updateSettings(
      @RequestBody SuperFlyConfig update) {
            ...
            e -> {
              ...
              return Mono.just(
                  ResponseEntity.status(500)
                      .body(
                          ApiResponse.error(
                              "Failed to save SuperFly configurations: " + e.getMessage())));
            });
  }
}
