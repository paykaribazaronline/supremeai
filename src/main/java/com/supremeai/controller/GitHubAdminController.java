...
public class GitHubAdminController {
  ...
  public Mono<ResponseEntity<List>> getOpenPullRequests(
      @PathVariable String owner, @PathVariable String repo, @RequestParam String installationId) {

    return gitHubAutomationService
        .getOpenPullRequests(owner, repo, installationId)
  ...
  public Mono<ResponseEntity<Map<String, Object>>> mergePullRequest(
    ...
    return gitHubAutomationService
        .mergePullRequest(
            ...
            repo,
            pullNumber,
            mergeRequest.getCommitTitle(),
            mergeRequest.getInstallationId())
        .map(ResponseEntity::ok);
  }
  ...
  public Mono<ResponseEntity<Map<String, String>>> triggerWorkflow(
    ...
    return gitHubAutomationService
        .triggerWorkflow(owner, repo, workflowId, request.getBranch(), request.getInstallationId())
        .then(
            Mono.just(
                ResponseEntity.ok(
                    Map.of("status", "success", "message", "Workflow triggered successfully"))));
  }
  ...
  public Mono<ResponseEntity<Map<String, Object>>> getLatestWorkflowRun(
    ...
    return gitHubAutomationService
        .getLatestWorkflowRun(owner, repo, workflowId, installationId)
        .map(ResponseEntity::ok)
        .defaultIfEmpty(ResponseEntity.notFound().build());
  }
