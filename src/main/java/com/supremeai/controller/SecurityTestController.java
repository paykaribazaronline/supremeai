...
/**
 * Security testing and validation endpoints. Helps verify Firebase authentication and user data
 * isolation.
 */
...
public class SecurityTestController {
  ...
  /**
   * POST /api/security/validate-firebase-token Validate a Firebase ID token and return detailed
   * info. Used to verify Firebase login is working correctly.
   */
  @PostMapping("/validate-firebase-token")
  public Mono<ResponseEntity<Map<String, Object>>> validateFirebaseToken(
    ...
    if (idToken == null || idToken.trim().isEmpty()) {
      return Mono.just(
          ResponseEntity.badRequest()
              .body(Map.of("valid", false, "message", "ID token is required")));
    }
    ...
    try {
      ...
      return Mono.just(
          ResponseEntity.ok(
              Map.of("valid", false, "message", "Invalid Firebase token: " + e.getMessage())));
    } catch (Exception e) {
      log.error("Unexpected error validating Firebase token", e);
      return Mono.just(
          ResponseEntity.internalServerError()
              .body(Map.of("valid", false, "message", "Validation error: " + e.getMessage())));
    }
  }

  /**
   * GET /api/security/current-user Get the currently authenticated user's info and verify session.
  ...
  public Mono<ResponseEntity<Map<String, Object>>> getCurrentUser() {
    ...
    if (auth == null || !auth.isAuthenticated() || auth.getName() == null) {
      return Mono.just(ResponseEntity.status(401).body(Map.of("authenticated", false)));
    }
    ...
    return userRepository
        ...
        .defaultIfEmpty(
            ResponseEntity.status(404)
                .body(Map.of("authenticated", true, "message", "User record missing")));
  }

  /**
   * GET /api/security/test-isolation Test endpoint to verify user data isolation. Tries to access a
   * different user's data to see if it's blocked. (This is a diagnostic endpoint for
   * admins/developers)
   */
  ...
  /**
   * GET /api/security/health-check Comprehensive security health check including: - Firebase
   * connectivity - JWT configuration - Firestore rules status - Session management
   */
