 ...
 *
 * <p>Uses STOMP over WebSocket (consistent with existing WebSocketConfig).
 *
 * <p>Destinations: - SEND to: /app/simulator/control/{sessionId} (admin → simulator) - SEND to:
 * /app/simulator/event/{sessionId} (simulator → backend) - SUBSCRIBE to:
 * /topic/simulator/{sessionId} (simulator receives commands) - SUBSCRIBE to:
 * /topic/simulator/events (admin receives events)
 */
@Controller
public class SimulatorWebSocketController {
  ...
  @Autowired private SimulatorSessionService sessionService;

  /**
   * Admin UI sends a control command to a simulator session.
   *
   * <p>Client sends STOMP message to: /app/simulator/control/{sessionId} Payload: { "type": "tap",
   * "x": 100, "y": 200 } or { "type": "input", "selector": "#email", "text": "test" }
   *
   * <p>This relays the command to the simulator frontend via topic.
   */
  @MessageMapping("/simulator/control/{sessionId}")
  public void handleControlCommand(
      @DestinationVariable String sessionId, @Payload Map<String, Object> command) {
    ...
    messagingTemplate.convertAndSend(
        "/topic/simulator/" + sessionId, Map.of("type", "command", "command", command));
   ...
   *
   * <p>Client sends to: /app/simulator/event/{sessionId} Payload: { "type": "heartbeat" } or {
   * "type": "log", "level": "ERROR", "message": "..." }
   */
  @MessageMapping("/simulator/event/{sessionId}")
  public void handleSimulatorEvent(
      @DestinationVariable String sessionId, @Payload Map<String, Object> event) {
  ...
  /**
   * Admin UI requests screenshot of active simulator. Sends command, simulator responds via event
   * with base64 image.
   */
  ...
  public void captureScreenshot(@Payload Map<String, Object> request) {
    ...
    messagingTemplate.convertAndSend(
        "/topic/simulator/" + sessionId, Map.of("type", "command", "command", cmd));
  }
}
