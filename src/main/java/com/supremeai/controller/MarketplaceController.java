...
/** MCP Marketplace Controller - Plan 24 Phase 4 Exposes MCP server/skill marketplace endpoints. */
@RestController
...
public class MarketplaceController {
  ...
  @Autowired private MCPMarketplaceService marketplaceService;

  /** GET /api/marketplace/servers List all available MCP servers. */
  @GetMapping("/servers")
  ...
  /** GET /api/marketplace/servers/{serverId} Get detailed info about a specific MCP server. */
  ...
  /** POST /api/marketplace/servers/{serverId}/install Install (enable) an MCP server. */
  ...
  public ResponseEntity<Map<String, Object>> installServer(@PathVariable String serverId) {
    ...
    if (!success) {
      return ResponseEntity.badRequest().body(Map.of("error", "Failed to install server"));
    }
  ...
  /** DELETE /api/marketplace/servers/{serverId}/uninstall Uninstall (disable) an MCP server. */
  ...
  public ResponseEntity<Map<String, Object>> uninstallServer(@PathVariable String serverId) {
    ...
    if (!success) {
      return ResponseEntity.badRequest().body(Map.of("error", "Failed to uninstall server"));
    }
  ...
  /** GET /api/marketplace/servers/{serverId}/usage-stats Get usage statistics for a server. */
  ...
  /**
   * GET /api/marketplace/servers/{serverId}/revenue-stats Get revenue/earnings statistics for a
   * server (if monetized).
   */
  ...
  /** GET /api/marketplace/trending Get trending MCP servers (most installed this week). */
  @GetMapping("/trending")
  public ResponseEntity<List<Map<String, Object>>> getTrending() {
    return ResponseEntity.ok(marketplaceService.getTrendingServers());
  }

  /** GET /api/marketplace/top-rated Get top-rated MCP servers. */
