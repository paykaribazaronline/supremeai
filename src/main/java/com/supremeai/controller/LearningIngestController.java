...
/**
 * Learning Ingest Controller - Public endpoints for VS Code Extension Receives real-time learning
 * data from VS Code without requiring admin auth
 */
@RestController
...
public class LearningIngestController {
  ...
  @Autowired private UserCodeLearningService userCodeLearningService;

  @Autowired private SystemLearningRepository systemLearningRepository;

  /** Receive code edit event from VS Code extension POST /api/v1/learning/code-edit */
  ...
  public ResponseEntity<Map<String, Object>> receiveCodeEdit(@RequestBody LearningEvent event) {
    ...
    try {
      userCodeLearningService.learnFromUserEdit(
          event.getTaskId(), event.getOriginalCode(), event.getEditedCode(), event.getContext());

      return ResponseEntity.ok(
          Map.of("success", true, "message", "Code edit learned successfully"));
    } catch (Exception e) {
  ...
  /** Receive error report from VS Code extension POST /api/v1/learning/error */
  @PostMapping("/error")
  public ResponseEntity<Map<String, Object>> reportError(@RequestBody ErrorEvent event) {
    ...
    try {
      ...
      userCodeLearningService.recordFailure(
          errorSignature, event.getCodeSnippet() != null ? event.getCodeSnippet() : "");

      return ResponseEntity.ok(Map.of("success", true, "message", "Error reported and learned"));
    } catch (Exception e) {
  ...
  /** Receive suggestion feedback (accept/reject) from VS Code POST /api/v1/learning/feedback */
  @PostMapping("/feedback")
  public ResponseEntity<Map<String, Object>> receiveFeedback(@RequestBody FeedbackEvent event) {
    log.info(
        "Received feedback: {} for suggestion: {}", event.isAccepted(), event.getSuggestionId());
    ...
    try {
      ...
      pattern.setContent(
          String.format(
              "User %s suggestion: %s",
              event.isAccepted() ? "accepted" : "rejected", event.getContext()));
  ...
  /** Get learning statistics for VS Code status bar GET /api/v1/learning/stats */
  @GetMapping("/stats")
  public ResponseEntity<Map<String, Object>> getStats() {
    try {
      ...
      return ResponseEntity.ok(Map.of("learningCount", 0, "status", "error"));
    }
  }

  /** Health check endpoint GET /api/v1/learning/health */
  @GetMapping("/health")
