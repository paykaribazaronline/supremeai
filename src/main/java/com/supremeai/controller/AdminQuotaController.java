...
public class AdminQuotaController {
  ...
  public Mono<ApiResponse<SystemConfig>> getQuotaConfig() {
    return Mono.just(
        ApiResponse.success("Quota configuration retrieved", configService.getConfig()));
  }
  ...
  public Mono<ApiResponse<SystemConfig>> updateQuotaConfig(@RequestBody SystemConfig newConfig) {
    ...
    if (newConfig.getTierMaxSimulatorInstalls() != null)
      current.setTierMaxSimulatorInstalls(newConfig.getTierMaxSimulatorInstalls());
