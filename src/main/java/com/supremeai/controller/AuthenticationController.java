...
/**
 * AuthenticationController - Handles user registration, login, and session management. Refactored
...
public class AuthenticationController {
  ...
  public Mono<ApiResponse<Map<String, Object>>> firebaseLogin(
      @Valid @RequestBody FirebaseLoginRequest request, HttpServletRequest httpRequest) {
    String idToken = request.idToken();
    String remoteAddr = httpRequest.getRemoteAddr();
    log.info("Firebase login attempt from {}", remoteAddr);

    return authenticationService
        .firebaseLogin(idToken, remoteAddr)
        .map(
            data -> {
  ...
  public Mono<ResponseEntity<ApiResponse<Map<String, Object>>>> register(
    ...
    return authenticationService
        .register(request.email(), request.password(), request.displayName(), remoteAddr)
        .map(
            savedUser ->
                    ...
                    .body(
                        ApiResponse.ok(
                            Map.of(
                                ...
                                "user",
                                convertToPublicUserMap(savedUser)))))
        .onErrorResume(
            e -> Mono.error(new ResponseStatusException(HttpStatus.BAD_REQUEST, e.getMessage())));
  }
  ...
  public Mono<ResponseEntity<ApiResponse<Map<String, Object>>>> validateToken(
    ...
    return Mono.fromCallable(
        ...
        .map(
            user ->
                ResponseEntity.ok(
                    ApiResponse.ok(Map.of("valid", true, "user", convertToPublicUserMap(user)))))
        .onErrorResume(
            e -> Mono.error(new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Invalid token")));
  }

  @GetMapping("/me")
  public Mono<ResponseEntity<ApiResponse<Map<String, Object>>>> getCurrentUser(
    ...
    return userRepository
        .findByFirebaseUid(context.getAuthentication().getName())
        .map(
            user ->
                ResponseEntity.ok(
                    ApiResponse.ok(Map.of("user", (Object) convertToPublicUserMap(user)))))
        .switchIfEmpty(
