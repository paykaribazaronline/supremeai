...
/**
 * Controller for website reverse engineering feature. Submits jobs to Pub/Sub queue for async
 * processing by Python FastAPI worker.
 */
@RestController
...
public class ReverseEngineeringController {
  ...
  @Autowired private ReverseEngineeringIntegrationService integrationService;
  ...
  public Mono<ResponseEntity<Map<String, Object>>> submitReverseEngineering(
    ...
    return integrationService
        .startJob(userId, url, taskType, customInstructions, extraParams)
        .map(
            job -> {
  ...
  /**
   * Get history of reverse engineering jobs. Frontend: GET /api/reverse-engineer/history?limit=50
   */
  @GetMapping("/history")
  @PreAuthorize("hasRole('ADMIN')")
                  ...
                  jobs.stream()
                      .map(
                          job -> {
                            ...
                            results.put(
                                "endpoints",
                                job.getDiscoveredApis() != null
                                    ? job.getDiscoveredApis()
                                    : List.of());
  ...
  /** Delete/cancel a reverse engineering job. Frontend: DELETE /api/reverse-engineer/job/{jobId} */
  @DeleteMapping("/job/{jobId}")
  @PreAuthorize("hasRole('ADMIN')")
  public ResponseEntity<Map<String, Object>> cancelJob(
      @PathVariable String jobId, Authentication auth) {
  ...
  public ResponseEntity<Map<String, Object>> completeJob(
      @PathVariable String jobId, @RequestBody Map<String, Object> result, Authentication auth) {
    ...
    integrationService
        .completeJob(jobId, discoveredApis)
        .subscribe(
  ...
  public ResponseEntity<Map<String, Object>> integrateWithCodeGen(
    ...
    integrationService
        ...
        .subscribe(
            job -> {
              logger.info(
                  "Integration triggered for job {}: generated app {}",
                  jobId,
                  job.getGeneratedAppId());
            },
            error -> {
