...
public class AdminControlController {
  ...
  public AdminControlController(
      ActivityLogRepository activityLogRepository, ProjectDNAHarvesterService dnaHarvesterService) {
  ...
  public Mono<ResponseEntity<Map<String, String>>> triggerDNAHarvest() {
    return dnaHarvesterService
        .harvestDNA()
        .then(
            Mono.just(
                ResponseEntity.ok(
                    Map.of("message", "Project DNA Harvesting triggered successfully"))));
  }

  @GetMapping("/history")
  public Mono<ResponseEntity<List<ActivityLog>>> getControlHistory() {
    return activityLogRepository.findAll().take(100).collectList().map(ResponseEntity::ok);
  }
}
