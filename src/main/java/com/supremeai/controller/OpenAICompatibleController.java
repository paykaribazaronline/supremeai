 ...
 *
 * <p>Provides a standard OpenAI-style chat completion endpoint that auto-detects the best available
 * model. Used by external tools like Continue.dev to integrate with SupremeAI without needing to
 * know specific providers.
 *
 * <p>Endpoint: POST /api/v1/chat/completions
 *
 * <p>Request format (OpenAI compatible): { "model": "auto" (optional, ignored - auto-detection is
 * always used), "messages": [{"role": "user", "content": "Your prompt"}] }
 *
 * <p>Authentication: Requires X-Authorized-Key header when SUPREMEAI_API_KEY env var is set. Rate
 * limiting is applied via RateLimiterFilter for all callers.
 *
 * <p>Response format: standard OpenAI chat completion response.
 */
@RestController
...
public class OpenAICompatibleController {
  ...
  @Autowired private SystemAutoDetectService autoDetectService;

  private final SecretManagerService secretManagerService;

  public OpenAICompatibleController(
      SystemAutoDetectService autoDetectService, SecretManagerService secretManagerService) {
  ...
  private ResponseEntity<Map<String, Object>> checkExternalApiKey(HttpServletRequest request) {
    ...
    if (expectedKey != null && !expectedKey.isBlank()) {
      ...
      if (!expectedKey.equals(providedKey)) {
        logger.warn(
            "Unauthorized /api/v1/chat/completions — missing or invalid X-Authorized-Key from IP: {}",
        ...
        return ResponseEntity.status(401)
            .body(
                Map.of(
                    "error", "UNAUTHORIZED",
                    "message",
                        "Missing or invalid X-Authorized-Key header. Set SUPREMEAI_API_KEY to enable this endpoint."));
      }
    }
    return null;
  }
  ...
  public Mono<ResponseEntity<Map<String, Object>>> chatCompletions(
            ...
            provider ->
                provider
                    ...
                    .onErrorResume(
                        e -> {
                          logger.error(
                              "Generation failed with provider {}: {}",
                              provider.getName(),
                              e.getMessage());
                              ...
                              ResponseEntity.status(502)
                                  .body(
                                      Map.of(
                                          ...
                                          "generation_failed",
                                          "message",
                                          "AI provider returned an error: " + e.getMessage())));
                        }))
        .onErrorResume(
            e -> {
                  ...
                  ResponseEntity.status(503)
                      .body(
                          Map.of(
                              ...
                              "message",
                                  "No AI provider is currently available. Check API keys and network.")));
            });
  }

  private ResponseEntity<Map<String, Object>> buildResponse(
      AIProvider provider, String prompt, String responseContent) {
