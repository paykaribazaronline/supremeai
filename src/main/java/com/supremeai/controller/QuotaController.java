...
/**
 * Quota management endpoints. Users can only view their own quota. Admins can view and manage any
 * user's quota.
 */
@RestController
...
public class QuotaController {
  ...
  @Autowired private QuotaPredictionService predictionService;
  ...
  /**
   * GET /api/quota/{userId} - Get quota usage for a user. Users can only view their own quota.
   * Admins can view any user's quota.
   */
  @GetMapping("/{userId}")
    ...
    if (!isAdmin && !auth.getName().equals(userId)) {
      return Mono.error(
          new ResponseStatusException(
              HttpStatus.FORBIDDEN, "Access denied: You can only view your own quota"));
    }
            ...
            stats ->
                ResponseEntity.ok(
                    Map.<String, Object>of(
                        ...
                        "lastUsedAt",
                            stats.getLastUsedAt() != null ? stats.getLastUsedAt().toString() : null,
    ...
    return quotaService
        ...
        .map(
            savedLog ->
                ResponseEntity.ok(
                    Map.<String, Object>of(
                        "status", "success", "message", "Quota reset for user " + userId)))
        .onErrorResume(
            e ->
                Mono.just(
                    ResponseEntity.status(500)
                        .body(Map.of("error", "Failed to reset quota: " + e.getMessage()))));
  }

  /**
   * GET /api/quota/{userId}/prediction - Get quota prediction for a user. Users can only view their
   * own prediction. Admins can view any user's prediction.
   */
  ...
  public Mono<ResponseEntity<Map<String, Object>>> getQuotaPrediction(@PathVariable String userId) {
    ...
    if (!isAdmin && !auth.getName().equals(userId)) {
      return Mono.error(
          new ResponseStatusException(
              HttpStatus.FORBIDDEN, "Access denied: You can only view your own quota prediction"));
    }

    return predictionService.getPrediction(userId).map(ResponseEntity::ok);
  }
    ...
    return predictionService
        ...
        .map(
            warnings ->
                ResponseEntity.ok(
                    Map.<String, Object>of(
                        "warnings", warnings, "timestamp", System.currentTimeMillis())));
  }
}
