...
public class AdminRuleController {
  ...
  public Mono<ResponseEntity<Map<String, Object>>> getRules(
    ...
    return (active_only
        ...
        .map(
            rules -> {
              List<Map<String, Object>> ruleList = rules.stream().map(this::entityToMap).toList();
              return ResponseEntity.ok(Map.of("success", true, "rules", ruleList));
            });
  }

  @PostMapping
  @Audited(resource = "protocol_rule", action = "CREATE")
  public Mono<ResponseEntity<Map<String, Object>>> createRule(
    ...
    return protocolRuleRepository
        .save(rule)
        .map(saved -> ResponseEntity.ok(Map.of("success", true, "rule", entityToMap(saved))));
  }

  @PutMapping("/{id}")
  @Audited(resource = "protocol_rule", action = "UPDATE")
  public Mono<ResponseEntity<Map<String, Object>>> updateRule(
    ...
    return protocolRuleRepository
        ...
        .flatMap(
            rule -> {
              ...
              rule.setSeverity((String) request.get("severity"));
              rule.setActive(
                  request.get("isActive") != null ? (Boolean) request.get("isActive") : true);
              rule.setUpdatedAt(LocalDateTime.now());
              return protocolRuleRepository.save(rule);
            })
        .map(updated -> ResponseEntity.ok(Map.of("success", true, "rule", entityToMap(updated))))
        .switchIfEmpty(
            Mono.just(
  ...
  public Mono<ResponseEntity<Map<String, Object>>> deleteRule(@PathVariable String id) {
    return protocolRuleRepository
        .deleteById(id)
        .thenReturn(
            ResponseEntity.ok(Map.of("success", true, "message", "Rule deleted successfully")));
  }
