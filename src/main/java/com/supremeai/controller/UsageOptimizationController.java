 ...
 *
 * <p>Endpoints: GET /api/optimization/usage - Get usage summary POST /api/optimization/select-model
 * - Get recommended model for a task GET /api/optimization/cache/stats - Get cache statistics
 */
@RestController
...
public class UsageOptimizationController {
  ...
  @Autowired private UsageOptimizationService optimizationService;
  ...
  public Mono<ResponseEntity<Map<String, Object>>> getUsageSummary() {
    ...
    return optimizationService.getUserUsageSummary(userId).map(ResponseEntity::ok);
  }

  /**
   * POST /api/optimization/select-model - Get recommended model for a task. Body: { "complexity":
   * "simple|moderate|complex|critical" }
   */
  @PostMapping("/select-model")
  public Mono<ResponseEntity<Map<String, Object>>> selectModel(
      @RequestBody Map<String, String> body) {
    String userId = getCurrentUserId();
    String complexity = body.getOrDefault("complexity", "moderate");

    return optimizationService
        .selectModelForTask(userId, complexity)
        .map(
            model -> {
