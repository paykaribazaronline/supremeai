...
public class ActivitySummaryController {
  ...
  public Mono<ResponseEntity<Map<String, Object>>> getActivitySummary(
    ...
    if (severity != null && !severity.isEmpty()) {
      recentActionsFlux =
          activityLogRepository.findBySeverityOrderByTimestampDesc(severity.toUpperCase());
    } else if (category != null && !category.isEmpty()) {
      recentActionsFlux =
          activityLogRepository.findByCategoryOrderByTimestampDesc(category.toUpperCase());
    } else {
      recentActionsFlux = activityLogRepository.findAll().take(100);
    }

    return Mono.zip(recentActionsFlux.collectList(), activityLogRepository.count())
        .map(
            tuple -> {
  ...
  public Mono<ResponseEntity<ActivityLog>> logActivity(@RequestBody ActivityLog log) {
    return activityLogRepository.save(log).map(ResponseEntity::ok);
  }
}
