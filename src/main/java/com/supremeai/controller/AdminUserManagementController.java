...
public class AdminUserManagementController extends BaseAdminController<Object, String> {
  ...
  public Mono<ResponseEntity<ApiResponse<Map<String, Object>>>> getUsers() {
    return wrapList(userRepository.findAll().map(facadeService::toUserMap), "users");
  }

  @GetMapping("/{userId}")
  public Mono<ResponseEntity<ApiResponse<Map<String, Object>>>> getUser(
      @PathVariable String userId) {
    return userAccountService
        .getUser(userId)
        .map(u -> ResponseEntity.ok(ApiResponse.ok(facadeService.toUserMap(u))))
        .switchIfEmpty(
            Mono.just(ResponseEntity.status(404).body(ApiResponse.error("User not found"))));
  }
  ...
  public Mono<ResponseEntity<ApiResponse<Map<String, Object>>>> updateUserTier(
  ...
  public Mono<ResponseEntity<ApiResponse<Map<String, Object>>>> deactivateUser(
    ...
    return userAccountService
        ...
        .map(user -> ResponseEntity.ok(ApiResponse.ok(facadeService.toUserMap(user))))
        .onErrorResume(
            e -> Mono.just(ResponseEntity.status(404).body(ApiResponse.error(e.getMessage()))));
  }
    ...
    return Mono.fromCallable(
            () -> {
              List<Map<String, Object>> tiers =
                  java.util.stream.Stream.of(UserTier.values())
                      .map(
                          tier -> {
                            ...
                            tierMap.put(
                                "displayName",
                                tier.name().charAt(0) + tier.name().substring(1).toLowerCase());
