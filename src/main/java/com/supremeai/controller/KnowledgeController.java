...
public class KnowledgeController {
    ...
    /**
     * DELETE /api/knowledge/clear-scraped
     * Purge all web-scraped knowledge entries (Admin only).
     */
    @DeleteMapping("/clear-scraped")
    @PreAuthorize("hasRole('ADMIN')")
    public Mono<ResponseEntity<Map<String, String>>> clearScrapedLearnings() {
        return knowledgeService.clearOldScrapedLearnings()
                .thenReturn(ResponseEntity.ok(Map.of(
                        "status", "success",
                        "message", "All web-scraped knowledge entries have been purged from Firestore.")));
    }
    ...
    public ResponseEntity<Map<String, Object>> handleScrapingTimeout(TimeoutException ex) {
        ...
        errorDetails.put("action_required", "Please verify the 'Recent Learnings' feed in a few moments.");
