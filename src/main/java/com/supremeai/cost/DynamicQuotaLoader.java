  ...
  public void loadQuotasFromFile(String filePath) {
    ...
    try (FileInputStream in = new FileInputStream(filePath)) {
      ...
      for (String key : properties.stringPropertyNames()) {
        // Expected format: ServiceName.FeatureName.Limit=Value OR
        // ServiceName.FeatureName.Period=Value
        ...
        if (key.endsWith(".Limit")) {
          ...
          if (parts.length == 2) {
            ...
            quotaManager.registerQuota(
                new QuotaDefinition(serviceName, featureName, limit, period));
          }
        }
      }
    } catch (IOException e) {
