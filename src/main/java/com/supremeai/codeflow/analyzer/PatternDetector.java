...
public class PatternDetector {
  ...
  private List<CodeRepository.PatternDetection> detectPatternsInFile(CodeRepository.CodeFile file) {
    ...
    for (CodeRepository.ClassInfo clazz : file.getClasses()) {
      ...
      if (isSingleton(clazz, file)) {
        patterns.add(
            createPattern("SINGLETON", "Singleton pattern detected", file, clazz.getLine(), 95));
      }

      // Factory pattern
      if (isFactory(clazz, file)) {
        patterns.add(
            createPattern("FACTORY", "Factory pattern detected", file, clazz.getLine(), 90));
      }

      // Observer pattern
      if (isObserver(clazz, file)) {
        patterns.add(
            createPattern("OBSERVER", "Observer pattern detected", file, clazz.getLine(), 85));
      }

      // Strategy pattern
      if (isStrategy(clazz, file)) {
        patterns.add(
            createPattern("STRATEGY", "Strategy pattern detected", file, clazz.getLine(), 85));
      }

      // Decorator pattern
      if (isDecorator(clazz, file)) {
        patterns.add(
            createPattern("DECORATOR", "Decorator pattern detected", file, clazz.getLine(), 80));
      }

      // Builder pattern
      if (isBuilder(clazz, file)) {
        patterns.add(
            createPattern("BUILDER", "Builder pattern detected", file, clazz.getLine(), 90));
      }

      // Repository pattern
      if (isRepository(clazz, file)) {
        patterns.add(
            createPattern("REPOSITORY", "Repository pattern detected", file, clazz.getLine(), 95));
      }

      // Service pattern
      if (isService(clazz, file)) {
        patterns.add(
            createPattern("SERVICE", "Service pattern detected", file, clazz.getLine(), 90));
      }

      // Controller pattern
      if (isController(clazz, file)) {
        patterns.add(
            createPattern("CONTROLLER", "Controller pattern detected", file, clazz.getLine(), 95));
      }
    }
  ...
  /** Detect cross-file patterns */
  private List<CodeRepository.PatternDetection> detectCrossFilePatterns(
      List<CodeRepository.CodeFile> files) {
    List<CodeRepository.PatternDetection> patterns = new ArrayList<>();
  ...
  private boolean isSingleton(CodeRepository.ClassInfo clazz, CodeRepository.CodeFile file) {
    ...
    boolean hasPrivateConstructor =
        file.getFunctions() != null
            && file.getFunctions().stream()
                .anyMatch(
                    f ->
                        f.getName().equals(clazz.getName())
                            && f.getIsPrivate() != null
                            && f.getIsPrivate());
  ...
  private boolean isFactory(CodeRepository.ClassInfo clazz, CodeRepository.CodeFile file) {
    ...
    boolean hasCreateMethods =
        clazz.getMethods() != null
            && clazz.getMethods().stream()
                .anyMatch(
                    m ->
                        m.getName() != null
                            && m.getName().matches("(create|make|build|newInstance).*")
                            && m.getReturnType() != null
                            && !m.getReturnType().equals("void"));

    // Check for interface/abstract class return types
    boolean returnsAbstraction =
        clazz.getMethods() != null
            && clazz.getMethods().stream()
                .anyMatch(
                    m ->
        ...
        clazz.getMethods() != null
            && clazz.getMethods().stream()
                .anyMatch(
                    m ->
                        m.getName() != null
                            && m.getName().matches("(subscribe|register|attach|addListener).*"));
        ...
        clazz.getMethods() != null
            && clazz.getMethods().stream()
                .anyMatch(
                    m ->
                        m.getName() != null
                            && m.getName().matches("(notify|publish|emit|broadcast).*"));
  ...
  private boolean isStrategy(CodeRepository.ClassInfo clazz, CodeRepository.CodeFile file) {
    ...
    boolean hasAlgorithmMethods =
        clazz.getMethods() != null
            && clazz.getMethods().stream()
                .anyMatch(
                    m ->
                        m.getName() != null
                            && m.getName().matches("(execute|run|process|calculate|algorithm).*")
                            && m.getReturnType() != null
                            && !m.getReturnType().equals("void"));

    // Check for context that uses strategy
    boolean hasContext =
        file.getClasses() != null
            && file.getClasses().stream()
                .anyMatch(
                    c ->
                        c.getName() != null
                            && (c.getName().contains("Context")
                                || c.getName().contains("Strategy")));
  ...
  private boolean isDecorator(CodeRepository.ClassInfo clazz, CodeRepository.CodeFile file) {
    ...
    boolean hasWrapper =
        clazz.getImplementsInterfaces() != null
            && clazz.getImplementsInterfaces().stream()
                .anyMatch(
                    i ->
                        file.getClasses() != null
                            && file.getClasses().stream()
                                .anyMatch(
                                    c ->
                                        c.getImplementsInterfaces() != null
                                            && c.getImplementsInterfaces().contains(i)));

    // Check for component reference
    boolean hasComponent =
        clazz.getFields() != null
                        ...
                        file.getClasses() != null
                            && file.getClasses().stream()
                                .anyMatch(
                                    c ->
                                        c.getImplementsInterfaces() != null
                                            && c.getImplementsInterfaces().contains(f)));
  ...
  private boolean isRepository(CodeRepository.ClassInfo clazz, CodeRepository.CodeFile file) {
    ...
    boolean hasCrudMethods =
        clazz.getMethods() != null
            && clazz.getMethods().stream()
                .anyMatch(
                    m ->
                        m.getName() != null
                            && m.getName().matches("(save|find|delete|update|create|get|list).*"));
  ...
  private boolean isService(CodeRepository.ClassInfo clazz, CodeRepository.CodeFile file) {
    boolean hasServiceName =
        clazz.getName() != null && clazz.getName().toLowerCase().contains("service");
    boolean hasServiceAnnotation =
        file.getContent() != null && file.getContent().contains("@Service");
  ...
  private boolean isController(CodeRepository.ClassInfo clazz, CodeRepository.CodeFile file) {
    boolean hasControllerName =
        clazz.getName() != null && clazz.getName().toLowerCase().contains("controller");
  ...
  private List<CodeRepository.PatternDetection> detectReactHooks(CodeRepository.CodeFile file) {
    ...
    for (String hook : hooks) {
      if (content.contains(hook + "(")) {
        patterns.add(
            createPattern(
                "REACT_" + hook.toUpperCase(), "React " + hook + " hook detected", file, 0, 95));
      }
    }
  ...
  /** Detect layered architecture */
  private List<CodeRepository.PatternDetection> detectLayeredArchitecture(
      List<CodeRepository.CodeFile> files) {
    List<CodeRepository.PatternDetection> patterns = new ArrayList<>();
    ...
    boolean hasController =
        files.stream()
            .anyMatch(
                f ->
                    f.getClasses() != null
                        && f.getClasses().stream().anyMatch(c -> isController(c, f)));
    ...
    boolean hasService =
        files.stream()
            .anyMatch(
                f ->
                    f.getClasses() != null
                        && f.getClasses().stream().anyMatch(c -> isService(c, f)));
    ...
    boolean hasRepository =
        files.stream()
            .anyMatch(
                f ->
                    f.getClasses() != null
                        && f.getClasses().stream().anyMatch(c -> isRepository(c, f)));
  ...
  /** Detect dependency injection */
  private List<CodeRepository.PatternDetection> detectDependencyInjection(
      List<CodeRepository.CodeFile> files) {
    List<CodeRepository.PatternDetection> patterns = new ArrayList<>();

    boolean hasAutowired =
        files.stream()
            .anyMatch(f -> f.getContent() != null && f.getContent().contains("@Autowired"));
    ...
    if (hasAutowired || hasInject) {
      patterns.add(
          createPattern(
              "DEPENDENCY_INJECTION", "Dependency injection pattern detected", null, 0, 95));
    }
  ...
  /** Detect module boundaries */
  private List<CodeRepository.PatternDetection> detectModuleBoundaries(
      List<CodeRepository.CodeFile> files) {
    List<CodeRepository.PatternDetection> patterns = new ArrayList<>();

    // Group files by directory
    Map<String, List<CodeRepository.CodeFile>> modules =
        files.stream()
            .collect(
  ...
  private CodeRepository.PatternDetection createPattern(
      String type, String description, CodeRepository.CodeFile file, int line, int confidence) {
