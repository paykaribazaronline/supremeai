...
/** Dependency graph analyzer Builds call graphs and computes blast radius */
@Component
public class DependencyAnalyzer {
  ...
  private CodeRepository.DependencyGraph.Node createNode(CodeRepository.CodeFile file) {
    ...
    // setFile, setType, setLinesOfCode, setComplexity, setFanIn, setFanOut, setCentralityScore,
    // setMetadata
    return node;
  }
  ...
  private String findFileForFunction(String functionName, List<CodeRepository.CodeFile> files) {
    for (CodeRepository.CodeFile file : files) {
      if (file.getFunctions() != null) {
        boolean found =
            file.getFunctions().stream().anyMatch(f -> f.getName().equals(functionName));
        if (found) {
          return file.getPath();
        }
      }
  ...
  private List<String> findCriticalPath(
    ...
    return nodes.stream()
        .sorted(
            (n1, n2) -> {
              // In a manual environment, we might need to implement getCentralityScore and
              // getFanOut manually in Node
              return 0;
            })
        .limit(5)
