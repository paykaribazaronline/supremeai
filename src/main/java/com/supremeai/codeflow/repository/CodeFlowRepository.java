...
/** Firestore repository for CodeFlow module Handles CRUD operations for code analysis data */
@Repository
public class CodeFlowRepository {
  ...
  public Optional<CodeRepository> findById(String id)
      throws ExecutionException, InterruptedException {
  ...
  public Optional<CodeRepository> findBySourceId(String sourceId)
      throws ExecutionException, InterruptedException {
    Firestore db = FirestoreClient.getFirestore();
    QuerySnapshot query =
        db.collection(COLLECTION_NAME).whereEqualTo("sourceId", sourceId).limit(1).get().get();
  ...
  public List<CodeRepository> findByOwnerId(String ownerId)
      throws ExecutionException, InterruptedException {
    Firestore db = FirestoreClient.getFirestore();
    QuerySnapshot query =
        db.collection(COLLECTION_NAME).whereEqualTo("ownerId", ownerId).get().get();
  ...
  public List<CodeRepository> findBySecuritySeverity(String severity)
    ...
    QuerySnapshot query = db.collection(COLLECTION_NAME).get().get();
  ...
  public long count() throws ExecutionException, InterruptedException {
    Firestore db = FirestoreClient.getFirestore();
    AggregateQuerySnapshot snapshot = db.collection(COLLECTION_NAME).count().get().get();
