...
/**
 * Error resolution workflow service Analyzes errors, identifies affected components, and generates
 * fixes
 */
@Service
public class ErrorResolutionService {
  ...
  public ErrorResolutionService(
      CodeFlowRepository repository, AIProviderFactory providerFactory, CodeAnalyzer codeAnalyzer) {
  ...
  public CodeRepository.ErrorAnalysis analyzeError(
    ...
    try {
      ...
      AIProvider aiProvider =
          Mono.fromCallable(
                  () ->
                      (AIProvider)
                          providerFactory
                              .getBestProviderForTask("error_analysis")
      ...
      // Parse AI response
      CodeRepository.ErrorAnalysis analysis =
          parseErrorAnalysis(response, errorType, stackTrace, repo);
    ...
    try {
      ...
      for (String frame : stackFrames) {
        for (CodeRepository.CodeFile file : repo.getFiles()) {
          if (frame.contains(file.getName()) || frame.contains(file.getPath().replace("/", "."))) {
      ...
      AIProvider aiProvider =
          Mono.fromCallable(
                  () ->
                      (AIProvider)
                          providerFactory
                              .getBestProviderForTask("fix_generation")
                              .subscribeOn(reactor.core.scheduler.Schedulers.boundedElastic())
                              .block(java.time.Duration.ofSeconds(30)))
              .subscribeOn(Schedulers.boundedElastic())
              .block(Duration.ofSeconds(35));
  ...
  public List<String> getCommonFixes(String errorType) {
    ...
    return commonFixes.getOrDefault(
        errorType,
        Arrays.asList(
  ...
  private CodeRepository.ErrorAnalysis parseErrorAnalysis(
    ...
    if (repo.getFiles() != null) {
      for (CodeRepository.CodeFile file : repo.getFiles()) {
        if (response.contains(file.getName()) || response.contains(file.getPath())) {
  ...
  private List<String> findRelatedComponents(String file, CodeRepository.DependencyGraph graph) {
