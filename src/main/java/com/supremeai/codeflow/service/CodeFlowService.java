...
/** Main service for CodeFlow module Handles code analysis, AI integration, and health scoring */
@Service
public class CodeFlowService {
      ...
      CompletableFuture.runAsync(
          () -> {
            try {
              ...
              try {
                repository.updateAnalysisStatus(repoId, CodeRepository.AnalysisStatus.FAILED);
              } catch (Exception ex) {
  ...
  public void performFullAnalysis(String repositoryId) throws Exception {
    ...
    try {
      ...
      Map<String, Integer> langStats =
          files.stream()
              .collect(
                  Collectors.groupingBy(
                      CodeRepository.CodeFile::getLanguage, Collectors.summingInt(f -> 1)));
