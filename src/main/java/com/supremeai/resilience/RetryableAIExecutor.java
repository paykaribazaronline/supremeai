 ...
 *
 * <p>Features: - Exponential backoff with jitter between retries - Configurable max attempts per
 * provider - Quota-aware: skips retry if quota is exhausted - Respects circuit breaker state -
...
public class RetryableAIExecutor {
  ...
  @Autowired private QuotaManager quotaManager;
    ...
    return Mono.defer(
        () -> {
          ...
          return operation
              ...
              .doOnError(
                  e -> log.warn("Retry attempt failed for {}: {}", providerName, e.getMessage()));
        });
  }

  /** Execute an AI call with retry and a custom circuit breaker reactively. */
  public <T> Mono<T> executeWithCircuitBreakerReactive(
      String providerName, String serviceName, CircuitBreaker circuitBreaker, Mono<T> operation) {
    return Mono.defer(
        () -> {
          if (!quotaManager.recordUsage(serviceName, "Requests", 0)) {
            return Mono.error(new QuotaExceededException("Quota exceeded for " + serviceName));
          ...
          if (circuitBreaker.getState() == CircuitBreaker.State.OPEN) {
            return Mono.error(
                new CircuitBreakerOpenException("Circuit breaker is OPEN for " + providerName));
          }
          ...
          return operation
              ...
              .doOnError(
                  e ->
                      log.warn(
                          "Reactive execution failed for {}: {}", providerName, e.getMessage()));
        });
  }
