...
public class QuotaService {
  ...
  /** Increment usage for an API key Returns true if successful, false if quota exceeded */
              ...
              return userApiKeyRepository
                  ...
                  .onErrorResume(
                      throwable -> {
                        ...
                        logger.warn(
                            "Concurrent modification detected for API key {}, retrying", apiKey);
                        return userApiKeyRepository
                            .findByApiKey(apiKey)
                            .flatMap(
                                retryApi -> {
                                  if (!"active".equalsIgnoreCase(retryApi.getStatus())) {
                                    return Mono.error(
                                        new IllegalArgumentException("Inactive API key"));
                                  }
  ...
  /**
   * Reset monthly usage for all APIs - runs on the 1st of every month at midnight Optimized for
  ...
  public static class ApiUsageStats {
    ...
    public ApiUsageStats(
        Long currentUsage, Long monthlyQuota, LocalDateTime lastUsedAt, boolean hasQuotaRemaining) {
