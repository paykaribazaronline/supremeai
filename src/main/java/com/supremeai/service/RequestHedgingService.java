...
/**
 * Request hedging service for critical paths. Sends requests to multiple providers simultaneously
 * and takes the first response.
 */
@Service
public class RequestHedgingService {
  ...
  /**
   * Execute request with hedging - send to multiple providers, take first response. Useful for
   * critical paths where latency is more important than cost.
   */
  public <T> T executeWithHedging(List<Supplier<T>> providers, long timeoutMs) {
    ...
    try {
      if (hedgedFuture != null) {
        ...
        T result =
            (T)
                CompletableFuture.anyOf(primaryFuture, hedgedFuture)
  ...
  public <T> CompletableFuture<T> executeWithHedgingAsync(
      List<Supplier<CompletableFuture<T>>> providers, long timeoutMs) {
