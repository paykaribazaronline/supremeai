...
/**
 * Manages active simulator runtime sessions. Tracks session metadata, heartbeat, and provides
 * lookup.
 */
@Service
public class SimulatorSessionService {
  ...
  public String registerSession(String appId, String deviceType) {
    String sessionId =
        "sess_" + System.currentTimeMillis() + "_" + sessionCounter.incrementAndGet();
    RuntimeSession session = new RuntimeSession(sessionId, appId, deviceType);
    sessions.put(sessionId, session);
    logger.info(
        "[SESSION] Registered new session: {} for app={} device={}", sessionId, appId, deviceType);
    return sessionId;
  }
