...
/**
 * Alerting service for circuit breaker trips and high error rates. Integrates with monitoring
 * systems for real-time notifications.
 */
@Service
public class AlertingService {
  ...
  private boolean isCircuitBreakerAlertingEnabled() {
    ...
    Object val =
        configService.getEffectiveSetting(
            "alerting.circuit-breaker.enabled", circuitBreakerAlertingEnabled);
  ...
  private double getErrorRateThreshold() {
    ...
    Object val =
        configService.getEffectiveSetting("alerting.error-rate.threshold", errorRateThreshold);
  ...
  private long getAlertCooldownMinutes() {
    ...
    Object val =
        configService.getEffectiveSetting("alerting.cooldown.minutes", alertCooldownMinutes);
    if (val instanceof Number) {
  ...
  public AlertingService(MeterRegistry meterRegistry) {
    ...
    this.circuitBreakerTrips =
        Counter.builder("alerts.circuitbreaker.trips")
            .description("Number of circuit breaker trip alerts")
            .register(meterRegistry);
    this.highErrorRateAlerts =
        Counter.builder("alerts.errorrate.high")
  ...
  public void sendCircuitBreakerAlert(String provider, String state, int failureCount) {
    ...
    if (shouldSendAlert(alertKey)) {
      log.warn(
          "CIRCUIT BREAKER ALERT: Provider {} is in {} state (failures: {})",
          provider,
          state,
          failureCount);
  ...
  public void sendHighErrorRateAlert(String endpoint, double errorRate, int totalRequests) {
    ...
    if (shouldSendAlert(alertKey)) {
      log.error(
          "HIGH ERROR RATE ALERT: Endpoint {} has error rate {}% ({} requests)",
  ...
  public static class AlertStats {
    ...
    public void setCircuitBreakerTrips(long circuitBreakerTrips) {
      this.circuitBreakerTrips = circuitBreakerTrips;
    }

    public long getHighErrorRateAlerts() {
      return highErrorRateAlerts;
    }

    public void setHighErrorRateAlerts(long highErrorRateAlerts) {
      this.highErrorRateAlerts = highErrorRateAlerts;
    }
    ...
    public static class Builder {
      ...
      public Builder circuitBreakerTrips(long val) {
        this.circuitBreakerTrips = val;
        return this;
      }

      public Builder highErrorRateAlerts(long val) {
        this.highErrorRateAlerts = val;
        return this;
      }

      public Builder recentAlertCount(int val) {
        this.recentAlertCount = val;
        return this;
      }
