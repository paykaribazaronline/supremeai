...
public class AutonomousQuestioningEngine {
  ...
  @Autowired private com.supremeai.service.SupremeAIBrain supremeAIBrain;
  ...
  /** NEURAL ROUTING: Instead of hardcoded Regex, we ask the Supreme Brain to classify intent. */
  public Mono<IntentType> classifyIntentAI(String input) {
    String prompt =
        signatureRegistry.getSignatures("INTENT_CLASSIFICATION_PROMPT").stream()
            .findFirst()
            .orElse("Classify the intent for: \"%s\"")
            .formatted(input);

    return supremeAIBrain
        .think("ORCHESTRATION", prompt)
        .map(
            response -> {
              try {
                    ...
                    () -> {
                      ...
                      if (intent == IntentType.GREETING) {
                        ...
                        if (userInput != null && userInput.trim().length() >= 3) {
                          ...
                          isComplete = false;
                          questions.add(
                              "Could you elaborate on your request? The input seems too brief.");
                        }
                      } else {
  ...
  public Mono<Double> calculateClarityScoreAI(String userInput, RequestType requestType) {
    String prompt =
        "Rate the clarity of this request for "
            + requestType
            + " from 0.0 to 1.0.\n"
            + "Input: \""
            + userInput
