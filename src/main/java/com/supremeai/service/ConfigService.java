...
public class ConfigService {
        ...
        .doOnNext(
            cfg -> {
              ...
              if (redisTemplate != null) {
                try {
                  redisTemplate.opsForValue().set(REDIS_CONFIG_KEY, cfg, CACHE_TTL);
                } catch (Exception e) {
        ...
        .switchIfEmpty(
            Mono.defer(
                () -> {
                  ...
                  return systemConfigRepository
                      .save(def)
                      .doOnNext(saved -> this.cachedConfig = saved);
                }));
  }
  ...
  public Mono<SystemConfig> updateConfig(
      SystemConfig newConfig, String actorUserId, String ipAddress) {
    logger.info(
        "[CONFIG] Update requested by {} from {}. Version: {}",
        actorUserId,
        ipAddress,
    ...
    logger.debug(
        "[CONFIG] Saving to Firestore: collections={}, thresholds={}, timeouts={}",
        newConfig.getCollections().size(),
        newConfig.getThresholds().size(),
        newConfig.getTimeouts().size());
            ...
            saved -> {
              ...
              if (redisTemplate != null) {
                try {
                  ...
                  logger.warn("Redis invalidate failed: {}", e.getMessage());
                }
              }
              return saved;
            })
