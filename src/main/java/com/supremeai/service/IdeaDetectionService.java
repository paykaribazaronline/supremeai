 ...
 *
 * <p>Monitors conversation text and detects "brilliant ideas" based on heuristic scoring (novelty
 * keywords + context signals). Detected ideas are queued for admin review.
 *
 * <p>Scoring approach: - Innovation keywords: +10 each - Monetization signals: +8 each -
 * Problem-solving framing: +6 each - Question marks / uncertainty: -2 (reduces speculation) -
...
public class IdeaDetectionService {
  ...
  private static final List<String> INNOVATION_KEYWORDS =
      List.of(
          "novel",
          "unique",
          "breakthrough",
  ...
  private static final List<String> MONETIZATION_KEYWORDS =
      List.of(
          "revenue",
          "monetize",
          "profit",
  ...
  private static final List<String> SOLUTION_KEYWORDS =
      List.of(
          "solve",
          "fix",
          "automate",
  ...
  public IdeaAnalysisResult analyze(String text, String source) {
    ...
    if (isBrilliant) {
      ...
      logger.info(
          "[IDEA] Brilliant idea detected from source={} score={} signals={}",
          source,
          score,
          matchedSignals.size());
    ...
    return IdeaAnalysisResult.notIdea(
        score, "Score below threshold (" + getDetectionThreshold() + ")");
  }

  /** Get all ideas pending admin review. */
  public List<DetectedIdea> getPendingReviewQueue() {
    return new ArrayList<>(
        reviewQueue.stream().filter(i -> i.getStatus() == IdeaStatus.PENDING).toList());
  }
  ...
  public static class DetectedIdea {
    ...
    public DetectedIdea(
        String ideaId, String textSnippet, String source, int score, List<String> signals) {
      this.ideaId = ideaId;
  ...
  public static class IdeaAnalysisResult {
    ...
    private IdeaAnalysisResult(
        boolean brilliant, int score, List<String> signals, String message, String queuedIdeaId) {
    ...
    public static IdeaAnalysisResult brilliant(int score, List<String> signals, String ideaId) {
      return new IdeaAnalysisResult(
          true, score, signals, "Brilliant idea detected and queued for review", ideaId);
    }
