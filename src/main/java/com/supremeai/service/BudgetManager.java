...
public class BudgetManager {
  ...
  private final java.util.Map<String, Double> userCosts =
  ...
  public String generateAlerts() {
    ...
    if (usagePercentage > 100) {
      return "CRITICAL: Budget exceeded! Current usage: "
          + String.format("%.2f", usagePercentage)
          + "%";
    } else if (usagePercentage > 90) {
      return "WARNING: Approaching budget limit. Current usage: "
          + String.format("%.2f", usagePercentage)
          + "%";
    }
    return "INFO: Budget is healthy. Current usage: "
        + String.format("%.2f", usagePercentage)
        + "%";
  }
}
