...
public class MonitoringService {
  ...
  /** Broadcast system resource metrics every 5 seconds. This replaces polling from the frontend. */
      ...
      if (metrics != null) {
        ...
        if (cpuUsage > CPU_ALERT_THRESHOLD) {
          broadcastLog(
              "ALERT",
              "System",
              "High CPU Usage detected: " + String.format("%.2f", cpuUsage) + "%");
        }
        if (memoryUsage > MEMORY_ALERT_THRESHOLD) {
          broadcastLog(
              "ALERT",
              "System",
              "High Memory Usage detected: " + String.format("%.2f", memoryUsage) + "%");
        }
