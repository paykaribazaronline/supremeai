...
/**
 * Root Cause Analysis and Automatic Self-Correction Service. Analyzes failures to identify root
 * causes and automatically applies fixes.
 */
@Service
public class RootCauseAnalysisService {
  ...
  @Autowired private EnhancedRandomForestPredictor failurePredictor;
  ...
  private static final Pattern ERROR_TYPE_PATTERN =
      Pattern.compile("([a-zA-Z]+Error|[a-zA-Z]+Exception):\\s*(.+)");
  private static final Pattern NULL_POINTER_PATTERN = Pattern.compile("NullPointer|null");
  private static final Pattern IMPORT_PATTERN =
      Pattern.compile("cannot find symbol|unresolved symbol|Import.*not found");
  ...
  public RootCauseAnalysis analyzeError(
    ...
    Map<EnhancedRandomForestPredictor.FeatureType, Double> features =
        extractErrorFeatures(errorMessage, codeContext);

    // Predict if this will be a failure
    EnhancedRandomForestPredictor.FailurePrediction prediction =
        failurePredictor.predict(errorSignature, features);
    ...
    CorrectionAction correction =
        rootCause != null ? rootCause.suggestedAction : CorrectionAction.MANUAL_REVIEW;
  ...
  /**
   * Record a failed correction attempt so the ML failure predictor learns from the miss. Called by
   * SelfHealingService when an auto-fix suggestion dissolves into an error.
   */
  public void recordFailedCorrection(
    ...
    log.info(
        "Recorded failed correction for fingerprint={} — ML predictor updated", errorSignature);
  }

  /** Extract features from error for ML prediction. */
  Map<EnhancedRandomForestPredictor.FeatureType, Double> extractErrorFeatures(
      String errorMessage, String codeContext) {
    ...
    features.put(
        EnhancedRandomForestPredictor.FeatureType.ERROR_FREQUENCY, history != null ? 1.0 : 0.0);
    ...
    features.put(
        EnhancedRandomForestPredictor.FeatureType.ERROR_MESSAGE_LENGTH,
  ...
  private String applyAutoCorrection(String code, RootCausePattern rootCause, String errorMessage) {
    ...
    try {
      ...
      log.error(
          "Failed to apply auto-correction for action {}: {}",
  ...
  private String addZeroCheck(String code) {
    ...
    while (matcher.find()) {
      ...
      String replacement =
          "(((" + denominator + ") != 0) ? (" + matcher.group(1) + " / " + denominator + ") : 0)";
  ...
  private String inferPackageName(String className) {
    ...
    Map<String, String> commonClasses =
        Map.of(
            "ArrayList", "java.util",
            "HashMap", "java.util",
            "List", "java.util",
  ...
  public Mono<Void> recordSuccessfulCorrection(String errorSignature, String correctedCode) {
    ...
    if (record != null) {
      ...
      return globalKnowledgeBase.recordSuccessWithPermission(
          errorSignature, correctedCode, "RootCauseAnalysisService", 0, 0.9);
    }
    return Mono.empty();
  }
  ...
  public Map<String, Object> getStatistics() {
    ...
    stats.put(
        ...
        correctionHistory.values().stream().filter(r -> r.wasSuccessful).count());
    return stats;
  }
