...
/**
 * Local ConfigService for local development profile. Provides configuration management without
 * Firestore dependency. This service is used when running with the "local" profile.
 */
@Profile("local")
@Service
public class ConfigServiceLocal {

  private SystemConfig cachedConfig;

  public ConfigServiceLocal() {
  ...
  private SystemConfig createDefaultConfig() {
    ...
    config.getTierMaxSimulatorInstalls().put(UserTier.ADMIN.name(), -1);
