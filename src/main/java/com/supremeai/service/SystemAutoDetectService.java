 ...
 * Fully reactive implementation using non-blocking pipelines.
 */
@Service
public class SystemAutoDetectService {
    ...
    /**
     * Get the best available provider reactively.
     * Returns cached provider if available and not expired.
     */
    public Mono<AIProvider> getProvider() {
        ...
        return providerRepository.findByStatus("active")
            ...
            .flatMap(dbProvider -> Mono.fromCallable(() -> providerFactory.getProvider(dbProvider.getName()))
    ...
    /**
     * Truly non-blocking health check — returns Mono<Boolean>.
     * No .block() calls; runs entirely on reactive pipeline.
     */
