...
/**
 * Reactive streams service with backpressure support. Handles high-volume scenarios with proper
 * flow control.
 */
@Service
public class ReactiveStreamService {
  ...
  /** Process items with backpressure using Reactor. Automatically handles flow control. */
  public <T, R> Flux<R> processWithBackpressure(
      List<T> items, Function<T, Mono<R>> processor, int concurrency) {
  ...
  public <T, R> Flux<R> processWithRateLimit(
      List<T> items, Function<T, Mono<R>> processor, int maxConcurrency, int rateLimitPerSecond) {

    return Flux.fromIterable(items)
        .window(rateLimitPerSecond)
        .concatMap(window -> window.flatMap(item -> processor.apply(item), maxConcurrency))
        .subscribeOn(Schedulers.boundedElastic());
  }
  ...
  public <T, R> Mono<List<R>> processParallel(
      List<T> items, Function<T, Mono<R>> processor, int parallelism) {
  ...
  public <T, R> Flux<R> processWithErrorHandling(
    ...
    return Flux.fromIterable(items)
        .flatMap(item -> processor.apply(item).onErrorResume(e -> Mono.just(errorHandler.apply(e))))
        .subscribeOn(Schedulers.boundedElastic());
  }

  /** Batch processing with backpressure. */
  public <T, R> Flux<List<R>> processBatches(
      List<T> items, Function<List<T>, Mono<List<R>>> batchProcessor, int batchSize) {
