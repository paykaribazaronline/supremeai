...
/**
 * Retry service with exponential backoff and jitter. Provides resilient retry logic for failed
 * operations.
 */
@Service
public class RetryService {
  ...
  /**
   * Execute with full jitter strategy (random delay up to calculated backoff). This is the most
   * effective jitter strategy for avoiding thundering herd.
   */
  public <T> T executeWithFullJitter(
      Supplier<T> operation, int maxRetries, long initialDelayMs, long maxDelayMs) {
    int attempts = 0;
    long delay = initialDelayMs;
