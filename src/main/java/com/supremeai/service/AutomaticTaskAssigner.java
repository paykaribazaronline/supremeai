 ...
 *
 * <p>When a new provider is added, this service: 1. Receives the provider's benchmark scores 2.
 * Auto-assigns it to task types where it scores >= threshold 3. Creates or updates task assignments
 * in the DB 4. Can remove poor performers during nightly rebalance
 */
@Service
public class AutomaticTaskAssigner {
  ...
  private static final List<TaskConfig> ALL_TASK_TYPES =
      List.of(
  ...
  public Mono<TaskProviderAssignment> autoAssignNewProvider(
      String providerId, Map<String, Double> scores) {
    ...
    return Flux.fromIterable(ALL_TASK_TYPES)
        ...
        .flatMap(
            task ->
                assignmentRepo
                    .findByTaskType(task.name)
                    .flatMap(
                        existing -> {
                          List<String> updatedProviders =
                              new ArrayList<>(existing.getProviderIds());
                    ...
                    .switchIfEmpty(
                        Mono.defer(
                            () -> {
                              ...
                              log.info(
                                  "Created new assignment: task='{}' -> provider={}",
                                  task.name,
                                  providerId);
        ...
        .doOnNext(
            assignments ->
                log.info(
                    "Provider {} auto-assigned to {} task types", providerId, assignments.size()))
        .flatMap(
            assignments -> {
  ...
  private List<String> keepTopProviders(List<String> providerIds, String taskType) {
    return providerIds.stream().limit(DEFAULT_MAX_PROVIDERS).collect(Collectors.toList());
  }
        ...
        .flatMap(
            assignment -> {
              ...
              return Flux.fromIterable(currentProviders)
                  .flatMap(
                      providerId ->
                          getLatestScore(providerId, taskType)
                  ...
                  .flatMap(
                      poorPerformers -> {
                        if (!poorPerformers.isEmpty()
                            && currentProviders.size() - poorPerformers.size()
                                >= assignment.getMinProviders()) {
                          for (String pp : poorPerformers) {
                            log.warn(
                                "Provider {} underperforming on {} (threshold: {})",
                                pp,
                                taskType,
                                assignment.getCapabilityThreshold());
                          }
                          ...
                          log.info(
                              "Removed {} poor performers from task '{}'",
                              poorPerformers.size(),
                              taskType);
  ...
  private Mono<Double> getLatestScore(String providerId, String taskType) {
    return providerRepo
        ...
        .onErrorResume(
            e -> {
              log.debug(
                  "Could not get score for {} on {}: {}", providerId, taskType, e.getMessage());
  ...
  public Mono<List<String>> getProvidersForTask(String taskType) {
    return assignmentRepo
        ...
        .switchIfEmpty(providerRepo.findByStatus("active").map(APIProvider::getId).collectList());
  }
