...
/**
 * Scheduled validation service for admin-managed APIProvider keys. Runs daily at 3 AM (server time)
 * and increments error streak for failed keys. Quarantines providers with 3 consecutive failures by
 * setting status="dead" and deadReason.
 *
 * <p>(এডমিন ম্যানেজ করা APIProvider কীগুলোর স্বয়ংকřiব preferably np胆碱તলদা সমronĐ ই永মতর কার্ক।)
 */
@Service
public class AdminProviderValidationService {
  ...
  private static final int ERROR_THRESHOLD =
      3; // Quarantine after 3 consecutive failures (prevents false positives)

  @Autowired private ProviderRepository providerRepository;

  @Autowired private AIProviderDiscoveryService discoveryService;

  @Autowired private APIHealthReportRepository healthReportRepository;

  /**
   * Daily cron (3 AM) to validate all active admin provider keys. Uses reactive parallel processing
   * (max 10 concurrent) to avoid timeouts. Updates consecutiveErrorDays, lastValidated, and
   * quarantines dead providers.
   */
  @Scheduled(cron = "0 0 3 * * *")
  public void validateAllActiveProviders() {
    log.info("Running scheduled admin provider auto-validation...");

    // Reactive pipeline: fetch all providers, filter active, validate concurrently, and generate
    // report.
    providerRepository
        .findAll()
        .filter(
                      ...
                      provider ->
                          discoveryService
                              ...
                              .map(
                                  valid -> {
                                    ...
                                    if (valid) {
                                      ...
                                      int streak =
                                          Optional.ofNullable(provider.getConsecutiveErrorDays())
                                                  .orElse(0)
                                              + 1;
                                      ...
                                      if (streak >= ERROR_THRESHOLD) {
                                        ...
                                        log.error(
                                            "Provider '{}' ({}) quarantined as DEAD. Reason: {}",
                                            provider.getName(),
                                            provider.getId(),
                                            deadReason);
                                      } else {
                                        log.warn(
                                            "Provider '{}' ({}) validation failed (streak: {}/{}) — status preserved as '{}'",
                                            provider.getName(),
                                            provider.getId(),
                                            streak,
                                            ERROR_THRESHOLD,
                              ...
                              .doOnNext(
                                  p ->
                                      log.info(
                                          "Provider '{}' validation complete. status={}",
                                          p.getName(),
                                          p.getStatus()))
                              .onErrorResume(
                                  e -> {
        ...
        .subscribe(null, err -> log.error("validateAllActiveProviders: pipeline failed", err));
  }
  ...
  private void generateAndSaveReport(List<APIProvider> providers) {
    ...
    List<Map<String, Object>> deadDetails =
        providers.stream()
            ...
            .map(
                p -> {
                  ...
                  map.put(
                      "reason",
                      p.getDeadReason() != null ? p.getDeadReason() : "Validation failed");
                  return map;
                })
            .collect(Collectors.toList());

    com.supremeai.model.APIHealthReport report =
        new com.supremeai.model.APIHealthReport(
            "REPORT_" + System.currentTimeMillis(), providers.size(), (int) active, (int) dead, 0);
  ...
  /**
   * On-demand provider validation — usable from ChatProcessingService RUN_AUDIT command. Scans all
   * active providers (max 10 concurrent), validates each against the discovery service, and stores
   * updated error-streak metadata. Returns a count summary map.
   */
        ...
        .flatMap(
            allProviders -> {
              if (allProviders == null || allProviders.isEmpty()) {
                ...
                return Mono.just(
                    Map.<String, Object>of("status", "ok", "total", 0, "valid", 0, "failed", 0));
              }
              List<APIProvider> activeProviders =
                  allProviders.stream()
                      .filter(
                          p ->
                              "active".equalsIgnoreCase(p.getStatus())
                      ...
                      provider ->
                          discoveryService
                                      ...
                                      if (streak >= ERROR_THRESHOLD) {
                                        ...
                                        provider.setDeadReason(
                                            "Quarantined after "
                                                ...
                                                + " consecutive validation failures");
                                        log.error(
                                            "Provider '{}' ({}) quarantined. Reason: {}",
                                            provider.getName(),
                                            provider.getId(),
                                            provider.getDeadReason());
                                      } else {
                                        log.warn(
                                            "Provider '{}' ({}) validation failed (streak: {}/{})",
                                            provider.getName(),
                                            provider.getId(),
                                            streak,
                                            ERROR_THRESHOLD);
                              ...
                              .onErrorResume(
                                  e -> {
                                    log.error(
                                        "testAllProviders: validation error for provider {}: {}",
                                        provider.getId(),
                                        e.getMessage(),
                                        e);
                                    return Mono.empty();
                  ...
                  .then(
                      Mono.fromSupplier(
                          () -> {
                            ...
                            summary.put(
                                "note", "active providers validated; 'dead' providers skipped");
                            log.info(
                                "testAllProviders: done — total={}, valid={}, failed={}",
