...
public class AIProviderDiscoveryService {
  ...
  public Flux<Map<String, Object>> discoverModels(String query) {
    ...
    Mono<List<Map<String, Object>>> hfModels =
        webClient
            .get()
            .uri(
                "https://huggingface.co/api/models?search={query}&filter=text-generation&sort=downloads&direction=-1&limit=20",
                searchQuery)
            .retrieve()
    ...
    Mono<List<Map<String, Object>>> orModels =
        webClient
            .get()
                  ...
                  return data.stream()
                      .filter(
                          model ->
                              query == null
                                  || query.isEmpty()
                                  || model.get("id").toString().contains(query))
                      .map(
                          model ->
  ...
  public Mono<Boolean> validateKey(String providerName, String apiKey) {
    logger.info(
        "[VALIDATION-DS] validateKey called: providerName={}, apiKeyLength={}",
        providerName,
        apiKey != null ? apiKey.length() : "null");
    try {
      com.supremeai.provider.AIProvider provider =
          providerFactory.getProvider(providerName, apiKey);
      return provider
          .generate("hi")
          .map(
              resp -> {
                logger.info(
                    "[VALIDATION-DS] generate response received for {}: length={}",
                    providerName,
                    resp != null ? resp.length() : "null");
                return resp != null && !resp.isEmpty();
              })
          .doOnError(
              e ->
                  logger.error(
                      "[VALIDATION-DS] generate() error for {}: {}", providerName, e.toString()))
          .onErrorReturn(false);
    } catch (Exception e) {
      logger.error(
          "[VALIDATION-DS] validateKey EXCEPTION for {}: {}",
          providerName,
  ...
  public Flux<Map<String, Object>> scanDeployments() {
    Flux<Map<String, Object>> ollamaModels =
        webClient
            .get()
    ...
    Flux<Map<String, Object>> cloudRunModels =
        Mono.fromCallable(
                () -> {
                  List<Map<String, Object>> result = new ArrayList<>();
                  try (ServicesClient client = ServicesClient.create()) {
                    LocationName parent = LocationName.of(gcpProjectId, gcpRegion);
                    for (com.google.cloud.run.v2.Service service :
                        client.listServices(parent).iterateAll()) {
                      ...
                      if (lower.contains("ai")
                        ...
                        result.add(
                            Map.<String, Object>of(
                                ...
                                service.getUri(),
                                "description",
                                "Cloud Run Deployment: " + simpleName));
                      }
                    }
                  } catch (Exception e) {
