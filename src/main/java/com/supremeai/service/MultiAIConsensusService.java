...
public class MultiAIConsensusService {
  ...
  public Mono<ConsensusResult> askAllAIs(
        ...
        .flatMap(
            providerName -> {
              ...
              return selfHealingService
                  .executeWithRetry(
                      () -> provider.generate(question), MAX_RETRIES, RETRY_BACKOFF_MS)
                  .map(
                      response -> {
                        ...
                        double confidence =
                            contextualRankingService.calculateProviderScore(
                                    providerName, taskType, null)
                                / 100.0;
                        ...
                        ProviderVote vote =
                            new ProviderVote(
                                providerName, response, confidence, System.currentTimeMillis());

                        recordProviderOutcome(
                            providerName, question, response, pDuration, confidence);
                        return vote;
                      })
                  .onErrorResume(
                      e -> {
                        logger.warn(
                            "Provider {} failed after retries: {}", providerName, e.getMessage());
                        return Mono.empty();
                      });
            })
        .collectList()
        ...
        .onErrorResume(
            ...
            e -> {
              ...
              return Mono.just(
                  new ConsensusResult(
                      question,
                      ThirdOpinionConstants.CONSENSUS_TIMEOUT,
                      List.of(),
                      0.0,
                      "TIMEOUT",
                      0.0,
  ...
  public Mono<ConsensusResult> askContextualAIs(String question, int count, long timeoutMs) {
    ContextualAIRankingService.ProviderSelection selection =
        contextualRankingService.selectBestProvider(question, null);
    ...
    List<ContextualAIRankingService.ProviderRanking> rankings =
        contextualRankingService.getRankingsForTask(selection.taskType);
