 ...
 *
 * <p>Responsibilities: - Track data with TTL (time-to-live) expiry - Soft delete → grace period →
 * hard delete pipeline - Scheduled cleanup jobs to prune expired entries - Recovery: restore
 * soft-deleted items within grace period
 */
@Service
public class DataLifecycleService {
  ...
  public void register(String dataId, String dataType, int ttlDays) {
    int defaultTtl =
        configService != null
            ? configService.getEffectiveSetting("lifecycle.default.ttl.days", defaultTtlDays)
            : defaultTtlDays;
  ...
  public void softDelete(String dataId) {
    ...
    int effectiveGrace =
        configService != null
            ? configService.getEffectiveSetting("lifecycle.grace.period.days", gracePeriodDays)
            : gracePeriodDays;
    entry.setHardDeleteAfter(LocalDateTime.now().plusDays(effectiveGrace));
    logger.info(
        "[LIFECYCLE] Soft-deleted {} hardDeleteAfter={}", dataId, entry.getHardDeleteAfter());
  }
  ...
  public Map<String, Object> getStats() {
    ...
    long active =
        registry.values().stream()
            .filter(e -> !e.isSoftDeleted() && LocalDateTime.now().isBefore(e.getExpiresAt()))
            .count();
    long softDeleted = registry.values().stream().filter(LifecycleEntry::isSoftDeleted).count();
    long expired =
        registry.values().stream()
            .filter(e -> !e.isSoftDeleted() && LocalDateTime.now().isAfter(e.getExpiresAt()))
            .count();
    return Map.of(
        "total", total,
        "active", active,
  ...
  public static class LifecycleEntry {
    ...
    public LocalDateTime getHardDeleteAfter() {
      return hardDeleteAfter;
    }

    public void setHardDeleteAfter(LocalDateTime hardDeleteAfter) {
