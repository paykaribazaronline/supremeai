...
/**
 * Service to handle GitHub App Authentication and Operations. Uses the App ID and Private Key to
 * generate JWTs and request Installation Tokens.
 */
@Service
public class GitHubAppService {
  ...
  public Mono<String> getAuthenticatedGitUrl(
      String repoOwner, String repoName, String targetInstallationId) {
    return getInstallationToken(targetInstallationId)
        .map(
            token ->
                String.format(
                    "https://x-access-token:%s@github.com/%s/%s.git", token, repoOwner, repoName));
  }
}
