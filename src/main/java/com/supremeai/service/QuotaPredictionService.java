...
/**
 * Predicts quota exhaustion using moving average of daily usage. Warns users 3 days before quota
 * runs out.
 */
@Service
public class QuotaPredictionService {
  ...
  /**
   * Predict days remaining until quota exhaustion. Returns days remaining, or -1 if quota is
   * unlimited.
   */
  ...
  public Mono<Map<String, Object>> getPrediction(String userId) {
    return Mono.zip(predictDaysRemaining(userId), userRepository.findByFirebaseUid(userId))
        .map(
            tuple -> {
              ...
              long remainingQuota =
                  user != null ? user.fetchMonthlyQuota() - user.getCurrentUsage() : 0;
  ...
  public Mono<List<Map<String, Object>>> getUsersNeedingWarning() {
    return userRepository
        ...
        .flatMap(
            user ->
                shouldWarn(user.getFirebaseUid())
