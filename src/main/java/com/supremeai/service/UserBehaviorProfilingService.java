...
/**
 * User Behavior Profiling and Smart Suggestion Engine. Learns from user interactions to provide
...
public class UserBehaviorProfilingService {
  ...
  private List<SmartSuggestion> getTimeBasedSuggestions(UserProfile profile) {
    ...
    Map<String, Long> timeBasedActions =
        profile.actions.stream()
  ...
  private List<SmartSuggestion> getCollaborativeSuggestions(UserProfile profile) {
    ...
    if (!similarUsers.isEmpty()) {
      ...
      Set<String> userActions =
          profile.actions.stream().map(a -> a.actionType).collect(Collectors.toSet());
  ...
  private static class UserProfile {
    ...
    void addAction(UserAction action) {
      ...
      synchronized (actions) {
        if (actions.size() > 1000) {
          List<UserAction> trimmed =
              new ArrayList<>(actions.subList(actions.size() - 1000, actions.size()));
          actions.clear();
    ...
    long getActiveDays() {
      return actions.stream()
          .map(
              a ->
                  LocalDateTime.ofEpochSecond(a.timestamp / 1000, 0, java.time.ZoneOffset.UTC)
                      .toLocalDate())
          .distinct()
          .count();
    }
