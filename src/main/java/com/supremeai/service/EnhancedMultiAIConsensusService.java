...
/**
 * Enhanced Multi-AI Consensus Service with iterative discussion and weighted voting. Supports 10+
 * AI providers with intelligent discussion rounds.
 */
@Service
public class EnhancedMultiAIConsensusService {
  ...
  @Autowired private AIRankingService aiRankingService;

  @Autowired private AIReasoningService aiReasoningService;

  @Autowired private EnhancedContentSanitizerService contentSanitizer;
  ...
  public EnhancedMultiAIConsensusService(
      @Qualifier("consensusTaskExecutor") ExecutorService consensusTaskExecutor) {
    this.executor = consensusTaskExecutor;
  }

  /**
   * Run iterative discussion among multiple AIs and reach consensus. Refactored to be fully
   * reactive to avoid blocking the Netty event loop.
   */
  public Mono<EnhancedConsensusResult> discussAndVote(
      String question, List<String> providerNames, int maxRounds) {
        ...
        .flatMap(
            initialVotes -> {
              if (initialVotes.size() < MIN_PROVIDERS_FOR_CONSENSUS) {
                return Mono.just(
                    new EnhancedConsensusResult(
                        question,
                        "Error: Not enough providers responded for consensus (need at least "
  ...
  private Mono<EnhancedConsensusResult> runDiscussionRounds(
      String question,
      List<String> providerNames,
      List<ProviderVote> currentVotes,
      int currentRound,
      int maxRounds) {
    if (currentRound > maxRounds) {
      ...
      if ("CONSENSUS_SPLIT".equals(finalConsensus.getStrength())) {
        return triggerDebate(question, currentVotes, providerNames)
            .map(
                debatedResult ->
                    finalizeConsensusResult(
                        question, debatedResult.votes, currentRound - 1, debatedResult.metadata));
      }

      return Mono.just(
          finalizeConsensusResult(question, currentVotes, currentRound - 1, new HashMap<>()));
    }
    ...
    List<String> disagreeingProviders =
        currentVotes.stream()
            .filter(
                v ->
                    !normalizeResponse(v.getResponse())
                        .equals(normalizeResponse(currentBestAnswer)))
            .map(ProviderVote::getProviderName)
            .collect(Collectors.toList());

    // If no one disagreed but consensus strength is weak/split, it means providers failed to
    // respond.
    // Re-querying all providers would cause an infinite loop. Break early instead.
    if (disagreeingProviders.isEmpty()) {
      log.info(
          "No active disagreement found (possible missing/failed votes). Breaking early to prevent infinite loop.");
      Map<String, Object> metadata = new HashMap<>();
      metadata.put("broken_early", true);
      metadata.put("reason", "no_disagreement");
      return Mono.just(finalizeConsensusResult(question, currentVotes, currentRound - 1, metadata));
    }

    List<String> providersToQuery = disagreeingProviders;
    log.info(
        "Cost optimization: querying only {} disagreeing providers instead of all {}",
        providersToQuery.size(),
        providerNames.size());
    ...
    return queryProvidersReactive(question, providersToQuery, discussionContext, currentRound)
        .flatMap(
            newVotes -> {
              ...
              return runDiscussionRounds(
                  question, providerNames, mergedVotes, currentRound + 1, maxRounds);
            });
  }

  private EnhancedConsensusResult finalizeConsensusResult(
      String question,
      List<ProviderVote> currentVotes,
      int completedRounds,
    ...
    aiReasoningService.logReasoning(
        ...
        String.format(
            "Strength: %s, Confidence: %.2f%%, Rounds: %d.",
  ...
  private Mono<List<ProviderVote>> queryProvidersReactive(
      String question, List<String> providerNames, String context, int round) {
                  ...
                  .flatMap(
                      provider -> {
                        ...
                        return selfHealingService
                            .executeWithRetry(() -> provider.generate(prompt), 2, 250L)
  ...
  private ConsensusResult calculateWeightedConsensus(String question, List<ProviderVote> votes) {
    ...
    double consensusPercentage =
        totalWeight > 0 ? (winningGroup.totalWeight / totalWeight) * 100.0 : 0.0;
  ...
  private String buildDiscussionContext(String question, List<ProviderVote> votes, int round) {
    ...
    Map<String, List<ProviderVote>> grouped =
        votes.stream()
            .collect(
    ...
    for (Map.Entry<String, List<ProviderVote>> entry : grouped.entrySet()) {
      ...
      context.append(representative, 0, truncateLen).append("...\n");
  ...
  private List<ProviderVote> mergeVotes(List<ProviderVote> existing, List<ProviderVote> newVotes) {
    ...
    for (ProviderVote newVote : newVotes) {
      ...
      if (existingVote == null || newVote.getConfidence() > existingVote.getConfidence()) {
  ...
  private void saveVoteToHistory(
      String question, List<ProviderVote> votes, String consensus, double confidence) {
    try {
  ...
  /**
   * Trigger Multi-Agent Debate (MAD) to resolve a split consensus. Selects a "Judge" AI to evaluate
   * conflicting responses.
   */
  private Mono<EnhancedConsensusResult> triggerDebate(
      String question, List<ProviderVote> votes, List<String> allProviders) {
    ...
    for (ProviderVote vote : votes) {
      groups
          .computeIfAbsent(normalizeResponse(vote.getResponse()), k -> new WeightedVoteGroup())
          .addVote(vote, getProviderWeight(vote.getProviderName()));
    }

    List<WeightedVoteGroup> sortedGroups =
        groups.values().stream()
    ...
    if (confA > 0.85 && confB > 0.85) {
      log.info(
          "Both conflicting groups have high confidence (>0.85). Skipping debate to save cost.");
      return Mono.just(
          new EnhancedConsensusResult(
              question,
    ...
    Set<String> conflictingProviders =
        sortedGroups.stream()
        ...
        allProviders.stream()
            ...
            .max(
                Comparator.comparingDouble(
                    p -> {
                      AIRankingService.ProviderRanking ranking =
                          aiRankingService.getRankingForProvider(p);
    ...
    return Mono.fromCallable(() -> providerFactory.getProvider(judgeName))
        ...
        .map(
            judgment -> {
              ...
              return new EnhancedConsensusResult(
                  question, judgment, newVotes, 0.9, "DEBATED", 0, metadata);
            })
        .subscribeOn(Schedulers.boundedElastic());
  }
  ...
  public static class EnhancedConsensusResult {
    ...
    public EnhancedConsensusResult(
        String question,
        String consensusAnswer,
        List<ProviderVote> votes,
        double confidence,
        String consensusStrength,
        int roundsCompleted,
        Map<String, Object> metadata) {
