...
/**
 * Automatically benchmarks providers across task types to discover their capabilities without
 * hardcoded scores.
 *
 * <p>When a new provider is added, this service: 1. Runs standardized tests 2. Measures performance
 * 3. Stores capability scores 4. Auto-assigns to appropriate tasks
 */
@Service
public class ProviderCapabilityAnalyzer {
  ...
  @Autowired private ProviderInitializationService providerInitService;
  ...
  private final List<TaskDefinition> taskDefinitions =
      List.of(
          ...
          new TaskDefinition(
              "code_review", "Code Review", "Review this code for bugs and suggest improvements"),
          new TaskDefinition(
              "debugging", "Debugging", "Fix this code that has a NullPointerException"),
          new TaskDefinition(
              "creative_writing",
              "Creative Writing",
              "Write a short story about a robot learning to paint"),
          new TaskDefinition(
              "summarization", "Summarization", "Summarize this article in 3 sentences"),
          new TaskDefinition(
              "question_answering",
              "Question Answering",
          ...
          new TaskDefinition(
              ...
              "Math Reasoning",
              "Solve: If a train travels 300 miles at 60mph, how long?"),
          new TaskDefinition(
              "bengali_translation", "Bengali Translation", "Translate: I am going to school"),
          new TaskDefinition(
              "vision_analysis", "Vision Analysis", "Describe what you see in this diagram"),
          new TaskDefinition(
              "long_context", "Long Context Analysis", "Summarize this 10K word document"));

  /** Cache benchmark results temporarily */
  private final Map<String, Map<String, Double>> benchmarkCache = new ConcurrentHashMap<>();

  /**
   * Run benchmarks for a provider and return capability scores. Fully reactive — no .block() calls.
   */
        ...
        .flatMap(
            provider -> {
              ...
              return Flux.fromIterable(taskDefinitions)
                  .flatMap(
                      task ->
                          runSingleBenchmarkReactive(provider, task)
                              .map(
                                  score -> {
                                    log.info(
                                        "  {} - {}: {}",
                              ...
                              .onErrorResume(
                                  e -> {
                                    log.warn(
                                        "Benchmark failed for {} on {}: {}",
                                        providerName,
                                        task.getName(),
                                        e.getMessage());
                                    return Mono.just(
                                        new AbstractMap.SimpleEntry<>(task.getName(), 0.0));
                                  }))
                  .collectList()
                  .map(
                      results -> {
                        ...
                        log.info(
                            "✅ Benchmark complete for {}: {} scores in {}ms",
                            providerName,
                            scores.size(),
                            duration);
  ...
  private double evaluateCompleteness(String response, TaskDefinition task) {
    ...
    switch (task.getName()) {
      case "code_generation":
        return (lower.contains("def ") || lower.contains("function") || lower.contains("class"))
            ? 0.9
            : 0.3;
