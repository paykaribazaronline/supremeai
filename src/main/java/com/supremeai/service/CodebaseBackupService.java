...
public class CodebaseBackupService {
    ...
    private final List<String> EXCLUDED_DIRS = Arrays.asList(
            "node_modules", ".gradle", "build", ".git", ".gemini", "out", "target", ".idea", ".vscode"
    );

    public CodebaseBackupService(TelegramStorageService telegramStorageService, 
    ...
    public Mono<String> createBackupAndUpload() {
        ...
        .flatMap(zipFile -> {
            ...
            return telegramStorageService.uploadFile(zipFile, remotePath)
                    .flatMap(url -> {
