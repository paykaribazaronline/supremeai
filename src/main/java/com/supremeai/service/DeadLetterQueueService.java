...
/**
 * Dead letter queue service for failed requests with retry scheduling. Stores failed requests and
...
public class DeadLetterQueueService {
  ...
  public void queueFailedRequest(
      String requestId, String provider, String requestData, String errorMessage, int retryCount) {
    ...
    FailedRequest failedRequest =
        new FailedRequest(
            requestId, provider, requestData, errorMessage, retryCount, Instant.now());
    ...
    log.info(
        "Queued failed request {} for provider {} (retry #{})", requestId, provider, retryCount);
  }
  ...
  public CompletableFuture<Boolean> processFailedRequest(
      String requestId, java.util.function.Supplier<Boolean> retryOperation) {
  ...
  public List<String> getScheduledRequests() {
    ...
    return redisTemplate.opsForSet().members(DLQ_SCHEDULED_KEY).stream().toList();
  }
