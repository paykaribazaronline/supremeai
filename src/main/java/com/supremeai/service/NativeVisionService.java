 ...
 *
 * <p>Implements lightweight computer vision using TensorFlow Lite / ONNX runtime for common tasks
 * like text extraction, object detection, and image classification.
 *
 * <p>This service provides fallback capabilities when external vision APIs are unavailable or
 * quota-limited, improving system resilience and reducing costs.
 *
 * <p>Plan 14 Enhancement: Native Vision Processing
 */
@Service
public class NativeVisionService {
  ...
  public void initialize() {
    if (nativeVisionEnabled) {
      try {
        ...
        modelLoaded = true;
        log.info(
            "[NATIVE-VISION] Native vision model loaded successfully from {}", nativeModelPath);
      } catch (Exception e) {
        log.warn(
            "[NATIVE-VISION] Failed to load native model: {}. Falling back to API-based vision.",
            e.getMessage());
        modelLoaded = false;
      }
    } else {
  ...
  /**
   * Process image using native on-device model for basic vision tasks. Returns results faster than
   * cloud APIs with no network latency.
   */
  ...
  /**
   * Extract text from image using native OCR model. Supports multiple languages including Bengali,
   * English, Hindi.
   */
  private Mono<NativeVisionResult> extractTextNative(float[] imageData) {
    try {
      ...
      if (extractedText != null && !extractedText.isEmpty()) {
        NativeVisionResult result =
            NativeVisionResult.success("text_extraction", extractedText, "native-tflite", 0.85f);
  ...
  private Mono<NativeVisionResult> classifyImageNative(float[] imageData) {
    try {
      ...
      return Mono.just(
          NativeVisionResult.success(
              "image_classification", category, "native-tflite", confidence));
    } catch (Exception e) {
  ...
  private Mono<NativeVisionResult> extractTableNative(float[] imageData) {
    try {
      ...
      if (tableData != null && !tableData.isEmpty()) {
        return Mono.just(
            NativeVisionResult.success("table_extraction", tableData, "native-tflite", 0.78f));
      } else {
  ...
  /**
   * Preprocess image for model inference. Normalizes pixel values and resizes to model input
   * dimensions.
   */
  ...
  public static class NativeVisionResult {
    ...
    public static NativeVisionResult success(
        String taskType, String result, String processor, float confidence) {
