...
/**
 * FirecrawlService handles browser scraping using the Firecrawl.dev API. It converts web pages into
...
public class FirecrawlService {
   ...
   *
   * <p>Logic: 1. If the URL is a known AI interface (Kimi/Claude), use System Browser (Playwright).
   * 2. If scraping preference is manually set to SYSTEM_BROWSER, use it. 3. Otherwise, try
   * Firecrawl API for high-quality LLM-ready markdown. 4. Fallback to System Browser if Firecrawl
   * fails or no keys available.
   */
  ...
  /**
   * Pro-Tip: Retrieves the list of high-technical web AI models from the database. Used to
   * dynamically route complex reasoning tasks to free web interfaces.
   */
  ...
  public List<String> getSpecificTaskWebAIs() {
    String list =
        configService.getEffectiveSetting("WEB_AI_SPECIFIC_TASK", "higgsfield.ai,runwayml.com");
    return Arrays.asList(list.split(","));
  }

  /**
   * Pro-Tip: Automatically determines if a URL belongs to a known free AI model to apply
   * specialized scraping/interaction logic.
   */
