...
/**
 * SystemConfigSeeder — seeds default system configuration into Firestore on first startup. Only
 * seeds global_settings. No hardcoded providers — admin configures via dashboard.
 */
@Component
public class SystemConfigSeeder {
    ...
    systemConfigRepository
        ...
        .flatMap(
            exists -> {
              if (!exists) {
                log.info(
                    "[CONFIG_SEED] No global_settings found — seeding default system config...");
                return systemConfigRepository.save(buildDefaultConfig());
              } else {
                log.info(
                    "[CONFIG_SEED] global_settings already exists — ensuring Telegram config is updated from environment...");
                return systemConfigRepository
                    ...
                    .flatMap(
                        config -> {
                          ...
                          if (tgConfig == null
                              || tgConfig.isEmpty()
                              || !Boolean.TRUE.equals(tgConfig.get("enabled"))) {
                            String telegramBotToken =
                                secretManagerService.getSecret("TELEGRAM_BOT_TOKEN");
                            if (telegramBotToken == null)
                              telegramBotToken = secretManagerService.getSecret("TG_BOT_TOKEN");
                            if (!telegramBotToken.isEmpty()) {
                              log.info(
                                  "[CONFIG_SEED] Found Telegram token in environment — updating existing config");
        ...
        .subscribe(
            config -> {
              log.info("[CONFIG_SEED] Default system config sync complete");
              // Seed/Update NVIDIA provider
              com.supremeai.model.APIProvider nvidia = new com.supremeai.model.APIProvider();
              nvidia.setId("nvidia");
  ...
  private Map<String, Object> buildTelegramConfig() {
    ...
    if (telegramChannelId == null)
      telegramChannelId = secretManagerService.getSecret("TG_CHANNEL_ID");
  ...
  private SystemConfig buildDefaultConfig() {
    ...
    log.debug(
        "[CONFIG_SEED] Built default config: model={} maintenance={}",
