 ...
 *
 * <p>Compares a new proposed plan against the existing system plan and: - Calculates a
 * compatibility score (0-100%) - Identifies conflicting areas - Provides an impact simulation
 * summary - Warns users of risk level without blocking execution (user autonomy preserved)
 */
@Service
public class PlanCompatibilityService {
  ...
  private static final List<String> BREAKING_KEYWORDS =
      List.of("delete", "drop", "remove", "deprecate", "replace", "migrate", "reset", "override");

  // Keywords that indicate additive/compatible changes
  private static final List<String> COMPATIBLE_KEYWORDS =
      List.of("add", "extend", "enhance", "improve", "support", "integrate", "optimize", "expand");
  ...
  public CompatibilityReport analyze(String existingPlan, String proposedPlan) {
    ...
    return new CompatibilityReport(
        score, risk, conflicts, compatibleAreas, warnings, impactSummary);
  }
