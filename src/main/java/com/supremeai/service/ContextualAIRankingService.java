...
/**
 * Contextual AI Ranking Service that automatically selects the best AI for each task type based on
...
public class ContextualAIRankingService {
  ...
  @Autowired private ProviderTierService providerTierService;
  ...
  private final Map<String, Map<TaskType, TaskPerformance>> performanceTracker =
      new ConcurrentHashMap<>();

  // Task-specific features for ML-based selection
  private final Map<TaskType, List<String>> taskKeywords =
      Map.of(
          TaskType.CODE_GENERATION,
              List.of("build", "create", "generate", "make", "implement", "code"),
          ...
          TaskType.TRANSLATION,
              List.of("translate", "convert", "to spanish", "to french", "to german"),
  ...
  public ProviderSelection selectBestProvider(String prompt, String requestedTaskType) {
    ...
    TaskType taskType =
        requestedTaskType != null ? TaskType.valueOf(requestedTaskType) : detectTaskType(prompt);
    ...
    double confidence =
        scores.get(bestProvider) / scores.values().stream().max(Double::compareTo).orElse(1.0);

    String reason =
        String.format(
            "Selected based on historical performance for %s tasks (score: %.2f)",
  ...
  public TaskType detectTaskType(String prompt) {
    ...
    for (TaskType task : TaskType.values()) {
      ...
      int count = keywords.stream().mapToInt(kw -> lowerPrompt.contains(kw) ? 1 : 0).sum();
  ...
  public double calculateProviderScore(String provider, TaskType taskType, String prompt) {
    Map<TaskType, TaskPerformance> providerPerf =
        performanceTracker.computeIfAbsent(provider, k -> new ConcurrentHashMap<>());
  ...
  /**
   * Get default score for providers with no history. Uses provider metadata from Firestore if
   * available, falls back to known defaults.
   */
  private double getDefaultScore(String provider, TaskType taskType) {
    ...
    return defaultScores.getOrDefault(provider, Map.of()).getOrDefault(taskType, 0.5);
  }
  ...
  public void recordTaskOutcome(
    ...
    TaskPerformance perf = providerPerf.computeIfAbsent(taskType, k -> new TaskPerformance());
    ...
    log.info(
        "Recorded outcome for {} on {}: success={}, quality={}",
        provider,
        taskType,
        success,
        qualityScore);
  ...
  public Map<String, Object> getStatistics() {
    ...
    int totalRecords =
        performanceTracker.values().stream()
  ...
  public static class ProviderSelection {
    ...
    public ProviderSelection(
        String providerName, TaskType taskType, double confidence, String reason) {
