...
public class ClaudeMemAndMem0Service {
    ...
    public String storeClaudeMemory(String sessionId, String content) throws IOException {
        if (claudeMemApiKey == null || claudeMemApiKey.isBlank()) {
            logger.warn("CLAUDE_MEM_API_KEY not configured; skipping memory store for session {}", sessionId);
            return content;
        }
    ...
    public List<Map<String, Object>> searchClaudeMemory(String sessionId, String query, int limit) throws IOException {
        if (claudeMemApiKey == null || claudeMemApiKey.isBlank()) {
    ...
    public List<Map<String, Object>> searchMem0(String userId, String query, int limit) throws IOException {
        if (mem0ApiKey == null || mem0ApiKey.isBlank()) {
            logger.warn("MEM0_API_KEY not configured; returning empty memories.");
            return List.of();
        }
        String url = mem0ApiUrl + "/memories/search?user_id=" + URLEncoder.encode(userId, StandardCharsets.UTF_8)
