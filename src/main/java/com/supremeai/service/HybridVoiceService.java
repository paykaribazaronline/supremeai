 ...
 *
 * <p>Provides voice-to-text processing with support for: - English (primary, high accuracy) -
 * Bengali/Bangla (supported via Web Speech API on frontend; normalized here) - Text normalization
 * and cleanup post-transcription
 *
 * <p>Architecture: Voice capture happens on the frontend (Web Speech API / MediaRecorder). The
 * transcribed text is sent to this service for normalization, language detection, and storage.
 * Actual STT conversion is browser-side to avoid latency and API costs.
 *
 * <p>Note: Bengali accuracy via Web Speech API is ~70-80%. This service applies post-processing
 * corrections for common Bengali transcription errors.
 */
@Service
public class HybridVoiceService {
  ...
  private static final Map<String, String> BENGALI_CORRECTIONS =
      Map.of(
          "ami", "আমি",
          "tumi", "তুমি",
          "apni", "আপনি",
  ...
  /**
   * Process transcribed text from the frontend voice capture. Normalizes, detects language, and
   * applies corrections.
   *
  ...
  public VoiceProcessingResult processTranscription(String rawText, String language) {
    ...
    logger.debug(
        "[VOICE] Processed {} chars lang={} hints={}",
  ...
  private DetectedLanguage detectLanguage(String text, String hint) {
    ...
    long bengaliChars = text.chars().filter(c -> c >= 0x0980 && c <= 0x09FF).count();
  ...
  public static class VoiceProcessingResult {
    ...
    public static VoiceProcessingResult empty() {
      return new VoiceProcessingResult(
          "", new DetectedLanguage("unknown", "auto"), List.of(), false);
    }
    ...
    public DetectedLanguage getLanguage() {
      return language;
    }

    public List<String> getIntentHints() {
      return intentHints;
    }
