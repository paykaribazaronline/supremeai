 ...
 * Core concept
 * ────────────
 * The admin creates/updates a SystemWorkRule that specifies:
 *   • WHAT kind of work parameter it controls (ruleKey, e.g. "LEARNING_AUTO_INTERVAL")
 *   • WHAT the correct value should be (value / structuredValue)
 *   • WHERE in Firestore it must be written (targetDoc + targetField)
 *
 * On every save, resolveConflicts() reads the target Firestore document,
 * compares the current value with the rule's value, and if they differ it
 * silently propagates the rule value into the document — eliminating the
 * conflict and recording the decision in an append-only decision log stored
 * alongside the rule document.
 */
@Service
public class SystemWorkRuleService {
    ...
    public Flux<SystemWorkRule> getAllRules() {
        return ruleRepository.findAll()
                .sort((a, b) -> {
                    int cat = (a.getCategory() != null && b.getCategory() != null)
    ...
    public Mono<SystemWorkRule> getRuleByKey(String ruleKey) {
        return ruleRepository.findById(ruleKey)
                .switchIfEmpty(Mono.error(new IllegalArgumentException("SystemWorkRule not found: " + ruleKey)));
    }

    // ─── WRITE ───────────────────────────────────────────────────────────────

    /**
     * Create or update a rule and fire conflict-resolution immediately so the
     * related Firestore documents are synchronised before the call returns.
     */
    public Mono<SystemWorkRule> saveRule(SystemWorkRule rule) {
        ...
        return ruleRepository.save(rule)
                ...
                .doOnSuccess(saved -> log.info(
                        ...
                        saved.getRuleKey(), saved.getValue(), saved.getCategory(),
                        saved.getTargetDoc(), saved.getLastSyncStatus()));
    }

    /**
     * Delete a rule. Does not revert the already-propagated changes — the admin
     * should be aware and set a replacement rule if needed.
     */
    public Mono<Void> deleteRule(String ruleKey) {
        return ruleRepository.deleteById(ruleKey)
                .then(Mono.<Void>fromRunnable(() -> log.info("[SystemWorkRule] Deleted rule '{}'", ruleKey)))
                .onErrorResume(e -> {
    ...
    /**
     * Reads the target document described by rule.targetDoc and compares the
     * current value of rule.targetField with rule.value.
     *
     * Conflict outcomes
     * ─────────────────
     *  NO_CONFLICT            → target value already matches, no write needed.
     *  CONFLICT_RESOLVED      → write succeeded; decisionAudit entry appended.
     *  CONFLICT_RESOLUTION_ACK → decisionAudit noted but Firestore write was deferred (no bean).
     *  PROPAGATION_FAILED     → Firestore write error after conflict detected.
     *  INVALID_TARGET         → targetDoc not in "collection/docId" format.
     *  NO_TARGET              → rule has no targetDoc.
     *  DISABLED               → rule.isEnabled() is false.
     *  FIRESTORE_UNAVAILABLE  → Firestore bean missing.
     */
    private Mono<SystemWorkRule> resolveConflicts(SystemWorkRule rule) {
        ...
        String targetField = (rule.getTargetField() != null && !rule.getTargetField().isBlank())
                ? rule.getTargetField()
                : rule.getRuleKey();
        ...
        return Mono.fromCallable(() -> {
                    ...
                    log.debug("[SystemWorkRule] Check '{}' @ {}/{}[{}]: current={}",
                            rule.getRuleKey(), collection, docId, targetField, previousValue);
                    ...
                    log.warn("[SystemWorkRule] CONFLICT in {}/{}[{}]: '{}' → '{}'. Propagating rule '{}'.",
                            collection, docId, targetField, previousValue, rule.getValue(), rule.getRuleKey());
                    return hasConflict;
                })
                .flatMap(hasConflictResult -> {
                    ...
                    return Mono.fromCallable(() -> {
                                ...
                                Object writeValue;
                                if ("OBJECT".equalsIgnoreCase(rule.getValueType()) && rule.getStructuredValue() != null) {
                                ...
                                Map<String, Object> auditUpdate = Map.of(
                                        "decisionAudit." + ts, decision,
                                        "lastSyncStatus", "CONFLICT_RESOLVED",
                                        "lastSyncedAt",   LocalDateTime.now()
                                );
                                firestore.collection("system_work_rules").document(rule.getRuleKey()).update(auditUpdate).get();
                                return decision;
                            })
                            .doOnSuccess(decision -> log.info(
                                    "[SystemWorkRule] CONFLICT_RESOLVED for '{}': {}", rule.getRuleKey(), decision))
                            .thenReturn(rule)
                            .doOnSuccess(r -> {
                            ...
                            .onErrorResume(e -> {
                                log.error("[SystemWorkRule] Propagation FAILED for '{}': {}", rule.getRuleKey(), e.getMessage(), e);
                ...
                .onErrorResume(e -> {
                    log.error("[SystemWorkRule] resolveConflicts failed for '{}': {}", rule.getRuleKey(), e.getMessage(), e);
                    rule.setLastSyncStatus("PROPAGATION_FAILED");
                    return Mono.just(rule);
                });
    }
    ...
    public Mono<Map<String, Object>> ensureDefaults() {
        SystemWorkRule r1 = new SystemWorkRule("LEARNING_AUTO_INTERVAL",
                "LEARNING", "Auto-learning trigger interval (minutes or cron expression).", "30min");
        r1.setValueType("CRON");
        r1.setTargetDoc("system_configs/global_settings");
        r1.setTargetField("timeouts.auto_learning_interval_min");

        SystemWorkRule r2 = new SystemWorkRule("LEARNING_SCRAPING_MODE",
                "LEARNING", "Internet scraping intensity / permission level.", "BALANCED");
        r2.setValueType("STRING");
        r2.setTargetDoc("system_configs/global_settings");
        r2.setTargetField("settings.learning_scraping_mode");

        SystemWorkRule r3 = new SystemWorkRule("LOGGING_ACTIVITY_ENABLED",
                "LOGGING", "Whether learning and admin activity logging is enabled.", "true");
        r3.setValueType("BOOLEAN");
        r3.setTargetDoc("system_configs/global_settings");
        r3.setTargetField("settings.logging_activity_enabled");

        SystemWorkRule r4 = new SystemWorkRule("EXECUTION_AUTO_APPROVE",
                "APPROVAL", "Auto-approve improvement proposals when AutoPilot is enabled.", "false");
        r4.setValueType("BOOLEAN");
        r4.setTargetDoc("system_configs/global_settings");
        r4.setTargetField("autoExecApprovalRequired");

        SystemWorkRule r5 = new SystemWorkRule("APPROVAL_QUEUE_STRATEGY",
                "APPROVAL", "How pending proposals are handled: allow | lock | manual.", "manual");
        r5.setValueType("STRING");
        r5.setTargetDoc("system_configs/global_settings");
        r5.setTargetField("settings.approval_queue_strategy");

        SystemWorkRule r6 = new SystemWorkRule("PROTOCOL_SEVERITY_THRESHOLD",
                "PROTOCOL", "Minimum protocol severity before auto-escalation is triggered.", "high");
        ...
        SystemWorkRule r7 = new SystemWorkRule("BROWSER_AUTO_MODE",
                "AUTOMATION", "Enables autonomous browser navigation and task execution.", "false");
        r7.setValueType("BOOLEAN");
        r7.setTargetDoc("system_configs/global_settings");
        r7.setTargetField("settings.browser_auto_mode");

        SystemWorkRule r8 = new SystemWorkRule("BROWSER_RESTRICTED_URLS",
                "SECURITY", "Comma-separated list of prohibited domains for the bot.",
        ...
        SystemWorkRule r9 = new SystemWorkRule("BROWSER_MAX_STEPS",
                "AUTOMATION", "Maximum number of autonomous steps allowed per session.", "50");
        r9.setValueType("INTEGER");
        r9.setTargetDoc("system_configs/global_settings");
        r9.setTargetField("settings.browser_max_steps");

        SystemWorkRule r10 = new SystemWorkRule("GITHUB_AUTO_PUSH",
                "AUTOMATION", "Automatically push generated code and updates to linked GitHub repository.", "false");
        r10.setValueType("BOOLEAN");
        r10.setTargetDoc("system_configs/global_settings");
        r10.setTargetField("settings.github_auto_push");

        SystemWorkRule r11 = new SystemWorkRule("GITHUB_ORG_NAME",
                "AUTOMATION", "Default GitHub organization or username for new app repositories.", "supremeai-apps");
