...
public class ECCToolsService {
  ...
  public byte[] ecdhAgree(String ownPrivateBase64, String peerPublicBase64)
      throws GeneralSecurityException {
    KeyFactory kf = KeyFactory.getInstance(KEY_ALG);
    PrivateKey ownPrivate =
        kf.generatePrivate(
            new java.security.spec.PKCS8EncodedKeySpec(
                Base64.getDecoder().decode(ownPrivateBase64)));
    PublicKey peerPublic =
        kf.generatePublic(
            new java.security.spec.X509EncodedKeySpec(
                Base64.getDecoder().decode(peerPublicBase64)));
  ...
  public boolean isAvailable() {
    try {
      return KeyPairGenerator.getInstance(KEY_ALG) != null
          && KeyAgreement.getInstance(KEY_AGREEMENT) != null;
