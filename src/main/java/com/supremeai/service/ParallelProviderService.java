...
/**
 * Parallel provider execution service for consensus voting. Executes requests across multiple
 * providers and aggregates results. FIXED: Removed blocking .join() calls, now fully reactive.
 */
@Service
public class ParallelProviderService {
  ...
  /**
   * Execute request across multiple providers in parallel. Returns first successful response.
  ...
  /**
   * Execute request across multiple providers and collect all responses. FIXED: Returns
   * CompletionStage instead of blocking.
   */
  public <T> CompletionStage<Map<String, T>> executeParallelAll(
      Map<String, ? extends CompletionStage<T>> providerRequests) {

    List<CompletionStage<Map.Entry<String, T>>> entryFutures =
  ...
  /**
   * Execute with consensus voting. Returns the most common response. FIXED: Returns CompletionStage
   * instead of blocking.
   */
  public <T> CompletionStage<T> executeWithConsensus(
      Map<String, ? extends CompletionStage<T>> providerRequests, int minAgreement) {

    return executeParallelAll(providerRequests)
        .thenApply(
            responses -> {
              // Count response frequencies
              Map<T, Long> responseCounts =
  ...
  /**
   * Execute with weighted voting based on provider reliability. FIXED: Returns CompletionStage
   * instead of blocking.
   */
                      ...
                      .collect(
                          Collectors.groupingBy(
                              ...
                              Collectors.summingDouble(
                                  entry -> providerWeights.getOrDefault(entry.getKey(), 1.0))));
