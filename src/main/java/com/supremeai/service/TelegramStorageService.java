...
public class TelegramStorageService {
    ...
    private static final String TELDRIVE_BOT_TOKEN = System.getenv().getOrDefault("TELDRIVE_BOT_TOKEN", System.getenv().getOrDefault("TG_BOT_TOKEN", ""));
    private static final String TELDRIVE_CHANNEL_ID = System.getenv().getOrDefault("TELDRIVE_CHANNEL_ID", System.getenv().getOrDefault("TG_CHANNEL_ID", ""));
    private static final String TELDRIVE_URL = System.getenv().getOrDefault("TELDRIVE_URL", "https://teldrive-lhlwyikwlq-uc.a.run.app");
    ...
    public void init() {
        ...
        this.dedicatedWebClient = webClientBuilder
                .baseUrl(TELDRIVE_URL)
                .clientConnector(new ReactorClientHttpConnector(httpClient))
                .build();

        logger.info("[TELEGRAM] Service initialized — URL: {}, SecretManager: {}, MaxUpload: {}MB",
                TELDRIVE_URL, secretManagerEnabled, MAX_UPLOAD_SIZE_BYTES / 1024 / 1024);
    }
    ...
    private java.util.Optional<String> resolveFromSecretManager(String secretId) {
        try {
            com.google.cloud.secretmanager.v1.SecretManagerServiceClient client = com.google.cloud.secretmanager.v1.SecretManagerServiceClient.create();
            com.google.cloud.secretmanager.v1.AccessSecretVersionRequest request = com.google.cloud.secretmanager.v1.AccessSecretVersionRequest.newBuilder()
    ...
    public Mono<String> uploadFile(File file, String remotePath) {
        ...
        logger.info("[TELEGRAM] Uploading: {} ({} bytes) to path: {}", file.getName(), file.length(), remoteFilePath);
                    ...
                    return executeWithRetry(() ->
                            dedicatedWebClient
                                    ...
                                    .map(response -> {
                                        ...
                                        logger.info("[TELEGRAM] Uploaded: {} → {} (compressed {} bytes → {} bytes, ratio: {}%)",
                                                file.getName(), fileUrl, file.length(), compressedData.length,
                                                String.format("%.1f", (1.0 - (double) compressedData.length / file.length()) * 100));
    ...
    public Mono<String> uploadData(byte[] data, String fileName, String remotePath) {
        ...
        logger.info("[TELEGRAM] Uploading data: {} ({} bytes) to path: {}", fileName, data.length, remoteFilePath);
                ...
                .flatMap(compressedData -> {
                    ...
                    return executeWithRetry(() ->
                            dedicatedWebClient
                                    ...
                                    .map(response -> {
                                        String fileUrl = (String) response.get("url");
                                        logger.info("[TELEGRAM] Uploaded data: {} → {} ({} bytes → {} bytes, ratio: {}%)",
                                                fileName, fileUrl, data.length, compressedData.length,
                                                String.format("%.1f", (1.0 - (double) compressedData.length / data.length) * 100));
                                        queueMetricUpdate(compressedData.length);
                                        return fileUrl != null ? fileUrl : "UPLOAD_SUCCESS";
                                    })
                    );
    ...
    public Mono<byte[]> downloadData(String remotePath) {
        ...
        return executeWithRetry(() ->
                ...
                .transformDeferred(CircuitBreakerOperator.of(circuitBreaker))
                .onErrorResume(e -> {
                    logger.error("[TELEGRAM] Data download failed from {}: {}", resolvedPath, e.getMessage());
    ...
    public Mono<String> getDownloadUrl(String fileId) {
        ...
        .transformDeferred(CircuitBreakerOperator.of(circuitBreaker))
        .onErrorResume(e -> {
            logger.error("[TELEGRAM] Failed to get download URL for {}: {}", fileId, e.getMessage());
    ...
    private void queueMetricUpdate(long newBytes) {
        ...
        pendingMetricUpdates.merge(threadName, String.valueOf(newBytes), (oldVal, newVal) ->
                String.valueOf(Long.parseLong(oldVal) + Long.parseLong(newVal)));
    ...
    private void flushPendingMetrics() {
        long totalBytes = pendingMetricUpdates.values().stream()
        ...
        try {
            ...
            configService.updateConfig(current, "system", "127.0.0.1")
                    .subscribe(
                            saved -> logger.debug("[TELEGRAM] Storage metrics updated: {} (+{} bytes)", newSizeFormatted, totalBytes),
                            error -> logger.error("[TELEGRAM] Failed to update storage metrics: {}", error.getMessage())
                    );
        } catch (Exception e) {
