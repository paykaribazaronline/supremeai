...
public class LearningArchiveService {
  ...
  public LearningArchiveService(
      SystemLearningRepository repository,
  ...
  public void runLearningArchiving() {
    ...
    isArchiving = true;
    logger.info(
        "[TELDRIVE-ARCHIVE] Starting system learning archival process (Interval: {}h)...",
        intervalHours);
    archiveOldLearnings()
        .doOnTerminate(
            () -> {
        ...
        .subscribe(
            ...
            error ->
                logger.error(
                    "[TELDRIVE-ARCHIVE] Error during archival process: {}",
                    error.getMessage(),
                    error));
  }
  ...
  private Mono<Void> archiveBatch(List<SystemLearning> learnings) {
    try {
      ...
      logger.info(
          "[TELDRIVE-ARCHIVE] Archiving {} learning records to Teldrive Cloud", learnings.size());

      return telegramStorageService
          .uploadData(jsonData, fileName, remotePath)
          .flatMap(
              url -> {
          ...
          .onErrorResume(
              e -> {
                logger.error(
                    "[TELDRIVE-ARCHIVE] Failed to archive learning records: {}", e.getMessage());
                return Mono.empty();
              });
    } catch (Exception e) {
  ...
  /**
   * Periodically check if it's time to sync back data from Telegram to Firebase based on admin
   * config.
   */
  @Scheduled(fixedDelay = 60000) // check every minute
  public void runLearningSync() {
    ...
    isSyncing = true;
    logger.info(
        "[TELDRIVE-SYNC] Starting system learning sync from Teldrive (Interval: {}h)...",
        intervalHours);
    syncFromTelegram()
        .doOnTerminate(
            () -> {
        ...
        .subscribe(
            ...
            error ->
                logger.error("[TELDRIVE-SYNC] Sync from Teldrive failed: {}", error.getMessage()));
  }

  public Mono<Void> syncFromTelegram() {
    return metadataRepository
        .findByCategory("LEARNING_ARCHIVE")
        .flatMap(
            metadata -> {
              logger.info("[TELDRIVE-SYNC] Syncing from archive file: {}", metadata.getFileName());
              return telegramStorageService
                  .downloadData(metadata.getRemotePath())
                  .flatMap(
                      data -> {
                        try {
                          List<SystemLearning> learnings =
                              objectMapper.readValue(
                                  ...
                                  objectMapper
                                      .getTypeFactory()
                                      .constructCollectionType(List.class, SystemLearning.class));

                          logger.info(
                              "[TELDRIVE-SYNC] Restoring {} records from {}",
                              learnings.size(),
                          ...
                          logger.error(
                              "[LEARN-SYNC] Failed to parse archive {}: {}",
                              metadata.getFileName(),
                              e.getMessage());
                          return Mono.empty();
                        }
