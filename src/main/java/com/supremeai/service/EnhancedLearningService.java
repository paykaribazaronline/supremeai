...
/**
 * Enhanced Learning Service - Central hub for all learning types Handles NLP, Multimodal,
 * Ecosystem, and Predictive learning
 */
@Service
public class EnhancedLearningService {
  ...
  @Autowired
  public EnhancedLearningService(
      SystemLearningRepository repository, SystemLearningService systemLearningService) {
  ...
  public Mono<SystemLearning> learnFromInteraction(
      String userId, String userInput, String aiResponse) {
  ...
  public Mono<SystemLearning> learnFromNLPInteraction(
      String userInput,
      String aiResponse,
      String provider,
      double qualityScore,
  ...
  public Mono<SystemLearning> learnFromMultimodalInteraction(
      String textPrompt,
      String imageUrl,
    ...
    List<String> tags =
        Arrays.asList("multimodal", "vision", "image_to_code", provider.toLowerCase());
    learning.setTags(tags);
  ...
  public Mono<SystemLearning> learnFromAPIUsage(
      String apiEndpoint,
      String provider,
      long responseTimeMs,
      boolean success,
    ...
    logger.info(
        "Ecosystem Learning captured: endpoint={}, provider={}, success={}",
        apiEndpoint,
        provider,
        success);
  ...
  public Mono<SystemLearning> learnFromAppGeneration(
      String requirement,
      String generatedAppType,
    ...
    learning.setContent(
        "Learned from app generation: "
            + requirement.substring(0, Math.min(100, requirement.length())));
    ...
    logger.info(
        "App Generation Learning captured: type={}, success={}, agent={}",
        generatedAppType,
        buildSuccess,
        agentUsed);
  ...
  public Mono<SystemLearning> learnPredictivePattern(
      String patternType,
      Map<String, Object> patternData,
      double confidence,
      String basedOnLearningId) {
    Map<String, Object> inputData = new HashMap<>();
  ...
  public Flux<SystemLearning> getPredictiveRecommendations(
      String taskType, Map<String, Object> taskFeatures) {
    return repository
        .findAll()
        ...
        .sort(
            (a, b) ->
                Double.compare(
                    b.getConfidenceScore() != null ? b.getConfidenceScore() : 0,
  ...
  public Mono<SystemLearning> applyLearning(String learningId) {
    return repository
        ...
        .flatMap(
            learning -> {
              learning.setTimesApplied(
                  (learning.getTimesApplied() != null ? learning.getTimesApplied() : 0) + 1);
    ...
    return repository
        ...
        .map(
            allLearnings -> {
                  ...
                  allLearnings.stream()
                      .collect(
                          Collectors.groupingBy(
                              l -> l.getLearningType() != null ? l.getLearningType() : "UNKNOWN",
                              Collectors.counting()));
              ...
              stats.put(
                  "successRate",
                  allLearnings.size() > 0 ? (double) totalSuccess / allLearnings.size() : 0.0);
  ...
  /**
   * System Learning Improvement Cycle Analyzes collected patterns, optimizes knowledge base, and
   * generates improvements. This is the main "system learning improve" command backend
   * implementation.
   *
    ...
    return repository
        ...
        .map(
            allLearnings -> {
              ...
              logger.info(
                  "System learning improvement cycle completed. Analyzed {} learnings, found {} opportunities",
                  allLearnings.size(),
                  opportunities.size());
  ...
  private Map<String, Object> analyzeLearningData(List<SystemLearning> learnings) {
    ...
    DoubleSummaryStatistics qualityStats =
        learnings.stream()
            .filter(l -> l.getQualityScore() != null)
            .mapToDouble(SystemLearning::getQualityScore)
            .summaryStatistics();
    ...
    List<SystemLearning> topApplied =
        learnings.stream()
            .sorted(
                Comparator.comparingInt(
                        (SystemLearning l) -> l.getTimesApplied() != null ? l.getTimesApplied() : 0)
                    .reversed())
            .limit(5)
            .collect(Collectors.toList());

    List<String> topAppliedIds =
        topApplied.stream().map(SystemLearning::getId).collect(Collectors.toList());
    ...
    Map<String, Long> providerCounts =
        learnings.stream()
            ...
            .collect(
                Collectors.groupingBy(SystemLearning::getRelatedProvider, Collectors.counting()));
  ...
  private List<Map<String, Object>> identifyImprovements(List<SystemLearning> learnings) {
    ...
    Map<String, Map<Boolean, Long>> categoryStats =
        learnings.stream()
            .collect(
                Collectors.groupingBy(
                    l -> l.getCategory() != null ? l.getCategory() : "unknown",
                    Collectors.groupingBy(
                        l -> l.getSuccess() != null && l.getSuccess(), Collectors.counting())));
    ...
    List<SystemLearning> lowQuality =
        learnings.stream()
  ...
  private List<String> generateRecommendations(
      List<SystemLearning> learnings, Map<String, Object> analysis) {
    List<String> recommendations = new ArrayList<>();

    @SuppressWarnings("unchecked")
    Map<String, Map<String, Double>> byType =
        (Map<String, Map<String, Double>>)
            analysis.getOrDefault("successRateByType", Map.<String, Map<String, Double>>of());
    ...
    byType.forEach(
        (type, typeMetrics) -> {
          ...
          if (rate < 0.6) {
            recommendations.add(
                "Increase training data for "
                    + type
                    + " learning type (success rate: "
    ...
    Map<String, Long> providerUsage =
        (Map<String, Long>) analysis.getOrDefault("providerUsage", Map.<String, Long>of());
