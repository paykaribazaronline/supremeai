...
public class ChatIntelligenceService {
  ...
  @Autowired private ChatRuleRepository ruleRepository;

  @Autowired private ChatPlanRepository planRepository;

  @Autowired private com.supremeai.repository.ChatAdminActionRepository adminActionRepository;
  ...
  public Mono<Void> handleIntelligence(
      String chatId, String message, Intent intent, String user, Double confidence) {
    ...
    if (intent == Intent.PROJECT_PLAN) {
      ...
      plan.setActive(true);
      logger.info(
          "Automatically detected and saving PROJECT_PLAN for chat {}: {}", chatId, message);
    ...
    if (intent == Intent.ADMIN_ACTION) {
      ...
      logger.info(
          "Automatically detected and saving ADMIN_ACTION for chat {}: {}", chatId, message);
