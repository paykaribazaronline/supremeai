...
/**
 * Tracks AI provider performance (success rates) and ranks them. Uses actual request outcomes to
 * provide auto-ranking.
 */
@Service
public class AIRankingService {
  ...
  public List<ProviderRanking> getRankings() {
    ...
    for (Map.Entry<String, ProviderStats> entry : providerStats.entrySet()) {
      ...
      rankings.add(
          new ProviderRanking(
              entry.getKey(), stats.successCount, stats.failureCount, successRate, valueScore));
    }
  ...
  public ProviderRanking getRankingForProvider(String provider) {
    ...
    return new ProviderRanking(
        provider, stats.successCount, stats.failureCount, stats.getSuccessRate(), valueScore);
  }
