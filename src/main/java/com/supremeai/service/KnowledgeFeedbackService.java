...
/**
 * Service to capture real-time feedback and system events as learned knowledge. Part of the
 * "Self-Correction Loop" for SupremeAI.
 */
@Service
public class KnowledgeFeedbackService {
  ...
  public Mono<SystemLearning> recordErrorLesson(
      String context, Throwable error, String suggestedFix) {
    String topic = "Error Lesson: " + context;
    String content =
        String.format(
            "Error [%s] occurred in context [%s]. Resolution/Lesson: %s",
  ...
  public Mono<SystemLearning> recordPerformanceLesson(
      String operation, long durationMs, String optimization) {
    String topic = "Performance Insight: " + operation;
    String content =
        String.format(
            "Operation [%s] took %dms. Optimization applied: %s",
  ...
  private SystemLearning createLesson(
      String topic, String category, String content, double confidence) {
