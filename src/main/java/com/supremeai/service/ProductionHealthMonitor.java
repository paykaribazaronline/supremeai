...
/**
 * Production Health Monitoring Service - Phase 4 Tracks system health, API provider status, and
 * performance metrics. Provides real-time monitoring data for dashboard and alerting.
 */
@Service
public class ProductionHealthMonitor {
  ...
  public ProductionHealthMonitor(io.micrometer.core.instrument.MeterRegistry meterRegistry) {
    ...
    meterRegistry.gauge(
        "supremeai.health.status",
        this,
        monitor -> "HEALTHY".equals(monitor.overallStatus) ? 1.0 : 0.0);
  ...
  public void recordSuccess(String provider, long responseTimeMs) {
    ...
    // Micrometer metrics
    meterRegistry
        .counter("supremeai.requests.total", "status", "success", "provider", provider)
        .increment();
    meterRegistry
        .timer("supremeai.requests.latency", "provider", provider)
  ...
  public void recordFailure(String provider, String error) {
    ...
    meterRegistry
        .counter(
            "supremeai.requests.total", "status", "failure", "provider", provider, "error", error)
        .increment();
  }
  ...
  private java.util.List<Map<String, Object>> computeAlerts() {
    ...
    if (total > 100) {
      ...
      if (rate < 95.0) {
        ...
        alert.put(
            "message", "Success rate dropped below 95%: " + Math.round(rate * 100) / 100.0 + "%");
    ...
    for (Map.Entry<String, AtomicLong> entry : providerErrors.entrySet()) {
      ...
      if (errors > 10) {
        ...
        alert.put(
            "message", "Provider " + entry.getKey() + " has " + errors + " consecutive errors");
  ...
  public void updateUptime() {
    ...
    if (failedRequests.get() > 0 && totalRequests.get() > 0) {
      ...
      if (!oldStatus.equals(overallStatus)) {
        logEvent(
            overallStatus.equals("HEALTHY") ? "SUCCESS" : "ALERT",
            "SYSTEM",
            "Global health status changed to " + overallStatus);
      }
    }
  }
