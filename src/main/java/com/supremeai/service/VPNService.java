...
public class VPNService {
    ...
    private final AtomicLong encryptedBytes = new AtomicLong(12400000000L); // Starts at 12.4 GB representation
        ...
        return vpnRepository.findById(id)
            .flatMap(conn -> {
                ...
                if (previousId != null && !previousId.equals(id)) {
                    disconnectPrevious = vpnRepository.findById(previousId)
                        .flatMap(prev -> {
                ...
                return disconnectPrevious.then(Mono.defer(() -> {
                    ...
                    logger.info("[VPN] Tunnel established. Host: {}, Simulated IP exit: {}, Native CLI support: {}", 
                            conn.getHost(), conn.getIpAddress(), hasNativeCli);
                            
                    return vpnRepository.save(conn);
                }));
            })
            .switchIfEmpty(Mono.error(new IllegalArgumentException("VPN Connection with ID " + id + " not found")));
    }
    ...
    public Mono<Map<String, Object>> getVPNStatus() {
        return vpnRepository.findAll()
            .collectList()
            .map(connections -> {
                long activeCount = connections.stream()
                ...
                if (activeCount > 0) {
                    VPNConnection active = connections.stream()
                        .filter(c -> "CONNECTED".equals(c.getStatus()))
                        .findFirst()
                        .orElse(null);
                    if (active != null) {
