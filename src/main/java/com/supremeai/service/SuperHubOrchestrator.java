...
public class SuperHubOrchestrator {
  ...
  public Mono<String> orchestrate(String taskDescription, Map<String, Object> context) {
    log.info(
        "[SUPER_HUB] Orchestrating task: {} (Context: {})",
        taskDescription,
        context != null ? context.size() : 0);

    return supremeAIBrain
        .identifyHub(taskDescription)
        .flatMap(
            hubId -> {
              ...
              try {
                ...
                return provider
                    ...
                    .onErrorResume(
                        e -> {
                          log.warn(
                              "[SUPER_HUB] Hub {} failed: {}. Trying next available cloud provider.",
                              hubId,
                              e.getMessage());
                ...
                log.warn(
                    "[SUPER_HUB] Could not create provider for hub {}: {}. Trying next available.",
                    hubId,
                    e.getMessage());
                return tryNextAvailableProvider(hubId, taskDescription);
              }
      ...
      try {
        ...
        return fallback
            ...
            .onErrorResume(
                e -> {
                  log.warn(
                      "[SUPER_HUB] Fallback provider {} also failed: {}",
                      providerName,
                      e.getMessage());
                  return Mono.empty();
                });
        ...
        log.debug(
            "[SUPER_HUB] Could not create fallback provider {}: {}", providerName, e.getMessage());
      }
    }
    return Mono.just(
        "[SUPER_HUB] All cloud providers failed. Please check provider status in admin dashboard.");
  }

  public Mono<String> executeDevelopmentTask(String codeRequest) {
    return providerFactory
        .getBestProviderForTask("CODE_GENERATION")
        .flatMap(
            provider ->
                provider
                    .generate(codeRequest)
                    .onErrorResume(
                        e -> {
                          log.warn(
                              "[SUPER_HUB] Dev task failed on best provider: {}. Trying any available.",
                              e.getMessage());
        ...
        .onErrorResume(
            e -> {
              log.warn(
                  "[SUPER_HUB] No best provider found for CODE_GENERATION: {}. Trying any available.",
                  e.getMessage());
              return tryNextAvailableProvider(null, codeRequest);
            });
  }
  ...
  public Mono<String> executeMarketingTask(String contentRequest) {
    return providerFactory
        .getBestProviderForTask("CREATIVE_WRITING")
        .flatMap(
            provider ->
                provider
                    ...
                    .onErrorResume(
                        e -> {
                          log.warn(
                              "[SUPER_HUB] Marketing task failed on best provider: {}. Trying any available.",
                              e.getMessage());
        ...
        .onErrorResume(
            e -> {
              log.warn(
                  "[SUPER_HUB] No best provider found for CREATIVE_WRITING: {}. Trying any available.",
                  e.getMessage());
              return tryNextAvailableProvider(null, contentRequest);
            });
  }
