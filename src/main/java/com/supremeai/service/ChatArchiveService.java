...
public class ChatArchiveService {
  ...
  public Mono<Void> archiveOldMessages() {
    ...
    return chatHistoryRepository
        ...
        .flatMap(
            allMessages -> {
              Map<String, List<ChatMessage>> groupedByUserId =
                  allMessages.stream()
                  ...
                  .flatMap(
                      entry -> {
                        ...
                        if (userMessages.size() > 50) {
                          ...
                          List<ChatMessage> toArchive =
                              userMessages.stream()
                                  .sorted((a, b) -> a.getTimestamp().compareTo(b.getTimestamp()))
                                  .limit(
                                      userMessages.size() - 20) // Keep at least 20 in Hot Storage
  ...
  private Mono<Void> archiveMessagesForUser(String userId, List<ChatMessage> messages) {
    try {
      ...
      logger.info(
          "[ARCHIVE] Archiving {} messages for user {} to Telegram", messages.size(), userId);
          ...
          .flatMap(
              url -> {
                ...
                return storageMetadataRepository
                    .save(metadata)
                    .then(
                        Flux.fromIterable(messages).flatMap(chatHistoryRepository::delete).then());
              })
          .onErrorResume(
              e -> {
