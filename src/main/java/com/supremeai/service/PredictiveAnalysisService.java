...
/**
 * Predictive Analysis Service - Analyzes past learnings to generate predictive patterns Uses
 * successful generations to improve future outcomes
 */
@Service
public class PredictiveAnalysisService {
  ...
  public PredictiveAnalysisService(
      SystemLearningRepository repository, EnhancedLearningService enhancedLearningService) {
  ...
  private Mono<SystemLearning> analyzePlatformSuccessPatterns(List<SystemLearning> learnings) {
    ...
    // Find best platform
    String bestPlatform =
        platformSuccessCounts.entrySet().stream()
    ...
    return enhancedLearningService.learnPredictivePattern(
        "BEST_PLATFORM_" + bestPlatform, patternData, confidence, "platform_analysis");
  }
  ...
  private Mono<SystemLearning> analyzeNLPProviderPatterns(List<SystemLearning> learnings) {
    Map<String, List<SystemLearning>> nlpByProvider =
        learnings.stream()
    ...
    // Find best provider
    String bestProvider =
        providerAvgQuality.entrySet().stream()
            .max(Map.Entry.comparingByValue())
            .map(Map.Entry::getKey)
            .orElse("unknown");
    ...
    return enhancedLearningService.learnPredictivePattern(
        "BEST_NLP_PROVIDER_" + bestProvider, patternData, confidence, "nlp_provider_analysis");
  }
  ...
  private Mono<SystemLearning> analyzeAPIReliabilityPatterns(List<SystemLearning> learnings) {
    Map<String, List<SystemLearning>> apiByProvider =
        learnings.stream()
    ...
    for (Map.Entry<String, List<SystemLearning>> entry : apiByProvider.entrySet()) {
      ...
      long success =
          entry.getValue().stream().filter(l -> l.getSuccess() != null && l.getSuccess()).count();
    ...
    String mostReliable =
        providerReliability.entrySet().stream()
            .max(Map.Entry.comparingByValue())
            .map(Map.Entry::getKey)
            .orElse("unknown");
    ...
    return enhancedLearningService.learnPredictivePattern(
        "MOST_RELIABLE_API_" + mostReliable, patternData, confidence, "api_reliability_analysis");
  }

  /** Pattern: Multimodal success patterns */
  private Mono<SystemLearning> analyzeMultimodalPatterns(List<SystemLearning> learnings) {
    List<SystemLearning> multimodalLearnings =
        learnings.stream()
  ...
  public Mono<Map<String, Object>> getRecommendationsForTask(
      String taskType, Map<String, Object> taskFeatures) {
    return enhancedLearningService
        .getPredictiveRecommendations(taskType, taskFeatures)
        .collectList()
        .map(
            patterns -> {
              ...
              if (!patterns.isEmpty()) {
                ...
                if (bestPattern.getInputData() != null) {
                  recommendations.put(
                      "recommendedProvider", bestPattern.getInputData().get("bestProvider"));
                  recommendations.put(
                      "recommendedPlatform", bestPattern.getInputData().get("bestPlatform"));
                }
              }
