...
/**
 * ZM-04: Database Schema Auto-Migration Detects schema changes and applies them automatically
 * via @PostConstruct. Performs a highly-efficient real Firestore batch migration.
 */
@Service
public class DatabaseSchemaMigrationService {
  ...
  public void runAutoMigrations() {
    ...
    firebaseRealtimeService
        .getData("migration_status")
        .subscribe(
            status -> {
              if (status == null || !status.containsKey("v2_firestore_applied")) {
                log.info(
                    "🚀 Applying Real Firestore Schema Batch Migration V2: System Learning Schema Versioning...");
                ...
                performFirestoreBatchMigration()
                    ...
                    .subscribe(
                        updatedCount -> {
                          log.info(
                              "✅ Firestore Batch Migration completed successfully. Updated {} documents.",
                              updatedCount);
                        ...
                        error ->
                            log.error("❌ Real Firestore Schema Batch Migration failed", error));
              } else {
  ...
  private Mono<Integer> performFirestoreBatchMigration() {
    return Mono.fromCallable(
        () -> {
          ...
          log.info(
              "Found {} total system_learning documents. Analyzing for migration...",
              documents.size());
          ...
          for (QueryDocumentSnapshot doc : documents) {
            ...
            if (needsMigration) {
              log.info(
                  "Document ID '{}' is missing fields. Queueing for batch migration...",
                  doc.getId());
              ...
              batch.update(
                  ...
                  2L,
                  "timesApplied",
                  doc.getLong("timesApplied") == null ? 0L : doc.getLong("timesApplied"),
                  "permanent",
                  doc.getBoolean("permanent") == null ? false : doc.getBoolean("permanent"));
