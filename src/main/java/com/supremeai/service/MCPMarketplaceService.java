...
/**
 * MCP Marketplace Service - Plan 24 Phase 4 Manages MCP server registry, skill marketplace, and
 * one-click install functionality. Implements the skill marketplace and MCP server discovery
 * (Pinokio/Ruflo compatible).
 */
@Service
public class MCPMarketplaceService {
  ...
  public MCPToolRegistration registerTool(
      String toolId,
      String name,
      String description,
  ...
  public SkillRegistration registerSkill(
      String skillId,
      String name,
      String description,
  ...
  public Map<String, Object> installSkill(String skillId, Map<String, Object> config) {
    ...
    return Map.of("success", true, "skillId", skillId, "name", skill.name, "status", "installed");
  }
  ...
  public Map<String, Object> connectServer(String name, String url, Map<String, String> auth) {
    try {
      ...
      return Map.of("success", true, "name", name, "url", url, "status", "connected");

    } catch (Exception e) {
      logger.error("Failed to connect to MCP server {}: {}", name, e.getMessage());
      return Map.of("success", false, "name", name, "error", e.getMessage());
    }
  }

  /** Autonomous Discovery: System finds a skill from the web and proposes it to Admin */
  public SkillRegistration proposeDiscoveredSkill(
      String sourceUrl,
      String name,
      String description,
      List<String> triggers,
