...
/**
 * Async non-blocking I/O service for external calls. Uses Java 11+ HttpClient for efficient async
 * operations.
 */
@Service
public class AsyncIOService {
  ...
  public CompletableFuture<java.util.List<String>> getAllAsync(
      java.util.List<String> urls, Duration timeout) {

    return CompletableFuture.allOf(
            urls.stream().map(url -> getAsync(url, timeout)).toArray(CompletableFuture[]::new))
        .thenApply(v -> urls.stream().map(url -> getAsync(url, timeout).join()).toList());
  }
}
