 ...
 *
 * <p>AdminDashboardController থেকে বিজনেস লজিক সরিয়ে এখানে আনা হয়েছে। কন্ট্রোলার এখন শুধু HTTP
 * রিকোয়েস্ট হ্যান্ডলিং ও রাউটিং করবে।
 *
 * <p>দায়িত্ব: - ড্যাশবোর্ড কনট্রাক্ট (contract) বিল্ড করা - ইউজার ডেটা ম্যাপিং - নেভিগেশন ও
 * কম্পোনেন্ট মেটাডেটা তৈরি - আপটাইম ক্যালকুলেশন
 */
@Service
public class AdminDashboardFacadeService {
  ...
  public List<Mono<?>> buildContractDataSources() {
    return Arrays.asList(
        ...
        activityLogRepository
            .findBySeverityOrderByTimestampDesc("CRITICAL")
            .count()
            .onErrorReturn(0L),
  ...
  public Map<String, Object> buildContract(Object[] data) {
    ...
    double successRate =
        totalProjects > 0 ? (double) completedProjects / totalProjects * 100 : 100.0;
    double healthScore =
        totalLogs > 0 ? Math.max(0, 100.0 - ((double) criticalErrors / totalLogs * 100)) : 100.0;

    Map<String, Object> contract = new HashMap<>();
    contract.put("contractVersion", "3.1.0-supremeai");
    contract.put("title", "SupremeAI Studio");
    contract.put(
        "description", "Enterprise-Grade Multi-Agent AI Orchestration & Cloud App Development");
  ...
  public Map<String, Object> createNavItem(
      String key, String label, String icon, String description, boolean enabled) {
  ...
  /**
   * Pushes a system suggestion to the admin dashboard. Creates a notification that can be viewed in
   * the dashboard UI.
   */
  public void pushSuggestion(String category, String message, String target, double score) {
    log.info(
        "[SYSTEM_LEARNING] Dashboard Notification: category={}, message={}, target={}, score={}",
        category,
        message,
        target,
        score);
  ...
  private Map<String, Object> buildStats(
    ...
    stats.put(
        "systemHealthReason",
        criticalErrors > 0
            ? criticalErrors + " critical system alerts detected"
    ...
    stats.put(
        ...
        Arrays.asList(
            Map.of(
                ...
                "Mon",
                "running",
                Math.max(0, runningProjects - 2),
                "completed",
                Math.max(0, completedProjects - 10)),
            Map.of(
                "t",
                "Tue",
                "running",
                Math.max(0, runningProjects - 1),
                "completed",
                Math.max(0, completedProjects - 5)),
        ...
        uiMetadata.getOrDefault(
            ...
            List.of(
                createNavItem(
                    ...
                    "AI Provider Hub",
                    "🔌",
                    "Manage LLM and AI service connections",
                    true)));
  }
        ...
        uiMetadata.getOrDefault(
            ...
            List.of(
                createComponent(
                    ...
                    "Intelligence",
                    true,
                    Map.of("endpoint", "/api/admin/providers/configured"))));
  }
}
