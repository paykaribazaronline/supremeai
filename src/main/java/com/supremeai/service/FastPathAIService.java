 ...
 *
 * <p>Groq first implementation with automatic failover Returns first valid response as fast as
 * possible
 */
@Service
public class FastPathAIService {
  ...
  /**
   * Fast path generation - Groq first with automatic failover Returns response in < 500ms when Groq
   * is healthy
   */
  public String generateFast(String prompt) {
    ...
    if (localCircuitBreaker.getState() == CircuitBreaker.State.CLOSED) {
      try {
        String response =
            localCircuitBreaker.executeSupplier(
                () -> {
  ...
  /**
   * Parallel consensus generation - returns first response immediately Runs all providers in
   * parallel in background for consensus
   */
