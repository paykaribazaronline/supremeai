...
/**
 * Manages Solo Mode Intelligence Features (SL-01 to SL-04) Ensures SupremeAI can operate even if
 * all external APIs are completely down.
 */
@Service
public class SoloModeManagerService {
  ...
  // SL-01: Local AI Model Auto-Download Tracker (Atomic for thread safety and preventing thread
  // leak)
  private final java.util.concurrent.atomic.AtomicBoolean isLocalModelDownloading =
      new java.util.concurrent.atomic.AtomicBoolean(false);
  private final java.util.concurrent.atomic.AtomicBoolean isLocalModelAvailable =
      new java.util.concurrent.atomic.AtomicBoolean(false);
  ...
  /**
   * SL-01: Local AI Model Auto-Download Triggered if all external providers fail. Auto-pulls the
   * smallest viable GGUF model (e.g., Phi-3-mini).
   */
        ...
        .flatMap(
            isHealthy -> {
              ...
              if (isLocalModelDownloading.compareAndSet(false, true)) {
                log.warn(
                    "All external providers failed. Initiating auto-download of {} GGUF fallback model via AirLLM sidecar at {}.",
                    fallbackModelName,
                    airllmHealthcheckUrl);

                return reactor.core.publisher.Flux.interval(
                    ...
                    .flatMap(
                        h -> {
                          log.info(
                              "{} successfully downloaded and loaded into memory via AirLLM sidecar.",
                              fallbackModelName);
                          isLocalModelDownloading.set(false);
                          isLocalModelAvailable.set(true);
                          return callAirLLMSidecar(prompt);
                    ...
                    .switchIfEmpty(
                        Mono.defer(
                            () -> {
                              log.warn(
                                  "AirLLM sidecar at {} is still initializing or downloading the model.",
                                  airllmHealthcheckUrl);
                              return Mono.just(
                                  "System is currently downloading and loading the offline fallback model ("
                                      + fallbackModelName
                                      + "). Please wait...");
                            }))
                    .doFinally(
                ...
                return Mono.just(
                    "System is currently downloading the offline fallback model ("
                        + fallbackModelName
                        + "). Please wait...");
              }
            });
  ...
  private Mono<String> callAirLLMSidecar(String prompt) {
    log.info(
        "[SoloMode] Calling AirLLM sidecar for fallback model: {} with prompt: {}",
        fallbackModelName,
        prompt);
    return providerRepository
        .findById(fallbackProviderId)
        .switchIfEmpty(providerRepository.findById(fallbackProviderId.toLowerCase()))
        .flatMap(
            airllmConfig -> {
              log.info(
                  "[SoloMode] Found fallback provider: baseUrl={}, model={}",
                  airllmConfig.getBaseUrl(),
                  airllmConfig.getModelName());
              AIProvider provider = providerFactory.createProviderFromConfig(airllmConfig);
              if (provider == null) {
                return Mono.error(
                    new RuntimeException("Could not construct provider for " + fallbackProviderId));
              }
        ...
        .onErrorResume(
            e -> {
              ...
              return Mono.just(
                  "[Local Fallback "
                      + fallbackModelName
                      + "] (Offline Mode Fallback response) Result for: "
                      + prompt);
            });
  }

  /**
   * SL-02: P2P Knowledge Sync SupremeAI instances can share learned knowledge entries via signed
   * Firestore writes.
   */
  public Mono<Void> broadcastP2PKnowledge(String topic, String learnedInsight) {
    ...
    syncData.put(
        "signature", "signed_" + UUID.randomUUID().toString()); // Placeholder for crypto signature

    return firebaseRealtimeService.setData("p2p_knowledge_sync/" + topic + "/" + nodeId, syncData);
  }

  /**
   * SL-03: Offline Knowledge Distillation Periodic job compresses Firestore system_learning into
   * updated core_knowledge.json offline snapshot.
   */
  @Scheduled(cron = "0 0 1 * * ?") // Every day at 1 AM
  public void distillOfflineKnowledge() {
    log.info("Starting Offline Knowledge Distillation...");
    firebaseRealtimeService
        .getData("system_learning")
        .subscribe(
            data -> {
  ...
  /** SL-04: Emergency Code Generation Template-based code scaffolding that works with zero AI. */
  public String generateEmergencyScaffold(String requirement) {
    log.info(
        "Triggering SL-04 Emergency Code Generation (Zero-AI Template Matching) for: {}",
        requirement);
    ...
    if (lowerReq.contains("spring") || lowerReq.contains("java") || lowerReq.contains("boot")) {
      code.append("@RestController\n")
          ...
          .append("    }\n")
          .append("}\n");
    } else if (lowerReq.contains("react")
        || lowerReq.contains("frontend")
        || lowerReq.contains("web")) {
  ...
  /**
   * SL-02: Step Limit Guard Checks if autonomous step execution should be allowed based on step
   * count and timeout. Uses AtomicLong for thread-safe stepStartTime to prevent race conditions.
   */
  private static final int MAX_STEPS = 15;

  private static final long TIMEOUT_MINUTES = 5;
  private final java.util.concurrent.atomic.AtomicInteger stepCounter =
      new java.util.concurrent.atomic.AtomicInteger(0);
  private final java.util.concurrent.atomic.AtomicLong stepStartTime =
  ...
  public boolean canExecuteAutonomousStep() {
    ...
    if (startTime > 0 && (System.currentTimeMillis() - startTime) > (TIMEOUT_MINUTES * 60 * 1000)) {
  ...
  /**
   * SL-03: Provider Recovery Attempts to recover failed providers by checking their health status.
   */
                    ...
                    .flatMap(
                        isHealthy -> {
                          if (Boolean.TRUE.equals(isHealthy)) {
                            ...
                            return providerRepository.save(provider).thenReturn(provider.getId());
                          }
                          return Mono.just("");
                        })
                    .onErrorResume(
                        e -> {
                          log.error(
                              "Error during health check recovery for provider {}: {}",
                              provider.getId(),
                              e.getMessage());
                          return Mono.just("");
                        }))
  ...
  /**
   * SL-04: Vision Service Graceful Degradation Returns text-only fallback when VisionService is
   * unavailable.
   */
