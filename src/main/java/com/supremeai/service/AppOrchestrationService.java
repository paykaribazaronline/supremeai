...
import com.supremeai.agentorchestration.OrchesResultContext;
...
/**
 * AppOrchestrationService — The "Brain" of the system.
 * Handles the end-to-end flow: Requirement -> Code Generation -> GitHub Repo -> Code Push.
 */
@Service
public class AppOrchestrationService {
     ...
     */
    public Mono<Map<String, Object>> runFullPipeline(String requirement, Map<String, String> githubConfig) {
        ...
        if (orchestrator == null) {
            return Mono.error(new IllegalStateException("Orchestrator unavailable. Check Firestore credentials."));
        }
        ...
        return Mono.fromCallable(() -> {
            ...
            .flatMap(orchestrationResult -> {
                ...
                @SuppressWarnings("unchecked")
                List<EntityDefinition> entities = (List<EntityDefinition>) generationContext.get("entities");
                
                log.info("Orchestration complete. Decisions: {}, Entities found: {}", 
                ...
                if (entities != null && !entities.isEmpty()) {
                    codeResult = codeGenerationService.generateAppWithAI(
    ...
    private Mono<Map<String, Object>> handleGitHubDeployment(Map<String, Object> codeResult, Map<String, String> explicitConfig) {
        @SuppressWarnings("unchecked")
        ...
        if (explicitConfig != null && explicitConfig.containsKey("repo") && explicitConfig.containsKey("owner")) {
            // Use explicit config
            ...
            return publishAgent.getAutoPushConfig()
                .flatMap(autoConfig -> {
