...
/**
 * Service to interact with Firebase Realtime Database. Wraps the blocking Admin SDK calls into
 * reactive Monos.
 */
@Service
public class FirebaseRealtimeService {
          ...
          FirebaseDatabase.getInstance()
              ...
              .setValue(
                  ...
                  (error, ref) -> {
                    if (error != null) {
                      sink.error(
                          new RuntimeException("Firebase RTDB write error: " + error.getMessage()));
                    } else {
                      sink.success();
                    }
                  });
  ...
  /** Save data to a specific path in Realtime Database. Alias for setData for compatibility. */
