 ...
 *
 * <p>Features: - Create Firebase Auth accounts from pre-saved email/password in Firestore - Bulk
 * account provisioning from stored credentials - User profile management (tier, status, quota) -
...
public class UserAccountService {
  ...
  /**
   * Create a single Firebase Auth user account with email and password. Also creates the
   * corresponding Firestore user document.
   */
  public Mono<User> createAccount(
      String email, String password, String displayName, UserTier tier) {
            ...
            existingUser -> {
              if (existingUser != null) {
                ...
                return userRepository
                    .findByFirebaseUid(existingUser.getUid())
                    .switchIfEmpty(
                        createUserDocument(
                            existingUser.getUid(),
                            email,
                            displayName != null ? displayName : existingUser.getDisplayName(),
                            tier));
              }
                  ...
                  .flatMap(
                      userRecord ->
                          createUserDocument(
                              userRecord.getUid(),
                              email,
                              displayName != null ? displayName : userRecord.getDisplayName(),
                              tier))
                  .doOnNext(
                      user ->
                          log.info(
                              "Created Firebase Auth account for: {} (uid: {})",
                              email,
                              user.getFirebaseUid()));
            });
  }
  ...
  public Mono<Map<String, Object>> createAccountsFromCollection(String collectionName) {
    if (firestore == null) {
      return Mono.error(
          new IllegalStateException("Firestore is not available. Ensure cloud profile is active."));
    }
        ...
        .flatMap(
            doc -> {
              ...
              if (email == null || password == null) {
                return Mono.just(
                    new BulkResult(BulkStatus.ERROR, doc.getId(), "missing email or password"));
              }

              UserTier tier =
                  tierStr != null ? UserTier.valueOf(tierStr.toUpperCase()) : UserTier.FREE;
        ...
        .map(
            results -> {
              ...
              summary.put(
                  "createdEmails",
                  results.stream()
                      .filter(r -> r.status == BulkStatus.CREATED)
                      .map(r -> r.identifier)
                      .toList());
              summary.put(
                  "errorDetails",
                  results.stream()
                      .filter(r -> r.status == BulkStatus.ERROR)
                      .map(r -> r.identifier + ": " + r.message)
                      .toList());
              return summary;
            });
  }

  private enum BulkStatus {
    CREATED,
    SKIPPED,
    ERROR
  }
  ...
  private Mono<User> createUserDocument(
      String uid, String email, String displayName, UserTier tier) {
                ...
                Mono.fromCallable(
                        () -> {
                          try {
                            ...
                            log.error(
                                "Failed to update custom claims for {}: {}",
                                userId,
                                e.getMessage());
                          }
                          return user;
                        ...
                        () -> {
                          try {
                            UserRecord.UpdateRequest request =
                                new UserRecord.UpdateRequest(userId).setDisabled(true);
                            getAuth().updateUser(request);
                          } catch (FirebaseAuthException e) {
                            log.error(
                                "Failed to disable Firebase Auth user {}: {}",
                                userId,
                                e.getMessage());
                          }
                          return user;
            ...
            user ->
                Mono.fromCallable(
                        () -> {
                          try {
                            UserRecord.UpdateRequest request =
                                new UserRecord.UpdateRequest(userId).setDisabled(false);
                            getAuth().updateUser(request);
                          } catch (FirebaseAuthException e) {
                            log.error(
                                "Failed to re-enable Firebase Auth user {}: {}",
                                userId,
                                e.getMessage());
                          }
                          return user;
