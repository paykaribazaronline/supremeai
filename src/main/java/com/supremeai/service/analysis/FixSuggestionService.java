...
public class FixSuggestionService {
  ...
  private static final Pattern EXPLANATION_PATTERN =
      Pattern.compile("EXPLANATION:\\s*(.+?)(?=FIXED_CODE:|$)", Pattern.DOTALL);
  private static final Pattern FIXED_CODE_PATTERN =
      Pattern.compile("FIXED_CODE:\\s*(.+?)(?=CONFIDENCE:|$)", Pattern.DOTALL);
  private static final Pattern CONFIDENCE_PATTERN = Pattern.compile("CONFIDENCE:\\s*([0-9.]+)");

  @Autowired
  public FixSuggestionService(
      AIProviderFactory providerFactory,
      FixPromptTemplates fixPromptTemplates,
  ...
  public Mono<List<AnalysisFix>> generateFixes(String jobId, List<AnalysisFinding> findings) {
    List<AnalysisFinding> fixableFindings =
        findings.stream()
    ...
    log.info(
        "Generating fix suggestions for {} HIGH/CRITICAL findings in job {}",
        fixableFindings.size(),
        jobId);
    ...
    return Flux.fromIterable(fixableFindings)
        ...
        .flatMap(
            fixes -> {
              ...
              return Flux.fromIterable(fixes).flatMap(fix -> fixRepository.save(fix)).collectList();
            })
              ...
              try {
                ...
                FixPromptTemplate.FixContext context =
                    FixPromptTemplate.FixContext.builder()
                        ...
                        .codeSnippet(
                            finding.getCodeSnippet() != null ? finding.getCodeSnippet() : "")
                ...
                log.debug(
                    "Sending fix prompt for finding {} in file {}",
                    finding.getId(),
                    finding.getFile());
                ...
                log.error(
                    "Error generating fix for finding {}: {}", finding.getId(), e.getMessage());
                return null;
              }
            })
        .subscribeOn(Schedulers.boundedElastic());
