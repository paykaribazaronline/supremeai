...
public class BrowserService {
    ...
    public Mono<String> searchAndScrape(String query, String engine, String baseUrl) {
        return Mono.fromCallable(() -> {
            try {
                ...
                String targetUrl = baseUrl.contains("%s") ? baseUrl.replace("%s", encodedQuery) : baseUrl + encodedQuery;
                
                log.info("🌐 [Level-3 Web Fallback] Initiating Fast-Path (Jsoup) connection via {}: {}", engine, targetUrl);
                ...
                if (selfHealingService.isDomainQuarantined(domain)) {
                    log.warn("🚫 [QUARANTINE] Domain {} is quarantined. Skipping Playwright, returning Jsoup fallback data.", domain);
                    return scrapedData;
                }
                
                // Hybrid Check: Jsoup ফেইল করলে বা JS/Bot Protection থাকলে Playwright ফায়ার করবে
                if (needsPlaywrightFallback(scrapedData)) {
                    log.warn("⚠️ [Level-3] JS-rendering or Bot Protection detected! Switching to Playwright (Hybrid Engine)...");
                    try {
    ...
    private boolean needsPlaywrightFallback(String text) {
        ...
        return lower.length() < 200 || lower.contains("enable javascript") 
            || lower.contains("checking your browser") || lower.contains("cloudflare") 
            || lower.contains("please wait...") || lower.contains("captcha");
    }
    ...
    public reactor.core.publisher.Mono<java.util.Map<String, Object>> getStatus() {
        return reactor.core.publisher.Mono.just(java.util.Map.of("status", "inactive"));
    }
    ...
    public reactor.core.publisher.Flux<com.supremeai.model.browser.BrowserActivity> getRecentActivity() {
        return reactor.core.publisher.Flux.empty();
    }

    public reactor.core.publisher.Flux<com.supremeai.model.browser.StoredCredential> getAllCredentials(String userId) {
        return reactor.core.publisher.Flux.empty();
    }

    public reactor.core.publisher.Mono<com.supremeai.model.browser.StoredCredential> saveCredential(com.supremeai.model.browser.StoredCredential credential) {
    ...
    public reactor.core.publisher.Flux<com.supremeai.model.browser.UrlPermission> getAllowedUrls(String userId) {
        return reactor.core.publisher.Flux.empty();
    }

    public reactor.core.publisher.Flux<com.supremeai.model.browser.UrlPermission> getDeniedUrls(String userId) {
        return reactor.core.publisher.Flux.empty();
    }

    public reactor.core.publisher.Mono<com.supremeai.model.browser.UrlPermission> addUrlPermission(com.supremeai.model.browser.UrlPermission permission) {
        return reactor.core.publisher.Mono.just(permission);
    }

    public reactor.core.publisher.Mono<com.supremeai.model.browser.UrlPermission> updateUrlPermission(String id, com.supremeai.model.browser.UrlPermission permission) {
        return reactor.core.publisher.Mono.just(permission);
    }
    ...
    public reactor.core.publisher.Flux<com.supremeai.model.browser.UrlPermissionRequest> getPermissionRequests() {
        return reactor.core.publisher.Flux.empty();
    }
    ...
    public reactor.core.publisher.Mono<com.supremeai.model.browser.BrowserTask> createActivityTask(String goal) {
    ...
    public reactor.core.publisher.Flux<com.supremeai.model.browser.BrowserFinding> getFindingsForTask(String id) {
        return reactor.core.publisher.Flux.empty();
    }

    public reactor.core.publisher.Mono<com.supremeai.model.browser.BrowserFinding> createFinding(com.supremeai.model.browser.BrowserFinding finding) {
    ...
    public reactor.core.publisher.Mono<com.supremeai.model.browser.BrowserActivity> recordActivity(String url, String action, String title, String reasoning) {
