    ...
    private Map<String, List<PatternRule>> initializeRules() {
        ...
        rules.put("CRITICAL", List.of(
            ...
            PatternRule.builder()
                ...
                .suggestion("Remove service-account.json from repo; use GOOGLE_APPLICATION_CREDENTIALS env var or Secret Manager")
                .build(),
            PatternRule.builder()
                .category("SECRETS")
                .pattern(Pattern.compile("(?i)(jwt[._-]?secret|signing[._-]?key)\\s*=\\s*[\"'][^\"']{8,}[\"']"))
                .message("JWT signing secret hardcoded in source code")
                .suggestion("Load JWT secret from environment variable: System.getenv(\"JWT_SECRET\")")
                .build(),
            PatternRule.builder()
                .category("SECRETS")
                .pattern(Pattern.compile("(?i)(gemini|openai|anthropic)[._-]?(api[._-]?key)\\s*=\\s*[\"'][A-Za-z0-9_\\-]{20,}[\"']"))
        ...
        rules.put("HIGH", List.of(
            ...
            PatternRule.builder()
                .category("SSRF")
                .pattern(Pattern.compile("(?i)(webClient|restTemplate|HttpClient).*\\burl\\b.*request\\.getParameter"))
                .message("Potential SSRF: HTTP client URL constructed from request parameter")
                .suggestion("Whitelist allowed URLs; never pass user input directly to HTTP client URL")
                .build(),
            PatternRule.builder()
                .category("DESERIALIZATION")
                .pattern(Pattern.compile("ObjectInputStream\\s*\\("))
                .message("Java native deserialization — insecure deserialization risk")
                .suggestion("Avoid ObjectInputStream with untrusted data; use JSON/Protobuf serialization instead")
                .build(),
            PatternRule.builder()
                .category("OPEN_REDIRECT")
                .pattern(Pattern.compile("(?i)(sendRedirect|RedirectView|redirect:).*request\\.getParameter"))
        ...
        rules.put("MEDIUM", List.of(
            ...
            PatternRule.builder()
                .category("EXPOSURE")
                .pattern(Pattern.compile("(?i)management\\.endpoints\\.web\\.exposure\\.include\\s*=\\s*\\*"))
                .message("All Spring Boot actuator endpoints exposed — sensitive data/control leak")
                .suggestion("Expose only needed endpoints: include=health,info; protect /actuator with .hasRole(\"ADMIN\")")
                .build(),
            PatternRule.builder()
                .category("PROTOTYPE_POLLUTION")
                .pattern(Pattern.compile("__proto__|constructor\\[prototype\\]|\\.prototype\\s*\\["))
                .message("Prototype pollution pattern detected in JavaScript/TypeScript")
                .suggestion("Validate/sanitize objects from user input; use Object.create(null) for maps")
                .build(),
            PatternRule.builder()
                .category("CONFIG_EXPOSURE")
                .pattern(Pattern.compile("(?i)storageBucket\\s*:\\s*\"[a-z0-9\\-]+\\.appspot\\.com\".*apiKey\\s*:\\s*\"AIza"))
                .message("Firebase client config hardcoded — verify this is intentional (client config is public)")
                .suggestion("Restrict Firebase with App Check and Firestore security rules to prevent misuse")
                .build(),
            PatternRule.builder()
                ...
                .suggestion("If backend should be private, remove flag. If intentionally public, add API-level auth (JWT)")
                .build()
        ));
        ...
        rules.put("LOW", List.of(
            ...
            PatternRule.builder()
                .category("STUB")
                .pattern(Pattern.compile("(?i)(throw new UnsupportedOperationException|return null; // TODO|not implemented)"))
