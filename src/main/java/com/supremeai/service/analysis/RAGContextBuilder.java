...
public class RAGContextBuilder {
  ...
  public RAGContext buildContext(
      String projectId, String agentQuery, List<String> allFiles, int topK, int maxTokens) {
    List<CodeChunk> relevantChunks =
        vectorSearchService.searchSimilarChunks(projectId, agentQuery, topK);

    if (relevantChunks.isEmpty()) {
      log.debug("No relevant chunks found for query, falling back to file scan");
      return new RAGContext(agentQuery, List.of(), List.of(), 0, 0, allFiles.size());
    }
    ...
    for (CodeChunk chunk : relevantChunks) {
      ...
      contextChunks.add(
          new ContextChunk(
              chunk.getFile(), chunk.getStartLine(), chunk.getEndLine(), chunk.getContent(), 1.0));

      totalTokens += estimatedTokens;
    }

    List<String> contextFiles =
        contextChunks.stream().map(ContextChunk::getFile).distinct().collect(Collectors.toList());

    return new RAGContext(
        agentQuery, contextChunks, contextFiles, totalTokens, contextFiles.size(), allFiles.size());
  }

  public RAGContext buildContextForFile(String projectId, String filePath, int maxTokens) {
    List<CodeChunk> chunks =
        vectorSearchService.searchSimilarChunks(projectId, filePath, DEFAULT_TOP_K);
    ...
    List<ContextChunk> contextChunks =
        chunks.stream()
            ...
            .map(
                c ->
                    new ContextChunk(
                        c.getFile(), c.getStartLine(), c.getEndLine(), c.getContent(), 1.0))
            .collect(Collectors.toList());

    int totalTokens =
        (int)
            (contextChunks.stream().mapToInt(c -> c.getContent().split("\n").length).sum()
                * AVG_TOKENS_PER_LINE);

    return new RAGContext(
        "analyze:" + filePath, contextChunks, List.of(filePath), totalTokens, 1, 1);
  }
  ...
  public static class RAGContext {
    ...
    public RAGContext(
        String agentQuery,
        List<ContextChunk> relevantChunks,
        List<String> contextFiles,
        int totalTokens,
        int filesScanned,
        int totalFiles) {
    ...
    public List<ContextChunk> getRelevantChunks() {
      return relevantChunks;
    }

    public void setRelevantChunks(List<ContextChunk> relevantChunks) {
      this.relevantChunks = relevantChunks;
    }
  ...
  public static class ContextChunk {
    ...
    public ContextChunk(
        String file, int startLine, int endLine, String content, double relevanceScore) {
      this.file = file;
