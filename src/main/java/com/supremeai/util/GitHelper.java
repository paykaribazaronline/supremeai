  ...
  public static void commitAndPushAll() {
    try {
      if (!hasUncommittedChanges()) {
        log.info("[GitHelper] No local changes to commit. Skipping sync.");
        return;
      }

      runGitCommand("add", ".");
      runGitCommand("commit", "-m", "Automated commit: system health and metadata sync");

      // Dynamically fetch all remotes instead of hardcoded list
      String remoteOutput = runGitCommand("remote");
      if (remoteOutput.isBlank()) {
        log.warn("[GitHelper] No remotes found to push to.");
        return;
      }
      String[] remotes = remoteOutput.split("\\s+");
