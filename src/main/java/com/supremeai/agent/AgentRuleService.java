...
/**
 * Enhanced Service to load and enforce global agent rules dynamically. Bridges the gap between
 * file-based rules and Firestore-based dynamic rules.
 */
@Service
public class AgentRuleService {

  @Autowired private DynamicInstructionService dynamicInstructionService;
  ...
  public Mono<String> enrichPrompt(String prompt, String taskType) {
    return dynamicInstructionService
        .getCombinedInstructions(taskType)
        .map(
            rules ->
                "### SUPREMEAI BUILD & ARCHITECTURE GUIDELINES ###\n"
  ...
  /**
   * Legacy support for static wrapping (not recommended for dynamic rules but kept for
   * compatibility)
   */
