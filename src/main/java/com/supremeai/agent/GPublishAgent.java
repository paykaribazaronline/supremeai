...
/**
 * GPublishAgent - Publish and Deploy Agent এই এজেন্টটি অ্যাপ পাবলিশিং এবং ডিপ্লয়মেন্টের জন্য
 * দায়িত্বপ্রাপ্ত
 */
@Service
public class GPublishAgent {
  ...
  @Autowired private com.supremeai.service.GitHubAutomationService githubAutomationService;

  @Autowired private com.supremeai.service.SystemWorkRuleService ruleService;
  ...
  public Mono<Map<String, String>> getAutoPushConfig() {
    return ruleService
        .getRuleByKey("GITHUB_AUTO_PUSH")
        .flatMap(
            pushRule -> {
              if (Boolean.parseBoolean(pushRule.getValue())) {
                return ruleService
                    .getRuleByKey("GITHUB_ORG_NAME")
                    .map(
                        orgRule -> {
                    ...
                    .onErrorResume(
                        e -> Mono.just(Map.of("autoPush", "true", "owner", "supremeai-apps")));
              }
  ...
  public Mono<Map<String, Object>> deployToGitHub(
      String owner, String repo, Map<String, String> files, String installationId) {
    ...
    return githubAutomationService
        .pushGeneratedCode(owner, repo, files, installationId)
        .flatMap(
            pushResult -> {
              // After successful push, try to enable GitHub Pages for a preview
              return githubAutomationService
                  .enablePages(owner, repo, installationId)
                  .map(
                      previewUrl -> {
                        ...
                        logger.info(
                            "গিটহাব ডিপ্লয়মেন্ট এবং পেজ এনাবল সফল হয়েছে: {}/{}", owner, repo);
                        return result;
                      });
            })
        .onErrorResume(
  ...
  public List<Question> analyzePublishingRequirements(String requirement) {
    try {
      ...
      String prompt =
          "একটি অ্যাপ পাবলিশিং এবং ডিপ্লয়মেন্টের জন্য নিম্নলিখিত প্রয়োজনীয়তা বিশ্লেষণ করুন: \""
              + requirement
              + "\"\n"
      ...
      logger.warn(
          "পাবলিশিং প্রয়োজনীয়তা বিশ্লেষণ ব্যর্থ হয়েছে, ডিফল্ট প্রশ্নগুলি ব্যবহার করা হচ্ছে: {}",
          e.getMessage());
  ...
  public Map<String, String> createPublishingPlan(String platform, Map<String, String> config) {
    ...
    switch (platform.toLowerCase()) {
      ...
      case "android":
        ...
        plan.put(
            "distributionMethod",
            config.getOrDefault("androidDistribution", "Google Play Console"));
  ...
  private List<Question> getDefaultPublishingQuestions() {
    ...
    questions.add(
        new Question(
            "publishPlatform",
            "কোন প্ল্যাটফর্মে পাবলিশ করবেন? (iOS, Android, Web, Desktop)",
            "CRITICAL"));
    questions.add(
        new Question(
            "publishStore",
            "কোন স্টোরে পাবলিশ করবেন? (App Store, Play Store, Vercel, GitHub)",
            "HIGH"));
    questions.add(
        new Question(
            "publishAutomation",
            "ডিপ্লয়মেন্ট অটোমেশন চান? (CI/CD, Manual, Semi-automated)",
            "HIGH"));
    questions.add(
        new Question("publishTesting", "টেস্টিং কীভাবে করবেন? (Beta, Alpha, Internal)", "MEDIUM"));
    questions.add(
        new Question(
            "publishUpdates", "আপডেট সিস্টেম কীভাবে হবে? (Auto, Manual, Scheduled)", "MEDIUM"));
    questions.add(
        new Question("publishAnalytics", "অ্যানালিটিক্স ট্র্যাকিং চান? (Yes, No)", "LOW"));
    return questions;
  }
  ...
  private List<Question> parseQuestions(String response) {
    try {
      ...
      return mapper.readValue(
          jsonArray, new com.fasterxml.jackson.core.type.TypeReference<List<Question>>() {});
    } catch (Exception e) {
