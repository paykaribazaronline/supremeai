...
/**
 * Performance Optimization Configuration Optimized for high-concurrency AI workloads with virtual
 * threads
 */
@Configuration
@EnableCaching
...
public class PerformanceConfig {
  ...
  private static final org.slf4j.Logger log =
      org.slf4j.LoggerFactory.getLogger(PerformanceConfig.class);
  ...
  /**
   * Shared ObjectMapper with Afterburner for 20-30% faster JSON serialization. This replaces all
   * individual ObjectMapper instances across providers. Afterburner module generates bytecode for
  ...
  /**
   * Virtual thread executor for handling high-concurrency requests efficiently. Provides 100x
   * better concurrency compared to traditional thread pools.
   */
  ...
  /** Async task executor for background processing. Optimized for CPU-intensive AI tasks. */
  ...
  public Executor asyncTaskExecutor() {
    ...
    executor.initialize();
    log.info(
        "Async task executor configured: core={}, max={}, queue={}",
        asyncCorePoolSize,
        asyncMaxPoolSize,
        asyncQueueCapacity);
    return executor;
  ...
  /**
   * IO-bound task executor for network operations. Separate pool for blocking IO operations to
   * avoid starving CPU threads.
   */
  ...
  /**
   * CPU-bound task executor for computation-heavy operations. Sized based on available processors.
   */
