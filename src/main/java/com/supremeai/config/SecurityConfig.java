...
public class SecurityConfig {
            ...
            auth ->
                auth
                    ...
                    .requestMatchers(
                        ...
                        "/ws/**",
                        "/error",
                        // /api/v1/chat/completions — gated by ExternalToolsAuthFilter
                        // (X-Authorized-Key)
                    ...
                    .hasRole("ADMIN")
                    .requestMatchers("/api/admin/chat/**")
                    .hasRole("ADMIN")
                    .requestMatchers("/api/self-healing/**")
                    .hasRole("ADMIN")
                    .requestMatchers("/api/healing/**")
                    .hasRole("ADMIN")
                    ...
                    .hasRole("ADMIN")
                    .requestMatchers("/api/v1/optimization/**")
                    .hasRole("ADMIN")
                    .requestMatchers("/api/v1/phase6/**")
                    .hasRole("ADMIN")
                    .requestMatchers("/api/v1/phase7/**")
                    .hasRole("ADMIN")
                    .requestMatchers("/api/v1/agents/phase8/**")
                    .hasRole("ADMIN")
                    .requestMatchers("/api/v1/agents/phase9/**")
                    .hasRole("ADMIN")
                    .requestMatchers("/api/v1/agents/phase10/**")
                    .hasRole("ADMIN")
                    .requestMatchers("/api/workflows/**")
                    .hasRole("ADMIN")
                    .requestMatchers("/api/ext/**")
                    .hasRole("ADMIN")
            ...
            ex ->
                ex.authenticationEntryPoint(
                        (req, res, ex2) -> {
                          ...
                          res.getWriter()
                              .write("{\"error\":\"Unauthorized\",\"message\":\"Please login\"}");
                        })
                    .accessDeniedHandler(
                        (req, res, ex2) -> {
                          res.setStatus(403);
  ...
  /**
   * CORS configuration for API endpoints. Uses whitelist from application properties
   * (cors.allowed-origins). Falls back to localhost development and production domain.
   */
  @Bean
  public CorsConfigurationSource corsConfigurationSource() {
    ...
    if (allowedOriginsCsv != null && !allowedOriginsCsv.trim().isEmpty()) {
      ...
      origins =
          Arrays.asList(
              "http://localhost:5173", "http://localhost:3000", "https://supremeai-a.web.app");
    }
