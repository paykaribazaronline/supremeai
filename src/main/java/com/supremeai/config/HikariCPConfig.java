 ...
 *
 * <p>Pool sizing is read from ${spring.datasource.hikari.*} defined in application.yml — the single
 * source of truth for all HikariCP tuning. These fallback defaults match application.yml so that
 * HikariCPConfig remains in sync if that file is overridden at runtime without a recompile.
 */
@Configuration
