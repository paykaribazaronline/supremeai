 ...
 *
 * Run with: --spring.profiles.active=seed-validation  OR  just start the app normally
 * (runs on every non-local/non-test profile by default).
 *
 * What it checks:
 *   ✅ At least MIN_USERS users exist in Firestore
 *   ✅ All MIN_PROVIDERS AI providers are registered
 *   ✅ User tier records (FREE/PRO/ADMIN) exist
 *   ✅ At least 5 knowledge domains registered
 *   ✅ System-learning collection has records
 *   ✅ All 3 knowledge domains present for system context (adding more for quality)
 *
 * If a check fails → logs WARN, does NOT block startup.
...
public class SeedDataValidator implements CommandLineRunner {
    ...
    public SeedDataValidator(UserRepository userRepository,
    ...
    public void run(String... args) throws Exception {
        ...
        if (userCount >= 3) {
            ...
            System.out.printf("[WARN] Users: %d found — run: node scripts/seed/seed-all-data.js --execute%n", userCount);
            didWarn = true;
        }
        ...
        if (providerCount >= 5) {
            ...
            System.out.printf("[WARN] AI Providers: %d found — run: node scripts/seed/seed-ai-providers.js --execute%n", providerCount);
            didWarn = true;
        }
        ...
        if (tierCount >= 3) {
            ...
            System.out.printf("[WARN] User Tiers: %d found — run: node scripts/seed/seed-all-data.js --execute%n", tierCount);
            didWarn = true;
        }
        ...
        if (domainCount >= 5) {
            ...
            System.out.printf("[WARN] Knowledge Domains: %d found — run: node scripts/seed/seed-all-data.js --execute%n", domainCount);
            didWarn = true;
        }
        ...
        if (learningCount >= 5) {
            ...
            System.out.printf("[WARN] System Learning: %d found — run: node scripts/seed/seed-all-data.js --execute%n", learningCount);
            didWarn = true;
        }
        ...
        if (entryCount >= 5) {
            ...
            System.out.printf("[WARN] Knowledge Entries: %d found — run: node scripts/seed/seed-all-data.js --execute%n", entryCount);
            didWarn = true;
        }
        ...
        if (perfCount >= 4) {
            ...
            System.out.printf("[WARN] Provider Performance: %d found — run: node scripts/seed/seed-all-data.js --execute%n", perfCount);
            didWarn = true;
        }
