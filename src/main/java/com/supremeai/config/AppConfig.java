 ...
 *
 * <p>Validates critical configuration values at startup to prevent runtime errors.
 *
 * <p>User Experience Benefits: - Early error detection with clear messages - Prevents application
 * from starting with invalid configuration - Reduces debugging time for configuration issues
 */
@Configuration
public class AppConfig {
  ...
  private void validateRedisConfig() {
    ...
    if (redisMinIdle > redisMaxActive) {
      throw new IllegalStateException(
          String.format(
              "Redis min-idle (%d) cannot be greater than max-active (%d)",
  ...
  private void validateThreadPoolConfig() {
    ...
    if (asyncCorePoolSize > asyncMaxPoolSize) {
      throw new IllegalStateException(
          String.format(
              "Async core pool size (%d) cannot be greater than max pool size (%d)",
    ...
    if (asyncQueueCapacity < 0) {
      throw new IllegalStateException(
          String.format(
              "Invalid async queue capacity: %d. Must be non-negative", asyncQueueCapacity));
    }
