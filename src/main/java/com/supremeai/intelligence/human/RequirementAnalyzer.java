...
/**
 * Solves the "User doesn't know how to explain what they want" problem. Analyzes the prompt for
 * vagueness and asks targeted multiple-choice questions with hints.
 */
@Service
public class RequirementAnalyzer {

  /**
   * Analyzes the user's prompt. If the prompt is too vague (e.g., "make a login page" or "fix the
   * bug"), it generates a Clarification object instead of guessing and writing wrong code.
   */
  public RequirementClarification analyzeRequirement(String userPrompt) {
    ...
    if ((lowerPrompt.contains("it doesn't work")
            || lowerPrompt.contains("fix bug")
            || lowerPrompt.contains("error"))
        && lowerPrompt.length() < 50
        && !lowerPrompt.contains("exception")) {
    ...
    if (lowerPrompt.contains("build") && lowerPrompt.contains("app") && lowerPrompt.length() < 40) {
      ...
      clarification.addOption(
          "Social / Chat", "I'll configure WebSockets for real-time messaging and User profiles.");
