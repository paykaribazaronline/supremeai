...
/**
 * Learns how a specific developer prefers to code and communicate. Now includes Automatic Language
 * Detection & Simplicity Tuning.
 */
@Service
public class HumanPreferenceProfiler {
  ...
  /**
   * Every time the user interacts with the AI, the system analyzes the prompt to update the user's
   * "DNA", including their native language.
   */
  public void learnFromInteraction(
      String userId, String prompt, String acceptedCode, boolean askedForExplanation) {
  ...
  /**
   * Simple heuristic language detection. In a real system, you would use an NLP library (like
   * Apache Tika or Google CLD3).
   */
  private void detectLanguageAndTone(String prompt, DeveloperDNA dna) {
    ...
    if (lowerPrompt.matches(".*[\\u0980-\\u09FF]+.*")
        || lowerPrompt.contains("koro")
        || lowerPrompt.contains("banaw")
        || lowerPrompt.contains("bhalo")) {
    ...
    // Detect Hindi
        ...
        || lowerPrompt.contains("karo")
        || lowerPrompt.contains("banao")
        || lowerPrompt.contains("hai")) {
      dna.setPreferredLanguage("Hindi");
      dna.setPrefersSimpleLanguage(true);
    }
    // Detect Spanish
    else if (lowerPrompt.contains("hola")
        || lowerPrompt.contains("hacer")
        || lowerPrompt.contains("por favor")) {
      dna.setPreferredLanguage("Spanish");
    }

    // Detect if user is struggling with tech terms (requests simple explanations)
    if (lowerPrompt.contains("simple")
        || lowerPrompt.contains("easy")
        || lowerPrompt.contains("non-technical")
        || lowerPrompt.contains("don't understand")) {
      dna.setPrefersSimpleLanguage(true);
    }
  }
}
