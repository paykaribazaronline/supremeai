...
public class DeveloperDNA {
  ...
  public boolean isPrefersSimpleLanguage() {
    return prefersSimpleLanguage;
  }
  ...
  public String generatePromptInjection() {
    ...
    if (!"English".equalsIgnoreCase(preferredLanguage)) {
      injection
          .append("CRITICAL: ALWAYS communicate with this user in ")
    ...
    if (prefersSimpleLanguage) {
      ...
      injection.append(
          "Explain programming concepts as if you are talking to a non-technical person.\n");
    }

    // 2. Explanation Style
    if (explanationScore > 55) {
      injection.append(
          "- The user prefers detailed explanations. Break down the logic step-by-step.\n");
    } else if (explanationScore < 30) {
      injection.append(
          "- The user is an expert who hates chatting. Output ONLY the raw code. NO explanations.\n");
    }

    // 3. Formatting
    injection
        .append("- Format code using ")
        .append(indentStyle)
        .append(" spaces for indentation.\n");
    injection
        .append("- Use ")
        .append(usesCamelCase ? "camelCase" : "snake_case")
        .append(" for variables.\n");
  ...
  public String getSummary() {
    return String.format(
        "Language:%s, SimpleTone:%b, ExpScore:%d",
        preferredLanguage, prefersSimpleLanguage, explanationScore);
  }
}
