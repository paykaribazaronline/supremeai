...
/**
 * Enhanced Random Forest implementation with proper feature engineering, auto-retraining, and
...
public class EnhancedRandomForestPredictor {
  ...
  public void recordFailure(
      String errorSignature, Map<FeatureType, Double> features, boolean actualFailure) {
  ...
  public FailurePrediction predict(String context, Map<FeatureType, Double> features) {
    ...
    String confidence =
        failureProbability > 0.7 ? "HIGH" : failureProbability > 0.4 ? "MEDIUM" : "LOW";
  ...
  private synchronized void retrainModel() {
    ...
    log.info(
        "Random Forest retrained successfully. Accuracy tracking: {}/{} predictions",
        correctPredictions,
        totalPredictions);
  }
  ...
  private static class EnhancedDecisionTree {
    ...
    private double calculateGini(
        double[][] features, int[] labels, int featureIdx, double threshold) {
      ...
      double giniLeft =
          1.0
              - leftLabels.values().stream()
                  .mapToDouble(c -> Math.pow((double) c / finalLeftCount, 2))
                  .sum();

      final int finalRightCount = rightCount;
      double giniRight =
          1.0
              - rightLabels.values().stream()
                  .mapToDouble(c -> Math.pow((double) c / finalRightCount, 2))
                  .sum();

      double totalCount = leftCount + rightCount;
      return (leftCount / totalCount) * giniLeft + (rightCount / totalCount) * giniRight;
    }
