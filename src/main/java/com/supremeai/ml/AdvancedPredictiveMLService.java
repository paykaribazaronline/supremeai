...
/**
 * Advanced Predictive ML Service with time-series forecasting. Uses ARIMA-like models for quota
 * prediction and user behavior forecasting.
 */
@Service
public class AdvancedPredictiveMLService {
  ...
  @Autowired private EnhancedRandomForestPredictor randomForestPredictor;
  ...
  public void recordQuotaUsage(String userId, long used, long quota) {
    TimeSeriesData data =
        userQuotaHistory.computeIfAbsent(userId, k -> new TimeSeriesData("quota_" + userId));
  ...
  public void recordSystemLoad(String serviceName, double load) {
    TimeSeriesData data =
        systemLoadHistory.computeIfAbsent(
            serviceName, k -> new TimeSeriesData("load_" + serviceName));
  ...
  public void recordUserActivity(String userId, String activityType) {
    TimeSeriesData data =
        userActivityHistory.computeIfAbsent(userId, k -> new TimeSeriesData("activity_" + userId));
  ...
  public QuotaPrediction predictQuotaExhaustion(
      String userId, long currentUsage, long monthlyQuota) {
  ...
  private QuotaPrediction predictQuotaExhaustion(
      String userId, TimeSeriesData data, long monthlyQuota) {
    ...
    List<Double> forecast =
        forecastARIMA(values, arCoefficients, maCoefficients, 30); // Next 30 days
  ...
  private List<Double> forecastARIMA(List<Double> history, double[] ar, double[] ma, int steps) {
    ...
    for (int s = 0; s < steps; s++) {
      ...
      for (int i = 0; i < ar.length && (forecast.size() - 2 - i) >= 0; i++) {
        prediction +=
            ar[i] * (forecast.get(forecast.size() - 1 - i) - forecast.get(forecast.size() - 2 - i));
      }
  ...
  private double calculatePredictionConfidence(List<Double> actual, List<Double> forecast) {
    ...
    double forecastTrend =
        forecast.isEmpty() ? 0 : forecast.get(forecast.size() - 1) - forecast.get(0);

    double trendMatch =
        1.0 - Math.abs(actualTrend - forecastTrend) / (Math.abs(actualTrend) + 0.01);
  ...
  private String analyzeTrend(List<Double> values) {
    ...
    double firstHalfAvg =
        values.subList(0, half).stream().mapToDouble(Double::doubleValue).average().orElse(0);
    double secondHalfAvg =
        values.subList(half, values.size()).stream()
  ...
  private void triggerN8nAlert(QuotaPrediction prediction) {
    this.webClient
        ...
        .doOnError(
            e ->
                log.error(
                    "Failed to trigger n8n alert for user {}: {}",
                    prediction.userId,
                    e.getMessage()))
  ...
  public static class UserActivityPrediction {
    ...
    public UserActivityPrediction(
        String userId, String predictedActivity, double confidence, Map<String, Object> metadata) {
      this.userId = userId;
  ...
  private static class TimeSeriesData {
    ...
    synchronized void addDataPoint(long timestamp, double value) {
      ...
      if (timestamps.size() > 1000) {
        ...
        timestamps =
            new ArrayList<>(timestamps.subList(timestamps.size() - 1000, timestamps.size()));
