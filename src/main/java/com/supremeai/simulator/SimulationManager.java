...
public class SimulationManager {
     ...
     */
    public Mono<SimulationScenario> createScenario(String userId, String appId, String name, String description, java.util.Map<String, Object> parameters) {
        ...
        SimulationScenario scenario = new SimulationScenario(scenarioId, userId, appId, name, description);
        if (parameters != null) {
    ...
    public Mono<SimulationResult> executeSimulation(String userId, String scenarioId) {
        ...
        return scenarioRepository.findById(scenarioId)
            ...
            .flatMap(scenario -> {
                ...
                SimulationResult result = new SimulationResult(resultId, scenarioId, userId, scenario.getAppId());
                ...
                return resultRepository.save(result)
                    .flatMap(savedResult -> {
                        ...
                        Mono.defer(() -> controllerEngine.runSimulation(scenario, savedResult)
                            .flatMap(executedResult -> resultAnalyzer.analyze(executedResult))
                            .flatMap(analyzedResult -> {
                                analyzedResult.setStatus("SUCCESS");
                                analyzedResult.getLogs().add("Simulation completed successfully at " + LocalDateTime.now());
                                return resultRepository.save(analyzedResult);
                            })
                            .onErrorResume(e -> {
                                logger.error("Simulation failed for result {}", savedResult.getResultId(), e);
                                savedResult.setStatus("FAILED");
                                savedResult.getLogs().add("Simulation failed: " + e.getMessage());
