...
public class ControllerEngine {
  ...
  /**
   * Executes the simulation using parameters configured in the scenario. Captures and records
   * performance metrics on the result object.
   */
    ...
    return Mono.delay(Duration.ofSeconds(2)) // Simulate startup and deployment delay
        .map(
            ignored -> {
              ...
              double cpuUsagePct =
                  15.0 + (concurrency.doubleValue() * 1.5) + (random.nextDouble() * 5.0);
              ...
              int responseTimeMs =
                  50 + (int) (concurrency.doubleValue() * 3.2) + random.nextInt(40);
              ...
              result
                  .getLogs()
                  .add(
                      "Simulation actions: [Tap Home -> Input Text -> Submit Form -> Rotate Device]");
              result
                  .getLogs()
                  .add(
                      String.format(
                          "Metrics collected: LoadTime: %dms, CPU: %.1f%%, Memory: %.1fMB, Errors: %.1f%%, RespTime: %dms",
