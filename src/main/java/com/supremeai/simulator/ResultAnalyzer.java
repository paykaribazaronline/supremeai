    ...
    public Mono<SimulationResult> analyze(SimulationResult result) {
        ...
        return Mono.fromCallable(() -> {
            ...
            if (metrics == null || metrics.isEmpty()) {
                recommendations.add("No metrics collected. Recommend running simulation with interactive parameters.");
            ...
            if (loadTimeMs > 1000) {
                recommendations.add("High startup latency detected (" + loadTimeMs + "ms). Consider enabling code-splitting, lazy-loading widgets, and optimizing heavy static assets.");
            } else {
                recommendations.add("Excellent startup latency (" + loadTimeMs + "ms). Keep using standard tree shaking and asset minification.");
            }

            // 2. Analyze CPU usage (Threshold: > 70%)
            if (cpuUsagePct > 70.0) {
                recommendations.add("High CPU peak load (" + String.format("%.1f", cpuUsagePct) + "%). Avoid unnecessary widget rebuilds and throttle active listener callbacks.");
            }

            // 3. Analyze Memory usage (Threshold: > 250MB)
            if (memoryUsageMb > 250.0) {
                recommendations.add("Memory footprints are high (" + String.format("%.1f", memoryUsageMb) + "MB). Optimize caching strategies and ensure all listeners, controllers, and streams are closed on widget dispose.");
            }

            // 4. Analyze Error rate (Threshold: > 0%)
            if (errorRatePct > 0.0) {
                recommendations.add("Failures detected during scenario execution (" + String.format("%.1f", errorRatePct) + "%). Audit exception handling logic and review console logs for uncaught network/null-pointer issues.");
            }

            // 5. Analyze Response time (Threshold: > 200ms)
            if (responseTimeMs > 200) {
                recommendations.add("High interactive response latency (" + responseTimeMs + "ms). Consider offloading intensive computations to background isolates (Dart) or optimizing server-side database indices.");
            }

            if (recommendations.isEmpty()) {
                recommendations.add("System is fully optimized! Performance parameters are within flawless thresholds.");
            }
