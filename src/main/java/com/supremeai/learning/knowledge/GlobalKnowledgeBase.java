...
public class GlobalKnowledgeBase {
  ...
  /**
   * Load memories from Firestore on startup. Uses reactive repository with blocking call suitable
   * for @PostConstruct.
   */
  public void loadMemories() {
    ...
    try {
      List<SolutionMemory> allSolutions =
      ...
      if (allSolutions != null) {
        for (SolutionMemory solution : allSolutions) {
          ...
          globalMemory.computeIfAbsent(errorSignature, k -> new ArrayList<>()).add(solution);
        }
  ...
  /**
   * Records a successful fix, checking with Admin Dashboard first for new solutions. Existing
   * solutions are updated with versioning.
   */
  public Mono<Void> recordSuccessWithPermission(
    ...
    for (SolutionMemory solution : solutions) {
      if (solution.getResolvedCode().equals(successfulCode)) {
        ...
        SolutionMemory updated =
            solution.createUpdate(
        ...
        log.info(
            "[Knowledge Base] Updated solution (new version: v{}) by {}",
            updated.getVersion(),
            aiProvider);
        saveMemory(updated);
        return Mono.empty();
    ...
    ImprovementProposal proposal =
        new ImprovementProposal(
            ...
            "AI Provider "
                + aiProvider
                + " successfully fixed this error. Should I add it to Global Memory?",
            "KNOWLEDGE_BASE",
            successfulCode);
            ...
            isApprovedImmediately -> {
              if (isApprovedImmediately) {
                SolutionMemory newMemory =
                    new SolutionMemory(
                        errorSignature, successfulCode, aiProvider, executionTimeMs, securityScore);
                solutions.add(newMemory);
                log.info("[Knowledge Base] Learned NEW solution automatically!");
                saveMemory(newMemory);
              } else {
                log.info(
                    "[Knowledge Base] Solution pending. Waiting for Admin to approve in the Dashboard.");
              }
  ...
  /**
   * Record a failure for an existing solution to help refine scoring. Creates a new version to
   * preserve history.
   */
    ...
    if (solutions != null) {
      for (SolutionMemory solution : solutions) {
        if (solution.getResolvedCode().equals(failedCode)) {
          ...
          SolutionMemory updated =
              solution.createUpdate(
                  failedCode, solution.getExecutionTimeMs(), solution.getSecurityScore());
          updated.setSuccessCount(solution.getSuccessCount());
          updated.setFailureCount(solution.getFailureCount() + 1);

          solutions.add(updated);
          log.warn(
              "[Knowledge Base] Penalized a solution that failed in production (v{})",
              updated.getVersion());
          saveMemory(updated);
          return Mono.empty();
        }
  ...
  /**
   * Find the best known solution for an error signature reactively. Returns Mono.empty() if no
   * solution meets minimum confidence threshold.
   */
  ...
  private String findBestCode(List<SolutionMemory> solutions) {
    ...
    solutions.sort(
        (s1, s2) -> Double.compare(s2.calculateSupremeScore(), s1.calculateSupremeScore()));
  ...
  private Mono<Void> loadSolutionsFromFirestore(String errorSignature) {
    ...
    return solutionMemoryRepository
        ...
        .flatMap(
            loaded -> {
              if (!loaded.isEmpty()) {
                List<SolutionMemory> existing =
                    globalMemory.computeIfAbsent(errorSignature, k -> new ArrayList<>());
                existing.addAll(loaded);
                log.debug(
                    "Loaded {} solutions for error signature {} from Firestore",
                    loaded.size(),
                    errorSignature);
              }
              return Mono.empty();
        ...
        .onErrorResume(
            e -> {
              log.error(
                  "Failed to load solutions from Firestore for {}: {}",
                  errorSignature,
                  e.getMessage());
              return Mono.empty();
            })
