...
/**
 * Service to learn from user-edited code and store patterns in the knowledge base. This enables
 * SupremeAI to improve over time based on user corrections and improvements.
 */
@Service
public class UserCodeLearningService {
  ...
  public UserCodeLearningService(
      ...
      @Value("${firestore.collection.system-learning:system_learning}")
          String systemLearningCollection) {
  ...
  private void loadExistingPatterns() {
    try {
      log.info(
          "Loading existing learning patterns from Firestore collection: {}",
          systemLearningCollection);
   ...
   */
  public void learnFromUserEdit(
      String taskId, String originalCode, String editedCode, String context) {
  ...
  private CodeDiffAnalysis analyzeCodeDiff(String originalCode, String editedCode) {
    ...
    int commonLines =
        originalLines.length + editedLines.length - addedLines.size() - removedLines.size();
  ...
  private SystemLearning createLearningPattern(
      String taskId, CodeDiffAnalysis diffAnalysis, String context) {
    ...
    content
        .append("User made ")
        .append(diffAnalysis.getAddedLines().size())
        .append(" additions and ")
        .append(diffAnalysis.getRemovedLines().size())
        .append(" removals. ");

    if (!diffAnalysis.getAddedLines().isEmpty()) {
      content
          .append("Key additions: ")
          .append(
              String.join(
                  ", ",
                  diffAnalysis
                      .getAddedLines()
                      .subList(0, Math.min(3, diffAnalysis.getAddedLines().size()))));
    }
  ...
  private String determineCategory(String context, CodeDiffAnalysis diffAnalysis) {
    ...
    List<Map.Entry<String, List<String>>> categoryKeywords =
        List.of(
    ...
    if (firestore != null) {
      try {
        DocumentSnapshot doc =
            firestore.collection(systemLearningCollection).document(patternId).get().get();
        if (doc.exists()) {
    ...
    if (firestore != null) {
      try {
        ApiFuture<QuerySnapshot> future =
            firestore.collection(systemLearningCollection).whereEqualTo("category", category).get();
        List<QueryDocumentSnapshot> documents = future.get().getDocuments();
  ...
  /** Record a failure pattern for learning. Used by VS Code extension to report errors. */
    ...
    try {
      ...
      pattern.setContent(
          "Error: "
              + errorSignature
              + "\nCode: "
              + failedCode.substring(0, Math.min(200, failedCode.length())));
      ...
      pattern.setMetadata(
          Map.of(
              ...
              errorSignature,
              "failedCode",
              failedCode.substring(0, Math.min(500, failedCode.length()))));
      ...
      if (firestore != null) {
        ...
        try {
          DocumentReference docRef =
              firestore
                  .collection(systemLearningCollection)
                  .add(data)
