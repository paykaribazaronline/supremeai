...
/**
 * Self-Learning Router - Plan 24 Phase 4 Implements SONA-style Q-learning for adaptive agent
 * routing. Routes tasks to the optimal agent/provider based on learned performance patterns.
 *
 * <p>Inspired by Ruflo's self-learning router (https://github.com/ruvnet/ruflo)
 */
@Service
public class SelfLearningRouter {
  ...
  public RoutingDecision routeTask(
      String taskCategory, String taskSignature, List<String> candidateAgents) {
  ...
  public void updateFromOutcome(
      String taskCategory, String taskSignature, String agentId, boolean success, long durationMs) {
    String state = encodeState(taskCategory, taskSignature);
    ...
    // Record outcome
    TaskOutcome outcome =
        new TaskOutcome(agentId, success, reward, durationMs, System.currentTimeMillis());
  ...
  private void trimHistory(String state, int maxEntries) {
    ...
    if (history != null && history.size() > maxEntries) {
      ...
      rewardHistory.put(
          state, new ArrayList<>(history.subList(history.size() - maxEntries, history.size())));
    }
  }
  ...
  public static class TaskOutcome {
    ...
    public TaskOutcome(
        String agentId, boolean success, double reward, long durationMs, long timestamp) {
      this.agentId = agentId;
