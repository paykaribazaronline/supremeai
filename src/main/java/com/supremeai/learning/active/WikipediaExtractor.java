...
/**
 * Wikipedia SiteExtractor - Scrapes recent tech-related edits and summaries from Wikipedia's public
 * API.
 *
 * <p>Authority: 0.75 (mid-range - good for general knowledge, not specific bugs)
 */
@Component
public class WikipediaExtractor implements SiteExtractor {
  ...
  @Override
  public List<ActiveInternetScraper.ScrapedIssue> scrape(
          ...
          if (summary != null && !summary.isEmpty()) {
            ActiveInternetScraper.ScrapedIssue issue =
                new ActiveInternetScraper.ScrapedIssue(
                    "Tech Update: " + title, summary, "Wikipedia");
  ...
  private static final List<String> TECH_KEYWORDS =
      List.of(
          "programming",
          "software",
          "computer",
  ...
  private String fetchWikipediaSummary(RestTemplate restTemplate, String title) {
    try {
      ...
      String url =
          String.format("https://en.wikipedia.org/api/rest_v1/page/summary/%s", encodedTitle);
      String response = restTemplate.getForObject(url, String.class);
      if (response != null) {
        JsonNode root = new com.fasterxml.jackson.databind.ObjectMapper().readTree(response);
