...
public class QueryClassifier {
  ...
  public QueryClassification classify(String domainName, List<String> keywords) {
    ...
    boolean isCode =
        fullQuery.contains("error")
            || fullQuery.contains("exception")
            || fullQuery.contains("bug")
            || fullQuery.contains("code")
            || fullQuery.contains("java")
            || fullQuery.contains("python");

    boolean isResearch =
        fullQuery.contains("paper")
            || fullQuery.contains("research")
            || fullQuery.contains("algorithm")
            || fullQuery.contains("arxiv")
            || fullQuery.contains("machine learning");

    boolean isWebDev =
        fullQuery.contains("html")
            || fullQuery.contains("css")
            || fullQuery.contains("javascript")
            || fullQuery.contains("mdn")
            || fullQuery.contains("react")
            || fullQuery.contains("frontend");
  ...
  public static class QueryClassification {
    ...
    public QueryClassification(
        boolean codeRelated, boolean researchRelated, boolean webDevRelated) {
    ...
    public boolean isResearchRelated() {
      return researchRelated;
    }

    public boolean isWebDevRelated() {
      return webDevRelated;
    }
  }
}
