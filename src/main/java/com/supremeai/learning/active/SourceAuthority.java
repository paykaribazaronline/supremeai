...
/**
 * Source authority hierarchy for conflict resolution. Higher weight sources override lower-weight
 * sources when confidence scores are similar.
 */
public enum SourceAuthority {
  ...
  public static SourceAuthority fromSourceName(String sourceName) {
    ...
    if (lower.contains("official")
        ...
        || lower.contains("docs.")
        || lower.contains("developer.")) return OFFICIAL_DOCS;
    if (lower.contains("github") || lower.contains("gitlab")) return GITHUB;
    if (lower.contains("stackoverflow")
        || lower.contains("stackexchange")
        || lower.contains("stack exchange")) return STACK_OVERFLOW;
    if (lower.contains("wikipedia")) return WIKIPEDIA;
    if (lower.contains("blog") || lower.contains("medium.com") || lower.contains("wordpress.com"))
      return BLOGS;
    return FORUMS;
  }
}
