...
/**
 * LearningActivityLogService - Centralized logging for all learning-related activities. Critical
 * for Phase 1 debugging and Phase 3 error analysis.
 *
 * <p>Logs: - Site access attempts (success/failure) - Scraping operations (sources, item counts,
 * errors) - Learning proposals (submitted, approved, rejected) - Solution versioning (create,
 * update, obsolete) - Quota consumption
 *
 * <p>These logs feed into: - Error analysis (Phase 3) - Pattern recognition (Phase 2) - Admin
 * dashboard metrics
 */
@Service
public class LearningActivityLogService {
  ...
  public void logScrapingSession(
      String source, int itemsCollected, long durationMs, boolean success) {
    log.info(
        "[SCRAPING] source={} items={} durationMs={} success={} timestamp={}",
        source,
        itemsCollected,
        durationMs,
        success,
  ...
  public void logProposalEvent(
      String proposalId, String eventType, String details, String adminUser) {
    log.info(
        "[PROPOSAL] id={} event={} details={} admin={} timestamp={}",
        proposalId,
        eventType,
        details,
        adminUser,
  ...
  public void logSolutionEvent(String solutionId, String eventType, String details) {
    log.info(
        "[SOLUTION] id={} event={} details={} timestamp={}",
        solutionId,
        eventType,
        details,
        LocalDateTime.now());
  ...
  public void logQuotaUsage(
      String userId, String operation, int unitsConsumed, int remainingQuota) {
    log.debug(
        "[QUOTA] userId={} operation={} consumed={} remaining={}",
        userId,
        operation,
        unitsConsumed,
        remainingQuota);
  ...
  public void logSanitization(String source, String contentHash, boolean approved, String reason) {
    if (approved) {
      ...
      log.warn(
          "[SANITIZE] source={} hash={} result=REJECTED reason={}", source, contentHash, reason);
    }
  }
