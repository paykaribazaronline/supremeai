...
/**
 * LearningQuotaService - Manages learning resource consumption quotas. Prevents runaway costs and
 * resource exhaustion from aggressive scraping/learning.
 *
 * <p>Supports both global admin limits and per-user limits. Quotas reset daily (midnight UTC) by
 * default.
 *
 * <p>Configuration (application.properties): - learning.quota.global.dailyMax=1000 (max learning
 * ops per day globally) - learning.quota.perUser.dailyMax=50 (max per user per day) -
...
public class LearningQuotaService {
  ...
  public boolean checkQuota(String userId, String operationType, int quotaUnits) {
    ...
    AtomicInteger userCounter =
        userDailyCounters.computeIfAbsent(userKey, k -> new AtomicInteger(0));
    int userCurrent = userCounter.get();
    int userAfter = userCurrent + quotaUnits;

    if (userAfter > perUserDailyMax) {
      log.warn(
          "[QUOTA] DENIED user={} op={} requested={} current={} limit={}",
          userKey,
          operationType,
          quotaUnits,
          userCurrent,
    ...
    if (globalAfter > globalDailyMax) {
      log.warn(
          "[QUOTA] DENIED user={} op={} GLOBAL_LIMIT_REACHED global={}/{} limit={}",
          userKey,
          operationType,
          globalAfter,
          globalDailyMax,
    ...
    if (((double) globalAfter / globalDailyMax) >= emergencyThreshold) {
      log.error(
          "[QUOTA] EMERGENCY: {}% of global quota used. Pausing learning recommended.",
    ...
    log.debug(
        "[QUOTA] APPROVED user={} op={} units={} userTotal={} globalTotal={}",
        userKey,
        operationType,
        quotaUnits,
        userCounter.get(),
        globalDailyCounter.get());
    return true;
  }
  ...
  public boolean canVisitSite(String userId) {
    ensureDailyReset();
    String userKey = userId != null ? userId : "anonymous";
    AtomicInteger counter = userDailyCounters.computeIfAbsent(userKey, k -> new AtomicInteger(0));
  ...
  public Map<String, Object> getQuotaStats() {
    ...
    stats.put(
        "globalPercentUsed",
        Math.round((globalDailyCounter.get() / (double) globalDailyMax) * 100));
  ...
  private void ensureDailyReset() {
    ...
    if (now.toLocalDate().isAfter(lastResetDate.toLocalDate())) {
      ...
      synchronized (this) {
        if (now.toLocalDate().isAfter(lastResetDate.toLocalDate())) {
          log.info(
              "[QUOTA] Daily reset: clearing {} user counters and global counter",
