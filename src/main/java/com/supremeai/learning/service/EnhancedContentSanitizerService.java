...
/**
 * Enhanced Content Sanitizer Service with advanced features: - Comprehensive PII detection and
 * masking - Malicious code detection - Content quality scoring - Security pattern analysis -
 * Machine learning-based pattern detection - Content validation - Audit logging
 */
@Service
public class EnhancedContentSanitizerService {
  ...
  private static final Pattern[] PII_PATTERNS = {
    ...
    Pattern.compile(
        "(api[_-]?key|token|secret|password|private[_-]?key)\\s*[:=]\\s*['\"\\\\]?[a-zA-Z0-9_\\-]{20,}['\"\\\\]?"),
    // JWT tokens
  ...
  public SanitizationResult sanitizeAndValidate(SolutionMemory solution, String source) {
    ...
    if (originalCode.length() < minContentLength) {
      SanitizationResult result = new SanitizationResult(false, "Content too short", 0.0, 0.0);
      resultCache.put(cacheKey, result);
      return result;
    }

    if (originalCode.length() > maxContentLength) {
      SanitizationResult result = new SanitizationResult(false, "Content too long", 0.0, 0.0);
      resultCache.put(cacheKey, result);
      return result;
    }
    ...
    if (containsMaliciousPatterns(sanitizedCode)) {
      SanitizationResult result =
          new SanitizationResult(false, "Malicious code detected", 0.0, 0.0);
      resultCache.put(cacheKey, result);
    ...
    if (immunitySystem != null && immunitySystem.isCodeInfected(sanitizedCode)) {
      SanitizationResult result = new SanitizationResult(false, "Known toxic pattern", 0.0, 0.0);
      resultCache.put(cacheKey, result);
    ...
    if (securityScore < minSecurityScore && !isTrustedSource(source)) {
      SanitizationResult result =
          new SanitizationResult(false, "Low security score", securityScore, qualityScore);
      resultCache.put(cacheKey, result);
    ...
    if (qualityScore < minQualityScore) {
      SanitizationResult result =
          new SanitizationResult(false, "Low quality score", securityScore, qualityScore);
      resultCache.put(cacheKey, result);
    ...
    SanitizationResult result =
        new SanitizationResult(true, "Validation passed", securityScore, qualityScore);
    resultCache.put(cacheKey, result);
    log.info(
        "[Sanitizer] ACCEPTED: Solution from {} passed validation (security: {}, quality: {})",
        source,
        securityScore,
        qualityScore);
    return result;
  ...
  public static class SanitizationResult {
    ...
    public SanitizationResult(
        boolean isValid, String message, double securityScore, double qualityScore) {
