...
/**
 * Enhanced Web Scraper Service with comprehensive features: - Rate limiting per domain - Content
 * caching with TTL - Deduplication - Technology detection - Source authority tracking - Integration
 * with sanitization - Quality scoring
 */
@Service
public class EnhancedWebScraperService {
  ...
  @Autowired private com.supremeai.service.ConfigService configService;
  ...
  private static final Pattern[] TECH_PATTERNS = {
    Pattern.compile(
        "(?i)\\b(java|kotlin|python|javascript|typescript|go|rust|c\\+\\+|c#|php|ruby|swift)\\b"),
  ...
  public ScrapedContent scrapeUrl(String url) {
    ...
    try {
      ...
      log.info(
          "Successfully scraped URL: {} (authority: {}, quality: {}, technologies: {})",
          url,
          authority,
          qualityScore,
          technologies);
  ...
  public SolutionMemory convertToSolution(ScrapedContent scraped, String errorSignature) {
    ...
    if (sanitizer.sanitizeAndValidate(candidate, scraped.getDomain()).isValid()) {
      ...
      log.warn("[Sanitizer] Dropped solution from {} (failed validation)", scraped.getDomain());
      return null;
    }
  }
  ...
  private boolean checkRateLimit(String url) {
    String domain = extractDomain(url);
    Semaphore limiter =
        domainLimiters.computeIfAbsent(domain, k -> new Semaphore(getRateLimitRequests()));

    Queue<Long> timestamps = requestTimestamps.computeIfAbsent(domain, k -> new LinkedList<>());
  ...
  private String extractDomain(String url) {
    ...
    try {
      String domain =
          url.replaceAll("^https?://", "").replaceAll("/.*$", "").replaceAll("^www\\.", "");
  ...
  private String extractMainContent(Document doc) {
    ...
    Elements contentElements =
        doc.select("article, main, .content, .post-content, .article-body, .markdown-body");
  ...
  private double calculateQualityScore(
      String content, String codeSnippets, Set<String> technologies) {
