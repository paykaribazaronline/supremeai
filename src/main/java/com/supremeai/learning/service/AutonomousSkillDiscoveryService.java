...
/**
 * Autonomous Skill Discovery Service Automatically finds resourceful websites (like skills.sh or AI
 * agent registries), scrapes their skill instructions, and injects them into SupremeAI's
...
public class AutonomousSkillDiscoveryService {
  ...
  @Autowired private EnhancedWebScraperService scraperService;
  ...
  /** Scheduled job to autonomously discover and learn new skills. Runs every 12 hours. */
  ...
  public void autonomousSkillDiscovery() {
    ...
    List<String> dynamicRegistries =
        Arrays.stream(registriesStr.split(","))
            .map(String::trim)
            .filter(s -> !s.isEmpty())
            .toList();
  ...
  private void proposeSkillToMarketplace(EnhancedWebScraperService.ScrapedContent content) {
    if (mcpMarketplaceService == null) {
      log.warn(
          "[Skill Discovery] MCPMarketplaceService unavailable; skipping marketplace proposal.");
      return;
    }
    ...
    mcpMarketplaceService.proposeDiscoveredSkill(
        content.getUrl(), title, description, triggers, steps, metadata);
