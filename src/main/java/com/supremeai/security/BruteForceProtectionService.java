...
/**
 * Service to prevent brute force login attacks. Tracks failed login attempts and locks accounts
...
public class BruteForceProtectionService {
  ...
  /**
   * Default max failed attempts before lockout. Can be overridden via bruteforce.max-attempts
   * property.
   */
  ...
  public boolean isLocked(String identifier) {
    ...
    if (record.isLocked()) {
      log.warn(
          "Account/IP {} is locked until {}", identifier, Instant.ofEpochMilli(record.lockUntil));
      return true;
    }
  ...
  public void recordFailedAttempt(String identifier) {
    ...
    if (record.count >= maxAttempts) {
      ...
      log.warn(
          "Account/IP {} locked for {} minutes after {} failed attempts",
          identifier,
          lockDurationMinutes,
          record.count);
    }
