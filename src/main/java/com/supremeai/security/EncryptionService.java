...
/**
 * AES-256-GCM encryption service for encrypting sensitive data at rest. Uses authenticated
 * encryption (GCM) for confidentiality and integrity.
 *
 * <p>Environment Variables: - API_ENCRYPTION_KEY: Base64-encoded 32-byte (256-bit) key Generate
...
public class EncryptionService {
  ...
  private SecretKey getSecretKey() {
    ...
    if (base64Key == null || base64Key.isBlank()) {
      log.warn(
          "API_ENCRYPTION_KEY not set! Using temporary in-memory key. Set the environment variable for production.");
    ...
    try {
      ...
      if (decodedKey.length != 32) {
        throw new IllegalArgumentException(
            "API_ENCRYPTION_KEY must be 32 bytes (256 bits) when decoded, got "
                + decodedKey.length
                + " bytes");
      }
  ...
  /** Encrypts plaintext using AES-256-GCM. Output format: base64(iv || ciphertext) */
  ...
  /**
   * Generates a base64-encoded 256-bit key for use as API_ENCRYPTION_KEY. Call this method, print
   * the result, then set it as environment variable.
   */
