...
public class JwtAuthFilter extends OncePerRequestFilter {
  ...
  protected boolean shouldNotFilter(HttpServletRequest request) throws ServletException {
    ...
    // This keeps static UI pages (e.g. /, /login.html) publicly reachable even with invalid
    // headers.
  ...
  protected void doFilterInternal(
    ...
    // Skip JWT validation if another filter (e.g., AuthenticationFilter) already authenticated the
    // user
    ...
    // The AuthenticationFilter will handle guest access, or the request will be rejected by
    // authorization rules
    if (token == null) {
    ...
    try {
      ...
      response
          .getWriter()
          .write("{\"error\":\"Token expired\",\"message\":\"Please login again\"}");
      return;
      ...
      response
          .getWriter()
          .write("{\"error\":\"Invalid token\",\"message\":\"Authentication token is invalid\"}");
      return;
    } catch (Exception e) {
