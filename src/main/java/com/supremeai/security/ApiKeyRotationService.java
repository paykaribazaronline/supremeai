...
public class ApiKeyRotationService {
  ...
  @Autowired private com.supremeai.service.ProviderTypeRegistry providerTypeRegistry;
  ...
  public Map<String, Object> testApiKey(UserApiKey key) {
    ...
    try {
      com.supremeai.model.ProviderTypeConfig typeConfig =
          providerTypeRegistry.getTypeConfig(providerLower);
      if (typeConfig != null && typeConfig.getExtraConfig() != null) {
      ...
      log.debug(
          "Could not load test config from Firestore for {}: {}", providerLower, e.getMessage());
    }
      ...
      try (Response response = httpClient.newCall(requestBuilder.build()).execute()) {
        ...
        result.put(
            ...
            valid
                ...
                : "API key test failed - status " + response.code());
      }
    } catch (Exception e) {
        ...
        .flatMap(
            key -> {
              ...
              if ((key.getAddedAt() != null && key.getAddedAt().isBefore(maxAgeDate))
                ...
                log.warn(
                    "Key {} for provider '{}' (user {}) is due for rotation. Marked rotation_due.",
                    key.getMaskedKey(),
                    key.getProvider(),
                    key.getUserId());
    ...
    return userApiKeyRepository
        ...
        .flatMap(
            allKeys -> {
              ...
              List<UserApiKey> rotationDue =
                  allKeys.stream()
                      .filter(k -> "rotation_due".equals(k.getStatus()))
                      .collect(Collectors.toList());
              ...
              return Flux.fromIterable(activeKeys)
                  ...
                  .then(
                      Mono.defer(
                          () -> {
                            String reportId =
                                "report_" + LocalDateTime.now().toString().replace(":", "-");
    ...
    return userApiKeyRepository
        ...
        .map(
            keys -> {
              ...
              long rotationDue =
                  keys.stream().filter(k -> "rotation_due".equals(k.getStatus())).count();
