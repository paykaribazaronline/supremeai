...
/** Plugin System - Plan 24 Week 13-14 Ruflo-style plugin architecture */
@Component
  ...
  public Plugin installPlugin(String pluginId) {
    try {
      ...
      Map<String, Object> metadata =
          restTemplate.getForObject("https://registry.npmjs.org/" + pluginId, Map.class);

      Plugin plugin = new Plugin();
      plugin.id = pluginId;
      plugin.name =
          metadata != null && metadata.containsKey("name")
              ? (String) metadata.get("name")
              : pluginId;
      plugin.description =
          metadata != null && metadata.containsKey("description")
