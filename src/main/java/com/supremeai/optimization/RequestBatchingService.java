 ...
 *
 * <p>Features: - Groups requests by (provider, model) with a short time window - Deduplicates
 * identical prompts within the batch window - Rate-limits outgoing batches per provider -
...
public class RequestBatchingService {
  ...
  public void init() {
    scheduler.scheduleAtFixedRate(
        this::flushAllBatches, BATCH_WINDOW_MS, BATCH_WINDOW_MS, TimeUnit.MILLISECONDS);
  }
  ...
  /**
   * Submit a single request and await the result. The request may be batched with others to the
   * same provider/model.
   *
  ...
  public <T> CompletableFuture<T> submit(
      String provider, String model, String prompt, Function<String, T> executor) {
    String batchKey = provider + "::" + model;
    BatchItem<T> item = new BatchItem<>(prompt, executor);

    PendingBatch<T> batch =
        (PendingBatch<T>)
  ...
  private <T> void executeBatch(String key, PendingBatch<T> batch) {
    ...
    Semaphore semaphore =
        providerConcurrency.computeIfAbsent(provider, p -> new Semaphore(MAX_CONCURRENT_BATCHES));
  ...
  private static class PendingBatch<T> {
    ...
    synchronized boolean isReadyToFlush() {
      return !items.isEmpty()
          && (Duration.between(createdAt, Instant.now()).toMillis() >= BATCH_WINDOW_MS
