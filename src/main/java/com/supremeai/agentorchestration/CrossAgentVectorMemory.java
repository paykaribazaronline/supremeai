 ...
 * Shared context store between Code Agent, Security Agent, and Deploy Agent.
 * Eliminates redundant context rebuilding across multi-agent workflows.
 */
@Service
public class CrossAgentVectorMemory {
    ...
    public void storeContext(String sessionId, String sourceAgent, String contextData, String taskType) {
        memoryStore.computeIfAbsent(sessionId, k -> Collections.synchronizedList(new ArrayList<>()))
    ...
    public String retrieveRelevantContext(String sessionId, String targetAgent, String currentTask) {
        ...
        for (MemoryEntry entry : entries) {
            ...
            if (!entry.sourceAgent.equals(targetAgent)) {
                sharedContext.append("From ").append(entry.sourceAgent)
                        .append(" (Task: ").append(entry.taskType).append("):\n")
                        .append(entry.contextData).append("\n\n");
            }
        }
