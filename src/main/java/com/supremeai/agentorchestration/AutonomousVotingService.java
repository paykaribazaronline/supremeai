...
public class AutonomousVotingService {
  ...
  @Autowired
  public AutonomousVotingService(
      @Qualifier("votingTaskExecutor") java.util.concurrent.Executor votingTaskExecutor) {
    this.executor = votingTaskExecutor;
  }

  /**
   * Resolve active providers at call time from the database — no hardcoded defaults. Falls back to
   * Firestore/DB query if no config property is set.
   */
  private List<String> resolveActiveProviders() {
    try {
      List<com.supremeai.model.APIProvider> active =
          providerRepository
              .findByStatus("active")
              .collectList()
    ...
    logger.warn(
        "[DynamicProviders] No active providers found in DB — solo mode active, returning empty list");
    return List.of();
  }
  ...
  public VotingDecision conductVote(String question, String context) {
    ...
    for (String providerName : providerList) {
      CompletableFuture<ProviderVote> future =
          CompletableFuture.supplyAsync(
              () -> {
                try {
                  ...
                  String response =
                      provider
                          .generate(buildPrompt(question, context))
                          .subscribeOn(reactor.core.scheduler.Schedulers.boundedElastic())
                          .block();
  ...
  private VotingDecision calculateConsensus(String question, List<ProviderVote> votes) {
    ...
    logger.info(
        "Voting complete. Consensus: {} with {} confidence",
