...
public class AIProviderFactory {
  ...
  private final com.github.benmanes.caffeine.cache.LoadingCache<String, Boolean>
      providerHealthCache =
          Caffeine.newBuilder()
  ...
  @Autowired private ProviderMetadataService providerMetadataService;
  ...
  @Autowired private com.supremeai.agent.AgentRuleService ruleService;

  @Autowired @Lazy private ContextualAIRankingService contextualRankingService;
  ...
  public AIProvider getProvider(String name, String overrideApiKey) {
    ...
    logger.info(
        "[Factory] getProvider called: name={}, normalizedName={}, overrideApiKeyLength={}",
        name,
        normalizedName,
        overrideApiKey != null ? overrideApiKey.length() : "null");

    if (isLocalFirstExcluded(normalizedName)) {
      logger.info(
          "[Factory] Provider '{}' is excluded under local-first policy; routing to stub/local fallback.",
          normalizedName);
      return stubLocalProvider;
    }

    APIProvider metadata =
        providerMetadataService != null
            ? providerMetadataService.getMetadata(normalizedName)
            : null;

    if (metadata == null) {
      logger.info(
          "[Factory] Provider '{}' not found in cache. Attempting DB query fallback...",
          normalizedName);
      try {
                  ...
                  .switchIfEmpty(
                      providerRepository
                          ...
                          .filter(
                              p -> {
                                String pName =
                                    p.getName() != null ? p.getName().toLowerCase().trim() : "";
                                String pId =
                                    p.getId() != null ? p.getId().toLowerCase().trim() : "";
                                String pDocId =
                                    p.getDocumentId() != null
                                        ? p.getDocumentId().toLowerCase().trim()
                                        : "";
                                return pName.equals(normalizedName)
                                    || pId.equals(normalizedName)
                                    || pDocId.equals(normalizedName);
                              })
                          .next())
                  .timeout(Duration.ofSeconds(5));
        }
        APIProvider dbProvider =
            lookupChain
                .subscribeOn(reactor.core.scheduler.Schedulers.boundedElastic())
        ...
        logger.error(
            "[Factory] DB query fallback failed for '{}': {}", normalizedName, e.getMessage(), e);
      }
    }

    logger.info(
        "[Factory] Metadata found for '{}': {}",
        normalizedName,
        metadata != null ? "YES (baseUrl=" + metadata.getBaseUrl() + ")" : "NO");
    ...
    if (metadata != null && metadata.getBaseUrl() != null && !metadata.getBaseUrl().isBlank()) {
      ...
      logger.info(
          "[Cloud-Only] Resolving provider '{}' from Firestore: baseUrl={}, model={}",
          normalizedName,
          metadata.getBaseUrl(),
          defaultModel);
      SupremeCloudProvider provider =
          new SupremeCloudProvider(key, normalizedName, defaultModel, metadata.getBaseUrl());
      injectMetadataService(provider);
      return provider;
    }

    ProviderTypeConfig typeConfig =
        providerTypeRegistry != null ? providerTypeRegistry.getTypeConfig(normalizedName) : null;
    logger.info(
        "[Factory] TypeConfig found for '{}': {}",
        normalizedName,
        typeConfig != null ? "YES (baseUrl=" + typeConfig.getDefaultBaseUrl() + ")" : "NO");
    if (typeConfig != null
        && typeConfig.getDefaultBaseUrl() != null
        && !typeConfig.getDefaultBaseUrl().isBlank()) {
      String key = resolveKey(overrideApiKey, null, normalizedName);
      String defaultModel = typeConfig.getDefaultModel();
      if (defaultModel == null || defaultModel.isBlank()) {
        throw new IllegalStateException(
            "No default model configured for provider type '"
                + normalizedName
                + "'. "
                + "Register a defaultModel in provider_types.");
      }
      logger.info(
          "[Cloud-Only] Resolving provider '{}' from provider_types: baseUrl={}, model={}",
          normalizedName,
          typeConfig.getDefaultBaseUrl(),
          defaultModel);
      SupremeCloudProvider provider =
          new SupremeCloudProvider(
              key, normalizedName, defaultModel, typeConfig.getDefaultBaseUrl());
      injectMetadataService(provider);
      return provider;
    }

    logger.warn(
        "[Factory] Provider '{}' not found in Firestore api_providers or provider_types. Using dynamic default.",
        normalizedName);
    try {
      AIProvider defaultProvider = getDefaultProvider();
      if (defaultProvider == null) {
        throw new IllegalArgumentException(
            "Unknown AI provider: " + name + " and no healthy default available.");
      }
      ...
      logger.error(
          "[Factory] getDefaultProvider failed for '{}': {}", normalizedName, e.getMessage(), e);
      throw new IllegalArgumentException(
          "Unknown AI provider: " + name + " and no healthy default available.", e);
    }
  }
  ...
  private String resolveKey(String overrideApiKey, String metadataApiKey, String providerName) {
    ...
    String serviceKey =
        aiProviderService != null ? aiProviderService.getActiveKey(providerName) : null;
  ...
  private String resolveModel(APIProvider metadata) {
    ...
    ProviderTypeConfig typeConfig =
        providerTypeRegistry != null
            ? providerTypeRegistry.getTypeConfig(metadata.getType())
            : null;
    if (typeConfig != null
        && typeConfig.getDefaultModel() != null
        && !typeConfig.getDefaultModel().isBlank()) {
    ...
    throw new IllegalStateException(
        "No model configured for provider '"
            ...
            + "or register a defaultModel in provider_types for type='"
            + metadata.getType()
            + "'.");
  }
  ...
  public AIProvider getEnforcedProvider(String name) {
    ...
    if (isLocalFirstExcluded(normalizedName)) {
      logger.info(
          "[Factory] getEnforcedProvider routed excluded provider '{}' to stub/local fallback.",
          normalizedName);
      return stubLocalProvider;
    }
  ...
  private AIProvider selectProviderByRankingOrFallback(String taskType, List<String> candidates) {
    try {
      ContextualAIRankingService.TaskType rankingTaskType =
      ...
      List<ContextualAIRankingService.ProviderRanking> rankings =
          contextualRankingService.getRankingsForTask(rankingTaskType);
  ...
  public AIProvider getDefaultProvider() {
    logger.info(
        "Dynamically searching for default provider from metadata cache (local-first mode)");
    ...
    if (activeProviders.isEmpty()) {
      logger.info(
          "[LOCAL-FIRST] No external AI providers configured. Using StubLocalProvider for offline operation.");
      return stubLocalProvider;
    }
  ...
  private boolean isLocalFirstExcluded(String normalizedName) {
    ...
    return n.equals("groq") || n.equals("openai") || n.equals("anthropic") || n.equals("deepseek");
  }
  ...
  public Flux<String> getActiveHelperProviderIds() {
    return providerRepository.findByStatus("active").map(APIProvider::getId);
  }
  ...
  public AIProvider createProviderFromConfig(APIProvider config) {
    ...
    if (baseUrl != null && !baseUrl.isEmpty()) {
      ...
      return new RuleEnforcingAIProvider(
          new SupremeCloudProvider(apiKey, name, defaultModel, baseUrl), ruleService);
    }
