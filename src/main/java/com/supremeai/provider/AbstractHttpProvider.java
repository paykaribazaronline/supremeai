...
/** Base HTTP provider implementation Eliminates 90% duplicate code across all REST API providers */
public abstract class AbstractHttpProvider implements AIProvider {
   ...
   *
   * <p>Features: - Connection pooling (max 100 connections) - HTTP/2 support for multiplexed
   * requests - Connection keep-alive - Retry on connection failure
   */
  protected static final OkHttpClient sharedHttpClient =
  ...
  /**
   * Sets the provider metadata service after construction. Used by AIProviderFactory instead of
   * reflection-based field injection. Spring @Autowired on the class field still works for
   * Spring-managed subclasses.
   */
  ...
  @Autowired protected ProviderMetadataService providerMetadataService;
  ...
  protected AbstractHttpProvider(
      String apiKey, String baseUrl, String defaultModel, ObjectMapper objectMapper) {
    this.apiKey = apiKey;
  ...
  /**
   * Each provider can override for custom response extraction. Default implementation handles
   * OpenAI-compatible format (choices[0].message.content). Providers like Gemini or Anthropic that
   * use different formats should override this.
   */
  ...
  /**
   * কমন রেসপন্স এক্সট্রাক্টর — OpenAI-compatible API ফরম্যাটের জন্য (OpenAI, DeepSeek, Groq,
   * Mistral, HuggingFace ইত্যাদি)
   *
   * <p>রেসপন্স ফরম্যাট: { "choices": [{ "message": { "content": "..." } }] }
   */
  protected final String extractOpenAICompatibleResponse(String responseBody, String providerName)
      throws Exception {
  ...
  public Map<String, Object> getCapabilities() {
    if (providerMetadataService != null) {
      ...
      if (meta != null) {
        ...
        String defaultModelFromMeta =
            (models != null && !models.isEmpty()) ? models.get(0) : defaultModel;
        return Map.of(
