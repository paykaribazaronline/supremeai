...
public class SupremeCoreProvider implements AIProvider {
    ...
    public SupremeCoreProvider(ProviderRepository providerRepository,
    ...
    public Mono<String> generate(String prompt) {
        logger.info("[SUPREME-CORE] Processing prompt via cloud orchestration pipeline: {}", truncate(prompt));

        if (providerRepository != null && providerFactory != null) {
            return generateFromCloud(prompt)
                    .switchIfEmpty(Mono.just("[SupremeAI Core] সকল cloud provider বর্তমানে অনুপলব্ধ। Admin প্যানেল থেকে provider চেক করুন।"));
        }

        return Mono.just("[SupremeAI Core] Cloud provider repository not configured. Please set up providers in Firestore.");
    }

    private Mono<String> generateFromCloud(String prompt) {
        return Flux.defer(() -> providerRepository.findAll())
                .onErrorResume(e -> {
                    logger.error("[SUPREME-CORE] Failed to load providers from Firestore: {}", e.getMessage());
                    return Flux.empty();
                })
                ...
                .flatMap(config -> {
                    ...
                    logger.info("[SUPREME-CORE] Delegating to cloud provider: {} (priority={})",
                            config.getName(), config.getPriority());
                    return cloudProvider.generate(prompt)
                            .onErrorResume(e -> {
                                logger.warn("[SUPREME-CORE] Cloud provider '{}' failed: {}. Trying next.",
