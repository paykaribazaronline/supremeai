...
/**
 * Generic SupremeCloudProvider for all GCP Cloud Run, HF Inference Endpoints, and Render
 * deployments. Supports both OpenAI-compatible and HF Inference API formats.
 */
public class SupremeCloudProvider extends AbstractHttpProvider {
  ...
  public SupremeCloudProvider(
      String apiKey, String providerName, String defaultModel, String baseUrl) {
    super(apiKey, baseUrl, defaultModel);
    this.providerName = providerName;
    this.isHfInference =
        providerName.startsWith("hf_") && baseUrl.contains("api-inference.huggingface.co");
  }
  ...
  public Map<String, Object> getCapabilities() {
    return Map.of(
        ...
        getModel(),
        "type",
        isHfInference ? "huggingface-inference" : "cloud-native");
  }
  ...
  protected Map<String, Object> createRequestBody(String prompt) {
    if (isHfInference) {
      return Map.of(
          ...
          prompt,
          "parameters",
          Map.of("max_new_tokens", 512, "temperature", 0.7, "return_full_text", false));
    }
    return Map.of(
        "model",
        getModel(),
