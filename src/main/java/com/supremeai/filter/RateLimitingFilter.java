...
public class RateLimitingFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(
    ...
    if (currentCount <= DEFAULT_RATE_LIMIT_PER_MINUTE) {
      ...
      response.setHeader(
          "X-RateLimit-Remaining", String.valueOf(DEFAULT_RATE_LIMIT_PER_MINUTE - currentCount));
      response.setHeader(
          "X-RateLimit-Reset",
          String.valueOf(
              Duration.between(Instant.now(), info.windowStart.plus(Duration.ofMinutes(1)))
                  .getSeconds()));
  ...
  /**
   * Get rate limiting key for the request. Uses API key if present, otherwise falls back to IP +
   * URI.
   */
