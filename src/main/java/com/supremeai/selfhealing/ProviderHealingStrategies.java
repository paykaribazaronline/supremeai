...
/**
 * প্রোভাইডার হিলিং স্ট্র্যাটেজি যা বিভিন্ন প্রোভাইডারের জন্য স্বয়ংক্রিয় হিলিং সমর্থন করে। এটি
 * প্রোভাইডার সুইচিং, API কী রোটেশন, এবং কনফিগারেশন পুনরুদ্ধার সমর্থন করে।
 */
@Component
public class ProviderHealingStrategies {
  ...
  @Autowired private AutoHealingStrategyService autoHealingService;

  @Autowired private UserApiKeyRepository apiKeyRepository;

  @Autowired private HealingEventRepository healingEventRepository;
  ...
  public AutoHealingStrategyService.HealingStrategy createProviderSwitchingStrategy(
    ...
    return (Exception error) -> {
      logger.info(
          "Attempting to switch from {} to {} due to error: {}",
          currentProvider,
          alternativeProvider,
          error.getMessage());
  ...
  public AutoHealingStrategyService.HealingStrategy createApiKeyRotationStrategy(
    ...
    return (Exception error) -> {
      logger.info(
          "Attempting to rotate API key for provider: {} for user: {} due to error: {}",
          providerName,
          userId,
          error.getMessage());
      ...
      try {
        ...
        return apiKeyRepository
            .findByUserIdAndProvider(userId, providerName)
            ...
            .flatMap(
                keys -> {
                  ...
                  UserApiKey nextKey =
                      keys.get(0); // In real logic, we'd pick one not currently in use

                  logger.info(
                      "Successfully identified new key for rotation: {}", nextKey.getLabel());
  ...
  public AutoHealingStrategyService.HealingStrategy createConfigRecoveryStrategy(
    ...
    return (Exception error) -> {
      logger.info(
          "Attempting to recover configuration for provider: {} due to error: {}",
          providerName,
          error.getMessage());
      ...
      try {
        ...
        logger.error(
            "Failed to recover configuration for provider {}: {}", providerName, e.getMessage());
        return false;
      }
    };
  }
  ...
  public void registerAllStrategies() {
    ...
    java.util.List<com.supremeai.model.APIProvider> activeProviders =
        providerRepository
            .findAll()
    ...
    for (com.supremeai.model.APIProvider p : activeProviders) {
      ...
      com.supremeai.model.ProviderTypeConfig typeConfig =
          providerTypeRegistry.getTypeConfig(providerType);
      if (typeConfig != null) {
      ...
      autoHealingService.registerHealingStrategy(
          providerType + "_auth_error", createApiKeyRotationStrategy(providerType, ""));
      registered++;
    }
    logger.info(
        "Registered {} healing strategies for {} active providers",
        registered,
  ...
  private String providerFallbackFromConfig(com.supremeai.model.ProviderTypeConfig typeConfig) {
    ...
    // Supports key "alternativeProvider" (string fallback) or "healingMitigation" (null → no
    // switch)
