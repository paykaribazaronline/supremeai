...
public class AIFallbackOrchestrator {
  ...
  /**
   * {@code true} when zero active AI providers are configured in Firestore. The system is running
   * in solo mode — all requests fall through to core_knowledge.json / autonomous_seed or the
   * airllm-sidecar local model.
   */
  private volatile boolean soloMode = false;

  public AIFallbackOrchestrator(
        ...
        .doOnComplete(
            () -> {
              if (activeCount.get() == 0) {
                soloMode = true;
                log.warn(
                    "[SOLO MODE] No active AI providers found in Firestore at startup. "
                ...
                log.info(
                    "[DYNAMIC PROVIDERS] {} active providers registered at startup",
                    activeCount.get());
              }
            })
        .subscribe();
  ...
  private void conditionallyCommitAndPush() {
    try {
      // Only push when we are on main branch, have uncommitted changes, and a dry‑run reports
      // pending pushes
      if (GitHelper.isOnMainBranch() && GitHelper.hasChanges() && GitHelper.dryPushHasPending()) {
        ...
        log.info(
            "[GitHelper] No conditional push needed (branch={}, changes={}, dryRunPending={}).",
            GitHelper.isOnMainBranch(),
            GitHelper.hasChanges(),
            GitHelper.dryPushHasPending());
      }
    } catch (Exception e) {
        ...
        .subscribe(
            count -> {
              ...
              if (newSoloMode != soloMode) {
                ...
                log.info(
                    "[SoloMode] Status changed dynamically to: {}",
                    soloMode ? "SOLO" : "CONNECTED");
              }
            });
  }
            ...
            knowledgeBase
                ...
                .switchIfEmpty(
                    Mono.defer(
                        () -> {
                          log.info(
                              "[BRAIN TIER 3] No solution in Firestore. Attempting web scraping via browser...");
                ...
                .switchIfEmpty(
                    Mono.defer(
                        () -> {
                          log.info(
                              "[BRAIN TIER 4] No web scrape result. Trying Helper AI providers...");
  ...
  private Mono<String> tryHelperAIProviders(
      String taskCategory, String errorSignature, String prompt, String userId) {
    ...
    return providerRepository
        ...
        .filter(
            p ->
                !fallbackProviderName.equalsIgnoreCase(p.getName())
                    && !fallbackProviderName.equalsIgnoreCase(p.getType()))
        .collectList()
        .flatMap(
            activeCloudProviders -> {
              // Get rankings
              List<com.supremeai.service.AIRankingService.ProviderRanking> rankings =
                  aiRankingService.getRankings();
              // Map provider name to ranking value for quick lookup
              Map<String, Double> rankMap =
                  rankings.stream()
                      .collect(
                          java.util.stream.Collectors.toMap(
                              r -> r.getProvider(), r -> r.getValueScore()));
              // Sort providers by ranking (higher score first)
              activeCloudProviders.sort(
                  (a, b) -> {
              ...
              if (activeCloudProviders.isEmpty()) {
                log.info(
                    "[Solo-First] No active cloud AI models configured. Relying purely on local Solo Mode.");
                return tryPrivateCloudFailover(taskCategory, prompt);
              }

              log.info(
                  "[Solo-First] Active cloud models found. Checking connection and taking help opportunistically...");
              ...
              if (!isServiceQuotaAvailable(serviceName)
                ...
                log.info(
                    "[Solo-First] Cloud provider {} quota exhausted or circuit open. Bypassing cloud assist.",
                    cloudProvider.getName());
                return tryPrivateCloudFailover(taskCategory, prompt);
              }
              ...
              return generateForProvider(
                  ...
                  .onErrorResume(
                      err -> {
                        log.warn(
                            "[Solo-First] Cloud connection not available or timeout: {}. Continuing purely with Solo Mode.",
                            err.getMessage());
                        return tryPrivateCloudFailover(taskCategory, prompt);
                      });
            });
              ...
              return coReasonWithLocalModel(taskCategory, prompt, generatedCode)
                  .flatMap(
                      optimizedCode -> {
                        ...
                        if (infected) {
                          log.error(
                              "-> [Orchestrator] Optimized code is toxic! Falling back to original...");
        ...
        .flatMap(
            keywords -> {
              ...
              return activeInternetScraper
                  .scrapeKnowledge(taskCategory, List.of(keywords.split("\\s+")))
                  .next()
                  .map(
                      issue -> {
                        log.info(
                            "[BRAIN TIER 3] Web scraping found result from: {}", issue.getSource());
  ...
  /**
   * Final failover to private cluster-local model (Solo Mode). The airllm-sidecar entry must be
   * configured as a regular provider in Firestore api_providers. No endpoint, type, or model is
   * hardcoded here.
   */
  private Mono<String> tryPrivateCloudFailover(String taskCategory, String prompt) {
    ...
    Mono<com.supremeai.model.APIProvider> lookupById =
        providerRepository
            .findById(fallbackProviderName)
            .switchIfEmpty(providerRepository.findById(fallbackProviderName.toLowerCase()))
            .onErrorResume(
                e -> {
                  log.warn(
                      "[SoloMode] Fallback provider '{}' lookup by id/lowercase failed: {}",
                      fallbackProviderName,
                      e.getMessage());
                  return Mono.empty();
                });

    Mono<com.supremeai.model.APIProvider> lookupByName =
        providerRepository
            .findAll()
    ...
    return lookupById
        ...
        .flatMap(
            airllmConfig -> {
              log.info(
                  "[SoloMode] Using fallback provider: baseUrl={}, type={}",
                  airllmConfig.getBaseUrl(),
                  airllmConfig.getType());
              AIProvider provider = providerFactory.createProviderFromConfig(airllmConfig);
              if (provider != null) {
                return provider.generate(prompt);
              }
              log.info(
                  "[SoloMode] Failed to create provider from config. Using StubLocalProvider.");
        ...
        .switchIfEmpty(
            Mono.defer(
                () -> {
                  log.info(
                      "[SoloMode] No local sidecar found in DB. Using StubLocalProvider for fully offline operation.");
                  return stubLocalProvider.generate(prompt);
                }))
  ...
  /**
   * Live Co-Reasoning Layer: The local AirLLM sidecar acts as an active quality auditor, code
   * optimiser, and local reasoning co-pilot. It runs alongside active cloud providers to polish and
  ...
  private Mono<String> coReasonWithLocalModel(
    ...
    log.info(
        "[Live Co-Reasoning] Invoking local GGUF model to audit and optimize cloud AI response...");
    ...
    return providerRepository
        ...
        .switchIfEmpty(
            Mono.fromCallable(
                () -> {
                  com.supremeai.model.APIProvider defaultConfig =
                      new com.supremeai.model.APIProvider();
                  defaultConfig.setName(fallbackProviderName);
                  defaultConfig.setBaseUrl("http://localhost:8081");
                  defaultConfig.setType("airllm-sidecar");
                  defaultConfig.setStatus("active");
        ...
        .timeout(
            Duration.ofSeconds(15)) // Strict 15s timeout to avoid dragging cloud response times
        .onErrorResume(
            e -> {
              log.warn(
                  "[Live Co-Reasoning] Local co-reasoning timed out or failed; returning original cloud response: {}",
                  e.getMessage());
              return Mono.just(originalOutput);
            })
        .map(
            optimized -> {
              ...
              log.info(
                  "[Live Co-Reasoning] Cloud response successfully optimized and polished by local model!");
              return optimized;
            });
  }

  /**
   * Returns {@code true} when the system is operating in solo mode — zero active AI providers are
   * configured in Firestore. All requests will fall through to the local knowledge base or the
   * airllm-sidecar local model.
   */
  ...
  private void recordIntelligenceGap(String taskCategory, String prompt, String failReason) {
    log.warn(
        "[Intelligence Gap] Recorded for future training. Task Category: {}, Reason: {}",
        taskCategory,
        failReason);
