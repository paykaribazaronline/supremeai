...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _save() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      ...
      content: Text(ok ? 'Settings saved'.tr() : 'Save failed'.tr(),
          ...
          return ListView(
            ...
            children: [
              _buildSection('AI Models'.tr(), [
                _buildTextField(
                    _modelController, 'Primary model', 'gemini-1.5-pro'),
                const SizedBox(height: 16),
                _buildTextField(
                    _smallModelController, 'Small model', 'gemini-1.5-flash'),
              ]),
              ...
              _buildSection('App Settings'.tr(), [
                _buildDropdown(
                    ...
                    (v) => setState(() => _shareMode = v!)),
                const SizedBox(height: 16),
                _buildSwitchTile(
                    'Full authority mode',
                    'Allow all core tools without confirmation',
                    _fullAuthority,
                    (v) => setState(() => _fullAuthority = v)),
                _buildSwitchTile(
                    'External directory access',
                    'Access directories outside workspace',
                    _externalDirectory,
                    (v) => setState(() => _externalDirectory = v)),
                const SizedBox(height: 16),
                _buildDropdown('Theme', _themeMode, ['system', 'light', 'dark'],
                    (v) => setState(() => _themeMode = v!)),
              ]),
              if (settingsProvider.error != null) ...[
                const SizedBox(height: 16),
                Text(settingsProvider.error!,
                    style:
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: Text(settingsProvider.isLoading
                      ? 'Saving...'.tr()
