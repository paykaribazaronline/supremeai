...
class _VpnScreenState extends State<VpnScreen> {
  ...
  Future<void> _refresh() async {
    ...
    setState(() {
      ...
      _nodes = List<Map<String, dynamic>>.from(
          data['nodes'] ?? data['data']?['nodes'] ?? const []);
      _loading = false;
      _activeNode = _nodes.indexWhere(
                  (n) => n['connected'] == true || n['active'] == true) >=
              0
          ? _nodes.firstWhere(
              (n) => n['connected'] == true || n['active'] == true,
              orElse: () => const {})['name']
  ...
  Future<void> _connect(String name) async {
    ...
    final result =
        await _api.action('vpn', {'action': 'connect', 'node': name});
    if (!mounted) return;
    setState(() {
      _connecting = false;
      _activeNode = name;
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 16),
          Text('${'vpn.secure_gateway'.tr()} (${_status.toUpperCase()})',
              style: const TextStyle(
                  color: Colors.white,
                  fontSize: 18,
  ...
  Widget _buildNodeCard(Map<String, dynamic> node) {
    ...
    final statusText =
        connected ? 'vpn.connected'.tr() : 'vpn.disconnected'.tr();
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(node['latency']?.toString() ?? 'Unknown latency',
                              style: const TextStyle(
  ...
  Widget _buildEmptyNodesTile() {
    return Container(
      ...
      decoration: BoxDecoration(
          ...
          border: Border.all(color: Colors.white10)),
      child: Center(
          child: Text('No VPN nodes configured'.tr(),
