...
export function useSkills(query = "") {
  return useQuery({
    ...
    queryFn: () =>
      postJSON<import("../types").Skill[]>("/api/skills/search", {
        query,
...
export function useCheckpoints() {
  return useQuery({
    ...
    queryFn: () =>
      fetchJSON<import("../types").Checkpoint[]>("/memory/checkpoints"),
  });
}
...
export function useSaveConfig() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (env_vars: Record<string, string>) =>
      postJSON("/admin-api/config", { env_vars }),
...
export function useCloudStats() {
  return useQuery({
    ...
    queryFn: () =>
      fetchJSON<import("../types").CloudStats>("/admin/cloud-distribution"),
    refetchInterval: 30000,
  });
}
...
export function useCIReports(limit = 20) {
  ...
  return useQuery({
    ...
    queryFn: () =>
      fetchJSON<import("../types").CIReport[]>(
        `/admin-api/ci-logs?limit=${limit}`,
      ),
