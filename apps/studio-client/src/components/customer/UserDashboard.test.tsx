...
describe("UserDashboard", () => {
  ...
  it("renders all four tab buttons", () => {
    ...
    expect(
      screen.getAllByRole("button", { name: /Home Feed/i }).length,
    ).toBeGreaterThanOrEqual(1);
    expect(
      screen.getAllByRole("button", { name: /Quick Presets/i }).length,
    ).toBeGreaterThanOrEqual(1);
    expect(
  ...
  it("switches to presets tab when clicked", () => {
    ...
    expect(
      getTabButton(/Quick Presets/i).classList.contains("bg-[#00f3ff]/20"),
    ).toBe(true);
  });
  ...
  it("shows project list on overview", () => {
    render(
      <UserDashboard {...defaultProps} projects={defaultProps.projects} />,
    );
