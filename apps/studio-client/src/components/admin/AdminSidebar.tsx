      ...
      <div className="bg-[#030611]/80 border border-[#00f3ff]/15 rounded-lg p-3 flex flex-col gap-2 text-xs font-mono">
        ...
        <div className="flex justify-between">
          ...
          <span className="text-indigo-400">
            {gcpHealth?.firestore_mode || "Local"}
          </span>
        </div>
        <div className="flex justify-between">
          <span className="text-slate-400">PubSub Queue:</span>
          <span className="text-purple-400">
            {gcpHealth?.pubsub_mode || "Local"}
          </span>
        </div>
      </div>
    </div>
      ...
      <div className="flex flex-col gap-2 max-h-48 overflow-y-auto">
        {skills.length === 0 ? (
          ...
          skills.map((skill) => (
            <div
              key={skill.id}
              className="bg-[#030611]/80 border border-[#00f3ff]/15 rounded p-2.5 text-xs"
            >
      ...
      <div className="flex flex-col gap-2 max-h-40 overflow-y-auto font-mono">
        {checkpoints.length === 0 ? (
          ...
          checkpoints.map((cp) => (
            <div
              key={cp.task_id}
              className="bg-[#030611]/80 border border-[#00f3ff]/15 rounded p-2 flex justify-between items-center text-[11px]"
            >
              <div className="min-w-0">
                <div className="text-slate-200 truncate" title={cp.task_id}>
                  {cp.task_id}
                </div>
