                ...
                >
                  ...
                  <div className="flex gap-1 mt-2">
                    {conv.tags?.map((tag: string) => (
                      <span
                        key={tag}
                        className="px-1 py-0.5 text-[9px] rounded bg-slate-800 text-slate-300 font-mono"
                      >
                        {tag}
                      </span>
                    ))}
            ...
            <Card title={`Session: ${selectedConv.session_id}`}>
              <div className="text-[10px] text-slate-500 mb-3 font-mono flex items-center gap-3">
                <span className="flex items-center gap-1">
                  <Clock size={10} /> {selectedConv.timestamp}
                </span>
                <span className="flex items-center gap-1">
                  <MessageSquare size={10} /> {selectedConv.turns} turns
                </span>
                <span className="flex items-center gap-1">
                  <Tag size={10} /> {selectedConv.tags?.length} tags
                </span>
              </div>
                ...
                {selectedConv.messages?.map((m: any, i: number) => (
                  <div
                    ...
                    className={`p-3 rounded-lg border text-xs font-mono ${
                      m.role === "user"
                        ? "border-[#00f3ff]/30 bg-[#00f3ff]/5 text-white"
                        : "border-slate-800 bg-slate-900/30 text-slate-400"
                    }`}
                  >
                    <div className="text-[9px] text-slate-500 mb-1 uppercase">
                      {m.role}
                    </div>
                    {m.content}
                  </div>
