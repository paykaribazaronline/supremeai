              ...
              <div className="flex flex-col gap-3">
                {ciReports.map((report) => (
                  <div
                    ...
                    onClick={() =>
                      setSelectedRun(
                        selectedRun?.id === report.id ? null : report,
                      )
                    }
                  ...
                  >
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-3 text-[10px] text-slate-500 mt-1 font-mono">
                            <span className="flex items-center gap-1">
                              <GitBranch size={10} /> {report.branch}
                            </span>
                            <span className="flex items-center gap-1">
                              <Clock size={10} />{" "}
                    ...
                    {selectedRun?.id === report.id && (
                      <div className="mt-4 pt-3 border-t border-slate-800 text-[10px] font-mono leading-relaxed text-slate-300">
                        <div className="grid grid-cols-2 gap-2 mb-3 text-slate-400">
                          <div>
                            ...
                            <span className="text-white">
                              {report.commit_sha.substring(0, 7)}
                            </span>
                          </div>
                          <div>
                            Event:{" "}
                            <span className="text-white">
                              {report.event_name.toUpperCase()}
                            </span>
                          </div>
                          <div>
                            Run ID:{" "}
                            <span className="text-white">{report.run_id}</span>
                          </div>
                          <div>
                            Recorded:{" "}
                            <span className="text-white">
                        ...
                        {report.jobs_summary &&
                          Object.keys(report.jobs_summary).length > 0 && (
                            <div className="mb-3">
                              <div className="text-slate-400 mb-1 font-bold">
                                Jobs Summary:
                              </div>
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-1.5">
                                {Object.entries(report.jobs_summary).map(
                                  ([job, info]: [string, any]) => (
                                    <div
                                      key={job}
                                      className="flex justify-between items-center p-1.5 rounded bg-slate-950 border border-slate-900"
                                    >
              ...
              {flags.map((flag) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    >
                      <div
                        className={`w-3 h-3 rounded-full bg-white transition-transform ${flag.enabled ? "translate-x-4" : "translate-x-0.5"}`}
                      />
                    </button>
                  </div>
                  {flag.enabled && (
                  ...
                  <div className="mt-2">
                    <Badge
                      variant={
                        flag.environment === "production" ? "success" : "info"
                      }
                    >
                      {flag.environment}
                    </Badge>
