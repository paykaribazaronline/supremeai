...
export function CostAuditor({ costReport }: CostAuditorProps) {
  const [limit, setLimit] = useState(150.0);
  const spent = 42.67;
  ...
  return (
    <div className="flex-grow bg-[#030611] p-6 overflow-y-auto font-sans">
      ...
      <div className="bg-gradient-to-br from-[#0c0d14] to-[#12131f] border border-slate-900 rounded-xl p-6 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-2xl font-bold text-[#00ff66] mt-1 font-mono">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        {providerCosts.map((prov) => {
          ...
          return (
            <div
              key={prov.name}
              className="bg-[#090a0f] border border-slate-900/60 rounded-xl p-4 flex flex-col gap-3"
            >
              <div className="flex justify-between items-center">
                ...
                <span className="text-[10px] font-bold text-slate-400 font-mono">
                  ${prov.spent.toFixed(2)} / ${prov.quota.toFixed(0)}
                </span>
              </div>
          ...
          <tbody className="divide-y divide-slate-900">
            {recentCharges.map((chg, idx) => (
              <tr key={idx} className="hover:bg-slate-800/10">
                ...
                <td className="p-3 text-right text-[#00ff66] font-bold">
                  ${chg.cost.toFixed(4)}
                </td>
              </tr>
            ))}
      ...
      <details className="mt-4">
        <summary className="text-[10px] text-slate-500 cursor-pointer font-mono select-none uppercase hover:text-slate-400">
          Show raw console output
        </summary>
