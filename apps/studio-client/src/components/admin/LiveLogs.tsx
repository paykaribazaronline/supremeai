...
export function LiveLogs({ liveLogs, setLiveLogs }: LiveLogsProps) {
  ...
  const infoCount = liveLogs.filter((log) =>
    log.toUpperCase().includes("INFO"),
  ).length;
  const warnCount = liveLogs.filter(
    (log) =>
      log.toUpperCase().includes("WARN") ||
      log.toUpperCase().includes("WARNING"),
  ).length;
  const errCount = liveLogs.filter(
    (log) =>
      log.toUpperCase().includes("ERROR") ||
      log.toUpperCase().includes("ERR") ||
      log.toUpperCase().includes("FAIL"),
  ).length;
  ...
  const filteredLogs = liveLogs.filter((log) => {
    ...
    if (filterLevel === "INFO")
      return matchesSearch && log.toUpperCase().includes("INFO");
    if (filterLevel === "WARN")
      return (
        matchesSearch &&
        (log.toUpperCase().includes("WARN") ||
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-4 pb-3 border-b border-[var(--border-color)]">
        <div className="flex flex-col gap-1">
          <span className="text-slate-400 font-bold uppercase tracking-wider text-[10px]">
            Real-time Live Stream (supremeai.log)
          </span>
      ...
      <div className="flex-grow flex flex-col gap-1 overflow-y-auto max-h-[70vh]">
        {filteredLogs.length === 0 ? (
          <div className="text-slate-500 italic">
            Listening for incoming server logs or no matching logs found...
          </div>
        ) : (
