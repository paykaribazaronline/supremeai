...
def _load_workspace_config() -> Dict[str, Any]:
    ...
    if config_path.exists():
        try:
            ...
            workspace_config = config.get("workspace", {}) # Access the nested 'workspace' key
            ...
            # Handle invalid JSON gracefully, return an empty config
            # In a production environment, this should ideally log a warning/error.
            return {}
    return {}
...
def _get_workspace_path(project_type: WorkspaceType) -> Path:
    ...
    full_config = _load_workspace_config()
    config = full_config.get("workspace", {}) # Get the nested workspace configuration
...
async def workspace_get_scoped_path(params: ScopedFilePathInput) -> str:
    ...
    # Normalize path separators for consistent checking across OS (e.g., Windows style on Linux)
    normalized_relative_path = params.relative_path.replace("\\", "/")
    ref_path = Path(normalized_relative_path)

    # Check for absolute paths or '..' components indicative of path traversal attempts
    if ref_path.is_absolute() or normalized_relative_path.startswith('/') or ".." in ref_path.parts:
        return json.dumps(
    ...
    try:
        ...
        if scoped_path.is_symlink():
            # This will raise ValueError if real_target is not under resolved_workspace
            Path(os.readlink(scoped_path)).resolve().relative_to(resolved_workspace)

        # This will raise ValueError if resolved_scoped is not under resolved_workspace
