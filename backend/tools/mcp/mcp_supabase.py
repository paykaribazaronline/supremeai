...
def _get_supabase_db_url() -> str:
    return getattr(settings, "supabase_database_url", "") or getattr(settings, "supabase_database_url", "")
...
async def supabase_execute_sql(params: ExecuteQueryInput) -> str:
    ...
    """
    admin_authorized = getattr(settings, "admin_authorized", "false").lower() == "true" or getattr(settings, "admin_authorized", "false").lower() == "true"
