...
def _collect_files(root_dir: str, changed_files: Optional[Set[str]] = None) -> List[str]:
    files = []
...
def _get_git_changed_files(root_dir: str, since: Optional[str] = None, until: Optional[str] = None) -> Optional[Set[str]]:
...
def _get_git_diff_summary(root_dir: str, since: Optional[str] = None, until: Optional[str] = None) -> str:
...
async def export_codebase_to_markdown(
    ...
    try:
        ...
        # Build headers
        header = f"# 📄 SupremeAI 2.0 Codebase Export\n"
        if clone_url:
            header += f"- **Source URL**: {clone_url}\n"
        else:
            header += f"- **Source**: Local Workspace\n"
