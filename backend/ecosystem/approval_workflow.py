...
class ProposalKind(enum.StrEnum):
    NEW_SOURCE = "NEW_SOURCE"; NEW_CAPABILITY = "NEW_CAPABILITY"
    CAPABILITY_PROMOTION = "CAPABILITY_PROMOTION"; CAPABILITY_ARCHIVE = "CAPABILITY_ARCHIVE"
    DEPLOYMENT = "DEPLOYMENT"; DB_MIGRATION = "DB_MIGRATION"
    SECRET_ROTATION = "SECRET_ROTATION"; HIGH_RISK_ACTION = "HIGH_RISK_ACTION"
...
class ProposalState(enum.StrEnum):
    ...
    EXECUTED = "EXECUTED"; SUPERSEDED = "SUPERSEDED"; EXPIRED = "EXPIRED"


_ALLOWED: dict[ProposalState, set[ProposalState]] = {
    ProposalState.PENDING: {ProposalState.APPROVED, ProposalState.REJECTED, ProposalState.DEFERRED,
                            ProposalState.EXPIRED, ProposalState.SUPERSEDED},
    ProposalState.APPROVED: {ProposalState.EXECUTED, ProposalState.EXPIRED, ProposalState.SUPERSEDED},
    ProposalState.REJECTED: {ProposalState.PENDING, ProposalState.SUPERSEDED},
    ProposalState.DEFERRED: {ProposalState.PENDING, ProposalState.EXPIRED, ProposalState.SUPERSEDED},
...
class ProposalStateError(Exception): pass
class ProposalCooldownError(Exception): pass
...
class ApprovalWorkflow:
    ...
    def _ensure_schema(self) -> None:
        with get_conn() as conn:
            ...
            conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.PROPOSALS_TABLE}_state ON {self.PROPOSALS_TABLE}(state)")
            conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.PROPOSALS_TABLE}_kind ON {self.PROPOSALS_TABLE}(kind)")
            conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.PROPOSALS_TABLE}_dedup ON {self.PROPOSALS_TABLE}(dedup_key)")
            ...
            conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.DECISIONS_TABLE}_prop ON {self.DECISIONS_TABLE}(proposal_id)")
            conn.commit()
    ...
    def propose(self, p: ApprovalProposal) -> ApprovalProposal:
        ...
        with get_conn() as conn:
            r = conn.execute(
                ...
                (p.dedup_key,)).fetchone()
        if r:
            age = (datetime.now(UTC) - datetime.fromisoformat(r["created_at"])).total_seconds()
            if age < _COOLDOWN_SECONDS and r["state"] in {ProposalState.PENDING.value, ProposalState.APPROVED.value}:
        ...
        with get_conn() as conn:
            ...
            conn.execute(
                ...
                (ProposalState.SUPERSEDED, datetime.now(UTC).isoformat(),
                 p.dedup_key, ProposalState.PENDING, p.proposal_id))
            conn.commit()
        return p

    def decide(self, proposal_id: str, decision: ApprovalDecision) -> ApprovalProposal:
        p = self.get(proposal_id)
        if p is None: raise ProposalStateError(f"Proposal {proposal_id} not found")
        if decision.decision not in {ProposalState.APPROVED, ProposalState.REJECTED, ProposalState.DEFERRED}:
    ...
    def mark_executed(self, proposal_id: str) -> ApprovalProposal:
        p = self.get(proposal_id)
        if p is None: raise ProposalStateError(proposal_id)
    ...
    def get(self, pid: str) -> ApprovalProposal | None:
        with get_conn() as conn:
            r = conn.execute(f"SELECT * FROM {self.PROPOSALS_TABLE} WHERE proposal_id=?", (pid,)).fetchone()
        return self._proposal_from(r) if r else None

    def list_pending(self, *, kind: ProposalKind | None = None,
                     priority: ProposalPriority | None = None, limit: int = 100) -> list[ApprovalProposal]:
    ...
    def requires_approval(self, kind: ProposalKind, *, risk_level: str = "MEDIUM") -> bool:
        high_risk_kinds = {ProposalKind.DB_MIGRATION, ProposalKind.SECRET_ROTATION,
                           ProposalKind.HIGH_RISK_ACTION, ProposalKind.DEPLOYMENT,
                           ProposalKind.CAPABILITY_PROMOTION}
        if kind in high_risk_kinds: return True
    ...
    def _proposal_insert_sql(self) -> str:
        cols = ("proposal_id,kind,priority,title,summary,payload,dedup_key,state,risk_level,"
    ...
    def _proposal_row(self, p: ApprovalProposal) -> tuple:
        return (p.proposal_id, p.kind, p.priority, p.title, p.summary, jdump(p.payload),
                p.dedup_key, p.state, p.risk_level, p.requested_by, p.tenant_id,
                jdump(p.correlation), p.created_at, p.updated_at, p.decided_at, p.executed_at, p.expires_at)

    def _proposal_from(self, r: Any) -> ApprovalProposal:
        return ApprovalProposal(proposal_id=r["proposal_id"], kind=ProposalKind(r["kind"]),
                              priority=ProposalPriority(r["priority"]), title=r["title"],
                              summary=r["summary"], payload=jload(r["payload"], {}),
                              dedup_key=r["dedup_key"], state=ProposalState(r["state"]),
                              risk_level=r["risk_level"], requested_by=r["requested_by"],
                              tenant_id=r["tenant_id"], correlation=jload(r["correlation"], {}),
                              created_at=r["created_at"], updated_at=r["updated_at"],
                              decided_at=r["decided_at"], executed_at=r["executed_at"],
    ...
    def _decision_row(self, d: ApprovalDecision) -> tuple:
        return (d.decision_id, d.proposal_id, d.decision, d.decided_by, d.rationale,
                jdump(d.correlation), d.decided_at)

    def _decision_from(self, r: Any) -> ApprovalDecision:
        return ApprovalDecision(decision_id=r["decision_id"], proposal_id=r["proposal_id"],
                              decision=ProposalState(r["decision"]), decided_by=r["decided_by"],
                              rationale=r["rationale"], correlation=jload(r["correlation"], {}),
...
def get_approval_workflow() -> ApprovalWorkflow:
    global _workflow
    if _workflow is None: _workflow = ApprovalWorkflow()
    return _workflow


__all__ = ["ProposalKind", "ProposalPriority", "ProposalState", "ApprovalProposal", "ApprovalDecision",
