...
class SourceState(enum.StrEnum):
    UNKNOWN = "UNKNOWN"; DISCOVERED = "DISCOVERED"; APPROVAL_PENDING = "APPROVAL_PENDING"
    ALLOWLISTED = "ALLOWLISTED"; BLOCKED = "BLOCKED"; DEFERRED = "DEFERRED"


class SourceCategory(enum.StrEnum):
    AI_DOCS = "AI_DOCS"; OSS_REPO = "OSS_REPO"; TECH_DOCS = "TECH_DOCS"; RESEARCH = "RESEARCH"
    STANDARDS = "STANDARDS"; PUBLIC_API = "PUBLIC_API"; TECH_BLOG = "TECH_BLOG"
    MODEL_PROVIDER_DOCS = "MODEL_PROVIDER_DOCS"; APPROVED_SITE = "APPROVED_SITE"
    APPROVED_DATASET = "APPROVED_DATASET"; UNKNOWN = "UNKNOWN"


_ALLOWED: dict[SourceState, set[SourceState]] = {
    SourceState.UNKNOWN: {SourceState.DISCOVERED, SourceState.BLOCKED},
    SourceState.DISCOVERED: {SourceState.APPROVAL_PENDING, SourceState.ALLOWLISTED, SourceState.BLOCKED, SourceState.DEFERRED},
    SourceState.APPROVAL_PENDING: {SourceState.ALLOWLISTED, SourceState.BLOCKED, SourceState.DEFERRED},
    ...
    SourceState.DEFERRED: {SourceState.APPROVAL_PENDING, SourceState.ALLOWLISTED, SourceState.BLOCKED},
}


class SourceStateError(Exception): pass
...
class SourceGovernance:
    ...
    def _ensure_schema(self) -> None:
        with get_conn() as conn:
            ...
            conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.SOURCES_TABLE}_state ON {self.SOURCES_TABLE}(state)")
            ...
            conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.LEARNED_TABLE}_value ON {self.LEARNED_TABLE}(value_score)")
            conn.commit()

    def discover(self, url: str, *, category: SourceCategory = SourceCategory.UNKNOWN) -> dict[str, Any]:
        now = datetime.now(UTC).isoformat()
        with get_conn() as conn:
            existing = conn.execute(f"SELECT * FROM {self.SOURCES_TABLE} WHERE url=?", (url,)).fetchone()
            if existing:
                conn.execute(f"UPDATE {self.SOURCES_TABLE} SET last_seen_at=? WHERE source_id=?",
            ...
            conn.commit()
            return {"source_id": source_id, "url": url, "category": str(category),
                    "state": str(state), "first_seen_at": now, "last_seen_at": now, "metadata": {}}

    def transition_source(self, source_id: str, to: SourceState) -> dict[str, Any]:
        with get_conn() as conn:
            r = conn.execute(f"SELECT * FROM {self.SOURCES_TABLE} WHERE source_id=?", (source_id,)).fetchone()
            if r is None: raise SourceStateError(f"Source {source_id} not found")
            current = SourceState(r["state"])
            if to not in _ALLOWED.get(current, set()):
                raise SourceStateError(f"Illegal transition {current} → {to}")
            now = datetime.now(UTC).isoformat()
            conn.execute(f"UPDATE {self.SOURCES_TABLE} SET state=?, last_seen_at=? WHERE source_id=?",
                         (to, now, source_id))
            conn.commit()
    ...
    def is_allowed(self, url: str) -> bool:
        with get_conn() as conn:
            r = conn.execute(f"SELECT state FROM {self.SOURCES_TABLE} WHERE url=?", (url,)).fetchone()
        if r:
            st = SourceState(r["state"])
            if st == SourceState.ALLOWLISTED: return True
            if st == SourceState.BLOCKED: return False
        policy = self.match_policy(url)
        if policy: return policy.state == SourceState.ALLOWLISTED
        return False
    ...
    def match_policy(self, url: str) -> SourcePolicy | None:
        with get_conn() as conn:
            rows = conn.execute(f"SELECT * FROM {self.POLICIES_TABLE} ORDER BY created_at ASC").fetchall()
        for r in rows:
            pat = r["url_pattern"]
            matched = False
            try:
    ...
    def list_learned(self, *, category: SourceCategory | None = None,
                     min_value: float = 0.0, limit: int = 100) -> list[LearnedItem]:
        ...
        with get_conn() as conn:
            rows = conn.execute(
                ...
                "ORDER BY value_score DESC, created_at DESC LIMIT ?", params).fetchall()
    ...
    def _policy_insert_sql(self) -> str:
        cols = ("policy_id,url_pattern,category,state,allowed_actions,source_weight,expires_at,"
                "created_by,created_at,updated_at")
        return f"INSERT INTO {self.POLICIES_TABLE} ({cols}) VALUES ({','.join(['?']*10)})"

    def _policy_row(self, p: SourcePolicy) -> tuple:
        return (p.policy_id, p.url_pattern, p.category, p.state, jdump(p.allowed_actions),
                p.source_weight, p.expires_at, p.created_by, p.created_at, p.updated_at)

    def _policy_from(self, r: Any) -> SourcePolicy:
        return SourcePolicy(policy_id=r["policy_id"], url_pattern=r["url_pattern"],
                          category=SourceCategory(r["category"]), state=SourceState(r["state"]),
    ...
    def _learned_insert_sql(self) -> str:
        cols = ("item_id,source_url,source_id,category,title,content,summary,embedding,"
    ...
    def _learned_row(self, i: LearnedItem) -> tuple:
        return (i.item_id, i.source_url, i.source_id, i.category, i.title, i.content, i.summary,
                jdump(i.embedding), i.value_score, i.reused_count, i.created_at, i.updated_at, i.pruned_at)

    def _learned_from(self, r: Any) -> LearnedItem:
        return LearnedItem(item_id=r["item_id"], source_url=r["source_url"], source_id=r["source_id"],
                         category=SourceCategory(r["category"]), title=r["title"], content=r["content"],
                         summary=r["summary"], embedding=jload(r["embedding"], []),
    ...
    def _source_row(r: Any) -> dict[str, Any]:
        return {"source_id": r["source_id"], "url": r["url"], "category": r["category"],
                "state": r["state"], "first_seen_at": r["first_seen_at"],
                "last_seen_at": r["last_seen_at"], "metadata": jload(r["metadata"], {})}


_governance: SourceGovernance | None = None
def get_source_governance() -> SourceGovernance:
    global _governance
    if _governance is None: _governance = SourceGovernance()
    return _governance


__all__ = ["SourceState", "SourceCategory", "SourcePolicy", "LearnedItem", "SourceGovernance",
           "get_source_governance", "SourceStateError"]
