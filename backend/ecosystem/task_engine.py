...
class TaskState(enum.StrEnum):
    ...
    CAPABILITY_CHECK = "CAPABILITY_CHECK"; RESOURCE_CHECK = "RESOURCE_CHECK"
    PREPARING = "PREPARING"; EXECUTING = "EXECUTING"; VERIFYING = "VERIFYING"
    REPAIRING = "REPAIRING"; DELIVERING = "DELIVERING"; COMPLETED = "COMPLETED"
...
_ALLOWED: dict[TaskState, set[TaskState]] = {
    ...
    TaskState.UNDERSTANDING: {TaskState.PLANNING, TaskState.ESCALATED, TaskState.FAILED, TaskState.CANCELLED},
    TaskState.PLANNING: {TaskState.CAPABILITY_CHECK, TaskState.ESCALATED, TaskState.FAILED, TaskState.CANCELLED},
    TaskState.CAPABILITY_CHECK: {TaskState.RESOURCE_CHECK, TaskState.ESCALATED, TaskState.FAILED, TaskState.CANCELLED},
    TaskState.RESOURCE_CHECK: {TaskState.PREPARING, TaskState.ESCALATED, TaskState.FAILED, TaskState.CANCELLED},
    TaskState.PREPARING: {TaskState.EXECUTING, TaskState.ESCALATED, TaskState.FAILED, TaskState.CANCELLED},
    TaskState.EXECUTING: {TaskState.VERIFYING, TaskState.FAILED, TaskState.ESCALATED, TaskState.CANCELLED},
    TaskState.VERIFYING: {TaskState.DELIVERING, TaskState.REPAIRING, TaskState.FAILED, TaskState.ESCALATED, TaskState.CANCELLED},
    TaskState.REPAIRING: {TaskState.EXECUTING, TaskState.FAILED, TaskState.ESCALATED, TaskState.CANCELLED},
    TaskState.DELIVERING: {TaskState.COMPLETED, TaskState.FAILED, TaskState.ESCALATED, TaskState.CANCELLED},
...
class TaskStateError(Exception): pass
class TaskNotFoundError(Exception): pass
class TaskRetryExceeded(Exception): pass
...
class TaskEngine:
    ...
    def _ensure_schema(self) -> None:
        with get_conn() as conn:
            ...
            conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.TABLE}_state ON {self.TABLE}(state)")
            conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.TABLE}_owner ON {self.TABLE}(owner)")
            conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.TABLE}_tenant ON {self.TABLE}(tenant_id)")
            conn.commit()
    ...
    def list(self, *, state: TaskState | None = None, owner: TaskOwner | None = None,
             tenant_id: str | None = None, limit: int = 200) -> list[TaskRecord]:
        clauses, params = [], []
    ...
    def transition(self, tid: str, to: TaskState, *, actor: str = "system",
                   error: str | None = None) -> TaskRecord:
        t = self.get(tid)
        if t is None: raise TaskNotFoundError(f"Task {tid} not found")
        if to not in _ALLOWED.get(t.state, set()):
            raise TaskStateError(f"Illegal transition {t.state} → {to}")
        # time-limit enforcement (§23)
        if t.time_limit_seconds and t.started_at and to not in {TaskState.COMPLETED, TaskState.FAILED,
        ...
        if error: sets["error"] = error
        sql = ", ".join(f"{k}=?" for k in sets)
        with get_conn() as conn:
            conn.execute(f"UPDATE {self.TABLE} SET {sql} WHERE task_id=?", list(sets.values()) + [tid])
            conn.commit()
        return self.get(tid)  # type: ignore[return-value]

    def deliver(self, tid: str, *, result: dict[str, Any] | None = None,
                artifacts: list[dict[str, Any]] | None = None, actor: str = "system") -> TaskRecord:
        t = self.get(tid)
        if t is None: raise TaskNotFoundError(tid)
        if t.state != TaskState.DELIVERING:
            raise TaskStateError(f"deliver requires DELIVERING state, got {t.state}")
        now = datetime.now(UTC).isoformat()
        sets: dict[str, Any] = {"updated_at": now, "state": TaskState.COMPLETED, "completed_at": now}
        if result is not None: sets["result"] = jdump(result)
        if artifacts is not None: sets["artifacts"] = jdump(artifacts)
        sql = ", ".join(f"{k}=?" for k in sets)
        with get_conn() as conn:
            conn.execute(f"UPDATE {self.TABLE} SET {sql} WHERE task_id=?", list(sets.values()) + [tid])
            conn.commit()
    ...
    def _insert_sql(self) -> str:
        cols = ("task_id,goal,owner,scope,state,plan,capability_requirements,resource_id,capability_id,"
    ...
    def _row(self, t: TaskRecord) -> tuple:
        return (t.task_id, t.goal, t.owner, jdump(t.scope), t.state, jdump(t.plan),
                jdump(t.capability_requirements), t.resource_id, t.capability_id,
                jdump(t.artifacts), jdump(t.result), jdump(t.success_criteria),
                jdump(t.verification_result), t.retry_count, t.retry_limit, t.time_limit_seconds,
                t.risk_level, jdump(t.correlation), t.created_by, t.tenant_id, t.audit_id, t.error,
                t.created_at, t.updated_at, t.started_at, t.completed_at)

    def _from(self, r: Any) -> TaskRecord:
        return TaskRecord(task_id=r["task_id"], goal=r["goal"], owner=TaskOwner(r["owner"]),
                         scope=jload(r["scope"], {}), state=TaskState(r["state"]),
                         plan=jload(r["plan"], {}),
                         capability_requirements=jload(r["capability_requirements"], []),
                         resource_id=r["resource_id"], capability_id=r["capability_id"],
                         artifacts=jload(r["artifacts"], []), result=jload(r["result"], {}),
                         success_criteria=jload(r["success_criteria"], {}),
                         verification_result=jload(r["verification_result"], {}),
                         retry_count=int(r["retry_count"] or 0), retry_limit=int(r["retry_limit"] or 3),
                         time_limit_seconds=r["time_limit_seconds"], risk_level=r["risk_level"],
                         correlation=jload(r["correlation"], {}), created_by=r["created_by"],
                         tenant_id=r["tenant_id"], audit_id=r["audit_id"], error=r["error"],
...
def get_task_engine() -> TaskEngine:
    global _engine
    if _engine is None: _engine = TaskEngine()
    return _engine


__all__ = ["TaskState", "TaskOwner", "TaskRecord", "TaskEngine", "get_task_engine",
