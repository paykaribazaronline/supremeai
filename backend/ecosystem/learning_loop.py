...
class LearningStage(enum.StrEnum):
    DISCOVERY = "DISCOVERY"; SOURCE_CHECK = "SOURCE_CHECK"; POLICY_GATE = "POLICY_GATE"
    RESEARCH = "RESEARCH"; KNOWLEDGE_RECORDED = "KNOWLEDGE_RECORDED"
    GAP_SIGNAL = "GAP_SIGNAL"; CAPABILITY_OPPORTUNITY = "CAPABILITY_OPPORTUNITY"
    PRACTICALITY_ANALYSIS = "PRACTICALITY_ANALYSIS"; PROPOSAL = "PROPOSAL"
    AWAITING_APPROVAL = "AWAITING_APPROVAL"; BUILDING = "BUILDING"; VALIDATING = "VALIDATING"
    REGISTERED = "REGISTERED"; REUSED = "REUSED"; REJECTED = "REJECTED"; ARCHIVED = "ARCHIVED"
...
_ALLOWED: dict[LearningStage, set[LearningStage]] = {
    ...
    LearningStage.RESEARCH: {LearningStage.KNOWLEDGE_RECORDED, LearningStage.GAP_SIGNAL, LearningStage.REJECTED},
    ...
    LearningStage.CAPABILITY_OPPORTUNITY: {LearningStage.PRACTICALITY_ANALYSIS, LearningStage.ARCHIVED},
...
class LearningStageError(Exception): pass
...
class LearningLoop:
    ...
    def _ensure_schema(self) -> None:
        with get_conn() as conn:
            ...
            conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.SIGNALS_TABLE}_stage ON {self.SIGNALS_TABLE}(stage)")
            ...
            conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.OPPORTUNITIES_TABLE}_stage ON {self.OPPORTUNITIES_TABLE}(stage)")
            conn.commit()
    ...
    def list_signals(self, *, stage: LearningStage | None = None, source: str | None = None,
                     limit: int = 100) -> list[EvolutionSignal]:
        clauses, params = [], []
        if stage: clauses.append("stage=?"); params.append(stage)
        if source: clauses.append("source=?"); params.append(source)
        where = ("WHERE " + " AND ".join(clauses)) if clauses else ""
        params.append(limit)
        with get_conn() as conn:
            rows = conn.execute(
                f"SELECT * FROM {self.SIGNALS_TABLE} {where} ORDER BY created_at DESC LIMIT ?", params).fetchall()
        ...
        with get_conn() as conn:
            ...
            if opp.signal_id:
                conn.execute(
                    ...
                    (opp.opportunity_id, datetime.now(UTC).isoformat(),
                     LearningStage.CAPABILITY_OPPORTUNITY, opp.signal_id))
            conn.commit()
        return opp

    def advance_stage(self, opportunity_id: str, to: LearningStage,
                       *, proposal_id: str | None = None) -> LearningOpportunity:
        with get_conn() as conn:
            r = conn.execute(
                f"SELECT * FROM {self.OPPORTUNITIES_TABLE} WHERE opportunity_id=?", (opportunity_id,)).fetchone()
            if r is None: raise LearningStageError(f"Opportunity {opportunity_id} not found")
            ...
            if proposal_id: sets["proposal_id"] = proposal_id
            if to == LearningStage.ARCHIVED: sets["archived_at"] = now
            sql = ", ".join(f"{k}=?" for k in sets)
            conn.execute(f"UPDATE {self.OPPORTUNITIES_TABLE} SET {sql} WHERE opportunity_id=?",
    ...
    def get_opportunity(self, oid: str) -> LearningOpportunity | None:
        with get_conn() as conn:
            r = conn.execute(f"SELECT * FROM {self.OPPORTUNITIES_TABLE} WHERE opportunity_id=?", (oid,)).fetchone()
        return self._opp_from(r) if r else None

    def list_opportunities(self, *, stage: LearningStage | None = None,
                           include_archived: bool = False, limit: int = 100) -> list[LearningOpportunity]:
        clauses, params = [], []
        if stage: clauses.append("stage=?"); params.append(stage)
        ...
        with get_conn() as conn:
            rows = conn.execute(
                f"SELECT * FROM {self.OPPORTUNITIES_TABLE} {where} ORDER BY created_at DESC LIMIT ?", params).fetchall()
        return [self._opp_from(r) for r in rows]

    def _signal_insert_sql(self) -> str:
        cols = ("signal_id,source,kind,content,value_score,stage,opportunity_id,correlation,"
                "created_at,updated_at")
        return f"INSERT INTO {self.SIGNALS_TABLE} ({cols}) VALUES ({','.join(['?']*10)})"

    def _signal_row(self, s: EvolutionSignal) -> tuple:
        return (s.signal_id, s.source, s.kind, jdump(s.content), s.value_score, s.stage,
                s.opportunity_id, jdump(s.correlation), s.created_at, s.updated_at)

    def _signal_from(self, r: Any) -> EvolutionSignal:
        return EvolutionSignal(signal_id=r["signal_id"], source=r["source"], kind=r["kind"],
                             content=jload(r["content"], {}), value_score=float(r["value_score"] or 0),
                             stage=LearningStage(r["stage"]), opportunity_id=r["opportunity_id"],
                             correlation=jload(r["correlation"], {}),
                             created_at=r["created_at"], updated_at=r["updated_at"])

    def _opp_insert_sql(self) -> str:
        cols = ("opportunity_id,signal_id,capability_hint,gap_description,predicted_value,"
    ...
    def _opp_row(self, o: LearningOpportunity) -> tuple:
        return (o.opportunity_id, o.signal_id, o.capability_hint, o.gap_description,
                o.predicted_value, o.predicted_effort, o.stage, o.proposal_id,
                jdump(o.correlation), o.created_at, o.updated_at, o.archived_at)

    def _opp_from(self, r: Any) -> LearningOpportunity:
        return LearningOpportunity(opportunity_id=r["opportunity_id"], signal_id=r["signal_id"],
                                 capability_hint=r["capability_hint"], gap_description=r["gap_description"],
                                 predicted_value=float(r["predicted_value"] or 0),
                                 predicted_effort=float(r["predicted_effort"] or 0),
                                 stage=LearningStage(r["stage"]), proposal_id=r["proposal_id"],
                                 correlation=jload(r["correlation"], {}),
                                 created_at=r["created_at"], updated_at=r["updated_at"],
...
def get_learning_loop() -> LearningLoop:
    global _loop
    if _loop is None: _loop = LearningLoop()
    return _loop


__all__ = ["LearningStage", "EvolutionSignal", "LearningOpportunity", "LearningLoop",
