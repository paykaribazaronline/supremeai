...
class DeploymentStatus(enum.StrEnum):
    PENDING = "PENDING"; IN_PROGRESS = "IN_PROGRESS"; SUCCEEDED = "SUCCEEDED"
    FAILED = "FAILED"; ROLLED_BACK = "ROLLED_BACK"; SUPERSEDED = "SUPERSEDED"


_ALLOWED: dict[DeploymentStatus, set[DeploymentStatus]] = {
    DeploymentStatus.PENDING: {DeploymentStatus.IN_PROGRESS, DeploymentStatus.FAILED, DeploymentStatus.SUPERSEDED},
    DeploymentStatus.IN_PROGRESS: {DeploymentStatus.SUCCEEDED, DeploymentStatus.FAILED, DeploymentStatus.ROLLED_BACK},
...
class DeploymentStateError(Exception): pass
class DeploymentNotFoundError(Exception): pass
...
class DeploymentTracker:
    ...
    def _ensure_schema(self) -> None:
        with get_conn() as conn:
            ...
            conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.TABLE}_resource ON {self.TABLE}(resource_id, started_at)")
            conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.TABLE}_commit ON {self.TABLE}(commit_sha)")
    ...
    def start(self, d: DeploymentRecord) -> DeploymentRecord:
        with get_conn() as conn:
            ...
            conn.execute(
                ...
                (DeploymentStatus.SUPERSEDED, datetime.now(UTC).isoformat(), d.resource_id,
                 DeploymentStatus.IN_PROGRESS, d.deployment_id))
            conn.commit()
        return d

    def finish(self, deployment_id: str, status: DeploymentStatus,
               *, notes: str = "", log_url: str | None = None) -> DeploymentRecord:
        d = self.get(deployment_id)
        if d is None: raise DeploymentNotFoundError(deployment_id)
        ...
        if log_url: sets["log_url"] = log_url
        sql = ", ".join(f"{k}=?" for k in sets)
        with get_conn() as conn:
            conn.execute(f"UPDATE {self.TABLE} SET {sql} WHERE deployment_id=?",
    ...
    def trace(self, correlation_key: str, value: str) -> list[DeploymentRecord]:
        ...
        with get_conn() as conn:
            rows = conn.execute(
                ...
                (f"$.{correlation_key}", value)).fetchall()
        return [self._from(r) for r in rows]

    def _insert_sql(self) -> str:
        cols = ("deployment_id,resource_id,commit_sha,branch,version,status,started_by,correlation,"
    ...
    def _row(self, d: DeploymentRecord) -> tuple:
        return (d.deployment_id, d.resource_id, d.commit_sha, d.branch, d.version, d.status,
                d.started_by, jdump(d.correlation), d.started_at, d.finished_at,
                d.log_url, d.rollback_of, d.notes, d.created_at, d.updated_at)

    def _from(self, r: Any) -> DeploymentRecord:
        return DeploymentRecord(deployment_id=r["deployment_id"], resource_id=r["resource_id"],
                             commit_sha=r["commit_sha"], branch=r["branch"], version=r["version"],
                             status=DeploymentStatus(r["status"]), started_by=r["started_by"],
                             correlation=jload(r["correlation"], {}),
                             started_at=r["started_at"], finished_at=r["finished_at"],
                             log_url=r["log_url"], rollback_of=r["rollback_of"], notes=r["notes"],
...
def get_deployment_tracker() -> DeploymentTracker:
    global _tracker
    if _tracker is None: _tracker = DeploymentTracker()
    return _tracker


__all__ = ["DeploymentStatus", "DeploymentRecord", "DeploymentTracker", "get_deployment_tracker",
