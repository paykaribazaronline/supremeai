...
class MCPOperationCategory(enum.StrEnum):
    OBSERVE = "OBSERVE"; ANALYZE = "ANALYZE"; ACT = "ACT"


class MCPOperationError(Exception): pass
class MCPOperationNotRegisteredError(Exception): pass
class MCPActionDenied(Exception): pass


_OBSERVE_OPS = {"list_resources", "get_resource", "get_health", "get_metrics", "get_logs",
                "get_deployment", "get_task_status", "get_capabilities", "get_kaggle_quota"}
_ANALYZE_OPS = {"find_capability", "detect_capability_gap", "forecast_capability",
                "trace_dependency", "correlate_error", "analyze_resource_usage"}
_ACT_OPS = {"create_capability", "activate_capability", "archive_capability", "restart",
            "deploy", "rollback", "trigger_job", "trigger_kaggle", "create_github_pr"}
...
class MCPSkeleton:
    ...
    def _ensure_schema(self) -> None:
        with get_conn() as conn:
            ...
            conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.TABLE}_op ON {self.TABLE}(operation)")
            conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.TABLE}_cat ON {self.TABLE}(category)")
            conn.commit()

    def register(self, operation: str, category: MCPOperationCategory,
                 handler: Callable[..., Any]) -> None:
        self._handlers[operation] = (category, handler)

    def call(self, operation: str, *, params: dict[str, Any] | None = None,
             correlation: dict[str, Any] | None = None, actor: str = "system") -> dict[str, Any]:
        params = params or {}
        corr = correlation or {}
        ...
        if category == MCPOperationCategory.ACT:
            ...
            if not allowed:
                self._record(operation, category, False, params, None,
                             "denied_by_governance", reason, corr)
        ...
        try:
            ...
            return {"operation": operation, "category": str(category),
                    "allowed": True, "result": result}
        except Exception as e:
            self._record(operation, category, False, params, None, str(e), reason, corr)
            raise

    def list_operations(self, *, category: MCPOperationCategory | None = None) -> list[dict[str, str]]:
        out = []
    ...
    def _record(self, operation: str, category: MCPOperationCategory, allowed: bool,
                params: dict[str, Any], result: Any, error: str | None, reason: str,
                correlation: dict[str, Any]) -> None:
        with get_conn() as conn:
            conn.execute(
                f"INSERT INTO {self.TABLE} (call_id, operation, category, allowed, params, "
                "result, error, reason, correlation, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (f"mcp-{uuid.uuid4().hex[:16]}", operation, str(category), int(allowed),
                 jdump(params), jdump(_to_serializable(result)), error, reason,
                 jdump(correlation), datetime.now(UTC).isoformat()))
            conn.commit()
...
def get_mcp_skeleton() -> MCPSkeleton:
    global _skeleton
    if _skeleton is None: _skeleton = MCPSkeleton()
    return _skeleton


__all__ = ["MCPOperationCategory", "MCPSkeleton", "get_mcp_skeleton",
