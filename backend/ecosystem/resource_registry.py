...
class ProviderKind(enum.StrEnum):
    RENDER = "RENDER"; GITHUB = "GITHUB"; KAGGLE = "KAGGLE"; SUPABASE = "SUPABASE"
    FIREBASE = "FIREBASE"; REDIS = "REDIS"; CI = "CI"; CUSTOM = "CUSTOM"


class ResourceState(enum.StrEnum):
    REGISTERED = "REGISTERED"; HEALTHY = "HEALTHY"; DEGRADED = "DEGRADED"
    CRITICAL = "CRITICAL"; UNKNOWN = "UNKNOWN"; MAINTENANCE = "MAINTENANCE"; OFFLINE = "OFFLINE"


class ResourceExistsError(Exception): pass
class ResourceNotFoundError(Exception): pass
class AdapterNotRegisteredError(Exception): pass
...
class ResourceRegistry:
    ...
    def _ensure_schema(self) -> None:
        with get_conn() as conn:
            ...
            conn.execute(f"CREATE UNIQUE INDEX IF NOT EXISTS idx_{self.TABLE}_ext ON {self.TABLE}(provider, external_id)")
            conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.TABLE}_state ON {self.TABLE}(state)")
            conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.TABLE}_provider ON {self.TABLE}(provider)")
    ...
    def register(self, r: ResourceRecord) -> ResourceRecord:
        with get_conn() as conn:
            existing = conn.execute(
                ...
                (r.provider, r.external_id)).fetchone()
            if existing: raise ResourceExistsError(f"Resource {r.provider}:{r.external_id} already exists")
    ...
    def list(self, *, provider: ProviderKind | None = None, state: ResourceState | None = None,
             tenant_id: str | None = None, limit: int = 200) -> list[ResourceRecord]:
        clauses, params = [], []
    ...
    def update_state(self, rid: str, state: ResourceState,
                     *, health: dict[str, Any] | None = None) -> ResourceRecord:
        now = datetime.now(UTC).isoformat()
        sets: dict[str, Any] = {"updated_at": now, "state": state}
        if health is not None: sets["last_health"] = jdump(health)
        sql = ", ".join(f"{k}=?" for k in sets)
        with get_conn() as conn:
            cur = conn.execute(f"UPDATE {self.TABLE} SET {sql} WHERE resource_id=?",
                               list(sets.values()) + [rid])
            if cur.rowcount == 0: raise ResourceNotFoundError(rid)
            conn.commit()
    ...
    def link_deployment(self, rid: str, deployment_id: str) -> ResourceRecord:
        now = datetime.now(UTC).isoformat()
        with get_conn() as conn:
            cur = conn.execute(f"UPDATE {self.TABLE} SET deployment_id=?, updated_at=? WHERE resource_id=?",
    ...
    def _load_adapter(self, provider: ProviderKind) -> BaseProviderAdapter:
        with get_conn() as conn:
            r = conn.execute(f"SELECT adapter_path FROM {self.ADAPTERS_TABLE} WHERE provider=?", (provider,)).fetchone()
        if r is None: raise AdapterNotRegisteredError(f"No adapter registered for provider {provider}")
        path = r["adapter_path"]
    ...
    def control(self, rid: str, action: str, **kwargs: Any) -> dict[str, Any]:
        ...
        r = self.get(rid)
        if r is None: raise ResourceNotFoundError(rid)
        adapter = self._load_adapter(r.provider)
        method = getattr(adapter, action, None)
        if method is None or not callable(method):
            raise AdapterNotRegisteredError(f"Action '{action}' not supported by {r.provider} adapter")
    ...
    def _insert_sql(self) -> str:
        cols = ("resource_id,provider,external_id,name,state,endpoint,metadata,deployment_id,"
    ...
    def _row(self, r: ResourceRecord) -> tuple:
        return (r.resource_id, r.provider, r.external_id, r.name, r.state, r.endpoint,
                jdump(r.metadata), r.deployment_id, r.owner, r.tenant_id, jdump(r.last_health),
                r.created_at, r.updated_at)

    def _from(self, r: Any) -> ResourceRecord:
        return ResourceRecord(resource_id=r["resource_id"], provider=ProviderKind(r["provider"]),
                            external_id=r["external_id"], name=r["name"],
                            state=ResourceState(r["state"]), endpoint=r["endpoint"],
                            metadata=jload(r["metadata"], {}), deployment_id=r["deployment_id"],
                            owner=r["owner"], tenant_id=r["tenant_id"],
...
def get_resource_registry() -> ResourceRegistry:
    global _registry
    if _registry is None: _registry = ResourceRegistry()
    return _registry


__all__ = ["ProviderKind", "ResourceState", "ResourceRecord", "BaseProviderAdapter",
