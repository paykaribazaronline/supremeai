...
class HealthStatus(enum.StrEnum):
    HEALTHY = "HEALTHY"; DEGRADED = "DEGRADED"; WARNING = "WARNING"
    CRITICAL = "CRITICAL"; UNKNOWN = "UNKNOWN"; MAINTENANCE = "MAINTENANCE"
...
def _status_rank(s: HealthStatus) -> int:
    return {HealthStatus.HEALTHY: 0, HealthStatus.MAINTENANCE: 1, HealthStatus.UNKNOWN: 2,
            HealthStatus.WARNING: 3, HealthStatus.DEGRADED: 4, HealthStatus.CRITICAL: 5}.get(s, 2)
...
class HealthAggregator:
    ...
    def _ensure_schema(self) -> None:
        with get_conn() as conn:
            ...
            conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.TABLE}_source ON {self.TABLE}(source, created_at)")
    ...
    def detect_memory_regression(self, *, hours: int = 24,
                                threshold_percent: float = 20.0) -> list[dict[str, Any]]:
        cutoff = (datetime.now(UTC) - timedelta(hours=hours)).isoformat()
        out: list[dict[str, Any]] = []
        with get_conn() as conn:
            sources = [r["source"] for r in conn.execute(
                f"SELECT DISTINCT source FROM {self.TABLE} WHERE created_at >= ?", (cutoff,)).fetchall()]
            for s in sources:
                rows = conn.execute(
                    f"SELECT * FROM {self.TABLE} WHERE source=? AND created_at >= ? ORDER BY created_at ASC",
                    (s, cutoff)).fetchall()
                if len(rows) < 2: continue
                first = self._from(rows[0]); last = self._from(rows[-1])
                delta = last.memory.current_mb - first.memory.current_mb
                pct = (delta / first.memory.current_mb * 100) if first.memory.current_mb > 0 else 0.0
                if pct > threshold_percent:
                    out.append({
                        "source": s, "first_mb": first.memory.current_mb,
                        "last_mb": last.memory.current_mb, "delta_mb": delta,
                        "percent_change": pct, "trend": last.memory.trend,
                    })
        return out

    def _insert_sql(self) -> str:
        cols = ("record_id,source,status,memory,cpu_percent,disk_percent,latency_ms,"
    ...
    def _row(self, h: UnifiedHealth) -> tuple:
        return (h.record_id, h.source, h.status, jdump(h.memory.model_dump()),
                h.cpu_percent, h.disk_percent, h.latency_ms, h.error_rate,
                jdump(h.custom_metrics), h.timestamp, h.created_at)

    def _from(self, r: Any) -> UnifiedHealth:
        mem_raw = jload(r["memory"], {"current_mb": 0.0, "peak_mb": 0.0, "limit_mb": 0.0,
                                      "percent": 0.0, "trend": "stable"})
        if not isinstance(mem_raw, dict): mem_raw = {}
        return UnifiedHealth(
            record_id=r["record_id"], source=r["source"], status=HealthStatus(r["status"]),
            memory=MemoryInfo(**mem_raw),
            cpu_percent=float(r["cpu_percent"] or 0), disk_percent=float(r["disk_percent"] or 0),
            latency_ms=float(r["latency_ms"] or 0), error_rate=float(r["error_rate"] or 0),
...
def get_health_aggregator() -> HealthAggregator:
    global _aggregator
    if _aggregator is None: _aggregator = HealthAggregator()
    return _aggregator


__all__ = ["HealthStatus", "MemoryInfo", "UnifiedHealth", "HealthAggregator", "get_health_aggregator"]
