...
class CorrelationContext:
    ...
    def as_headers(self) -> dict[str, str]:
        ...
        if self.task_id: h["x-correlation-task-id"] = self.task_id
        if self.job_id: h["x-correlation-job-id"] = self.job_id
        if self.deployment_id: h["x-correlation-deployment-id"] = self.deployment_id
        if self.resource_id: h["x-correlation-resource-id"] = self.resource_id
        if self.capability_id: h["x-correlation-capability-id"] = self.capability_id
        return h
