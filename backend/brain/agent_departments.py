    ...
    def execute(self, role: str, task: str, context: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            if result.get("success") or result.get("text"):
                return {"role": role_key, "success": True, "output": result.get("text", ""), "cost": result.get("cost", 0.0)}
            return {"role": role_key, "success": False, "error": result.get("error", "unknown")}
