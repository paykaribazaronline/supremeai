...
class LegalAgent:
    ...
    RISK_INDICATORS = {
        "unlimited_liability": [r"unlimited\s+liability", r"liability\s+shall\s+not\s+be\s+limited", r"no\s+cap\s+on\s+damages"],
        "indemnification": [r"indemnify", r"indemnification", r"hold\s+harmless"],
        "ip_assignment": [r"assign\s+all\s+rights", r"work\s+for\s+hire", r"intellectual\s+property\s+transfer"],
        "non_compete": [r"non[\s-]?compete", r"shall\s+not\s+compete"],
        "termination": [r"terminat\w*\s+for\s+convenience", r"terminate\s+this\s+agreement"],
        "auto_renewal": [r"auto[\s-]?renew", r"automatic\s+renewal", r"renew\s+automatically"],
        "jurisdiction": [r"exclusive\s+jurisdiction", r"governed\s+by\s+the\s+laws\s+of"],
        "ambiguous": [r"reasonable\s+efforts", r"best\s+efforts", r"material\s+adverse\s+effect"],
    }
