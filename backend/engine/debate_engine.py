...
class JudgeAgent:
    ...
    async def evaluate_proposals(self, task_prompt: str, proposals: List[Proposal]) -> Dict[str, Any]:
        ...
        return {
            "decision": DebateState.CONSENSUS,
            "best_proposal": best_proposal,
            "feedback": "Consensus reached successfully."
        }
...
class ConsensusOrchestrator:
    ...
    async def run_debate_cycle(self, task_prompt: str) -> Optional[Proposal]:
        """
        ...
        while self.iteration < self.max_iterations:
            ...
            await swarm_streamer.broadcast("DEBATE_UPDATE", {"session_id": self.session_id, "state": self.current_state, "proposals_count": len(self.proposals)})
            ...
            if judge_result["decision"] == DebateState.CONSENSUS:
                ...
                await swarm_streamer.broadcast("DEBATE_UPDATE", {"session_id": self.session_id, "state": self.current_state, "winning_agent": judge_result["best_proposal"].agent_id})
                return judge_result["best_proposal"]
            else:
                self.current_state = DebateState.RETHINKING
                await swarm_streamer.broadcast("DEBATE_UPDATE", {"session_id": self.session_id, "state": self.current_state, "feedback": judge_result.get("feedback")})
