"""Ecosystem user-facing routes — task submission, capability discovery, MCP.

বাংলা: এই router-টি user-facing surface। admin/governance endpoints আলাদা
router-এ (ecosystem_admin.py) — যাতে is_admin flag আলাদাভাবে enforce হয়।

ROADMAP §22 — user submits goal → task engine নেয়।
ROADMAP §45 — MCP skeleton এখান থেকে expose হয়।
ROADMAP §14 — capability search REUSE > ADAPT > EXTEND > CREATE।
"""

from __future__ import annotations

from typing import Any

from fastapi import APIRouter, Depends, HTTPException, Query
from pydantic import BaseModel, Field

from ecosystem import (
    Capability,
    CapabilityLifecycleState,
    ProviderKind,
    ResourceRecord,
    ResourceState,
    TaskOwner,
    TaskState,
    get_capability_registry,
    get_mcp_skeleton,
    get_resource_registry,
    get_task_engine,
    get_health_aggregator,
    get_deployment_tracker,
)
from ecosystem.correlation import new_correlation_context

router = APIRouter(prefix="/api/v1/ecosystem", tags=["ecosystem"])


# ---------------------------------------------------------------------------
# Capability discovery (ROADMAP §14)
# ---------------------------------------------------------------------------


class CapabilitySearchRequest(BaseModel):
    requirement: str
    signature_hint: str | None = None
    category_hint: str | None = None
    limit: int = 10


@router.get("/capabilities")
def list_capabilities(
    state: str | None = None,
    category: str | None = None,
    limit: int = Query(200, le=500),
) -> list[dict[str, Any]]:
    """ROADMAP §12 — list registered capabilities."""
    caps = get_capability_registry().list(
        state=CapabilityLifecycleState(state) if state else None,
        category=category,
        limit=limit,
    )
    return [c.model_dump() for c in caps]


@router.post("/capabilities/search")
def search_capabilities(req: CapabilitySearchRequest) -> dict[str, Any]:
    """ROADMAP §14 — REUSE > ADAPT > EXTEND > CREATE search."""
    caps = get_capability_registry().search_for_requirement(
        req.requirement,
        signature_hint=req.signature_hint,
        category_hint=req.category_hint,
        limit=req.limit,
    )
    return {
        "requirement": req.requirement,
        "candidates": [c.model_dump() for c in caps],
        "rule": "REUSE > ADAPT > EXTEND > CREATE",
        "gap_detected": len(caps) == 0,
    }


@router.get("/capabilities/{capability_id}")
def get_capability(capability_id: str) -> dict[str, Any]:
    cap = get_capability_registry().get(capability_id)
    if cap is None:
        raise HTTPException(404, f"Capability {capability_id} not found")
    return cap.model_dump()


# ---------------------------------------------------------------------------
# Task engine (ROADMAP §22)
# ---------------------------------------------------------------------------


class TaskSubmitRequest(BaseModel):
    goal: str
    owner: str = "USER"
    scope: str = "USER_WORKSPACE"
    success_criteria: dict[str, Any] = Field(default_factory=dict)
    capability_requirements: list[dict[str, Any]] = Field(default_factory=list)
    risk_level: str = "medium"
    tenant_id: str | None = None
    created_by: str = "user"


@router.post("/tasks")
def submit_task(req: TaskSubmitRequest) -> dict[str, Any]:
    """ROADMAP §22 — submit a goal; the autonomous engine takes over."""
    new_correlation_context(user_id=req.created_by, tenant_id=req.tenant_id)
    try:
        owner = TaskOwner(req.owner)
    except ValueError:
        raise HTTPException(400, f"invalid owner: {req.owner}")
    task = get_task_engine().submit(
        goal=req.goal,
        owner=owner,
        scope=req.scope,
        created_by=req.created_by,
        tenant_id=req.tenant_id,
        success_criteria=req.success_criteria,
        capability_requirements=req.capability_requirements,
        risk_level=req.risk_level,
    )
    return task.model_dump()


@router.get("/tasks")
def list_tasks(
    state: str | None = None,
    owner: str | None = None,
    limit: int = Query(100, le=500),
) -> list[dict[str, Any]]:
    tasks = get_task_engine().list(
        state=TaskState(state) if state else None,
        owner=TaskOwner(owner) if owner else None,
        limit=limit,
    )
    return [t.model_dump() for t in tasks]


@router.get("/tasks/{task_id}")
def get_task(task_id: str) -> dict[str, Any]:
    task = get_task_engine().get(task_id)
    if task is None:
        raise HTTPException(404, f"Task {task_id} not found")
    return task.model_dump()


class TaskTransitionRequest(BaseModel):
    to_state: str
    patch: dict[str, Any] | None = None
    error: str | None = None
    actor: str = "system"


@router.post("/tasks/{task_id}/transition")
def transition_task(task_id: str, req: TaskTransitionRequest) -> dict[str, Any]:
    try:
        to_state = TaskState(req.to_state)
    except ValueError:
        raise HTTPException(400, f"invalid state: {req.to_state}")
    try:
        task = get_task_engine().transition(
            task_id, to_state, actor=req.actor, patch=req.patch, error=req.error
        )
    except Exception as exc:
        raise HTTPException(409, str(exc))
    return task.model_dump()


class TaskArtifactRequest(BaseModel):
    artifact: dict[str, Any]


@router.post("/tasks/{task_id}/artifacts")
def attach_artifact(task_id: str, req: TaskArtifactRequest) -> dict[str, Any]:
    task = get_task_engine().record_artifact(task_id, req.artifact)
    return task.model_dump()


class TaskDeliverRequest(BaseModel):
    result: dict[str, Any]
    actor: str = "system"


@router.post("/tasks/{task_id}/deliver")
def deliver_task(task_id: str, req: TaskDeliverRequest) -> dict[str, Any]:
    task = get_task_engine().deliver(task_id, req.result, actor=req.actor)
    return task.model_dump()


# ---------------------------------------------------------------------------
# Resources / health / deployments (ROADMAP §36, §40, §41)
# ---------------------------------------------------------------------------


class ResourceRegisterRequest(BaseModel):
    name: str
    provider: str
    type: str = "web_service"
    environment: str = "production"
    repository: str | None = None
    region: str | None = None
    dependencies: list[str] = Field(default_factory=list)
    capabilities: list[str] = Field(default_factory=list)
    metadata: dict[str, Any] = Field(default_factory=dict)
    provider_config_ref: str | None = None
    owner: str = "system"
    tenant_id: str | None = None


@router.get("/resources")
def list_resources(
    provider: str | None = None,
    environment: str | None = None,
    state: str | None = None,
    limit: int = Query(200, le=500),
) -> list[dict[str, Any]]:
    resources = get_resource_registry().list(
        provider=ProviderKind(provider) if provider else None,
        environment=environment,
        state=ResourceState(state) if state else None,
        limit=limit,
    )
    return [r.model_dump() for r in resources]


@router.post("/resources")
def register_resource(req: ResourceRegisterRequest) -> dict[str, Any]:
    try:
        provider = ProviderKind(req.provider)
    except ValueError:
        raise HTTPException(400, f"invalid provider: {req.provider}")
    resource = ResourceRecord(
        name=req.name,
        provider=provider,
        type=req.type,
        environment=req.environment,
        repository=req.repository,
        region=req.region,
        dependencies=req.dependencies,
        capabilities=req.capabilities,
        metadata=req.metadata,
        provider_config_ref=req.provider_config_ref,
        owner=req.owner,
        tenant_id=req.tenant_id,
    )
    return get_resource_registry().register(resource).model_dump()


@router.get("/resources/{resource_id}")
def get_resource(resource_id: str) -> dict[str, Any]:
    r = get_resource_registry().get(resource_id)
    if r is None:
        raise HTTPException(404, f"Resource {resource_id} not found")
    return r.model_dump()


@router.get("/health")
def ecosystem_health() -> dict[str, Any]:
    """ROADMAP §41, §47 — unified composite health roll-up."""
    agg = get_health_aggregator()
    return {
        "composite": str(agg.composite_status()),
        "resources": [h.model_dump() for h in agg.all_latest()],
        "top_memory": [h.model_dump() for h in agg.top_memory_consumers(5)],
    }


@router.get("/deployments")
def list_deployments(
    resource_id: str | None = None,
    commit_sha: str | None = None,
    limit: int = Query(50, le=200),
) -> list[dict[str, Any]]:
    tracker = get_deployment_tracker()
    if commit_sha:
        return [d.model_dump() for d in tracker.list_by_commit(commit_sha)]
    if resource_id:
        return [d.model_dump() for d in tracker.list_by_resource(resource_id, limit=limit)]
    return []


@router.get("/deployments/trace/{commit_sha}")
def trace_commit(commit_sha: str) -> dict[str, Any]:
    """ROADMAP §40 — 'which commit caused which incident' answer."""
    return get_deployment_tracker().trace(commit_sha)


# ---------------------------------------------------------------------------
# MCP skeleton (ROADMAP §45)
# ---------------------------------------------------------------------------


class MCPCallRequest(BaseModel):
    operation: str
    arguments: dict[str, Any] = Field(default_factory=dict)


@router.get("/mcp/manifest")
def mcp_manifest() -> dict[str, Any]:
    return get_mcp_skeleton().manifest()


@router.post("/mcp/call")
async def mcp_call(req: MCPCallRequest) -> dict[str, Any]:
    """ROADMAP §45 — single generic MCP entry. Governance-gated for ACT ops."""
    return await get_mcp_skeleton().call(req.operation, arguments=req.arguments)


__all__ = ["router"]
