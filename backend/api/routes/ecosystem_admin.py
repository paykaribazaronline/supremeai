...
# বাংলা: বিদ্যমান admin auth dependency — approval_manager.py-র একই pattern।
# Production-এ core.security.authentication.auth_middleware.verify_admin_session_fail_closed
# ব্যবহার হবে (JWT ভিত্তিক)।
# Local test-এ ECOSYSTEM_DEV_BYPASS_ADMIN_AUTH=true দিলে simple Bearer token দিয়ে
# admin endpoints test করা যায়। ⚠️ কখনো production-এ এই bypass চালু করবেন না।
import os

_DEV_BYPASS = os.getenv("ECOSYSTEM_DEV_BYPASS_ADMIN_AUTH", "false").lower() == "true"

if _DEV_BYPASS:
    _DEV_TOKEN = os.getenv("ADMIN_TOKEN", "")

    async def _dev_admin_auth(request: Any) -> dict[str, Any]:
        """Local dev-only admin auth via simple Bearer token (TEST ONLY)."""
        from fastapi import HTTPException
        auth = request.headers.get("Authorization", "")
        if not auth.startswith("Bearer "):
            raise HTTPException(401, "Missing Bearer token")
        if auth[7:] != _DEV_TOKEN:
            raise HTTPException(403, "Invalid admin token")
        return {"role": "admin", "source": "dev-bypass"}

    _ADMIN_DEP = [Depends(_dev_admin_auth)]
else:
    try:
        from core.security.authentication.auth_middleware import (  # type: ignore[import-not-found]
            verify_admin_session_fail_closed,
        )
        _ADMIN_DEP = [Depends(verify_admin_session_fail_closed)]
    except Exception:  # noqa: BLE001 — graceful if auth module path differs in forks
        _ADMIN_DEP = []
