...
def mock_db_context():
    ...
    mock_factory.create_specialized_agent = AsyncMock(return_value={"agent_name": "MockAgent", "execution_steps": []})
...
async def test_fallback_layer2_success(mock_db_context):
    ...
    with (
        patch("database.session.AsyncSessionLocal", return_value=mock_session),
        patch("core.agent_factory.DynamicAgentFactory", return_value=mock_factory),
    ):
        response = await router.execute_scraping_task(task_prompt="Extract pricing", contextual_url="https://example.com/products")
