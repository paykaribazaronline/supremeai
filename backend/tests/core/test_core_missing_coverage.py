...
class TestSettingsValidators:
    ...
    def test_computed_fields_read_from_vault(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "core.config.secret_vault.fetch_secret", lambda k, *a, **kw: f"val-{k}"
        )
        s = Settings()
...
class TestConfigCacheMissingBranches:
    ...
    async def test_refresh_async_db_failure_uses_defaults(self):
        ...
        cache = ConfigCache()
        with patch(
            "database.session._get_session_maker", side_effect=RuntimeError("db down")
        ):
...
class TestEventBusMissingBranches:
    ...
    async def test_handler_failure_routes_to_dlq(self):
        from core.messaging.event_bus import (DeadLetterQueueItem, ErrorEvent,
                                              ErrorEventBus)
