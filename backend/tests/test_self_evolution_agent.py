...
def mock_fitness_engine():
    engine = MagicMock()
    engine.metrics = {"Skill_A": {"success_count": 10, "failure_count": 5, "total_latency": 15.0, "token_cost": 0.0, "reuse_count": 15}}
...
async def test_evaluate_skill_skips_below_min_runs(agent, mock_fitness_engine):
    mock_fitness_engine.metrics = {"Skill_S": {"success_count": 2, "failure_count": 0, "total_latency": 2.0, "token_cost": 0.0, "reuse_count": 2}}
...
async def test_refactor_prompt_contains_source_code(agent, mock_auto_skill_creator, tmp_path):
    ...
    base = tmp_path
    with patch("backend.evolution.self_evolution_agent.Path") as mock_path_cls:
        instance = MagicMock()
