...
async def test_episodic_memory_flow():
    ...
    episodic.store_episode(
        task_type="sorting", input_data="Write a Python sorting algorithm"
    )
...
def test_long_term_memory_user_context():
    ltm = LongTermMemory()
    ltm.store_user_preference(
        user_id="usr_99", key="preferred_language", value="Bengali"
    )
...
def test_behavioral_guard_anomalies():
    ...
    res2 = guard.record_action(
        agent_id="ag_01", action_type="cmd", prompt_or_command="rm -rf /"
    )
