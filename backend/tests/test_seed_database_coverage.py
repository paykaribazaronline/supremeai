...
class TestInitFtsDB:
    ...
    def test_init_fts_db_creates_table(self):
        ...
        with patch.dict(
            "sys.modules", {"tools.knowledge.local_search_rag": MagicMock()}
        ):
...
class TestUpsertFTS:
    ...
    def test_upsert_fts_inserts_new_row(self):
        ...
        with patch.dict(
            ...
            cursor = conn.execute(
                "SELECT title, content, source FROM knowledge_fts WHERE rowid=1"
            )
            row = cursor.fetchone()
...
class TestSeedAll:
    ...
    def test_seed_all_with_rag_failure(self):
        ...
        with patch.dict(
            ...
            with (
                patch(
                    "tools.seed_database.LocalSearchRAG",
    ...
    def test_seed_all_skips_init_and_helpers(self):
        ...
        with patch.dict(
            ...
            with (
                patch(
                    "tools.seed_database.os.listdir",
                    return_value=["__init__.py", "helpers.py"],
                ),
                patch("tools.seed_database.LocalSearchRAG") as mock_rag,
                patch("tools.seed_database.logger"),
            ):
    ...
    def test_seed_all_module_load_error(self):
        ...
        with patch.dict(
            ...
            with (
                patch(
                    "tools.seed_database.os.listdir", return_value=["test_module.py"]
                ),
                patch("tools.seed_database.os.path.exists", return_value=True),
                patch(
                    "tools.seed_database.importlib.util.spec_from_file_location",
                    return_value=None,
                ),
                patch("tools.seed_database.LocalSearchRAG") as mock_rag,
                patch("tools.seed_database.logger"),
    ...
    def test_seed_all_module_exec_error(self):
        ...
        with patch.dict(
            ...
            with (
                patch("tools.seed_database.os.path.exists", return_value=True),
                patch(
                    "tools.seed_database.os.listdir", return_value=["broken_module.py"]
                ),
                patch("tools.seed_database.LocalSearchRAG") as mock_rag,
                patch("tools.seed_database.logger"),
                patch(
                ...
                mock_spec_obj.loader.exec_module.side_effect = ImportError(
                    "Module broken"
                )
                result = seed_all()
                # seed_all কোনো রিটার্ন ভ্যালু দেয় না (None রিটার্ন করে)
