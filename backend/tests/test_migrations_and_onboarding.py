...
class TestMigrationFiles:
    ...
    def test_referral_schema_has_required_tables(self):
        ...
        required_tables = ["referral_codes", "referral_redemptions", "credit_wallets", "credit_ledger"]
    ...
    def test_tenant_schema_has_required_tables(self):
        ...
        required_tables = ["tenant_limits", "sso_configs", "offline_sync_logs", "tenant_usage"]
        for table in required_tables:
...
class TestOnboardingAPI:
    ...
    async def test_complete_onboarding_endpoint(self):
        ...
        with patch("api.routes.onboarding._validate_api_key", new_callable=AsyncMock, return_value=True):
            with patch("api.routes.onboarding._save_user_preferences", return_value=True):
    ...
    async def test_onboarding_invalid_key_still_succeeds(self):
        ...
        with patch("api.routes.onboarding._validate_api_key", new_callable=AsyncMock, return_value=False):
            with patch("api.routes.onboarding._save_user_preferences", return_value=True):
                result = await complete_onboarding(payload)
...
class TestViralReferralEngine:
    def test_generate_referral_code_format(self):
        ...
        with patch("tools.viral_referral_engine.db") as mock_db:
            ...
            with patch.object(engine, "_save_local"):
                with patch.object(engine, "_load_local", return_value={"codes": {}, "wallets": {}}):
    ...
    def test_calculate_reward_bronze(self):
        ...
        with patch("tools.viral_referral_engine.db") as mock_db:
            mock_db.client = None
            with patch.object(engine, "_load_local", return_value={"codes": {}, "wallets": {}, "redemptions": [{"referrer_id": "u1"}]}):
    ...
    def test_fraud_detection_same_ip(self):
        ...
        fake_history = [
            {"referrer_id": "u1", "new_user_id": f"new{i}", "created_at": time.time(), "metadata": {"ip_address": "1.2.3.4"}}
            for i in range(3)
        ]
        with patch("tools.viral_referral_engine.db") as mock_db:
            mock_db.client = None
            with patch.object(engine, "_load_local", return_value={"codes": {}, "wallets": {}, "redemptions": fake_history}):
