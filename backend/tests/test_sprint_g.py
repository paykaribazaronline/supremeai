    ...
    async def test_synthesize_no_key_uses_fallback(self):
        ...
        with patch.object(tts, "_edge_tts", new_callable=AsyncMock) as mock_edge:
            mock_edge.return_value = {
                ...
                "provider": "edge-tts", "audio_path": "/tmp/test.mp3",
                "text_length": 5,
            }
    ...
    async def test_synthesize_elevenlabs_success(self):
        ...
        with patch.object(tts, "_elevenlabs", new_callable=AsyncMock) as mock_el:
            mock_el.return_value = {
                "status": "success", "language": "bn", "provider": "elevenlabs",
                "audio_path": "/tmp/tts_bn_abc.mp3", "text_length": 10,
            }
...
class TestCommentThreadAI:
    ...
    async def test_handle_pr_comment_posts_to_github(self):
        ...
        with patch.object(ai, "_llm", new_callable=AsyncMock, return_value="Fix: use None check"):
            with patch.object(ai, "_post_pr_comment", new_callable=AsyncMock) as mock_post:
                mock_post.return_value = {"status": "success", "comment_url": "https://github.com/..."}
                result = await ai.handle_pr_comment(
                    repo_full_name="owner/repo",
                    pr_number=1,
    ...
    async def test_webhook_responds_to_trigger(self):
        ...
        with patch.object(ai, "handle_pr_comment", new_callable=AsyncMock) as mock_handle:
            mock_handle.return_value = {"status": "success", "action": "code_fix_proposed"}
            result = await ai.handle_github_webhook({
                "action": "created",
                "comment": {"body": "@supremeai fix this bug", "path": "main.py", "line": 10, "id": 123},
                "repository": {"full_name": "owner/repo"},
...
class TestTenantAdminAPI:
    ...
    async def test_create_tenant_applies_tier_defaults(self):
        ...
        _local_store.clear()
        with patch("api.routes.tenant_admin._get_db", return_value=None):
            with patch("api.routes.tenant_admin._db_get_tenant", new_callable=AsyncMock, return_value=None):
    ...
    async def test_update_tenant_changes_tier(self):
        ...
        existing = {
            "tenant_id": "my-org", "org_name": "My Org",
            "billing_tier": "free", "requests_per_minute": 20,
            "max_tokens_per_day": 50000, "max_concurrent_sessions": 2,
        }
        with patch("api.routes.tenant_admin._db_get_tenant", new_callable=AsyncMock, return_value=existing):
            with patch("api.routes.tenant_admin._db_upsert_tenant", new_callable=AsyncMock, return_value=True):
                with patch("api.routes.tenant_admin._get_db", return_value=None):
                    try:
    ...
    async def test_delete_tenant(self):
        ...
        with patch("api.routes.tenant_admin._db_get_tenant", new_callable=AsyncMock, return_value=existing):
            with patch("api.routes.tenant_admin._db_delete_tenant", new_callable=AsyncMock, return_value=True):
