...
def test_moe_prompt_classification():
    ...
    # Reasoner prompts
    assert (
        SupremeMoERouter.classify_prompt(
            "Calculate the theorem proof for calculus equation"
        )
        == ExpertType.REASONER
    )
    ...
    # General prompts
    assert (
        SupremeMoERouter.classify_prompt("What is the capital of France?")
        == ExpertType.GENERAL
    )
...
def test_moe_model_chain_generation():
    ...
    coder_chain = SupremeMoERouter.get_model_chain(
        "import numpy as np; print(np.array([1,2]))"
    )
...
async def test_llm_gateway_moe_integration(monkeypatch):
    ...
    res = await gateway.async_generate(
        "Explain python list comprehension", use_moe=True
    )
