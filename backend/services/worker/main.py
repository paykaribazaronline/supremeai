...
async def poll_and_execute() -> None:
    ...
    try:
        ...
        logger.error("redis package not installed"); sys.exit(1)
    ...
    while True:
        ...
        except Exception as e: logger.error(f"poll error: {e}"); await asyncio.sleep(POLL_INTERVAL)


async def _execute(task: dict, redis) -> None:
    await asyncio.sleep(1)
    result = {"task_id": task.get("task_id"), "status": "completed",
              "result": {"message": f"processed by {WORKER_ID}"}, "completed_at": time.time()}
...
async def heartbeat() -> None:
    ...
    try:
        ...
        r = aioredis.from_url(REDIS_URL, decode_responses=True)
        while True:
            await r.hset("ecosystem:workers", WORKER_ID, json.dumps({"id": WORKER_ID, "last_heartbeat": time.time()}))
            await asyncio.sleep(10)
    except Exception as e: logger.debug(f"heartbeat: {e}")


async def health_server() -> None:
    from aiohttp import web
    async def health(request): return web.json_response({"status": "ok", "worker_id": WORKER_ID})
    app = web.Application(); app.router.add_get("/health", health); app.router.add_get("/", health)
    runner = web.AppRunner(app); await runner.setup()
    site = web.TCPSite(runner, "0.0.0.0", int(os.getenv("PORT", "8001"))); await site.start()
