...
from models.sentinel import SystemIncident, ApiEndpoint, SystemDependency
                ...
                async with httpx.AsyncClient(timeout=10.0) as client:
                    for ep in endpoints:
                        ...
                        try:
                            ...
                            ep.last_check_at = datetime.now(timezone.utc)
    ...
    async def audit_dependencies(self):
        ...
        try:
            async with AsyncSessionLocal() as session:
                ...
                for dep in deps:
                    dep.last_audit_at = datetime.now(timezone.utc)
                await session.commit()
        except Exception as e:
