...
class Settings(BaseSettings):
    ...
    target_repo_branch: str = Field(
        default="main", validation_alias="TARGET_REPO_BRANCH"
    )
    target_repo_token: SecretStr = Field(
        default=SecretStr(""), validation_alias="TARGET_REPO_TOKEN"
    )
    workspace_base_dir: str = Field(
        default="storage/workspaces", validation_alias="WORKSPACE_BASE_DIR"
    )
    ...
    cost_tier_economy_limit: float = Field(
        default=0.02, validation_alias="COST_TIER_ECONOMY_LIMIT"
    )
    cost_tier_premium_limit: float = Field(
        default=0.50, validation_alias="COST_TIER_PREMIUM_LIMIT"
    )
    ...
    def validate_cors_origins(cls, v: list[str], info: ValidationInfo) -> list[str]:
        ...
        if env in {"production", "staging"}:
            ...
            if (
                field in {"user_cors_origins", "admin_cors_origins", "cors_origins"}
                or not field
            ):
