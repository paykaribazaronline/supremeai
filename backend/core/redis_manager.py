...
class SecureRedisManager:
    ...
    async def is_rate_limited(self, key: str, max_requests: int, window_seconds: int) -> bool:
        ...
        if self.client is None:
            ...
            raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Security infrastructure unavailable. Request safely denied.")
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Authentication and rate verification engine failure. Access blocked."
            ) from redis_err
