...
class LLMGateway:
    ...
    def _load_routing_policy(self) -> dict[str, Any]:
        ...
        return {
            "complexity_rules": {"easy": ["groq/llama-3.3-70b-versatile"], "medium": ["gemini/gemini-3.5-flash"], "hard": ["openai/gpt-4o-mini"]},
    ...
    def _setup_callbacks(self):
        ...
        def failure_callback(kwargs, exception_obj, start_time, end_time):
            model = kwargs.get("model", "unknown")
            logger.error(f"🔴 [LLMGateway Failure] Model: {model} failed! | Error: {str(exception_obj)} | " f"Duration: {end_time - start_time:.2f}s")

        litellm.success_callback = [success_callback]
        litellm.failure_callback = [failure_callback]

    async def acompletion(
        self, prompt: str | list[dict[str, Any]], task_type: str = "general", stream: bool = False, timeout: float = 12.0, model: str | None = None
    ) -> Any:
        """
