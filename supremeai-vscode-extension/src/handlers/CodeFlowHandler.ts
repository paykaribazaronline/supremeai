...
export class CodeFlowHandler {
  ...
  public register(): void {
    this.context.subscriptions.push(
      ...
      vscode.workspace.onDidSaveTextEditor(this.onFileSave.bind(this)),
      this.statusBarItem
    );
   ...
   */
  private async onFileSave(e: vscode.TextDocumentWillSaveEvent): Promise<void> {
    const config = vscode.workspace.getConfiguration('supremeai');
    const autoAnalyze = config.get<boolean>('autoAnalyzeOnSave', false);

    if (autoAnalyze && (e.document.languageId === 'javascript' || e.document.languageId === 'typescript')) {
      // Debounced analysis
      setTimeout(() => {
        this.analyzeCodeFlow(e.document.uri);
      }, 1000);
    }
  }
