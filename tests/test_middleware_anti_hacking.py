...
from backend.middleware.anti_hacking import (AntiHackingContextMiddleware,
                                             _octet3)
...
async def test_dispatch_admin_request_no_previous_context():
    ...
    with patch("backend.middleware.anti_hacking.redis_manager") as mock_redis_manager:
        ...
        mock_redis_manager.get_cache = AsyncMock(
            return_value=None
        )  # No previous context
        ...
        with patch("backend.middleware.anti_hacking.send_otp") as mock_send_otp:
            # Mock call_next to return a simple response
            call_next = AsyncMock(
                return_value=JSONResponse(content={"test": "response"})
            )
...
async def test_dispatch_admin_request_matching_context():
    ...
    with patch("backend.middleware.anti_hacking.redis_manager") as mock_redis_manager:
        ...
        mock_redis_manager.get_cache = AsyncMock(
            return_value=json.dumps(previous_context)
        )
        mock_redis_manager.set_cache = AsyncMock()
...
async def test_dispatch_admin_request_context_mismatch_alert_only():
    ...
    request = Request(
        {
            ...
            "headers": [
                (
                    b"x-forwarded-for",
                    b"192.168.2.100",
                ),  # Different IP + different subnet
                (b"cf-ipcountry", b"US"),
                (
                    b"user-agent",
                    b"completely-different-agent",
                ),  # Different UA → OTP fires
    ...
    with patch("backend.middleware.anti_hacking.settings") as mock_settings:
        ...
        with patch(
            "backend.middleware.anti_hacking.redis_manager"
