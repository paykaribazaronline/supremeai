...
from backend.services.internet_monitor_service import (
    InternetMonitorService, get_internet_monitor_service,
    initialize_internet_monitor_service, internet_monitor_service,
    start_internet_monitoring, stop_internet_monitoring)
...
async def test_start_monitoring_already_running():
    ...
    with patch("backend.services.internet_monitor_service.logger") as mock_logger:
        ...
        mock_logger.warning.assert_called_once_with(
            "Internet monitoring is already running"
        )
...
async def test_start_monitoring_exception():
    ...
    service.agent.start_monitoring_loop = MagicMock(
...
async def test_stop_monitoring_when_not_running():
    ...
    with patch("backend.services.internet_monitor_service.logger") as mock_logger:
        ...
        # Verify that a warning was logged
        mock_logger.warning.assert_called_once_with(
            "Internet monitoring is not running"
        )
