...
from backend.core.health_check import (ComprehensiveHealthChecker,
...
async def test_check_database_exception():
    ...
    with patch(
        "backend.core.health_check.time.time", side_effect=Exception("DB Error")
    ):
        result = await checker.check_database()
        assert result.status == HealthStatus.UNHEALTHY
        assert "failed" in result.message
...
async def test_check_external_services_exception():
    ...
    with patch("backend.core.health_check.settings") as mock_settings:
        type(mock_settings).gemini_api_key = PropertyMock(
...
async def test_check_all_comprehensive():
    ...
    with patch.object(checker, "check_application") as mock_app, patch.object(
        checker, "check_redis"
    ) as mock_redis, patch.object(checker, "check_database") as mock_db, patch.object(
...
async def test_check_all_with_degraded_component():
    ...
    with patch.object(checker, "check_application") as mock_app, patch.object(
        ...
        mock_redis.return_value = HealthCheckResult(
            HealthStatus.DEGRADED, "Redis degraded"
        )
        mock_db.return_value = HealthCheckResult(HealthStatus.HEALTHY, "DB OK")
        mock_ext.return_value = HealthCheckResult(HealthStatus.HEALTHY, "Ext OK")
        mock_mem.return_value = HealthCheckResult(HealthStatus.HEALTHY, "Mem OK")
...
async def test_check_all_with_unhealthy_component():
    ...
    with patch.object(checker, "check_application") as mock_app, patch.object(
        ...
        mock_app.return_value = HealthCheckResult(HealthStatus.HEALTHY, "App OK")
        mock_redis.return_value = HealthCheckResult(
            HealthStatus.UNHEALTHY, "Redis failed"
        )
        mock_db.return_value = HealthCheckResult(HealthStatus.HEALTHY, "DB OK")
        mock_ext.return_value = HealthCheckResult(HealthStatus.HEALTHY, "Ext OK")
        mock_mem.return_value = HealthCheckResult(HealthStatus.HEALTHY, "Mem OK")
...
async def test_check_all_with_exception():
    ...
    with patch.object(
        checker, "check_application", new_callable=AsyncMock
        ...
        checker, "check_database", new_callable=AsyncMock
    ) as mock_db, patch.object(
        checker, "check_external_services", new_callable=AsyncMock
    ) as mock_ext, patch.object(
        checker, "check_memory", new_callable=AsyncMock
    ) as mock_mem, patch.object(
        checker, "check_disk", new_callable=AsyncMock
    ) as mock_disk:
