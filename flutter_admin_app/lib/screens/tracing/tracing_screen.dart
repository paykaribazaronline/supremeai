...
class _TracingScreenState extends State<TracingScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadAll() async {
    ...
    setState(() {
      ...
      if (results[1].success) _recentTraces = (results[1].data as List<dynamic>?) ?? [];
      if (results[2].success) _errorTraces = (results[2].data as List<dynamic>?) ?? [];
      if (!results[0].success) _error = results[0].error ?? 'ট্রেসিং তথ্য লোড করা যায়নি';
    });
  }
    ...
    return Scaffold(
      ...
      body: _isLoading
          ...
          : _error != null
              ...
              : TabBarView(
                  controller: _tabController,
                  children: [_buildStatsTab(), _buildRecentTab(), _buildErrorsTab()],
                ),
    );
  }
        ...
        child: Column(
          children: [
            Card(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Row(children: [
                      Icon(Icons.route, color: Color(AppConstants.primaryColor)),
                      const SizedBox(width: 8),
                      const Text('ট্রেসিং পরিসংখ্যান', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                    ]),
                    const Text('(প্রতিটি API কল কতটা সময় নিচ্ছে ও কোথায় সমস্যা হচ্ছে)',
            ...
            GridView.count(
              ...
              crossAxisSpacing: 8,
              children: [
                _buildStatCard('মোট ট্রেস', '${s['totalTraces'] ?? 0}', Icons.timeline, AppConstants.primaryColor),
                _buildStatCard('ত্রুটি ট্রেস', '${s['errorTraces'] ?? 0}', Icons.error, AppConstants.errorColor),
                _buildStatCard('গড় সময়', '${s['avgDuration'] ?? 0}ms', Icons.speed, AppConstants.infoColor),
                _buildStatCard('সক্রিয় পাথ', '${s['activePaths'] ?? 0}', Icons.route, AppConstants.successColor),
              ],
            ),
          ],
        ),
    ...
    return RefreshIndicator(
      ...
      child: _recentTraces.isEmpty
          ? const Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              Text('(API কল হলে এখানে দেখা যাবে)', style: TextStyle(fontSize: 12, color: Colors.grey)),
            ]))
          : ListView.builder(
    ...
    return RefreshIndicator(
      ...
      child: _errorTraces.isEmpty
          ? const Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              Text('(ভালো খবর - কোনো সমস্যা নেই)', style: TextStyle(fontSize: 12, color: Colors.grey)),
            ]))
          : ListView.builder(
              padding: const EdgeInsets.all(AppConstants.paddingMedium),
  ...
  Widget _buildTraceCard(dynamic trace, bool isError) {
    ...
    final traceId = '${map['traceId'] ?? map['id'] ?? ''}'.length > 12
        ? '${map['traceId'] ?? map['id'] ?? ''}'.substring(0, 12) : '${map['traceId'] ?? map['id'] ?? ''}';
    ...
    final isSuccess = status.startsWith('2') || status == 'SUCCESS' || status == 'OK';

    return Card(
      margin: const EdgeInsets.only(bottom: 8),
      child: ExpansionTile(
        leading: CircleAvatar(
          backgroundColor: (isError || !isSuccess ? Colors.red : Colors.green).withOpacity(0.1),
          child: Icon(isError || !isSuccess ? Icons.error : Icons.check,
              color: isError || !isSuccess ? Colors.red : Colors.green, size: 18),
        ),
        title: Row(children: [
          Container(
            ...
            child: Text(method, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.blue)),
          ),
          const SizedBox(width: 8),
          Expanded(child: Text(path, style: const TextStyle(fontSize: 13), overflow: TextOverflow.ellipsis)),
        ]),
        subtitle: Text('$duration • $traceId', style: const TextStyle(fontSize: 11)),
        children: [
          Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                _buildDetailRow('ট্রেস ID', '${map['traceId'] ?? map['id'] ?? 'N/A'}'),
                ...
                if (map['error'] != null) _buildDetailRow('ত্রুটি', '${map['error']}'),
              ],
            ),
          ),
        ],
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(child: Text(value, style: TextStyle(fontSize: 12, fontFamily: label.contains('ID') ? 'monospace' : null))),
        ],
      ),
    );
  }
