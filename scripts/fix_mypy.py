...
fix(
    ...
    "def __init__(self, db_path: str = None, config: SlidingWindowConfig = None):",
    "def __init__(self, db_path: typing.Optional[str] = None, config: typing.Optional[SlidingWindowConfig] = None):",
)
fix_re(
    "backend/memory/sliding_window.py",
    r"^import sqlite3\n",
...
fix(
    "backend/core/gcp_pubsub_queue.py",
...
fix(
    "backend/tools/gcp_cloud_functions.py",
    'os.getenv("GCP_PROJECT_ID").rstrip',
    '(os.getenv("GCP_PROJECT_ID") or "").rstrip',
)

fix(
    "backend/core/semantic_cache.py",
    'vector = json.loads(row["vector"]) if row["vector"] else None',
    'vector = json.loads(str(row["vector"])) if row["vector"] else None',
)

fix(
    "backend/tools/local_search_rag.py",
    "query_tf = {}",
...
fix(
    "backend/tools/local_search_rag.py",
    "self.index.upsert",
    "if self.index: self.index.upsert",
)
fix_re(
    "backend/tools/local_search_rag.py",
    r"^import math\n",
...
fix(
    ...
    "LocalSearchRAGClass: typing.Any = None",
)
fix(
    "backend/api/routes/knowledge.py",
    "KnowledgeBaseIndexerClass = None",
    "KnowledgeBaseIndexerClass: typing.Any = None",
)
fix(
    "backend/api/routes/knowledge.py",
    "except ImportError:\n    sqlite3 = None",
...
fix_re(
    "backend/core/gcp_firestore.py", r"^import json\n", "import json\nimport typing\n"
)
...
fix(
    ...
    "self.http_client: typing.Any = None",
)

fix(
    "backend/core/auto_remediation.py",
...
fix_re(
    "backend/brain/model_router.py", r"^import json\n", "import json\nimport typing\n"
)
