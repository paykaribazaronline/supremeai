...
PROJECT_DIR = os.path.abspath(sys.argv[1] if len(sys.argv) > 1 and not sys.argv[1].startswith('--') else ".")
DRY = "--dry" in sys.argv
...
def scan(fp):
    ...
    for i, line in enumerate(lines):
        ...
        if s.startswith('import static'): continue
        if '.*' in s:                     continue
    ...
    for i in ib:
        ...
        if fqn.endswith('.*'):
            ...
            pat1 = re.compile(r'(?<![.\w\d])' + re.escape(simple) + r'(?=[\s\(\)\{\}\,\;\<\>\/\.\:]|$)')
...
for fp in sorted(set(
    ...
    if idxs:
        with open(fp) as f:
            flines = f.readlines()
        all_fixes.append((os.path.relpath(fp, PROJECT_DIR), idxs,
                          [(i+1, flines[i].strip()) for i in sorted(idxs)]))

total = sum(len(r) for _, _, r in all_fixes)
if DRY:
    print(f"[DRY-RUN] {len(all_fixes)} files, {total} unused import lines would be removed:\n")
