...
def collect_unused(fp, member_idx, dry=False):
    ...
    for i, line in enumerate(lines):
        ...
        if re.match(r'^import\s+(?:static\s+)?[\w.]+\s*;\s*$', line_stripped) and '.*' not in line_stripped:
            import_set.add(i)
    if not import_set:
        return set(), []
    ...
    bt = ''.join(
        ...
        if i not in import_set
        and not lines[i].strip().startswith('package ')
    )
...
def main():
    ...
    print(f"=== [{ 'DRY RUN' if DRY else 'APPLIED' }] {ne} files, {nr} unused import lines removed ===\n")
