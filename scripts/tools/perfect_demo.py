...
from scripts.supremeai_mcp_server import (
    SupremeAIMCP,
    general_chat,
    generate_code,
    review_code,
...
async def demo_mcp_tools():
    ...
    code_result = await generate_code(
        "python", "Create a class for managing a todo list"
    )
...
"""
    ...
    print("\n3. General Chat Tool:")
    chat_result = await general_chat(
        "What is machine learning?", "I'm learning about AI"
    )
...
async def demo_system_health():
    ...
    print(
        f"API Rotation Health: {health_data.get('api_rotation', {}).get('system_health', 0):.1f}%"
    )

    api_status = health_data.get("api_rotation", {})
    print(
        f"Active Accounts: {api_status.get('active_accounts', 0)}/{api_status.get('total_accounts', 0)}"
    )
...
async def demo_failover():
    ...
    if used_provider in rotator.providers:
        ...
        rotator.providers[used_provider].status = rotator.providers[
            used_provider
        ].status.FAILED
