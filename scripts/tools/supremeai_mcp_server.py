...
class SupremeAIMCP:
    ...
    def _load_vm_model_config(self) -> dict:
        ...
        candidates = [
            os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "vm_models_config.json"
            ),
            os.path.join(
                os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                "vm_models_config.json",
            ),
        ]
        ...
        logger.warning(
            "[MCP] No vm_models_config.json found — VM model routing unavailable"
        )
        return {}
    ...
    async def execute_with_strategy(
        self, user_input: str, intent: dict, strategy: dict
    ) -> dict:
        ...
        return {
            ...
            "knowledge_base": {
                ...
                "user_preferences": len(
                    self.knowledge_base.get("user_preferences", {})
                ),
            },
            "overall_health": self._calculate_overall_health(
                vm_status, rotation_status
            ),
        }
    ...
    def _calculate_overall_health(
        self, vm_status: dict, rotation_status: dict
    ) -> float:
        ...
        if method == "initialize":
            return {
                ...
                "result": {
                    ...
                    "serverInfo": {"name": "supremeai-core", "version": "1.0.0"},
                },
            }
        ...
        if method == "tools/call":
            ...
            try:
                ...
                return {
                    ...
                    "error": {
                        "code": -32602,
                        "message": f"Bad arguments for {tool_name}: {e}",
                    },
                }
            except Exception as e:
...
async def generate_code(language: str, description: str, complexity: str = "medium"):
    ...
    result = await mcp.execute_with_strategy(
        f"Write {language} code: {description}", intent, strategy
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test", action="store_true", help="Run internal tests instead of MCP server"
    )
