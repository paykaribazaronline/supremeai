...
def get_directory_tree(root, include_contents, max_file_size):
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        tree_lines.append(
            f"{indent}{rel_dir.name if rel_dir.name != '.' else 'project-root'}/"
        )
...
def main():
    ...
    print(
        f"Directories/files scanned. Found {len(files_data)} files with content to include.",
        flush=True,
    )

    markdown = build_markdown(root, tree_lines, files_data, extensions, args.contents)
