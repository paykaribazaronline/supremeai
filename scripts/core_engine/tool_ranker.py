...
class ToolRanker:
    ...
    def rank_results(
        ...
        self.logger.info(
            f"Ranking complete. Top score: {ranked_results[0].total_score:.3f}"
            if ranked_results
    ...
    def _calculate_factor_scores(
        self, result: "SearchResult", query: str, context: dict[str, Any] = None
    ) -> dict[str, float]:
        """
        ...
        )  # Already calculated by search engine
    ...
    def _calculate_popularity_score(
        self, item: dict[str, Any], source: "CatalogSource"
    ) -> float:
        ...
        if source == CatalogSource.AWESOME_SELFHOSTED:
            ...
            if isinstance(stars, (int, float)) and stars > 0:
                ...
                score = min(
                    1.0, math.log10(max(stars, 1)) / math.log10(10000)
                )  # Normalize to 10k stars
            else:
    ...
    def _calculate_quality_score(
        ...
        has_tests = bool(
            item.get("tests") or item.get("testing") or item.get("test_coverage")
        )
    ...
    def _calculate_community_score(
        ...
        if isinstance(forks, (int, float)) and forks > 0:
            score += min(
                0.3, math.log10(max(forks, 1)) / math.log10(1000)
            )  # Up to 0.3 for 1k forks
        if isinstance(contributors, (int, float)) and contributors > 0:
            score += min(
                0.2, math.log10(max(contributors, 1)) / math.log10(100)
            )  # Up to 0.2 for 100 contributors
        if isinstance(watchers, (int, float)) and watchers > 0:
            score += min(
                0.2, math.log10(max(watchers, 1)) / math.log10(1000)
            )  # Up to 0.2 for 1k watchers
    ...
    def _calculate_maturity_score(
        ...
        # Look for version/maturity indicators
        version = (
            item.get("version") or item.get("release") or item.get("current_version")
        )
        ...
        if any(
            stable_indicator in status
            for stable_indicator in ["stable", "production", "lts", "release"]
        ):
            score += 0.2
...
def demo_ranker():
    ...
    for i, result in enumerate(search_results[:5], 1):
        print(
            f"  {i}. [{result.source.value}] {result.name} (score: {result.relevance_score:.2f})"
        )
    ...
    for i, ranked_result in enumerate(ranked_results[:5], 1):
        ...
        print(
            f"     Factors: P{ranked_result.factor_scores.get('popularity', 0):.2f} "
