...
def run_radon_cc() -> str:
    ...
    try:
        result = subprocess.run(
            cmd, capture_output=True, text=True, timeout=180  # 3 minute timeout
        )
...
def generate_report(
    dead_code: list, complex_functions: list, low_maintainability: list
) -> str:
    ...
    report = rf"""# Dead Code and Code Quality Report
...
"""
    ...
    if dead_code:
        ...
        report += (
            "3. Consider running tests after removal to ensure nothing breaks.\n\n"
        )
    ...
    if complex_functions:
        ...
        report += (
            "3. Look for opportunities to extract helper functions or classes.\n\n"
        )
...
def main() -> int:
    ...
    print(rf"📂 Scanning directories: {', '.join(TARGET_DIRS)}")
    print(rf"🚫 Excluding patterns: {', '.join(EXCLUDE)}")
    print(rf"🎯 Minimum confidence: {MIN_CONFIDENCE}%")
    ...
    print(rf"📄 Report saved to: {output_path}")

    # Print summary
    print(r"\n📈 Summary:")
    print(rf"   🐛 Dead code items: {len(dead_code)}")
    print(rf"   ⚠️  Complex functions: {len(complex_functions)}")
    print(rf"   📉 Low maintainability: {len(low_maintainability)}")
