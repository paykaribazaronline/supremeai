...
def generate_docs():
    ...
    try:
        commits = (
            subprocess.check_output(["git", "log", "-n", "10", "--format=%H"])
            .decode()
            .splitlines()
        )
        for commit in commits:
            try:
                ...
                (changes_dir / f"change_{commit}.md").write_text(
    ...
    change_files = sorted(
        changes_dir.glob("change_*.md"), key=os.path.getmtime, reverse=True
    )
