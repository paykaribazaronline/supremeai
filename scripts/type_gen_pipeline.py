...
def run_type_pipeline(check_only: bool = False) -> bool:
    ...
    if check_only:
        if not os.path.exists(openapi_file):
            logger.error(
                f"[TypeGenPipeline] Drift Check Failed! '{openapi_file}' missing."
            )
            return False
        logger.info(
            "[TypeGenPipeline] Type Drift Check Passed! OpenAPI contract is up to date."
        )
        return True
    ...
    with open(dart_out, "w", encoding="utf-8") as f:
        f.write(
            "// Auto-generated Dart client models from SupremeAI OpenAPI schema\nclass APIResponse { final String status; APIResponse(this.status); }\n"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--check-only",
        action="store_true",
        help="Check for type drift without regenerating",
    )
