    ...
    def load_rules(self):
        ...
        return {
            "risk_rules": {
                ...
                "HIGH": [
                    {
                        "pattern": r"DATABASE_URL=.*localhost",
                        "message": "Local DB in staging+",
                    },
                    {
                        "pattern": r"REDIS_URL=.*localhost",
                        "message": "Local Redis in staging+",
                    },
                    {
                        "pattern": r"LOG_LEVEL=debug",
                        "message": "Verbose logging in prod",
                    },
                ],
                "MEDIUM": [],
            },
        ...
        for i, env1 in enumerate(self.envs):
            for env2 in self.envs[i + 1 :]:
                if env1 in env_configs and env2 in env_configs:
                    self.compare_envs(env1, env_configs[env1], env2, env_configs[env2])
                        ...
                        self.issues.append(
                            {
                                "risk": risk_level,
                                "file": (
                                    f".env.{env}" if env != "development" else ".env"
                                ),
