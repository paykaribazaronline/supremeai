...
for service in SERVICES:
    ...
    if get_res.status_code == 200:
        ...
        print(
            f"  ⚠️ Warning: Could not fetch existing env vars for {name} (HTTP {get_res.status_code}). Proceeding with local values."
        )
    ...
    payload = [
        {"key": k, "value": v} for k, v in merged_envs.items() if k and v is not None
    ]
    ...
    if put_res.status_code == 200:
        ...
        print(
            f"  ❌ Failed to update {name}: HTTP {put_res.status_code} - {put_res.text}"
        )
