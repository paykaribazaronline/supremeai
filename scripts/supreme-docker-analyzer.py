...
class SupremeDockerAnalyzer:
    ...
    def load_limits(self):
        ...
        if limits_path.exists():
            with open(limits_path, "r") as f:
                ...
                return data.get("limits", {}).get(
                    self.service_name, data.get("limits", {}).get("default", {})
                )
    ...
    def analyze(self):
        ...
        try:
            ...
            return {
                "status": "FAIL",
                "reason": f"Could not inspect image: {e}",
                "size_mb": 0,
            }
        ...
        try:
            history_output = subprocess.check_output(history_cmd, shell=False).decode(
                "utf-8"
            )
            layers = []
