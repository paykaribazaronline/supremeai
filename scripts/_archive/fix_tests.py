...
replace_in_file(
    f"{tests_dir}/test_style_learner.py", "extract_style_guidelines", "analyze_codebase"
)
replace_in_file(
    f"{tests_dir}/test_sprint_c_tools.py",
    "extract_style_guidelines",
    "analyze_codebase",
)
...
def fix_diagram_test():
    ...
    content = content.replace(
        'patch.object(converter, "_get_vision_client")',
...
def fix_blockchain_test():
    ...
    with open(path, "r", encoding="utf-8") as f:
        content = f.read()
    content = content.replace(
        'patch.object(agent, "_get_model_router")',
        'patch("brain.model_router.ModelRouter.async_route_and_generate")',
    )
    content = content.replace(
        "mock_router.return_value.async_route_and_generate.return_value",
...
def fix_pr_reviewer_test():
    ...
    content = content.replace(
        "assert 'style_issues' in []", "assert 'style_issues' not in []"
    )
    with open(path, "w", encoding="utf-8") as f:
        f.write(content)
