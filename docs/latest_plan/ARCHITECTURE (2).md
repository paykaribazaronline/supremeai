...
Neural Chat follows a strict 4-level "Local-First, Cloud/Web-Fallback" architecture to ensure 100% uptime and the highest accuracy for user queries.

```mermaid
flowchart TD
    User[User Message] --> AQE{Autonomous Questioning Engine\n(Intent & Context Router)}
    
    AQE -->|GREETING| L0[Level 0: Instant Local Response]
    AQE -->|AMBIGUOUS| L1[Level 1: Interactive Clarification]
    AQE -->|FACTUAL / TASK| L2_RAG[Level 2: Pure Java NLP RAG]
    
    L2_RAG --> CheckSim{Similarity Score > 0.55?}
    CheckSim -->|Yes| L2_Answer[Primary AI Generates Answer from Local DB]
    CheckSim -->|No| MagicLoop[Level 3: The Magic Loop]
    
    MagicLoop --> Parallel{Parallel Execution}
    Parallel --> |Local NLP Search| L2_Cache[KnowledgeService: N-Gram Similarity]
    Parallel --> |Semantic Router| L3_Scrape[Primary AI Selects Target URL -> Hybrid Browser (Jsoup+Playwright) Scrapes]
    L2_Cache --> Combine[Combine Local Memory + Web Data]
    L3_Scrape --> Combine
    Combine --> L3_Answer[Primary AI Synthesizes Final Answer]
    L3_Answer --> SaveDB[Auto-Learn: Save to System Learning]
    
    L3_Answer --> Fallback{If AI Fails}
    L2_Answer --> Fallback
    Fallback -->|Failover / Timeout| L4[Level 4: Multi-AI Routing/Voting Council]
```
