...
# ── Knowledge & Continuous-Learning Endpoints ──────────────────────────────
# Implements: continuous-knowledge-improvement.md + supremeai-agent-learning-rules.md

@app.route('/api/knowledge/lookup', methods=['POST'])
def knowledge_lookup():
    """Search the local knowledge base for a matching answer. (Zero-AI mode)"""
    data = request.json or {}
    message = data.get('message', '')

    if not message:
        return jsonify({"success": False, "message": "message field required"}), 400

    kb_answer = chat_system.knowledge.lookup(message)

    if not kb_answer:
        return jsonify({
            "success": True,
            "found": False,
            "message": "No local knowledge entry matched this query "
                       "(knowledge-gap flag stored internally)"
        })
    ...
    return jsonify({
        ...
        "found":     True,
        "match": {
            "source":   kb_answer.get("source", "core_kb"),
            "task":     kb_answer.get("task", ""),
            "solution": kb_answer.get("solution", ""),
            "confidence": kb_answer.get("confidence", 0.0),
            "match_score": kb_answer.get("match_score", 0.0),
        }
    })


@app.route('/api/knowledge/correction', methods=['POST'])
def knowledge_correction():
    """Capture a user correction to grow the knowledge base. Per rules §1 #4."""
    data = request.json or {}

    for field in ("original_message", "incorrect_response", "corrected_by", "corrected_content"):
        if field not in data:
            return jsonify({"success": False, "message": f"'{field}' is required"}), 400

    try:
        entry = chat_system.capture_correction(
            original_message    = data["original_message"],
            incorrect_response  = data["incorrect_response"],
...
@app.route('/api/knowledge/error-pattern', methods=['POST'])
def knowledge_error_pattern():
    """Record an error-signature + fix pattern. Per rules §1 #1, #5."""
    data = request.json or {}

    for field in ("error_signature", "error_message", "context", "fix_applied"):
        if field not in data:
            return jsonify({"success": False, "message": f"'{field}' is required"}), 400

    try:
        entry = chat_system.capture_error_pattern(
...
def learning_stats():
    """Admin-only: Knowledge-engine health and growth metrics.
    Per have to keep eyes on supremeai plugin…md + continuous-knowledge rules.
    """
...
@app.route('/api/admin/learning/gaps')
def learning_gaps():
    """Admin-only: List open knowledge gaps that need attention."""
    if not request.args.get('token'):
        return jsonify({"success": False, "message": "Admin token required"}), 401

    gaps = [e for e in chat_system.knowledge.learning_log
            if e.get("type") == "knowledge_gap" and not e.get("resolved", False)]
    return jsonify({"success": True, "open_gaps": gaps, "count": len(gaps)})


@app.route('/api/admin/learning/resolve-gap', methods=['POST'])
def resolve_gap():
    """Admin-only: Mark a knowledge gap as resolved and optionally add solution."""
    data = request.json or {}
    query_hash = data.get("query_hash")
    resolved   = data.get("resolved", True)

    found = False
    for e in chat_system.knowledge.learning_log:
        if e.get("type") == "knowledge_gap" and e.get("query_hash") == query_hash:
            e["resolved"] = resolved
            found = True

    if found:
        chat_system.knowledge._save_learning_log()
        return jsonify({"success": True})

    return jsonify({"success": False, "message": "Gap not found"}), 404
