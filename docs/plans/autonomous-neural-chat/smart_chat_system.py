...
class SmartChatSystem:
    def __init__(self, db_path: str = None, kb_dir: str = None):
        ...
        Args:
            ...
            kb_dir:  নলেজ ম্যানেজার ডিরেক্টরি
                      (ডিফল্ট: প্ৰজেক্ট রূট — knowledge files reside there)
        """
        self.classifier = ChatClassifier()
        self.db_manager = DatabaseManager(db_path)

        # Default knowledge directory → parent of this package directory
        if kb_dir is None:
            kb_dir = str(Path(__file__).parent.parent)

        self.knowledge  = KnowledgeManager(kb_dir)
    ...
    def process_message(self, user_id: str, message: str, is_admin: bool = False) -> Dict[str, Any]:
        ...
        """
        # Track interaction for self-monitoring
        self.knowledge.record_interaction()

        # চ্যাট ডাটাবেজে সেভ করা
        chat_id = self.db_manager.save_chat(user_id, message, is_admin)

        # ── ① Local knowledge-base lookup (zero-AI offline answer) ──────────
        # Per continuous-knowledge-improvement.md: AI MUST search local knowledge
        # before returning any response.
        kb_answer = self.knowledge.lookup(message)
        if kb_answer:
            self.knowledge.local_fallback_hits += 1
            return {
                "chat_id":       chat_id,
                "message":       message,
                "chat_type":     "noreply",
                "confidence":    round(kb_answer.get("match_score", 0.0), 4),
                "reason":        "answered from local knowledge base",
        ...
        # Per continuous-knowledge-improvement.md §1 trigger #6, #9.
        gap = self.knowledge.check_and_log_gap(message)
    ...
    def capture_correction(self,
        ...
        """Record a user correction to build knowledge. Per knowledge rules §1 #4."""
        return self.knowledge.capture_correction(
            original_message, incorrect_response, corrected_by, corrected_content
        )
    ...
    def capture_error_pattern(self,
        ...
        """Record an error-signature + fix for future fallback. Per knowledge rules §1 #1."""
        return self.knowledge.capture_error_pattern(
            error_signature, error_message, context, fix_applied, success
        )

    def get_learning_stats(self) -> Dict[str, Any]:
        """Return health and growth metrics for the knowledge engine. Per rules §2 #13."""
