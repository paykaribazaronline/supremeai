...
const ProviderModal: React.FC<Props> = ({
  visible,
  editingProvider,
  onCancel,
  onSubmit,
  ...
  const fetchDiscoveredModels = async (query: string = "") => {
    ...
    try {
      const response = await authUtils.fetchWithAuth(
        `/api/admin/providers/discover?query=${query}`,
      );
      if (response.ok) {
  ...
  const handleSuggestRoles = async () => {
    ...
    try {
      const response = await authUtils.fetchWithAuth(
        `/api/admin/providers/${editingProvider.id}/roles`,
      );
      if (response.ok) {
        const result = await response.json();
        ...
        message.success(
          `সিস্টেম ${suggestions.join(", ")} রোলগুলো সাজেস্ট করেছে`,
        );
      }
    } catch (err) {
      ...
      <Form form={form} layout="vertical" onFinish={onSubmit}>
        ...
        <Form.Item label="Search & Choose AI Model" required>
          <Form.Item
            ...
            noStyle
            rules={[
              { required: true, message: "কমপক্ষে একটি মডেল সিলেক্ট করুন" },
            ]}
          >
            <Select
              mode="multiple"
        ...
        <Form.Item
          ...
          initialValue="local"
          help="Determines which group this model appears in on the admin dashboard"
        >
          <Select>
            <Option value="api">
              🔑 API Key — Regular API key providers (OpenAI, Anthropic, etc.)
            </Option>
            <Option value="gcloud">
              ☁️ GCloud Deploy — Firebase / Google Gemini / Vertex AI models
            </Option>
            <Option value="local">
              🖥️ Local / Ollama — Self-hosted Ollama or local models
              (Recommended)
            </Option>
          </Select>
        </Form.Item>
