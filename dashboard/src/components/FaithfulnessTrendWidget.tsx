...
const FaithfulnessTrendWidget: React.FC = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "/api/knowledge/recent-scraped?limit=50",
        );
  ...
  return (
    <Card className="glass-card" style={{ marginBottom: 20 }}>
      <Title level={4} style={{ color: "#00f3ff" }}>
        🧠 Knowledge Faithfulness Trend
      </Title>
      <div style={{ height: 250, width: "100%" }}>
        <ResponsiveContainer>
          <LineChart data={data}>
            ...
            />
            <XAxis dataKey="date" stroke="#8b5cf6" tick={{ fontSize: 10 }} />
            <YAxis domain={[0, 1]} stroke="#8b5cf6" />
            <Tooltip
              contentStyle={{
      ...
      <div
        style={{ marginTop: 10, fontSize: 11, color: "rgba(255,255,255,0.45)" }}
      >
