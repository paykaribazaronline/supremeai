...
const KingModePanel: React.FC = () => {
  ...
  const fetchStatus = async () => {
    ...
    try {
      ...
      if (statusRes.ok) {
        ...
        setError({
          message: "You do not have permission to access King Mode controls.",
          recoverable: false,
        });
      }
      if (pendingRes.ok) {
  ...
  const changeMode = async (newMode: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError({
          message: "Session expired. Please log in again to change modes.",
          recoverable: false,
        });
        ...
        setError({
          message: `Failed to change mode to ${newMode}. The server returned an error.`,
          recoverable: true,
          action: "Retry",
        });
      }
  ...
  const forceStop = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError({
          message:
            "System is unresponsive. Try refreshing the page or contacting support.",
          recoverable: true,
          action: "Refresh",
        });
      } else {
        setError({
          message: "Failed to stop operations. Please try again.",
          recoverable: true,
          action: "Retry",
        });
      }
  ...
  const resumeOperations = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError({
          message: "Failed to resume operations. Please try again.",
          recoverable: true,
          action: "Retry",
        });
      }
  ...
  const handlePendingAction = async (
    ...
    try {
      const res = await authUtils.fetchWithAuth(
        `/api/admin/control/pending/${actionId}/${decision}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        },
      ...
      if (res.ok) {
        ...
        setError({
          message: `This action no longer exists. It may have been handled by another admin.`,
          recoverable: true,
          action: "Refresh",
        });
      } else {
        setError({
          message: `Failed to ${decision} the action. Please try again.`,
          recoverable: true,
          action: "Retry",
        });
      }
  ...
  const parseProposalDescription = (desc: string) => {
    ...
    try {
      const currentMatch =
        desc.match(/১\.\s*\*\*বর্তমান অবস্থা[^*]*\*\*\s*:(.*?)(?=২\.)/s) ||
        desc.match(/১\.\s*\*\*বর্তমান অবস্থা[^*]*\*\*(.*?)(?=২\.)/s);
      const proposedMatch =
        desc.match(/২\.\s*\*\*সাজেশন[^*]*\*\*\s*:(.*?)(?=৩\.)/s) ||
        desc.match(/২\.\s*\*\*সাজেশন[^*]*\*\*(.*?)(?=৩\.)/s);
      const reasonMatch =
        desc.match(/৩\.\s*\*\*কেন অ্যাপ্রুভ[^*]*\*\*\s*:(.*?)(?=৪\.)/s) ||
        desc.match(/৩\.\s*\*\*কেন অ্যাপ্রুভ[^*]*\*\*(.*?)(?=৪\.)/s);
      const impactMatch =
        desc.match(/৪\.\s*\*\*উন্নতি[^*]*\*\*\s*:(.*)/s) ||
        desc.match(/৪\.\s*\*\*উন্নতি[^*]*\*\*(.*)/s);
  ...
  return (
    <div>
      <Alert
        message={
          <>
            <CrownOutlined /> King Mode — Full Admin Override Control
          </>
        }
      ...
      <Row gutter={16} style={{ marginBottom: 24 }}>
        <Col span={6}>
          <Card>
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: 12, color: "#8c8c8c" }}>Current Mode</div>
              <Tag
                color={modeColor}
                style={{ fontSize: 20, padding: "4px 16px", marginTop: 8 }}
              >
        ...
        <Col span={6}>
          <Card>
            <Statistic
              ...
              valueStyle={{
                color: status.pendingCount > 0 ? "#faad14" : "#52c41a",
              }}
            />
          </Card>
        </Col>
        ...
        <Col span={6}>
          <Card>
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: 12, color: "#8c8c8c" }}>Last Change</div>
              <div style={{ fontSize: 14, marginTop: 8 }}>
                {status.lastModeChange
                  ? new Date(status.lastModeChange).toLocaleString()
                  : "N/A"}
              </div>
            </div>
          </Card>
      ...
      >
        <Space size="large">
          <Button
            type={status.mode === "AUTO" ? "primary" : "default"}
          ...
          <Button
            type={status.mode === "WAIT" ? "primary" : "default"}
          ...
          <Popconfirm
            title="Force stop ALL operations?"
            onConfirm={forceStop}
            okText="STOP"
            okType="danger"
          >
            <Button
              ...
              icon={<StopOutlined />}
              size="large"
              type={status.mode === "FORCE_STOP" ? "primary" : "default"}
            >
              FORCE STOP
            </Button>
          </Popconfirm>
      ...
      <Card
        title={
          <>
            Pending Actions <Badge count={pendingActions.length} />
          </>
        }
      >
          ...
          <List
            ...
            renderItem={(action) => {
              const parsedProposal =
                action.type === "kingsmode-pending" ||
                action.description.includes("Improvement Proposal")
              ...
              return (
                <List.Item
                  ...
                  style={{
                    display: "block",
                    padding: "16px",
                    background: parsedProposal ? "#fafafa" : "#fff",
                  ...
                  >
                    <div>
                      <Title level={5} style={{ margin: 0 }}>
                        <Tag
                          color={
                            action.severity === "critical" ? "red" : "blue"
                          }
                        >
