...
import type { AnalysisJob, AnalysisFinding } from "../types";
...
const AnalysisResultsTable: React.FC<AnalysisResultsTableProps> = ({
        ...
        <Space>
          <Tooltip title="View Code Snippet">
            <Button
              ...
              icon={<EyeOutlined />}
              onClick={() =>
                copyToClipboard(record.codeSnippet || record.message)
              }
  ...
  const completedJobs = jobs.filter(
    (j) => j.status === "COMPLETED" && j.findings && j.findings.length > 0,
  );
              ...
              render: (name: string, record: any) => (
                <Space>
                  ...
                  {record.incrementalUsed && (
                    <Tooltip
                      title={`Incremental: ${record.changedFiles || 0} changed files`}
                    >
