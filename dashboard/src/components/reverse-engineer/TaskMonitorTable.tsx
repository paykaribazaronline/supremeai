...
const TaskMonitorTable: React.FC<TaskMonitorTableProps> = ({
      ...
      render: (text: string, record: Job) => (
        <Space direction="vertical" size={0}>
          ...
          <Space>
            ...
            {record.results?.alternative_suggestions &&
              record.results.alternative_suggestions.length > 0 && (
                <Badge
                  count={<BulbOutlined style={{ color: "#faad14" }} />}
                  offset={[5, 0]}
                >
    ...
    },
    {
      ...
      render: (_: any, record: Job) => (
        <Space>
          ...
          {["PENDING", "ANALYZING", "GENERATING", "RUNNING"].includes(
            record.status,
          ) && (
  ...
  return (
    <Card
      ...
      title={
        <Space>
          <PlayCircleOutlined style={{ color: "#52c41a" }} /> Live Task Monitor
        </Space>
      }
      extra={
