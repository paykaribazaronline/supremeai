...
const AISuggestionInformer: React.FC<AISuggestionInformerProps> = ({
  ...
  return (
    <Card
      ...
      title={
        <Space>
          <BulbOutlined style={{ color: "#faad14" }} /> {title}
        </Space>
      }
      extra={
        <Badge
          status="processing"
          text={`${suggestions?.length || 0} Active Insights`}
        />
      }
      style={{ marginBottom: 24, borderLeft: "4px solid #faad14", ...style }}
    >
      <Paragraph type="secondary" style={{ fontSize: 13 }}>
        Based on current {context} metrics, SupremeAI suggests the following
        enhancements to make the system more profound.
      </Paragraph>
