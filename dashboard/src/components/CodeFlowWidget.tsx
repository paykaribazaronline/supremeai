...
import { Card, Statistic, Progress, Badge, Tooltip, Button, Spin, Alert, List, Tag, Typography, Row, Col, Divider } from 'antd';
import { 
  EyeOutlined, 
  BugOutlined, 
...
const CodeFlowWidget: React.FC<CodeFlowWidgetProps> = ({
  ...
  if (loading && !analysis) {
    return (
      <Card className="codeflow-widget" style={{ background: '#16161a', border: '1px solid #262626' }}>
        <div style={{ textAlign: 'center', padding: compact ? 20 : 40 }}>
          <Spin size="large" />
          <div style={{ marginTop: 16, color: '#a3a3a3' }}>Analyzing {repositoryName}...</div>
        </div>
      </Card>
    );
  }
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 0 }}>
            ...
            <Text style={{ color: '#f0f0f2', fontSize: 14, fontWeight: 500 }} ellipsis>
              {repositoryName}
            </Text>
          </div>
          ...
          <Badge
            status={analysis.analysisStatus === 'COMPLETED' ? 'success' : 
                    analysis.analysisStatus === 'ANALYZING' ? 'processing' : 'default'}
          />
        </div>
      </Card>
    );
    ...
    >
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} sm={12} md={6}>
          ...
          >
            <Statistic
              ...
              valueStyle={{ 
                color: getGradeColor(healthData?.grade || analysis.healthGrade || 'F'),
                fontSize: 24
              }}
        ...
        <Col xs={24} sm={12} md={6}>
          ...
          >
            <div style={{ marginBottom: 8 }}>
              <SecurityScanOutlined style={{ color: '#ff7875', marginRight: 8 }} />
            ...
            <Row gutter={8}>
              <Col span={12}>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ color: '#f0f0f2', fontSize: 18, fontWeight: 600 }}>
        ...
        <Col xs={24} sm={12} md={6}>
          ...
          >
            <div style={{ marginBottom: 8 }}>
              <ThunderboltOutlined style={{ color: '#00ff9d', marginRight: 8 }} />
      ...
      {((analysis.detectedPatterns?.length || 0) > 0 ||
        ...
        <Row gutter={16} style={{ marginTop: 16 }}>
          {analysis.detectedPatterns && analysis.detectedPatterns.length > 0 && (
            <Col xs={24} sm={8}>
              <Card size="small" style={{ background: '#0a0a0c', border: '1px solid #262626' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <CodeOutlined style={{ color: '#1890ff' }} />
                  <Text style={{ color: '#a3a3a3' }}>Patterns</Text>
                  <Badge count={analysis.detectedPatterns.length} style={{ marginLeft: 'auto' }} />
                </div>
                <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                  {analysis.detectedPatterns.slice(0, 3).map((p: any, i: number) => (
                    <Tag key={i} color="blue" style={{ fontSize: 10 }}>
                      {p.patternType}
                    </Tag>
                  ))}
                  {analysis.detectedPatterns.length > 3 && (
                    <Tag style={{ fontSize: 10 }}>+{analysis.detectedPatterns.length - 3} more</Tag>
                  )}
                </div>
              </Card>
            </Col>
              ...
              <Card size="small" style={{ background: '#0a0a0c', border: '1px solid #262626' }}>
                ...
                <div style={{ marginTop: 8 }}>
                  <Text style={{ color: '#ff7875', fontSize: 11 }}>
                    {analysis.circularDependencies[0].files.length} files involved
                  </Text>
                </div>
              </Card>
            </Col>
              ...
              <Card size="small" style={{ background: '#0a0a0c', border: '1px solid #262626' }}>
                ...
                <div style={{ marginTop: 8 }}>
                  <Text style={{ color: '#faad14', fontSize: 11 }}>
                    {analysis.deadCode.filter((d: any) => d.type === 'UNUSED_FUNCTION').length} unused functions
                  </Text>
                </div>
              </Card>
            </Col>
            ...
            renderItem={(item: any) => (
              <List.Item style={{ 
                ...
                <List.Item.Meta
                  ...
                  title={<Text style={{ color: '#f0f0f2', fontSize: 12 }}>{item.type}</Text>}
                  description={
                ...
                />
                <Badge count={item.confidence} style={{ 
