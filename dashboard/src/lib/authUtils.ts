...
function emulatorStub(url: string, fullUrl: string): Response {
  ...
  if (url.startsWith("/api/self-healing/detect")) {
    return jsonOk({
      suggestion:
        "No known fix found in local knowledge base. Logged for next session.",
      confidence: 0.4,
    });
  }
  ...
  if (url === "/api/admin/providers/test-key") {
    return jsonOk({
      ok: true,
      message: "Provider test key stub — emulator mode",
    });
  }
  ...
  if (url === "/api/admin/security/cyber/audit") {
    return jsonOk({
      reportId: "emulator-audit-" + Date.now(),
      findings: [],
  ...
  if (url === "/api/admin/security/cyber/learn") {
    return jsonOk({
      ok: true,
      message: "Learning cycle started in emulator mode",
    });
  }
  ...
  if (url === "/api/admin/learning/status") {
    return jsonOk({
      ...
      mode: "AUTO",
      modeDescription:
        "Continuous autonomous intelligence optimization cycle active.",
  ...
  if (
    url === "/api/admin/knowledge/domains" ||
    url.startsWith("/api/admin/knowledge/domains?")
  ) {
    return jsonOk([
      {
        id: "domain-ai-safety",
    ...
    return jsonOk([
      {
        ...
        description:
          "Auto-rotate SSH deploy keys and Git credentials every 24 hours to prevent unauthorized repository access.",
        confidence: 0.95,
      ...
      },
      {
        ...
        description:
          "Dynamically reroute traffic from unresponsive AI endpoints to healthier models to ensure zero service disruption.",
        confidence: 0.89,
  ...
  if (
    ...
    url === "/api/admin/providers/configured"
  ) {
    const rawProviders =
      sessionStorage.getItem("supremeai_dynamic_providers") ||
      localStorage.getItem("supremeai_dynamic_providers");
    ...
    return jsonOk({
      ...
      sources: Array.from(
        new Set(providers.map((p: any) => p.source || "custom")),
      ),
    });
  }
  ...
  if (
    ...
    url === "/api/admin/providers/health-stats"
  ) {
    return jsonOk({
      total: 7,
      active: 7,
  ...
  // Rules
  if (url.match(/^\/api\/admin\/rules\/[^\/]+\/toggle$/)) {
    return jsonOk({ ok: true });
  }
  if (url.match(/^\/api\/admin\/rules\/[^\/]+$/)) {
    return jsonOk({ id: url.split("/").pop(), ok: true });
  }
  ...
  // ── Browser
  if (url === "/api/browser/tasks") {
    return jsonOk({ tasks: [] });
  }
  if (url.match(/^\/api\/browser\/tasks\/[^\/]+\/(start|pause|stop)$/)) {
    return jsonOk({ ok: true });
  }
  if (url.match(/^\/api\/browser\/tasks\/[^\/]+\/findings$/)) {
    return jsonOk({ findings: [] });
  }
  if (url.match(/^\/api\/browser\/tasks\/[^\/]+$/)) {
    return jsonOk({ ok: true });
  }
  // ── Chat ────────────────────────────────────────────────────────────────
...
export const authUtils = {
  ...
  isAdmin(): boolean {
    ...
    return (
      user?.role === "admin" || user?.tier === "admin" || user?.tier === "ADMIN"
    );
  },
  ...
  getCurrentUser(): AuthUser | null {
    const raw =
      sessionStorage.getItem(FIREBASE_USER_KEY) ||
      localStorage.getItem(FIREBASE_USER_KEY);
    if (!raw) return null;
    try {
      let userStr = raw;
  ...
  async fetchWithAuth(url: string, options: any = {}) {
    ...
    try {
      ...
      if (!fullUrl.includes("/health") && !fullUrl.includes("providers")) {
        console.debug(
          "[fetchWithAuth] Optional endpoint unavailable:",
          fullUrl,
        );
      }
