...
const AdminAutoBrowser: React.FC = () => {
  ...
  const handleStartTask = async (taskId: string) => {
    ...
    try {
      const res = await authUtils.fetchWithAuth(
        `/api/browser/tasks/${taskId}/start`,
        { method: "POST" },
      );
      if (res.ok) {
        notification.success({ message: "Task started" });
        fetchTasks();
      }
    } catch {
      notification.error({ message: "Failed to start task" });
    }
  };
  ...
  const handlePauseTask = async (taskId: string) => {
    ...
    try {
      const res = await authUtils.fetchWithAuth(
        `/api/browser/tasks/${taskId}/pause`,
        { method: "POST" },
      );
      if (res.ok) {
      ...
      notification.error({ message: "Failed to pause task" });
    }
  };
  ...
  const handleStopTask = async (taskId: string) => {
    ...
    try {
      const res = await authUtils.fetchWithAuth(
        `/api/browser/tasks/${taskId}/stop`,
        { method: "POST" },
      );
      if (res.ok) {
        notification.success({ message: "Task stopped" });
        fetchTasks();
      }
    } catch {
      notification.error({ message: "Failed to stop task" });
    }
  };
  ...
  const handleViewFindings = async (taskId: string) => {
    try {
      const res = await authUtils.fetchWithAuth(
        `/api/browser/tasks/${taskId}/findings`,
      );
      if (res.ok) {
        const data = await res.json();
        Modal.info({
      ...
      notification.error({ message: "Failed to load findings" });
    }
  };
    ...
    >
      <div style={{ marginBottom: 24 }}>
        <Title level={2} style={{ color: "#fff", margin: 0 }}>
          <RocketOutlined /> Autonomous Browser
        </Title>
        <Text type="secondary">
          AI-driven autonomous web navigation and task execution
        </Text>
      </div>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card className="glass-card" bordered={false}>
            <Statistic
              title={
                <Text style={{ color: "rgba(255,255,255,0.45)" }}>Running</Text>
              }
              value={runningCount}
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card className="glass-card" bordered={false}>
            <Statistic
              title={
                <Text style={{ color: "rgba(255,255,255,0.45)" }}>Failed</Text>
              }
      ...
      <Card
        ...
        title={
          <>
            <PlusOutlined /> Create Autonomous Task
          </>
        }
        ...
        <Row gutter={[16, 16]} align="middle">
          <Col xs={24} md={8}>
            <Input
              ...
              prefix={
                <RocketOutlined style={{ color: "rgba(255,255,255,0.2)" }} />
              }
              style={{
          ...
          <Col xs={24} md={12}>
            <Input
              ...
              prefix={
                <BulbOutlined style={{ color: "rgba(255,255,255,0.2)" }} />
              }
              style={{
                background: "rgba(255,255,255,0.03)",
                border: "1px solid rgba(255,255,255,0.05)",
          ...
          <Col xs={24} md={4}>
            <Button
              ...
              disabled={!newTaskName || !newTaskGoal || isGuest}
              block
              style={{
      ...
      <Card
        ...
        title={
          <>
            <HistoryOutlined /> Autonomous Tasks
          </>
        }
        extra={
          <Button
            ...
            <tbody>
              {tasks.length === 0 ? (
                <tr>
                  ...
                  >
                    No autonomous tasks yet. Create one above to get started.
                  </td>
                </tr>
              ) : (
                tasks.map((task) => (
                      ...
                      <Space size="small">
                        ...
                        {(task.status === "completed" ||
                          ...
                          <Button
                            size="small"
                            icon={<EyeOutlined />}
                            onClick={() => handleViewFindings(task.id)}
                          >
                            Findings
                          </Button>
                        )}
