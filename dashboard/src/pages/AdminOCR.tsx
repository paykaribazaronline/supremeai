...
const AdminOCR: React.FC = () => {
  ...
  const handleDelete = async (id: string) => {
    if (isGuest) return;
    try {
      const res = await authUtils.fetchWithAuth(`/api/ocr/history/${id}`, {
        method: "DELETE",
      });
      if (res.ok) {
  ...
  const handleExport = async (id: string, format: "json" | "xlsx") => {
    ...
    try {
      const res = await authUtils.fetchWithAuth(
        `/api/ocr/history/${id}/export?format=${format}`,
      );
      if (res.ok) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (s: string) => (
        <Tag
          color={
            s === "COMPLETED" ? "green" : s === "PROCESSING" ? "blue" : "red"
          }
        >
          {s}
        </Tag>
    ...
    },
    {
      ...
      key: "createdAt",
      render: (t: string) => (
        <Text style={{ color: "var(--text-dim)", fontSize: 12 }}>{t}</Text>
      ),
    },
    {
      title: "Actions",
      ...
      render: (_: any, record: OCRResult) => (
        <Space>
          <Button
            ...
            onClick={() =>
              message.info(
                `Preview: ${record.textPreview?.substring(0, 100)}...`,
              )
            }
          >
            View
          ...
          <Button
            size="small"
            icon={<DownloadOutlined />}
            onClick={() => handleExport(record.id, "xlsx")}
          >
            XLSX
          </Button>
          <Button
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          marginBottom: 32,
          borderBottom: "1px solid rgba(235, 47, 150, 0.2)",
          paddingBottom: 24,
        }}
      >
          ...
          <Col>
            ...
            >
              ...
              <span
                style={{
                  color: "#eb2f96",
                  textShadow: "0 0 10px rgba(235, 47, 150, 0.3)",
                }}
              >
                OCR Engine
              </span>
            ...
            <Text style={{ color: "var(--text-dim)", fontSize: 14 }}>
              Neural character recognition for complex script analysis and
      ...
      {scanning && (
        <Card
          ...
          style={{ marginBottom: 24, background: "rgba(235, 47, 150, 0.05)" }}
        >
          <Space direction="vertical" style={{ width: "100%" }}>
            <Text style={{ color: "#eb2f96", fontWeight: 700 }}>
              Processing document...
            </Text>
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card className="glass-card" style={{ textAlign: "center" }}>
            ...
            <div style={{ color: "#eb2f96", fontSize: 28, fontWeight: 800 }}>
              {results.length > 0
                ? Math.round(
                    results.reduce((a, b) => a + b.confidence, 0) /
                      results.length,
                  )
                : 0}
              %
      ...
      <Card
        ...
        title={
          <>
            <FileTextOutlined /> OCR Scan History
          </>
        }
        extra={
          <Badge count={results.length} style={{ background: "#eb2f96" }} />
        }
      >
        <Table
          columns={columns}
          ...
          loading={loading}
          locale={{
            emptyText: "No OCR scans yet. Upload an image or PDF to begin.",
          }}
        />
      </Card>
      ...
      <div
        className="glass-card"
        style={{ marginTop: 24, background: "rgba(0, 243, 255, 0.05)" }}
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          >
            Quantum Vision Ready. Structured export to .XLSX and .JSON supported
            upon activation.
          </Text>
        </div>
      </div>
