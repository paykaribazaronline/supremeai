...
const AdminLearning: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [statusResp, domainsResp, recsResp, sourcesResp] =
        await Promise.all([
  ...
  const handleEmergencyPause = async () => {
    ...
    try {
      const res = await authUtils.fetchWithAuth(
        "/api/admin/learning/emergency-pause",
        {
          method: "POST",
      ...
      if (res.ok) {
        message.success(
          status?.emergencyPaused ? "Learning resumed" : "Learning paused",
        );
        fetchData();
      } else message.error("Failed to toggle learning pause");
    } catch {
      message.error("Failed to toggle learning pause");
    } finally {
  ...
  const handleModeChange = async (mode: string) => {
    ...
    try {
      ...
      message.error("Failed to change learning mode");
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleManualTrigger = async () => {
    ...
    try {
      const res = await authUtils.fetchWithAuth(
        "/api/admin/learning/manual-trigger",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        },
      ...
      message.error("Failed to trigger training");
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleIntervalChange = async (interval: number) => {
    try {
      ...
      if (res.ok) {
        message.success(`Interval set to ${interval} minutes`);
        fetchData();
      }
    } catch {
  ...
  const handleCyberResearch = async (topic: string) => {
    ...
    try {
      const res = await authUtils.fetchWithAuth(
        "/api/admin/learning/cyber-research",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      message.error("Failed to start cyber research");
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleRunAudit = async () => {
    ...
    try {
      ...
      message.error("Self-audit failed");
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleApprove = async (id: string) => {
    try {
      const res = await authUtils.fetchWithAuth(
        `/api/admin/learning/proposals/${id}/approve`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        },
  ...
  const handleDecline = async (id: string) => {
    try {
      const res = await authUtils.fetchWithAuth(
        `/api/admin/learning/proposals/${id}/decline`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        },
      ...
      message.error("Failed to decline proposal");
    }
  };
  ...
  const tabItems = [
    {
      ...
      label: (
        <span className="tab-label">
          <ExperimentOutlined /> NEURAL MATRIX
        </span>
      ),
      children: (
    ...
    },
    {
      ...
      label: (
        <span className="tab-label">
          <BookOutlined /> KNOWLEDGE DOMAINS
        </span>
      ),
      children: (
        <KnowledgeDomainsTab
    ...
    },
    {
      ...
      label: (
        <span className="tab-label">
          <BulbOutlined /> EVOLUTION PROPOSALS
        </span>
      ),
      children: (
        ...
        <Row justify="space-between" align="bottom">
          <Col>
            ...
            <Text style={{ color: "var(--text-dim)", fontSize: 14 }}>
              Monitor AI neural training, expand knowledge domains, and
              authorize evolution paths.
            </Text>
          </Col>
          <Col>
            <Space>
