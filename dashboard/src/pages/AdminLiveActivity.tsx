  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        ...
        <Space>
          <Button icon={<SyncOutlined />} onClick={fetchActivities} className="glass-action-button">
            Refresh
          </Button>
          <Button icon={<ClearOutlined />} onClick={() => setActivities([])} className="glass-action-button">
            Clear
          </Button>
        </Space>
      </div>
          ...
          renderItem={(activity) => (
            <List.Item key={activity.id} style={{ padding: '12px' }}>
              <List.Item.Meta
                ...
                description={<Text style={{ color: 'rgba(255,255,255,0.7)' }}>{activity.details}</Text>}
              />
