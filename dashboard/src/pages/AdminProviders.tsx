...
import {
  Provider,
  ProviderHealthStats as StatsType,
...
const AdminProviders: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState<"activationStatus" | "modelName">(
    "activationStatus",
  );
  ...
  const handleWakeUpTest = async () => {
    ...
    try {
      const res = await authUtils.fetchWithAuth(
        "/api/self-healing/health-check/now",
        { method: "POST" },
      );
      if (res.ok) {
        ...
        <Row justify="space-between" align="bottom">
          <Col>
            ...
            <Text style={{ color: "var(--text-dim)", fontSize: 14 }}>
              Manage global LLM orchestration and model health.
            </Text>
          </Col>
          <Col>
            <Space>
          ...
          <div className="glass-card" style={{ minHeight: 600 }}>
            ...
            >
              <div style={{ display: "flex", gap: 16, flex: 1, maxWidth: 600 }}>
                ...
                >
                  <Select.Option value="activationStatus">
                    Status Rank
                  </Select.Option>
