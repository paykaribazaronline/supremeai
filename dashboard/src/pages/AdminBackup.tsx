...
const AdminBackup: React.FC = () => {
    ...
    try {
      const response = await authUtils.fetchWithAuth(
        "/api/admin/backup/trigger",
        {
          method: "POST",
          body: JSON.stringify({
            name: `Vault Sync ${new Date().toLocaleTimeString()}`,
          }),
        },
      );
      if (response.ok) {
  ...
  return (
    ...
    >
        ...
        <Row justify="space-between" align="bottom">
          <Col>
            ...
            <Text style={{ color: "var(--text-dim)", fontSize: 14 }}>
              Secure system snapshots, encrypted neural weights, and operator
              state backups.
            </Text>
          </Col>
          <Col>
        ...
        <Col span={24}>
          <div className="glass-card" style={{ minHeight: 600 }}>
            <div className="glass-card-title">
              Historical Archive Matrix <DatabaseOutlined />
            </div>
            <Table
              dataSource={backups}
              ...
              columns={[
                ...
                },
                {
                  ...
                  render: (_, r) => (
                    <Space>
                      <Button icon={<DownloadOutlined />} size="small" ghost>
                        Retrieve
                      </Button>
                      <Button
                        icon={<DeleteOutlined />}
      ...
      <div
        className="glass-card"
        style={{ marginTop: 24, background: "rgba(16, 185, 129, 0.05)" }}
      >
        <Text
          style={{
            color: "var(--success)",
        ...
        >
          Vault Protocol: All system snapshots are sharded and distributed
          across redundant secure nodes.
        </Text>
      </div>
    </motion.div>
  );
